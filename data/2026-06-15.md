...
Recommendation: ★★☆☆☆ (30/100) - 与 Zotero 兴趣画像中的 dark matter, dynamical mass 等主题/作者匹配。

TL;DR: 本文首次证明重子塔利-费舍尔关系(BTFR)能同时直接敏感于宇宙学参数(Ω_m和σ_8)、天体物理反馈(超新星和活动星系核)以及温暗物质粒子质量(M_wdm)，可作为宇宙学和基础物理的新型直接探针。基于DREAMS宇宙学磁流体模拟套件，通过模拟推断和深度神经网络训练，该框架能以亚百分比偏差、~2.6%和~3.9%的中位精度恢复Ω_m和σ_8，以~30-35%精度约束M_wdm，并能限制超新星反馈参数。


<details>
  <summary>Details</summary>
Motivation: BTFR长期以来被视为基本标度关系和校准距离指示器，可用于检验星系形成演化理论及探测宇宙膨胀。本文旨在探索BTFR是否也能同时直接敏感于宇宙学参数、暗物质性质和天体物理反馈，从而作为宇宙学和基础物理的新型直接探针。

Method: 利用DREAMS宇宙学磁流体模拟套件进行基于模拟的推断，训练归一化流形式的深度神经网络，在给定BTFR测量值条件下估计Ω_m、σ_8、M_wdm及三个天体物理自由参数的后验分布。

Result: 该框架能以亚百分比偏差恢复Ω_m和σ_8，中位精度分别约为2.6%和3.9%；能以约30-35%精度约束温暗物质粒子质量M_wdm；能限制超新星反馈参数，但无法约束活动星系核反馈参数。

Conclusion: BTFR除了作为距离指示器和约束重子循环及反馈机制外，还可作为宇宙学和基础物理的直接独立探针，为未来平方千米阵列(SKA)的探索开辟了新途径。
...
### [2] [Impact of 21-cm foreground mitigation strategies on reionization power spectrum constraints](https://arxiv.org/abs/2606.14682)
*Sambit K. Giri,Florent Mertens*

Main category: astro-ph.CO

Recommendation: ★★☆☆☆ (25/100) - 与 Zotero 兴趣画像中的 halo, intergalactic medium 等主题/作者匹配。

TL;DR: 论文评估了两种21-cm信号干扰消除策略（前景回避与前景移除），发现两者均会导致天体物理参数（如最小恒星形成晕质量、电离逃逸分数）产生约1σ的系统偏差，其中回避策略由于视场覆盖受限而具有更宽的置信区间。全局再电离历史可在95%置信区间内恢复，但晚期电离中性分数存在持续偏差。多红移推断易受污染影响，排除污染区间可显著减少偏差，但需独立质量指标。在相同尺度下，两种策略恢复的功率谱相似。


<details>
  <summary>Details</summary>
Motivation: 21-cm信号在再电离时期揭示星际介质信息，但受到比其强数个量级的天体物理前景污染。为提取宇宙学信号，需有效缓解前景干扰，因此评估不同缓解策略的效能与偏差至关重要。

Method: 采用两种前景缓解策略：(i) 前景回避——将分析限制在傅里叶空间的EoR窗口；(ii) 前景移除——利用高斯过程回归基于光谱平滑性统计分离前景与信号。通过贝叶斯推断比较两种策略在恢复天体物理参数与再电离历史方面的表现。

Result: 两种策略均导致约1σ的系统偏差，回避策略后验分布更宽。全局再电离历史可在95%置信区间内恢复，但晚期中性分数存在持续偏差。多红移推断中污染区间会引入偏差，排除后显著改善。在相同尺度下两种策略恢复的功率谱相似。

Conclusion: 两种前景缓解策略均无法完全消除系统偏差，尤其对晚期再电离中性分数的推断存在固有困难。多红移分析需建立独立的质量指标识别污染区间。在可比尺度下，两种策略的性能差异主要源于可用数据的视场覆盖范围。
...
### [3] [Impact of non-Gaussian likelihood on cosmological constraints from the thermal Sunyaev--Zel'dovich power spectrum: a simulation-based inference analysis](https://arxiv.org/abs/2606.14622)
*Licong Xu,Íñigo Zubeldia,James Alvey,Boris Bolliet,Anthony Challinor*

Main category: astro-ph.CO

Recommendation: ★★☆☆☆ (25/100) - 与 Zotero 兴趣画像中的 halo 等主题/作者匹配。

TL;DR: 本文通过模拟推断验证了普朗克热SZ功率谱分析中高斯似然假设的准确性，发现其在宇宙学参数约束上无偏，但模拟推断方法能更好地处理非高斯尾部特征。


<details>
  <summary>Details</summary>
Motivation: 热SZ功率谱受稀有、大质量、低红移星系团的显著贡献，其统计量呈非高斯分布，标准高斯似然在大尺度上的有效性存疑，需进行系统验证。

Method: 采用基于晕模型的全球康普顿-y图模拟，训练神经后验估计器和似然估计器，将模拟推断结果与标准高斯似然假设的约束进行对比，仅使用ℓ<1000的多极矩数据。

Result: 高斯似然假设给出的宇宙学参数约束是无偏的，但模拟推断显示残余前景振幅的后验分布略有展宽，表明非高斯性主要影响前景参数而非宇宙学参数。

Conclusion: 对于普朗克级别的热SZ分析，高斯似然假设在宇宙学推断上足够准确；模拟推断可作为超越解析近似的非高斯似然建模的有效验证工具。
...
### [4] [Constraints on Stable Scalar-Tensor Dark Energy from DESI Data and Solar System Tests](https://arxiv.org/abs/2606.14014)
*Husam Adam,Mark P. Hertzberg,Daniel Jiménez-Aguilar*

Main category: astro-ph.CO

Recommendation: ★★☆☆☆ (25/100) - 与 Zotero 兴趣画像中的 dark energy 等主题/作者匹配。

TL;DR: 该论文研究了具有四次多项式非最小耦合和线性标量势的标量-张量（quintessence）暗能量模型的可行性，发现满足DESI约束和局部引力测试的参数带非常狭窄。


<details>
  <summary>Details</summary>
Motivation: 探索在早期宇宙中场能稳定稳定的标量-张量暗能量模型，并检验其与最新观测数据的兼容性。

Method: 系统扫描二次和四次非最小耦合及势能斜率构成的参数空间，将模型预测与DESI暗能量状态方程约束及太阳系测试、牛顿常数时间变化等局部引力约束进行对比。

Result: 识别出满足所有约束的极窄参数带。

Conclusion: 该模型在参数空间的可行区域非常有限，表明这类模型面临严格的观测限制。
...
### [5] [Optimal marked statistics from the Effective Field Theory of Large Scale Structure](https://arxiv.org/abs/2606.14332)
...
TL;DR: 本文在有效场论框架下提出了一种简单且鲁棒的标记密度场功率谱分析方法，通过优化标记函数，可将宇宙学参数（Ω_m、σ_8、h）的约束精度提升1.2-8.3倍，展示了标记统计量在提取非线性密度场额外信息方面的潜力。


<details>
  <summary>Details</summary>
Motivation: 传统的两点关联函数分析只能提取密度场的部分信息，作者希望探索标记统计量（marked statistics）这一非线性变换方法，以从密度场中提取更多宇宙学信息，从而提高参数约束精度。

Method: 基于有效场论（EFT）框架，构建标记密度场的功率谱，并优化标记函数的选择以最大化对宇宙学参数（Ω_m、σ_8、h）的约束能力，同时对小尺度反项进行边际化处理。

Result: 优化后的标记函数可将参数误差棒降低：Ω_m提升约1.30-2.3倍，σ_8提升约1.3-8.3倍，h提升约1.2-2.5倍。优化标记具有简单函数形式，且可基于对密度场的响应做出物理解释。

Conclusion: 标记统计量是提取超越标准两点分析之外额外宇宙学信息的有效工具，为未来进一步探索密度场非线性变换提供了新途径。
...
### [6] [Controlling instrumental systematics for the BICEP inflation survey](https://arxiv.org/abs/2606.13942)
...
TL;DR: BICEP系列实验利用南极观测的CMB极化数据，旨在约束暴胀引力波。预计2024年数据可将张量标量比r的约束精度提升至σ(r)~0.005（含去透镜），长期目标（2034年前）达σ(r)~0.001。为确保精度，需将仪器系统误差控制在σ(r)的20%以内，本文重点介绍了校准程序、系统误差分析与模拟方法，尤其关注波束系统误差这一主要来源，并给出了初步估计与未来应对策略。


<details>
  <summary>Details</summary>
Motivation: 随着BICEP实验灵敏度不断提升，仪器系统误差必须被控制在前所未有的低水平，以避免对原初引力波探测造成污染，确保对张量标量比r的约束精度。

Method: 通过完善的校准程序（光学、光谱、偏振响应表征）、端到端模拟评估各类系统误差对宇宙学参数的影响，重点分析波束系统误差，并制定识别与缓解策略。

Result: 预计2024年数据可达到σ(r)~0.005的精度，长期目标σ(r)~0.001；系统误差控制目标为σ(r)的20%以内，给出了初步的系统污染水平估计。

Conclusion: BICEP有望在未来实现对原初引力波的高精度探测，但需持续严格的系统误差控制；本文提出的校准与分析方法为当前及未来CMB极化实验提供了重要参考。
...
### [7] [GATOS N: Extended circumnuclear dust emission in nearby Seyfert galaxies surveyed by JWST/MIRI](https://arxiv.org/abs/2606.14477)
...
Recommendation: ★★★★☆ (61/100) - 与 Zotero 兴趣画像中的 jwst, dust, molecular, dusty, molecular gas 等主题/作者匹配。
...
### [8] [CANUCS/Technicolor Data Release 2: A Catalogue of Galaxy Structural Parameters in up to 29 HST+JWST bands and a Multi-Wavelength Exploration of the Galaxy Size-Mass Relation at $0.6 < z \leq 4$](https://arxiv.org/abs/2606.13781)
...
Recommendation: ★★★☆☆ (56/100) - 与 Zotero 兴趣画像中的 jwst, star-forming galaxies, james webb space, webb space telescope, space telescope 等主题/作者匹配。

TL;DR: 基于JWST数据对约4,100个恒星形成星系（0.6<z≤4）进行形态学研究，测量了大小-恒星质量-红移-波长四维关系，发现约10^9.5 M⊙的关键交叉质量，并发布了约41,000个星系的形态参数数据。


<details>
  <summary>Details</summary>
Motivation: 系统揭示星系大小如何随恒星质量、红移和静止帧波长变化，探究星系从弥散到致密形态转变的临界质量。

Method: 利用JWST NIRCam 19个中/宽带滤光片观测CANUCS和Technicolor巡天数据，使用GALFIT测量~4,100个恒星形成星系（0.6<z≤4，质量8.5<log(M*/M⊙)≤11.5）的形态参数，分析大小-质量-波长关系。

Result: 发现大小-质量关系斜率随静止帧波长存在梯度，在~10^9.5 M⊙处存在临界交叉质量，标志着星系从弥散到致密形态的转变。

Conclusion: 提出约10^9.5 M⊙为星系形态转变的特征质量，并发布了包含~41,000个星系在29个JWST+HST滤光片下的结构参数数据。
...
### [9] [Lensed hot stars with HST in the 2030s](https://arxiv.org/abs/2606.14392)
*J. M. Diego*

Main category: astro-ph.GA

Recommendation: ★★★☆☆ (49/100) - 与 Zotero 兴趣画像中的 lensed, james webb space, webb space telescope, space telescope, james webb 等主题/作者匹配。

TL;DR: 本文论证了哈勃太空望远镜（HST）在紫外和光学波段相对于未来的罗曼望远镜仍具有独特优势，并聚焦于利用HST研究红移z>0.5的透镜热星科学，直到2040年代宜居世界天文台发射。


<details>
  <summary>Details</summary>
Motivation: 响应"为2030年代哈勃科学制定路线图"的号召，论证HST在紫外/光学波段不可替代的科学价值，为其在罗曼发射后继续运行提供科学依据。

Method: 通过对比HST与罗曼望远镜在紫外/光学波段的技术参数（分辨率、像素尺寸、采样率等），分析HST的独特优势；聚焦于透镜热星（z>0.5）这一具体科学案例。

Result: HST在紫外波段具有不可替代性；在光学波段对点源的分辨率优于罗曼望远镜；罗曼的传感器像素尺寸约为HST的3倍，导致点扩散函数欠采样，空间分辨率较差。

Conclusion: HST在紫外和光学波段的独特能力使其成为特定科学案例（如透镜热星研究）的最佳仪器，应持续运行直到2040年代宜居世界天文台发射。
...
### [10] [GA-NIFS: The interplay between feedback and star formation at 3 < z < 9 probed by JWST/NIRSpec IFU](https://arxiv.org/abs/2606.13819)
...
Recommendation: ★★★☆☆ (47/100) - 与 Zotero 兴趣画像中的 jwst, starburst, spatially resolved, starburst galaxies 等主题/作者匹配。

TL;DR: 利用JWST/NIRSpec IFU对6个红移3~9的星系进行空间分辨研究，发现大质量星系中AGN驱动的电离外流与过去1-3亿年内的核区熄灭事件相关，而高红移星暴星系未显示熄灭迹象，表明AGN外流在早期宇宙中已能塑造星系的恒星形成历史。


<details>
  <summary>Details</summary>
Motivation: 理解星暴和活动星系核（AGN）驱动的反馈机制对星系生长、恒星形成熄灭以及星系与中央黑洞协同演化的基本作用。

Method: 利用JWST/NIRSpec IFU的低分辨率（R~100）和高分辨率（R~2700）模式，对6个红移3<z<9的星系（包括星暴主导和AGN主导系统）进行空间分辨的恒星种群分析与外流性质研究。

Result: 大质量星系（M*>5×10¹⁰ M☉）中强AGN驱动电离外流与核区（<3 kpc）过去1-3亿年内的熄灭事件相关；高红移（z>5）低质量星暴星系未显示熄灭；弱外流的大质量星系无熄灭历史；一个AGN宿主星系显示近期（1-3千万年）复苏迹象。

Conclusion: AGN驱动的外流在早期宇宙中已能对大质量星系的恒星形成历史起到关键塑造作用。
...
### [11] [Reconstructing the orbits of Milky Way dwarf galaxies: An LMC perspective](https://arxiv.org/abs/2606.13787)
...
Recommendation: ★★★☆☆ (43/100) - 与 Zotero 兴趣画像中的 milky way, dwarf galaxies, star formation rate, formation rate 等主题/作者匹配。

TL;DR: 该研究通过构建银河系与小麦哲伦云相互作用的动态引力势，对72个银河系矮卫星的轨道进行积分，提供了迄今最大规模的轨道参数目录，并识别了可能的LMC捕获卫星及卫星与麦哲伦云之间的轨道-恒星形成率关联。


<details>
  <summary>Details</summary>
Motivation: 理解银河系矮卫星的轨道历史对于揭示其演化过程及将当前性质置于动力学背景中至关重要，但现有研究缺乏大规模、多势场模型的系统轨道重建。

Method: 基于文献中72个矮星的精确6维相空间坐标，使用六种考虑银河系与大麦哲伦云（LMC）相互作用的时变引力势模型进行轨道积分，计算轨道参数、束缚状态及下落时间。

Result: 识别出10个可能的LMC卫星（部分经历极近掠过LMC恒星盘）；SMC最近一次绕LMC的近心点（约8 kpc，约170 Myr前）与直接碰撞模型的预测一致；发现SMC之前多次近心点与两个麦哲伦云中的恒星形成率峰值存在时间上的巧合；识别出Grus II和Tucana IV可能是最近被LMC捕获的银河系卫星。

Conclusion: 大多数矮卫星在过去50亿年内位于银河系维里半径内；LMC与SMC的轨道-恒星形成率关联支持因果关系；发现了银河系卫星被LMC捕获的候选体。
...
### [12] [From THESAN-ZOOM to JWST: Predicting ionizing photon escape and the rise of UV-bright reionization sources](https://arxiv.org/abs/2606.14671)
...
Recommendation: ★★☆☆☆ (40/100) - 与 Zotero 兴趣画像中的 jwst, formation rate, mark vogelsberger 等主题/作者匹配。

TL;DR: 利用THESAN-ZOOM模拟的35,000多个星系样本，通过随机森林回归模型预测莱曼连续谱光子逃逸分数(f_esc)和逃逸率，发现SFR比值和气体-恒星质量比是f_esc的最佳诊断指标，而UV绝对星等主导逃逸率预测，由此构建的再电离历史与观测一致。


<details>
  <summary>Details</summary>
Motivation: 理解宇宙再电离的来源和演化是天体物理学的核心挑战，其中早期星系电离光子的逃逸机制是主要不确定性来源。

Method: 使用THESAN-ZOOM宇宙学辐射流体动力学模拟中的超过35,000个星系样本，训练随机森林回归模型，利用多种间接诊断指标预测LyC光子逃逸分数(f_esc)和逃逸率。

Result: SFR比值和气体-恒星质量比是f_esc的最强诊断指标；UV绝对星等主导逃逸率预测；基于UV光度函数构建的再电离历史与观测约束一致，避免了离子化光子预算危机。

Conclusion: 再电离的主体发生在z≈8之后，由UV明亮的星系驱动，M_UV < -17的星系群提供了主要贡献。
...
Recommendation: ★★☆☆☆ (40/100) - 与 Zotero 兴趣画像中的 halo, cosmic time, cosmic noon, hydrodynamical simulations 等主题/作者匹配。

TL;DR: 利用多个大尺度流体动力学模拟，发现类星体（活动星系核）与暗物质晕质量之间的关系是非线性的：中等质量晕（~10¹² M☉）中AGN最活跃，而更大质量的晕中AGN活动反而被抑制（淬灭）。模拟结果与观测基本一致，表明黑洞吸积与晕增长之间的耦合较弱。


<details>
  <summary>Details</summary>
Motivation: 观测和聚类分析表明类星体存在于较窄的暗物质晕质量范围（10¹²⁻¹³ M☉）内，但此前缺乏自洽的星系形成模型对该图景的直接验证。作者利用多个大尺度流体动力学模拟来量化AGN辐射光度与宿主晕质量之间的关系。

Method: 使用TNG300、TNG-Cluster、FLAMINGO L1_m8、L2p8_m9等大体积模拟，以及Illustris、EAGLE、TNG100、Simba等小体积模拟，分析AGN辐射光度L_bol≥10⁴² erg/s的源，统计其与宿主晕质量的关系。

Result: 1）AGN光度-晕质量关系呈高度非线性，在大质量端（≥10¹² M☉）出现平台或下降（高质端淬灭）；2）TNG模拟中类星体（~10⁴⁵⁻⁴⁷ erg/s）通常寄宿于10¹²⁻¹²·⁵ M☉晕中，而FLAMINGO模拟中类星体中位质量可达~10¹²·⁸ M☉；3）固定晕质量下AGN光度弥散很大（~3个量级），而固定光度下晕质量弥散较小（<1个量级），表明晕增长与黑洞瞬时吸积耦合较弱。

Conclusion: 模拟预测的类星体宿主晕质量与观测估计基本一致。最亮的AGN存在于越来越稀有的晕中，但并不总是栖息于最巨大的晕中，支持了AGN在大质量晕中被淬灭的图景。
...
### [14] [A new CIGALE module for modeling AGN emission lines](https://arxiv.org/abs/2606.14643)
*Hao Zhang,Patrice Theule,Veronique Buat,Denis Burgarella,Estelle Pons,Mederic Boquien*

Main category: astro-ph.GA

Recommendation: ★★☆☆☆ (36/100) - 与 Zotero 兴趣画像中的 emission lines, dust, dust attenuation 等主题/作者匹配。

TL;DR: 本文为新开发的CIGALE软件[nebular_AGN]模块，可高效模拟和拟合高红移AGN的宽线区和窄线区发射线，并估计其物理性质。


<details>
  <summary>Details</summary>
Motivation: 高红移AGN的大量发现对发射线诊断工具提出了更高要求，现有工具难以有效推导AGN及其宿主星系的性质。

Method: 利用Cloudy光谱合成代码构建数据库，基于CIGALE已有的X射线和吸积盘连续谱生成辐射场，结合AGN几何与尘埃设置定义气体云物理参数。

Result: 该模块能近似重现类星体发射线轮廓，覆盖关键发射线比值，金属丰度与经验公式一致，并与其它光致电离模型进行了对比。

Conclusion: [nebular_AGN]模块可有效用于AGN发射线诊断，且证实尘埃消光律在SED拟合中具有重要作用。
...
### [15] [Turbulence in the multi-phase circumgalactic medium: bridging TNG50 and idealized kpc-scale simulations](https://arxiv.org/abs/2606.13777)
*Jonas Biba,Dylan Nelson*

Main category: astro-ph.GA

Recommendation: ★★☆☆☆ (36/100) - 与 Zotero 兴趣画像中的 circumgalactic medium, halo, milky way 等主题/作者匹配。

TL;DR: 本研究利用TNG50宇宙学模拟和理想化湍流盒模拟，分析了银河系大小星系周围CGM的湍流特性，发现其以亚音速流为主，并研究了湍流如何通过密度涨落触发热不稳定性并促进冷热气体混合。


<details>
  <summary>Details</summary>
Motivation: 研究辐射冷却与湍流的相互作用如何塑造CGM的动力学结构，理解冷气体团块的形成与演化机制。

Method: 首先分析TNG50宇宙学磁流体模拟中CGM的湍流特性，然后基于物理约束在AREPO代码中实现湍流驱动算法，运行高分辨率理想化湍流盒模拟，参数化研究冷却与混合时标的影响。

Result: CGM主要为亚音速流（马赫数0.1-0.7），压缩与螺线分量大致平衡（b~0.3-0.6）；冷气体（~10^4 K）的丰度和演化强烈依赖于湍流强度和冷却效率的竞争。

Conclusion: 湍流驱动的密度涨落可触发热不稳定性并促进冷热相混合，冷气体的形成取决于冷却时标与混合时标的相对大小。
...
### [16] [Identifying Warped Galaxies in Pan-STARRS and Euclid using Deep Convolutional Neural Network](https://arxiv.org/abs/2606.14152)
...
TL;DR: 本文利用深度学习模型（Zoobot convNext-nano）对Pan-STARRS和Euclid巡天中的侧向星系图像进行分类，以83%的准确率识别出具有翘曲盘结构的星系，并发现翘曲星系具有更蓝的颜色、更年轻的星族和更高的恒星形成活动。


<details>
  <summary>Details</summary>
Motivation: 星系翘曲盘是常见但难以探测的结构，因为星系外围通常非常暗淡。作者希望利用深度成像巡天和机器学习方法，提高对这类微弱形态特征的检测效率。

Method: 基于Pan-STARRS EGIPS星表，在1000张侧向星系FITS图像上微调Zoobot convNext-nano模型，以区分翘曲和非翘曲侧向星系，然后应用于更大样本及Euclid Q1数据，并使用LayerCAM进行可解释性分析。

Result: 模型分类准确率达83%；在高置信度阈值（≥0.85）下识别出2088个翘曲星系和1398个非翘曲星系；模型能泛化到不同分辨率的Euclid数据；翘曲星系具有更低的轴比、更高的不对称性、更蓝的颜色、更年轻的星族和增强的恒星形成。

Conclusion: 深度学习方法能有效识别翘曲等微弱形态特征，可用于当前及未来大型成像巡天中星系结构性质的研究。
...
### [17] [On the survival of dark matter spikes: Stellar and compact-object perturbations](https://arxiv.org/abs/2606.13761)
*Theophanes K. Karydas,Francesca Scarcella,Bradley J. Kavanagh,Gianfranco Bertone*

Main category: astro-ph.GA

Recommendation: ★★☆☆☆ (30/100) - 与 Zotero 兴趣画像中的 dark matter, density profile 等主题/作者匹配。

TL;DR: 本文利用银河系中心作为观测约束的案例，研究暗物质密度尖峰在恒星和黑洞引力扰动下的演化，发现恒星散射会消耗远距离的暗物质分布，但近距离的恒星扰动和过去的极端质量比旋进事件对暗物质过密度的破坏非常有限，最终暗物质过密度基本保持完整。


<details>
  <summary>Details</summary>
Motivation: 建立对超大质量黑洞附近暗物质分布的现实预期，这对于评估暗物质环境对引力波信号的影响至关重要。

Method: 以银河系中心为观测约束案例，建模核星团对暗物质粒子的引力散射、已知近距离恒星（S2、S38）的引力扰动，以及过去10 Gyr内恒星质量黑洞（约10 M☉）的极端质量比旋进事件对暗物质密度剖面的累积影响。

Result: 核星团散射在~10⁻¹ pc尺度消耗暗物质分布，但在产生引力波信号的更小尺度上影响不大；S2和S38在~10⁻³ pc尺度引起可忽略的密度变化；在~10⁻⁵ pc及以下尺度，多次极端质量比旋进事件仅将暗物质密度降至初始值的约82%。

Conclusion: 至少在银河系中心，在恒星扰动和合理的恒星质量黑洞并合历史下，中心黑洞周围的暗物质过密度预计基本保持完整。
...
### [18] [Uncovering DM distribution by combining stellar kinematics and integrated HI spectra: method validation](https://arxiv.org/abs/2606.14271)
*Yu Lei,Meng Yang,Ling Zhu*

Main category: astro-ph.GA

Recommendation: ★★☆☆☆ (25/100) - 与 Zotero 兴趣画像中的 halo, dark matter 等主题/作者匹配。

TL;DR: 本文提出一种联合恒星运动学（IFU观测）和HI气体运动学（单次积分HI谱）来测定星系暗物质分布的方法，并在TNG50模拟的58个星系（116个侧向/端向视角）中验证。结果显示，该方法能准确恢复总质量、恒星质量和暗物质质量轮廓，在5 kpc内精度与仅用IFU数据相当，而在20 kpc处暗物质质量的不确定性从约85%降至约30%。但gNFW暗物质密度斜率存在系统性低估，改用2-20 kpc间的密度斜率则可较好恢复。该方法在揭示暗物质分布方面具有可靠性，适用于大样本星系研究。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以同时利用恒星和气体运动学信息准确测定星系暗物质分布，尤其在外围区域不确定性较大。作者希望结合IFU恒星运动学与单次HI谱线数据，构建更精确的暗物质分布测量方法。

Method: 采用三轴轨道叠加Schwarzschild模型描述恒星运动学，理想化盘模型描述HI气体，两者受同一引力势支配。势能包含恒星质量、广义NFW暗物质晕和中心黑洞。在TNG50模拟的58个星系中生成侧向和端向两种视角的模拟数据（共116个），进行方法验证。

Result: 模型能恢复总质量、恒星质量和暗物质质量轮廓；2-20 kpc范围内暗物质质量恢复的中位系统偏差小于20%。5 kpc内暗物质质量统计不确定性与仅用IFU数据相当；20 kpc处相对不确定度从约85%降至约30%。gNFW暗物质密度斜率系统性低估，但2-20 kpc间的密度斜率恢复良好。

Conclusion: 该方法能可靠揭示暗物质分布，尤其在外围区域显著提高了暗物质质量的约束精度，适用于大样本星系观测分析。
...
### [19] [The S-PLUS Fornax Project (S+FP): Fornax-like clusters in cosmological hydrodynamical simulations](https://arxiv.org/abs/2606.14109)
...
Recommendation: ★★☆☆☆ (25/100) - 与 Zotero 兴趣画像中的 hydrodynamical simulations 等主题/作者匹配。

TL;DR: 利用EAGLE和IllustrisTNG模拟生成Fornax星系团的合成测光目录，与S-PLUS观测数据对比，验证了模拟在重现星系颜色-星等关系、空间分布等关键特征方面的可靠性。


<details>
  <summary>Details</summary>
Motivation: Fornax星系团是研究致密环境中星系演化的理想实验室，S-PLUS巡天提供了前所未有的测光数据，需要借助模拟来解读这些观测发现的历史、现状和未来演化。

Method: 从EAGLE和IllustrisTNG模拟中选取Fornax类似系统，使用SKIRT辐射转移代码生成12个S-PLUS波段的合成测光数据，包括数据立方体、模拟图像、光谱能量分布、星等和颜色。

Result: 模拟合成的测光和光谱与S-PLUS观测吻合良好；再现了颜色-星等关系、团核到Fornax A区的空间子结构；模拟星系在更大团心距处更蓝。

Conclusion: 尽管存在少许偏差，但选择标准和合成测光方法有效，当前宇宙学模拟能够可靠地再现Fornax星系团的关键总体特征。
...
### [20] [A novel data-driven approach to extract stellar population properties from galaxy spectra using absorption indices](https://arxiv.org/abs/2606.13791)
...
TL;DR: 本文利用PCA方法分析星族合成模型，在三维空间中打破了年龄-金属度简并，并发现Hγ_A和Hδ_A指数的差异可用于识别近期爆发性星族形成行为。


<details>
  <summary>Details</summary>
Motivation: 在复杂机器学习方法难以解释的背景下，PCA作为一种无需参数拟合、易于解释的降维方法，被用于分析星系光谱中的星族参数，以建立更直观的潜在空间。

Method: 选取六种主要光谱吸收指数，对大量复合星族模型进行PCA降维，构建基于前三特征向量的潜在空间，并分析各特征向量的物理含义。

Result: 三维PCA空间成功打破了年龄-金属度简并；最后一个特征向量揭示了Hγ_A与Hδ_A之间的"拉锯"关系，可用于识别约0.5-1 Gyr时间尺度上的近期爆发行为。

Conclusion: 基于纯数据的PCA方法与标准SPS模型拟合技术结果一致，为不同巡天或观测/合成数据库中的星族研究提供了可靠的替代方案。
