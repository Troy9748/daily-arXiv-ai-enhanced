...
### [1] [Chaotic Molecular Gas in Five Dusty Star-forming Galaxies in the Spiderweb Protocluster at $z = 2.16$](https://arxiv.org/abs/2604.27071)
...
### [2] [Molecular Outflows in the Nucleus of the Nearby Compton-thick AGN NGC 3079](https://arxiv.org/abs/2604.27159)
...
### [3] [Dense cores and filaments in M16: Enhanced formation efficiency in the stellar feedback-driven shell](https://arxiv.org/abs/2604.27584)
...
TL;DR: 利用赫歇尔望远镜数据，在M16鹰状星云中识别出233个致密核和111条纤维结构，发现由NGC 6611星团驱动的壳层结构显著增强了纤维和核的形成效率，提供了大质量恒星形成正反馈的观测证据。


<details>
  <summary>Details</summary>
Motivation: 探究大质量恒星形成过程中，电离辐射、星风等反馈机制如何影响周围分子云的结构演化，特别是壳层压缩如何触发次级恒星形成活动。

Method: 使用赫歇尔望远镜的高分辨率面密度和温度图，采用hires算法构建图像，getsf方法提取源和纤维结构，对M16区域的壳层内外部进行径向分析。

Result: 识别出233个核和111条纤维，纤维中位宽度0.4 pc，中位线密度61 M⊙/pc，76%超临界；壳层内纤维形成效率提高2.3倍，核密度提升1.5倍；观测面密度超过碎裂临界阈值约8倍，碎裂时标(1.5-2.0 Myr)与壳层动力学年龄(1.0-1.3 Myr)相当。

Conclusion: 揭示了壳层压缩→纤维形成→核形成的层级碎裂序列，为大质量恒星通过正反馈触发周围分子云中次级结构形成提供了清晰的观测证据。
...
### [4] [Applications of 1.4 GHz diagnostics to Type Ia Supernova host galaxies](https://arxiv.org/abs/2604.27988)
*S. Ramaiya,M. J. Jarvis,M. Vincenzi,M. Sullivan,I. H. Whittam*

Main category: astro-ph.GA

Recommendation: ★★★☆☆ (56/100) - 与 Zotero 兴趣画像中的 dust, ska 等主题/作者匹配。

TL;DR: 本文研究了利用1.4 GHz射电数据替代远红外（FIR）SED拟合来估算Ia型超新星（SN Ia）宿主星系恒星形成率（SFR）的可行性，发现约84%的宿主星系在SFR-M*平面上的区域分类与基于FIR的方法一致，且标准化的nuisance参数（α, β, M）在各子区域内高度一致，表明射电SFR是一种可靠且可扩展的替代方案，尤其适用于即将开展的LSST等时域巡天项目。


<details>
  <summary>Details</summary>
Motivation: Ia型超新星标准化参数随宿主星系恒星形成率-恒星质量（SFR-M*）平面的系统变化表明，在宇宙学推断中需要纳入星系SFR信息。然而，传统SFR估算依赖远红外（FIR）SED拟合，而即将开展的LSST等时域巡天将缺乏充分的FIR覆盖，因此需要寻找替代的SFR示踪物。

Method: 利用1.4 GHz射电诊断重建SFR-M*平面，与基于FIR约束的SED估算的SFR进行对比。将SN Ia宿主星系分为三个区域：低质量区（Region 1）、高质量恒星形成区（Region 2）和高质量被动区（Region 3），比较两种方法下区域分类的一致性以及各区域内SN Ia标准化参数（α, β, M）的差异。

Result: 约84%的SN宿主星系在使用射电与FIR约束SED-derived SFRs时获得相同的区域分类。两种SFR-M*平面重建方法得到的各子区域内SN Ia标准化参数值一致，最大偏差出现在Region 3，约为1.1σ。在射电SFR-M*的三个子区域内，确认了SN Ia标准化参数（特别是β）存在区域依赖的变化。

Conclusion: 射电SFR校准是一种可靠且可扩展的宿主星系分类方法，尤其适用于缺乏FIR覆盖的时域巡天。随着SKA等射电连续谱巡天对LSST足迹的高覆盖，射电SFR方法将在构建精确宇宙学的稳健SN Ia子样本中发挥越来越重要的作用。
...
### [5] [Extended Universal Rotational Curve of Spiral Galaxies](https://arxiv.org/abs/2604.27320)
...
TL;DR: 本文利用SPARC样本的扩展HI旋转曲线，将普适旋转曲线从光学半径$R_{opt}$延伸至$2R_{opt}$，验证了其在暗物质主导区域仍保持普适性，为星系质量模型和暗物质本质研究提供了新约束。


<details>
  <summary>Details</summary>
Motivation: 推动普适旋转曲线范式的发展，利用新数据将其观测范围拓展到暗物质主导的外围区域，以更好地约束星系质量模型并探索暗物质粒子的性质。

Method: 基于SPARC样本的扩展HI旋转曲线，将URC的构建从光学半径$R_{opt}$延伸到$2R_{opt}$，采用双归一化旋转曲线分析其普适性。

Result: SPARC样本构建的URC在$2R_{opt}$范围内仍保持普适性，双归一化旋转曲线坍缩为单一轮廓。

Conclusion: 扩展的URC为理解重子物质与暗物质在星系旋转曲线中的相互作用提供了新见解，强化了URC范式，并为星系质量模型和暗物质本质研究提供了新约束。
...
TL;DR: 本文通过三个宇宙学模拟（FIREbox、NIVARIA-LG、Recal-EAGLE）研究了HI-rich无星暗物质晕（failed halos）的性质，发现不同模拟中的样本在HI-气体-晕质量平面上占据不同区域，并预测在本地宇宙的HI贫乏区域比高红移HI丰富区域更可能发现这类天体。


<details>
  <summary>Details</summary>
Motivation: 如果HI-rich无星暗物质晕确实存在，它们将为揭示暗物质晕的性质提供重要线索。本文旨在系统研究这类天体在不同宇宙学模拟中的性质差异及其成因。

Method: 对比分析三种先进宇宙学模拟（FIREbox、NIVARIA-LG、Recal-EAGLE）在红移零处的HI-rich failed halos样本，比较其与观测候选体Cloud-9的数值相似性，并分析它们在M_HI-M_gas-M_200质量平面上的分布特征。

Result: 不同模拟的failed halos样本在质量平面上分布各异：FIREbox范围狭窄，NIVARIA-LG覆盖更广的M_200范围且M_HI和M_gas更高，Recal-EAGLE的M_HI与FIREbox相似但气体和晕质量更低。与Cloud-9的差异可能源于环境因素或气体自屏蔽处理的差异。

Conclusion: 建议优先在本地宇宙的HI贫乏区域搜索HI-rich无星暗物质晕，而非高红移的HI丰富区域，并呼吁投入资源进行后续光谱观测以确认其是否真正缺少恒星成分及评估其隔离状态。
...
### [7] [The Critical Mass in Galaxy Evolution](https://arxiv.org/abs/2604.27477)
...
TL;DR: 利用Horizon Run 5模拟，发现星系临界质量（约10^12.5 M⊙）源于热气体晕的形成抑制冷气体流入，导致恒星形成效率下降，从而驱动恒星-总质量比的转折。


<details>
  <summary>Details</summary>
Motivation: 探究星系临界质量的物理起源，即许多星系性质和标度关系发生根本性转变的质量阈值。

Method: 使用Horizon Run 5宇宙学模拟，将恒星-总质量比分解为恒星-重子质量比和重子保留率，分析大质量中央星系的演化。

Result: 恒星-重子质量比变化超3倍，在10^12.4-12.7 M⊙达到峰值；重子保留率变化最多30%。临界质量10^12.5 M⊙与热气体晕形成相关，次临界质量约10^11 M⊙对应气体保留率峰值。

Conclusion: 临界质量由气体吸积性质的转变驱动：热气体晕抑制冷气体流入，降低原位恒星形成效率，导致恒星质量增长放缓，恒星-总质量比出现转折。
...
TL;DR: 黑洞吸积率低时，喷流和风共同作用才能有效产生AGN反馈，二者单独均不足以在椭圆星系中产生强湍流和抑制冷气体凝聚。


<details>
  <summary>Details</summary>
Motivation: 以往AGN反馈模型通常仅包含带自由参数的喷流，忽略了风的作用，且未解析邦迪半径以准确计算吸积率。

Method: 对孤立椭圆星系进行水动力学模拟，解析邦迪半径，使用GRMHD模拟参数，分别激活喷流和风以研究各自及联合效应。

Result: 只有喷流和风同时作用才能产生强湍流（通过开尔文-亥姆霍兹不稳定性），提高中心气体熵，抑制冷气体凝聚和恒星形成；单独作用均无效。湍流以螺线管型为主，能谱近似科尔莫戈罗夫幂律，耗散率约10^{-27} erg/cm³/s，与观测一致。

Conclusion: 研究强调同时考虑喷流和风对理解AGN反馈在椭圆星系演化中的作用至关重要。
...
TL;DR: 利用Gaia数据追踪16个低质量年轻星协的起源，发现其中12个与更大结构相关，揭示了气泡反馈、云碰撞等恒星形成机制，并指出低质量星协对理解星系恒星形成过程至关重要。


<details>
  <summary>Details</summary>
Motivation: Gaia探测到一批与大型星协或巨分子云脱离的低质量星协，但其恒星形成环境未知，需要揭示其起源和形成机制。

Method: 利用Gaia新速度、年龄数据与文献星表，对16个低质量星协（质量≲100 M☉，年龄≲50 Myr）进行动力学回溯追踪。

Result: 发现3组目前分散的星群共享形成位置；12个星协与更大复杂结构有联系，其中6个从多代恒星形成事件向外运动；解释了Local Bubble和Orion-Eridanus Bubble的形态及Sco-Cen、Orion OB1的形成；Leo星协可能由中间速度云与猎户座气体碰撞形成。

Conclusion: 最低质量的年轻星协尚未被发现，这些星群对揭示塑造星系恒星形成的小尺度气体过密区域具有关键作用。
...
### [10] [Coupling between stellar and HI lopsidedness in Milky Way-type galaxies from the Auriga Superstars cosmological simulations](https://arxiv.org/abs/2604.27057)
...
TL;DR: 本文利用Auriga Superstars宇宙学zoom-in模拟，分析了九个银河系类型星系中恒星和中性氢（HI）的不对称性（lopsidedness）。研究发现，老年恒星（>0.5Gyr）与HI在形态不对称性上强相关，反映了整体引力势的扭曲；而年轻恒星（<0.5Gyr）则追踪螺旋臂上的不对称恒星形成。恒星形态与运动学不对称性强相关，但HI的相关性较弱。强棒状星系往往具有更对称的盘和更高的中心恒星质量密度。潮汐相互作用（质量比>1:50）会同时诱发恒星和HI的不对称性，而平滑气体吸积主要影响HI和年轻恒星。


<details>
  <summary>Details</summary>
Motivation: 盘状星系中普遍存在不对称性，但其起源和演化仍不清楚。以往研究通常分别考察恒星和气体的不对称性，而联合分析能更有效地探测驱动不对称性的机制、近期星系演化以及星系环境的影响。

Method: 使用Auriga Superstars宇宙学zoom-in模拟中的九个银河系类型星系，利用高恒星质量分辨率降低噪声并增强盘面结构可见性。通过计算面密度分布和径向速度图的一阶傅里叶模式（m=1），在0.5到1个恒星光学半径范围内一致地量化恒星和气体的形态及运动学不对称性。

Result: 在z=0时，老年恒星与HI的形态不对称性强相关；年轻恒星则追踪螺旋臂上的不对称恒星形成。恒星形态与运动学不对称性强相关，而HI的相关性较弱且运动学不对称性占主导。恒星不对称性与棒强度呈反相关。潮汐相互作用（质量比>1:50）同时诱导恒星和HI的不对称性，而平滑气体吸积主要影响HI和年轻恒星。

Conclusion: 不对称性是研究内部盘演化、气体吸积以及跨宇宙时间环境相互作用的有效诊断工具。
...
### [11] [Cosmological Observational Tests in the JWST Era. II: The Tolman Test](https://arxiv.org/abs/2604.27867)
...
TL;DR: 本研究利用ASTRODEEP-JWST星表中的6,860个星系，测试了Tolman检验（星系表面亮度随红移的变化），发现表面亮度确实随距离增加而降低，但观测趋势与标准宇宙学模型预测的(1+z)^-4衰减规律存在显著偏离。


<details>
  <summary>Details</summary>
Motivation: 检验经典宇宙学测试——Tolman检验，即星系表面亮度与红移z的关系，以验证标准宇宙学模型的正确性。

Method: 使用ASTRODEEP-JWST测光星表中6,860个具有可靠光谱红移的星系，分析其平均表面亮度随红移的变化。

Result: (a) 星系平均表面亮度确实随距离增加而降低；(b) 观测趋势与标准宇宙学模型预测的~(1+z)^-4衰减规律存在显著偏离。

Conclusion: 观测结果不支持标准宇宙学模型对Tolman检验的预测，可能对现有宇宙学模型提出挑战。
...
### [12] [Primordial black hole dark matter from axion inflation](https://arxiv.org/abs/2604.27496)
...
TL;DR: 本文重新审视了通过赝标量-暴胀子耦合的U(1)规范场产生原初黑洞（PBH）的机制，发现即使暴胀子梯度能量密度高度次主导（仅为动能的10⁻⁴–10⁻³），PBH仍可解释全部小行星质量范围的暗物质，同时该机制不可避免地产生LISA可探测的随机引力波背景。


<details>
  <summary>Details</summary>
Motivation: 改进现有文献中关于PBH通过赝标量-暴胀子耦合产生的计算精度，纳入均匀反作用机制下的数值规范场模函数、最新的PBH丰度计算方法，以及密度扰动统计不确定性。

Method: 采用数值计算的规范场模函数在均匀反作用框架下工作，结合最先进的PBH丰度计算，并引入密度扰动（δρ）统计的不确定性。

Result: 在小行星质量范围内，PBH可构成全部暗物质，即使暴胀子梯度能量密度仅占动能的10⁻⁴–10⁻³；同时产生的随机引力波背景幅度可被LISA探测。

Conclusion: 该PBH产生机制在高度次主导的暴胀子梯度能量下依然有效，并提供了通过LISA探测的引力波信号来区分不同密度扰动统计的间接方法。
...
### [13] [Axion dark matter from extended misalignment with a constant-$ω_φ$ pre-oscillatory phase and dark radiation](https://arxiv.org/abs/2604.27954)
...
TL;DR: 本文扩展了轴子预振荡机制，引入具有恒定状态方程$ω_φ$的预振荡相，并考虑与暗辐射的耦合，通过贝叶斯分析对参数空间进行约束，发现该模型无法解决$H_0$和$S_8$张力，但给出了严格的参数限制。


<details>
  <summary>Details</summary>
Motivation: 扩展标准轴子预振荡错位机制，并探索是否可以通过轴子与暗辐射耦合来解决宇宙学中$H_0$和$S_8$的持续张力问题。

Method: 引入具有恒定状态方程$ω_φ\in[-1,1]$的预振荡相（由跟踪势产生），在辐射主导时期势能快速过渡到余弦势；考虑轴子与暗辐射耦合及衰变；使用贝叶斯分析方法结合当前宇宙学数据对参数空间进行约束。

Result: 轴子诱导的暗辐射无法解决$H_0$和$S_8$张力；数据倾向于$ω_φ$为负值；对称破缺标度$f_φ\in[80,1.5\times10^{10}]~\mathrm{TeV}$，对应轴子质量$m_φ\in[10^{-20},10^{-2}]~\mathrm{eV}$。

Conclusion: 该模型不能缓解现有的宇宙学张力，但当前数据对其参数空间施加了严格的上限和下限约束。
...
TL;DR: 本文利用天籁柱面阵列探路者（TCPA）的21厘米强度映射数据，结合无人机测量的高精度主波束模型与多尺度前景抑制策略（mPCA-UWTS），首次将球面傅里叶-贝塞尔（SFB）分解应用于观测数据，实现了三维功率谱的精确估计，为未来SKA和完整天籁阵列等大规模巡天提供了高效诊断工具。


<details>
  <summary>Details</summary>
Motivation: 21厘米强度映射中，天体物理前景信号比宇宙学信号强约五个数量级，传统平天近似存在偏差，亟需发展精确的前景抑制方法和严格处理宽场曲率效应的功率谱估计技术。

Method: 利用无人机实测高精度主波束模型进行天图重建；提出mPCA-UWTS策略（各向同性球面未抽样小波变换+独立主成分分析）进行多尺度前景减除；采用球面傅里叶-贝塞尔（SFB）分解估计三维功率谱，严格处理宽角和视线方向曲率效应。

Result: 无人机校准显著提升了重建精度；mPCA-UWTS有效抑制了强前景；SFB框架成功隔离了系统污染并恢复了成团信号，避免了平天近似引入的偏差。

Conclusion: 这是SFB形式主义首次应用于观测21厘米强度映射数据，证明其作为计算高效且可扩展的诊断工具的有效性，为下一代宽场21厘米巡天（如SKA和完整天籁阵列）奠定了方法基础。
