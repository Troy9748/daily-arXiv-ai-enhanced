...
### [1] [An Obscured Tidal Disruption Event Uncovered by Its Mid- and Near-Infrared Dust Echo in a Star-Forming Galaxy](https://arxiv.org/abs/2605.29616)
...
TL;DR: 本研究报告了星系SDSS J010320.39+140152.5中一次红外耀发事件，其峰值亮度达5.4×10⁴³ erg/s，持续约一年。通过近红外定位确认该事件发生在星系核（偏移≤100 pc），光谱能量分布符合约900 K的尘埃热辐射。模型推断峰值紫外光度约(4-10)×10⁴⁴ erg/s，总释放能量约(0.9-2)×10⁵² erg。研究排除了超新星可能性，认为该红外耀发起源于尘埃遮蔽的潮汐瓦解事件（TDE），而非变脸活动星系核。该事件代表了近年来新兴的尘埃遮蔽TDE类别中的一个典型案例，这类事件被光学巡天遗漏，部分解释了TDE宿主星系观测偏差，是全面理解TDE群体中关键但常被忽视的组成部分。


<details>
  <summary>Details</summary>
Motivation: 光学巡天对TDE的观测存在宿主星系偏差，许多TDE因尘埃遮蔽而未被探测到。本研究旨在通过红外波段发现并分析这类被遗漏的TDE，以更全面地理解TDE群体。

Method: 从MIRONG样本中选取中红外爆发现象，进行近红外定位观测、光谱能量分布（SED）分析，并利用尘埃辐射转移模型推断紫外光度和总释放能量。

Result: 红外耀发峰值光度为5.4×10⁴³ erg/s，持续约一年，与星系核位置重合（偏移≤100 pc），尘埃温度约900 K，峰值紫外光度约(4-10)×10⁴⁴ erg/s，总能量约(0.9-2)×10⁵² erg。

Conclusion: 该红外耀发更可能源于尘埃遮蔽的TDE而非变脸AGN，代表了被光学巡天遗漏的TDE类别的重要案例，有助于解释TDE宿主星系观测偏差。
...
### [2] [Compact near infrared sources in the center of the extraordinary galaxy IC 860](https://arxiv.org/abs/2605.29149)
...
TL;DR: IC 860是一个极端气体聚集的明亮红外星系，核心存在致密遮蔽核(CON)。HST图像分析发现其中心分子区(CMZ)包含两个致密明亮天体：一个是新确认的CON（遮蔽中心源），另一个是偏移约0.2角秒的IC860-a（可能是大质量年轻星团或入侵核），两者共存揭示了该星系不寻常的演化历史。


<details>
  <summary>Details</summary>
Motivation: IC 860因分子气体高度集中在中心、存在致密遮蔽核(CON)而具有特殊研究价值。此前近红外探测到发光致密核源，但CON的结构、性质及与CMZ的关系尚不明确，因此本研究旨在解析CMZ的物理特性及其与CON的关联。

Method: 利用HST图像研究IC 860中心区域结构；基于Gaia位置校正HST图像坐标以确定IC860-a坐标；对HST图像进行测光获取星等、颜色及V波段尘埃光学深度（采用前景屏幕尘埃模型）；利用消光校正测光获得IC860-a和CMZ的近红外光度。

Result: IC 860存在多个致密中心光度源：大部分近红外光度来自CMZ，而LIRG-CON是位于CMZ中心的遮蔽区域。IC860-a位于CMZ东北侧，与CON偏移约0.2角秒，光度约1E9 Lsun，可能为大质量年轻星团或入侵核。CON首次被确认为遮蔽的中心源。

Conclusion: IC 860内部CMZ包含两个致密明亮天体：CON和IC860-a。两者的共存是IC 860富含气体的CMZ非寻常演化的进一步标志。
...
### [3] [Modeling Solitonic Cores, Stabilization of Bar, and Suppression of Bar Dissolution in DDO 168 via GPP Formalism: A Detailed Analysis of Bose--Einstein Condensate/Fuzzy Dark Matter Halo Structure and Bar Dynamics in the Dwarf Galaxy DDO 168](https://arxiv.org/abs/2605.29314)
...
TL;DR: 本文在BEC/FDM（超轻玻色子暗物质）框架下模拟了矮不规则星系DDO 168，发现其中心平坦的旋转曲线与轴子质量约1.3×10⁻²³ eV的孤子核一致，有效缓解了冷暗物质模型的尖点-核问题，并支持弱棒结构的长期存在。


<details>
  <summary>Details</summary>
Motivation: 解决ΛCDM模型中的尖点-核问题（cusp-core problem），即观测到的矮星系中心密度平坦，与N体模拟预测的陡峭尖点不符。

Method: 基于Bose-Einstein凝聚体/模糊暗物质框架，求解Gross-Pitaevskii-Poisson方程的基态解，利用SPARC数据拟合DDO 168的内旋曲线，并验证孤子质量-半径关系。

Result: 轴子质量m = (1.3⁺⁰·³₋₀·₂)×10⁻²³ eV；孤子核特征半径Rc = 2.40⁺⁰·²⁴₋₀·²² kpc；2.47 kpc内质量约(1.5±0.2)×10⁹ M☉；成功再现平坦内旋曲线，弱H I棒可在降低的Chandrasekhar动力学摩擦下存活。

Conclusion: BEC/FDM框架为DDO 168提供了内部自洽的描述，同时再现了观测旋曲线、缓解了尖点-核矛盾，并在保守动力学假设下允许长期存在的弱棒结构。
...
### [4] [Equilibrium Core and Vortex Solutions of Bose Einstein Condensate Dark Matter around a Black Hole](https://arxiv.org/abs/2605.29069)
...
TL;DR: 本文构建了黑洞周围玻色-爱因斯坦凝聚暗物质(BECDM)的稳态解，包括基态核心解和涡旋解，分析了自相互作用和黑洞质量对密度分布及稳定性的影响，发现吸引性自相互作用下存在最大质量，且稳定涡旋解可能在物理相关形成场景中存在。


<details>
  <summary>Details</summary>
Motivation: 研究黑洞周围BECDM的稳态构型，探索不同参数下的密度分布和稳定性，特别是吸引性自相互作用和涡旋解的存在性与物理可行性。

Method: 采用虚时方法求解轴对称稳态方程，构建从基态核心解到一阶非零缠绕数涡旋解的系列解族，并通过基于焓泛函的转向点判据评估解的稳定性。

Result: 成功构建了宽泛参数范围内的BECDM稳态解族；发现吸引性自相互作用(g<0)下存在最大质量阈值；识别出稳定与不稳定分支；证明稳定涡旋解可在黑洞周围存在。

Conclusion: 黑洞周围BECDM基态核心解在非相互作用情况下可作为吸引子，而吸引性自相互作用下的最大质量约束可推断参数范围；稳定涡旋解的存在表明其可能与物理形成场景兼容。
...
TL;DR: 本文论证了在2030年代扩大HST/COS紫外光谱观测的必要性，以研究红移z=1-2类星体的极紫外波段，为未来Habitable Worlds Observatory提供关键数据基础。


<details>
  <summary>Details</summary>
Motivation: HST是当前唯一的高分辨率紫外光谱设施，但其探测器灵敏度正在下降，同时新发现的类星体数量快速增长（Milliquas、UVQS以及即将到来的Rubin、Roman、Euclid巡天），观测窗口正在快速收窄。需要抓住时机开展对z>1类星体的极紫外（EUV）光谱观测。

Method: 利用HST/COS的远紫外波段对红移z=1-2的中间红移类星体的静止系极紫外（1-4 Ryd, 228-912 Å）吸收线谱和连续谱进行观测，通过红移效应将EUV光子移至COS可观测波段。

Result: 扩大HST紫外轨道分配将带来暖热CGM/IGM科学的阶跃式进展，并产出首个系统性的类星体极紫外能谱分布（EUV SED）普查数据集。

Conclusion: 这些数据将成为HWO科学的基础低红移锚点，建议在2030年代扩大HST/COS对中间红移AGN静止系极紫外的光谱数据库。
...
### [6] [Too many protoclusters? Reconciling the overabundance of cluster progenitors within the first billion years of the Universe](https://arxiv.org/abs/2605.28930)
...
TL;DR: JWST观测发现高红移原星系团候选体数量远超ΛCDM预测，本文通过修正观测与模拟之间的不一致性（如晕质量定义、演化追踪方式），完全消除了这一矛盾，发现64%的候选体实际是原星系群，没有候选体能演变为类似Coma的超大星系团。


<details>
  <summary>Details</summary>
Motivation: JWST揭示了z>5处原星系团候选体数量远超ΛCDM理论预测（超出约2-300倍，存在2.3σ-4σ的张力学），亟需解决这一观测与理论之间的严重矛盾。

Method: 分析文献中5<z<9的原星系团候选体，与TNG-Cluster和TNG300模拟对比，修正了观测质量估计（在>50倍virial半径孔径内累积恒星质量）与模拟（仅virial半径内晕质量）的不一致性，改用前向追踪而非后向追踪的演化方法，并考虑了合并候选体和宇宙方差。

Result: 修正不一致后完全消除了张力学；64%候选体实为原星系群（M200c<10^14 M⊙），无一能演变为Coma-like星系团（M200c>10^15 M⊙）；这些超密区域仍是宇宙网极端节点，可能驱动早期电离区和宇宙恒星形成率密度。

Conclusion: 需要更谨慎地对比观测与模拟的高红移原星系团，建议使用拉格朗日尺度上晕质量总和作为改进的选择标准。
...
TL;DR: 本文利用MUSE深度观测研究了旋涡星系W2246f（红移z~0.09）的空间分辨恒星与电离气体性质，发现其中心区域以年老、贫金属恒星为主且无恒星形成（退休区），而盘区仍在进行恒星形成，符合由内向外熄灭（inside-out quenching）的演化图景，是一个典型的cLIER星系。


<details>
  <summary>Details</summary>
Motivation: 在大样本巡天时代，单个星系的细节之美往往被忽视。本文旨在通过超深度的MUSE观测，以优于常规IFU巡天的空间分辨率，深入理解W2246f星系的形成与演化过程。

Method: 利用MUSE光谱数据对该旋涡星系进行空间分辨分析，包括：恒星和气体运动学、恒星族群的星族合成（年龄、金属丰度径向轮廓）、电离气体性质（消光、Hα通量、氧丰度O3N2校准、Hα恒星形成率），以及使用WHAN和WHaD图诊断电离源。

Result: 发现负的恒星金属丰度梯度；质量加权年龄在盘面均匀平坦，而光度加权年龄呈负梯度；星系中心区域气体金属丰度、恒星形成率密度下降，且存在年老的光度加权星族；WHAN/WHaD图显示中心为退休区，盘区为恒星形成区。

Conclusion: W2246f是一个典型的cLIER星系：中心千秒差距由年老、贫金属恒星主导，LIER辐射主要来自热演化星，而盘区仍在形成恒星。该结果支持由内向外熄灭（inside-out quenching）的演化场景。
...
### [8] [High-S/N Quasar Observations with HST/COS: Deep Fields for Spectroscopy](https://arxiv.org/abs/2605.29949)
...
TL;DR: 本文提出利用HST/COS对约20个QSO进行深紫外光谱巡天（信噪比>30，分辨率20 km/s），以产生类似哈勃深场（HDF/UDF）的遗产数据集，推动星系周介质（CGM）、星际介质（IGM）、银河系弥散气体及AGN外流等前沿科学研究，并为未来Habitable Worlds Observatory的紫外科学铺路。


<details>
  <summary>Details</summary>
Motivation: 当前MAST档案中缺乏高信噪比的紫外光谱数据，而哈勃望远镜仍具备出色的紫外观测能力；现有深紫外光谱的缺失限制了CGM/IGM化学丰度、电离状态、重子及金属预算等前沿研究，亟需类似哈勃深场（HDF/UDF）的遗产级紫外光谱数据集。

Method: 利用HST/COS对0.5<z<1.5的约20个QSO进行深度紫外光谱观测，目标信噪比>30，分辨率20 km/s，生成高灵敏度紫外遗产数据集。

Result: 可达到H I柱密度log N=12.6的极低探测灵敏度，覆盖多种紫外金属谱线，可探测金属丰度低至[Z/H]≈-2的介质；能对CGM/IGM、银河系及本星系群的高速云和气体流、以及AGN外流的极紫外辐射区进行前所未有的精细研究。

Conclusion: 该深紫外光谱巡天将产生具有长远影响力的遗产数据集，弥补当前紫外光谱领域的关键空白，支撑多领域前沿科学，并为Habitable Worlds Observatory的未来紫外观测提供技术和方法上的先导。
...
TL;DR: 利用HST slitless grism光谱观测了3个z~2.3的原星系团中42个恒星形成星系的空间分辨气体金属丰度梯度，发现约69%的成员星系呈现正（倒转）金属丰度梯度，远高于场星系。研究表明这些正梯度与相对于场质量-金属丰度关系而言贫金属的星系强烈相关，提示原星系团环境中大量纯净气体向星系中心区域的增强内流稀释了中心金属丰度，导致观测到的倒转梯度。


<details>
  <summary>Details</summary>
Motivation: 在宇宙正午时期，高密度环境（overdensities）对星系内部化学结构的影响仍存在争议，缺乏直接的观测证据。

Method: 利用HST slitless grism光谱从MAMMOTH-Grism巡天中获取42个z~2.3原星系团中恒星形成星系的空间分辨气体金属丰度梯度，并与场星系进行对比。

Result: 1) 69%的原星系团成员星系呈现正（倒转）金属丰度梯度，比例显著高于场星系；2) 正梯度与星系贫金属性质强相关，尤其在大质量星系(log(M*/M⊙)>9.95)中更为显著。

Conclusion: 密原型星系团环境中的星系经历了增强的纯净气体向中心区域的内流，稀释了中心金属丰度并产生倒转梯度，表明环境效应在宇宙恒星形成峰值时期主动调节气体吸积和化学再分布。
...
### [10] [A New Cartwheel-like Collisional Ring Galaxy](https://arxiv.org/abs/2605.29618)
...
TL;DR: 发现了一个新的卡通车式碰撞环星系PGC 1112751，命名为“Eridanus Wheel”（EW），它由星系正面碰撞形成，具有内外环和辐条结构，与著名卡通车星系相似。


<details>
  <summary>Details</summary>
Motivation: 碰撞环星系作为研究恒星形成、密度波传播以及星系盘对外部强扰动响应的理想实验室，发现此类天体具有重要的科学价值。

Method: 通过对DESI Legacy成像巡天数据进行系统视觉检查，发现并确认了该星系；利用griz滤光片进行光度学研究，分析其形态和颜色特征。

Result: EW在红移z=0.0856处，具有清晰的内环和弥散外环，环间有“辐条”结构；与可能的闯入者星系之间有暗淡光学桥，投影距离约60 kpc；它是一个具有强径向颜色梯度的巨晚型星系。

Conclusion: EW的形态最可能由与一个早型星系的近期正面碰撞解释，其进一步演化将很可能形成具有低表面亮度盘的星系。
...
TL;DR: 利用DESI-eRASS1数据，研究了1171个X射线AGN和45374个非AGN星系在恒星形成主序上的性质，发现AGN与恒星形成的关系受宿主星系形态（盘状vs球状）和宇宙红移的调控。


<details>
  <summary>Details</summary>
Motivation: 恒星形成与超大质量黑洞增长之间的相互作用是星系演化的核心问题，但宿主星系形态如何调控恒星形成增强和AGN触发尚不清楚。

Method: 基于DESI-eRASS1数据集，选取1171个X射线AGN和45374个非AGN恒星形成星系（z≤1.5），通过匹配控制样本量化AGN宿主中的恒星形成（SFR_norm），并分析其与X射线光度（L_X）和比吸积率（λ_sBHAR）的关系；利用Sersic指数区分盘主导和球主导系统，测量AGN发生率随主序偏移量（Δ_MS）的变化。

Result: SFR_norm在低到中等L_X时接近1，在高L_X时增加，且该转变向高质量系统偏移；此趋势依赖形态：盘主导AGN在中等L_X下SFR增强，球主导则保持为1；AGN发生率随Δ_MS增加而显著上升，高红移时更陡峭，且该关系也依赖形态和红移。

Conclusion: 恒星形成与AGN活动的联系受全局气体可用性控制，但被宿主星系结构和宇宙学时代所调制；AGN的绝对功率比按恒星质量归一化的吸积效率与宿主星系的恒星形成联系更紧密。
...
### [12] [A Designer's Guide to Lunar Far-Side Interferometer Array: Power Spectrum Measurement and Cosmological Constraints from the Dark Ages](https://arxiv.org/abs/2605.29276)
...
TL;DR: 本文对月球远侧干涉阵列探测宇宙黑暗时代21厘米信号进行了定量预测，证明了通过优化阵列配置可将信噪比提升两个数量级，并需约3万个傅里叶模式才能实现与Planck 2018相当的暴胀模型约束能力，同时揭示了热噪声对高红移和小尺度观测的严重限制。


<details>
  <summary>Details</summary>
Motivation: 利用宇宙黑暗时代中性氢的21厘米发射线作为宇宙学模型和早期宇宙物理的探测工具，特别是约束暴胀模型中的谱指数运行α_s，因为黑暗时代线性微扰理论在更小尺度上仍然有效，可提供超越当前大尺度结构和CMB巡天的独立探测途径。

Method: 首先通过量化小质量暗物质晕对21厘米信号的贡献来验证线性假设的有效性；然后建立适用于任意数量台站或子阵列的干涉仪基线密度分布的广义解析框架；结合实际噪声模型，确定达到探测阈值所需的配置，并分析总收集面积在不同台站间的分布对信噪比的影响。

Result: 将总收集面积分布到多个台站可将功率谱的信噪比在可调的小尺度上提升最多两个数量级；月球阵列需要约30,000个探测傅里叶模式才能实现σ(α_s)=0.034的暴胀约束，该结果与Planck 2018结果相当；热噪声在高红移和小尺度上严重侵蚀模式，这一限制此前在文献中未被充分认识。

Conclusion: 月球远侧干涉阵列有潜力成为约束暴胀模型的新独立探测手段，尽管热噪声对高红移和小尺度观测构成显著限制，但适当优化配置仍可达到与CMB观测相竞争的能力。
...
### [13] [Model Independent Probe of Variation of Cosmic Opacity with Redshift](https://arxiv.org/abs/2605.29805)
*Savita Gahlaut,Meetu Luthra*

Main category: astro-ph.CO

Recommendation: ★★★☆☆ (60/100) - 与 Zotero 兴趣画像中的 lensing, dust 等主题/作者匹配。

TL;DR: 本文利用强引力透镜数据和Ia型超新星Pantheon+样本，通过模型无关方法研究了宇宙不透明度随红移的变化。平均而言宇宙是透明的，但在某些红移区间（如0.3<z≤0.4）存在显著不透明度偏差。


<details>
  <summary>Details</summary>
Motivation: 宇宙不透明度可能因尘埃分布、颗粒性质及光子衰减效率的不均匀性而存在空间变化，现有方法多依赖特定模型，本文旨在提出一种模型无关的方法来研究宇宙不透明度随红移的变化。

Method: 利用强引力透镜数据构建与不透明度无关的共动距离函数，结合超新星Ia Pantheon+数据估计依赖不透明度的共动距离，通过距离对偶关系约束不透明度参数。

Result: 在Pantheon+样本的红移范围（0.01≤z≤2.26137）内，宇宙平均是透明的。但在0.3<z≤0.4红移区间，不透明度参数ε=-0.4283^{+0.1914}_{-0.2027}，存在显著偏差。

Conclusion: 当前超新星Ia观测表明不透明度参数随红移变化，这一结果可能对从超新星观测推导的宇宙学参数值产生重要影响。
...
### [14] [The observer power spectrum for lightcone statistics, integrated relativistic observables and wide angle effects](https://arxiv.org/abs/2605.29806)
...
TL;DR: 本文提出了一种新的观测者功率谱定义，通过在固定光锥坐标的空间超曲面上对观测者位置进行傅里叶变换，使得任何可观测量的统计谱都是对角的，避免了传统方法中的模式混合问题。


<details>
  <summary>Details</summary>
Motivation: 传统傅里叶空间中的功率谱由于广角效应、径向演化和透镜等积分效应破坏了平移不变性，导致非对角化的模式混合。作者希望定义一种更自然的功率谱，能够保持统计均匀性，使谱保持对角形式。

Method: 在固定光锥坐标的空间超曲面上对观测者位置进行傅里叶变换，而不是在单一光锥上对源位置进行变换。这样形成的场具有可移动的光线腿，观测者超曲面的统计均匀性保证了谱的对角性。

Result: 证明了该观测者功率谱对于任何可观测星（无论是局部的还是积分的）都是对角的，不会受到模式混合的影响；并展示了如何从该谱投影恢复大尺度结构分析中使用的各种两点统计量。

Conclusion: 该方法可直接扩展到高阶统计量，并可用于构建观测星系计数涨落的相对论性核，包括密度、红移空间畸变、多普勒效应、透镜放大和ISW效应等贡献。
...
### [15] [Late-time Quantum Vacuum Decay and its Cosmological Implications](https://arxiv.org/abs/2605.30259)
...
TL;DR: 本文研究了宇宙晚期量子真空衰变对宇宙学观测的影响，发现当前数据允许真空能量最多减少50%（z_t<1），并存在一个约z_t~7、涉及10%暗物质转换的最佳模型可缓解与ΛCDM的张力，但CMB各向异性对慢速或稀疏的晚期相变施加了强约束。


<details>
  <summary>Details</summary>
Motivation: 亚稳态真空景观的存在意味着我们的宇宙可能在晚期经历过量子真空衰变，但目前缺乏对这一现象的系统观测检验，因此需要探索如何用宇宙学观测来约束晚期量子隧穿过程。

Method: 构建了一组唯象模型，其中晚期量子隧穿改变真空能，并将暗物质子组分转化为暗辐射（可能伴随畴壁产生）；利用DESI DR2重子声学振荡数据、DES-Dovekie/Pantheon+/Union3超新星距离测量以及压缩CMB似然函数进行约束。

Result: 当前距离测量允许真空能量在z_t<1时最多降低50%；暗物质转换+畴壁模型可缓解与ΛCDM的张力，最佳拟合在z_t~7且约10%暗物质参与相变；CMB各向异性强约束慢速晚期相变，最小模型中真空能降低约10%（z_t量级为1），非最小模型可通过密度依赖隧穿规避约束。

Conclusion: 晚期量子真空衰变是一个可检验的宇宙学现象，本文为景观场景下的亚稳态真空物理提供了具体的观测约束手段。
...
### [16] [Dominated-Convergence Failure in Cosmological Perturbation Theory and a Numerical Foundation for BBGKY+ZA](https://arxiv.org/abs/2605.28943)
*Svetlin V. Tassev*
...
TL;DR: 本文揭示了宇宙学扰动论中密度场展开的勒贝格控制收敛定理障碍，并提出了基于BBGKY层次和不展开位移的替代方案。


<details>
  <summary>Details</summary>
Motivation: 标准微扰论展开在特定条件下发散，其原因并非展开本身，而是无穷求和与傅里叶积分交换违背了勒贝格控制收敛定理。有效场论需要平滑短尺度来恢复该问题，作者希望从根本上解决这一障碍。

Method: 从数学上证明扰动展开中的DCO问题，并对比EPT和LPT中该问题的表现。提出不展开密度场、改用BBGKY层次演化的方法，利用Zel'dovich近似初始化并闭合系统，数值积分了ZA相空间两点函数。

Result: 明确了DCO是EPT收敛受平流项控制的根本原因，也是LPT有效区域受限的根源。成功数值积出了ZA相空间两点函数，为BBGKY+ZA方案提供了构建模块。

Conclusion: BBGKY+ZA方法可从根本上避免DCO，使EFTs专注于质量守恒以外的物理，有望将微扰论推进到壳交叉之后。
