...
### [1] [High-Resolution ALMA Imaging for a Gravitationally-lensed Quasar at $z=6.5$: Constraining the AGN Contribution to Galactic-Scale Dust Heating](https://arxiv.org/abs/2606.11084)
...
### [2] [Molecular Gas Structure and Star Formation Diversity in Stephan's Quintet Revealed by ACA CO(1-0) Mapping](https://arxiv.org/abs/2606.10557)
...
### [3] [Aether-SHELLQs: JWST integral-field spectroscopy of candidate obscured quasars at z ~ 6](https://arxiv.org/abs/2606.10160)
...
TL;DR: 利用JWST NIRSpec IFU观测6个z~6的星系，发现两个有宽Balmer线（FWHM>3000 km/s）的AGN，四个仅有窄线。宽线出现与Lyα光度强相关（>10^44 erg/s时AGN占比>77%，低于时<15%）。AGN宿主紧凑，其他星系有延展电离气体（~10 kpc）和高恒星形成率（60-600 M⊙/yr），其中仅一个显示旋转特征。这些天体填补了经典类星体与JWST发现的低光度AGN（如小红点）之间的空白。


<details>
  <summary>Details</summary>
Motivation: 为了研究z~6高红移宇宙中处于中等光度区间的AGN和星系的性质，填补经典亮类星体与JWST新发现的低光度AGN（如小红点）之间的观测空白。

Method: 使用JWST NIRSpec IFU对6个SHELLQs巡天选出的候选遮蔽类星体进行光谱成像观测，分析Balmer线和禁线的线型、Lyα光度、尘埃消光修正、空间结构和运动学特征。

Result: 发现2个有宽Balmer线的AGN和4个仅有窄线的星系；宽线出现与Lyα光度强相关，高于10^44 erg/s时AGN比例>77%，低于时<15%；尘埃消光修正导致Lyα光度不合理，表明多区气体；AGN宿主紧凑，其他星系延展至10 kpc，恒星形成率60-600 M⊙/yr；仅一个显示旋转运动学。

Conclusion: 这些中等光度天体连接了经典亮类星体与JWST发现的低光度AGN（如小红点）两个种群，对理解高红移黑洞增长和星系演化具有重要意义。
...
### [4] [Learning the Universe: The Structure of Dust Attenuation Curves in Galaxy Simulations](https://arxiv.org/abs/2606.10027)
...
TL;DR: 本文通过TNG50/TNG100模拟和SKIRT辐射传输代码，发现灰尘消光曲线需要4个参数来描述，并利用符号回归推导出一个新的四参数消光模型，该模型优于现有参数化方法，且参数具有明确物理意义。


<details>
  <summary>Details</summary>
Motivation: 灰尘消光是星系SED拟合和前向建模中系统不确定性的主要来源，但消光曲线的函数形式缺乏系统性研究，特别是究竟需要多少自由参数以及哪种解析表达式最能捕捉消光曲线多样性。

Method: 使用TNG50/TNG100星系合成消光曲线库，结合SKIRT辐射传输代码和三种尘埃混合物（银河系、SMC、恒星尘埃），通过信息有序瓶颈分析确定参数数量，再使用符号回归推导新模型。

Result: 发现恰好需要4个参数来捕捉消光曲线多样性；新四参数模型优于现有参数化方法；四个参数分别对应UV bump强度、FUV斜率、UV-bump过渡曲率和光学波段大尺度斜率；参数与星系性质的相关性主要由恒星形成率面密度、金属丰度和尘埃几何结构决定。

Conclusion: 提供了将四个参数与准可观测星系性质关联的符号回归标度关系，为SED拟合和前向建模提供了无需辐射传输计算的物理驱动方法。
...
### [5] [Formation of Parallel Stellar Streams through Encounters with Dark Matter Subhalos and Intermediate-Mass Black Holes](https://arxiv.org/abs/2606.10859)
...
TL;DR: 本文提出暗物质子晕和游走的中等质量黑洞等暗弱扰动体会在恒星流中留下平行结构的动力学印记，通过解析建模和N体模拟验证了单条恒星流与暗扰动体相遇后可分裂为两条平行流，并讨论了如何与其他形成过程区分。


<details>
  <summary>Details</summary>
Motivation: 暗物质子晕和游走的中等质量黑洞难以通过电磁观测直接探测，但了解其丰度对理解星系形成与演化至关重要，现有方法缺乏有效探测手段。

Method: 采用解析建模和N体模拟方法，研究恒星流与暗扰动体（暗物质子晕、中等质量黑洞）的相互作用过程。

Result: 单条恒星流与暗扰动体相遇后可分裂为两条平行结构，暗扰动体可产生垂直于恒星流延伸方向的密度耗损，形成平行恒星流形态而非传统的间隙状特征。

Conclusion: 平行恒星流可作为暗扰动体的动力学印记，为探测暗物质子晕和游走中等质量黑洞提供新途径，拓展了流-子晕相互作用的理论图景。
...
### [6] [Learning the Universe at High Redshifts: Impact of Accretion Modeling on Early Black Hole Growth](https://arxiv.org/abs/2606.10036)
...
TL;DR: JWST发现的高红移超大质量黑洞对现有理论提出挑战。本研究比较了三种吸积模型（Bondi-Hoyle模型和两种自由落体模型）对早期黑洞增长的影响，发现Bondi吸积在大质量种子下易产生失控增长，而自由落体模型对低质量种子更有效。模拟表明，通过丰富的重种子形成或弱反馈的Bondi吸积，可在z≳9时组装约10^6-10^7 M⊙的黑洞。


<details>
  <summary>Details</summary>
Motivation: JWST观测发现早期宇宙（z≳9）存在超大质量黑洞（>10^6 M⊙），这对现有的黑洞种子形成和吸积模型构成挑战，需要研究不同吸积模型能否解释这些观测。

Method: 使用宇宙学模拟，比较三种吸积模型：Bondi-Hoyle模型（Ṁ∝M²）和两种自由落体模型（Ṁ∝M^{1/2}和Ṁ∝M），研究不同种子质量和反馈强度下黑洞的增长情况。

Result: Bondi吸积对重种子（~10^5 M⊙）产生更强的失控增长，但对声速敏感易被反馈抑制；自由落体模型对低质量种子（10^{3-4} M⊙）更有效但增长有限；黑洞合并对Bondi模型有更强促进作用。

Conclusion: 模拟可在z≳9时组装约10^6-10^7 M⊙的黑洞，需要两种条件之一：丰富的重种子形成驱动黑洞合并，或采用弱反馈的Bondi吸积模型。
...
### [7] [From Dense Gas Clouds to Supermassive Black Hole Seeds: Hybrid Hydro/Direct $N$-body Simulations of Runaway Collision-driven Intermediate-mass Black Hole Formation](https://arxiv.org/abs/2606.10000)
...
TL;DR: 本文利用自洽的星团尺度模拟（Enzo-Abyss代码），发现高红移环境中致密金属贫乏气体云可通过失控碰撞形成质量343-5108 M☉的极重恒星（VMS），其直接塌缩产生中等质量黑洞（IMBH），并通过爱丁顿限制气体吸积和潮汐瓦解事件（TDEs）在100 Myr内增长至约62000 M☉，为高红移超大质量黑洞（SMBH）种子提供了可行形成路径。


<details>
  <summary>Details</summary>
Motivation: JWST在高红移发现了大量致密恒星系统，但超大质量黑洞种子如何在这些环境中形成仍不清楚。本文旨在不引入特设播种机制的前提下，研究致密恒星系统中SMBH种子的自然形成过程。

Method: 使用升级版流体动力学代码Enzo-Abyss，结合直接N体引力方法和恒星演化，模拟初始致密、金属贫乏气体云在不同湍流条件下的演化，追踪从气体云到VMS形成、塌缩为IMBH及其后续增长的全过程。

Result: 1) 形成了类似早期核星团（NSC）的致密星团；2) 通过失控碰撞形成了343-5108 M☉的极重恒星（VMS），且不依赖于恒星风反馈强度；3) VMS直接塌缩产生IMBH后，通过气体吸积和TDEs增长；4) 最乐观模型中，质量吸积率为1.64×10⁻⁴ M☉/yr，TDEs贡献23%；5) 初始6747 M☉的IMBH可在100 Myr内增长至约62000 M☉。

Conclusion: 在单一自洽框架下，本数值研究表明VMS可在致密气体云中形成并塌缩为IMBH，这些IMBH能够作为高红移观测到的SMBH的可行种子，无需引入特设的播种机制。
...
### [8] [Through the Veil: Ly$α$ Illuminates the Host Galaxies of Little Red Dots](https://arxiv.org/abs/2606.09970)
...
TL;DR: 本文研究了JWST发现的"小红点(LRDs)"中的Lyα发射，通过110个z≥4的样本发现Lyα主要来自宿主星系而非致密核区，支持LRDs的双组分结构模型。


<details>
  <summary>Details</summary>
Motivation: LRDs是JWST深场中发现的一类神秘致密红色源，其物理本质尚不清楚。Lyα作为中性氢的灵敏示踪剂，可用于探测LRDs中可能存在的特殊气体结构，从而揭示其本质。

Method: 基于110个光谱确认的z≥4 LRDs样本（均含NIRSpec/PRISM Lyα覆盖），检测到32个具有显著Lyα发射（S/N≥3）的源。对其中13个z≥5.5的LRDs，利用HST/ACS或JWST/NIRCam宽带成像构建了扣除连续谱后的Lyα空间分布图。

Result: (1) Lyα光度与等值宽度分布与正常恒星形成星系一致，但Lyα/Hα比值系统偏低；(2) Lyα光度与[O III]光度的相关性更强，而非[O III]等值宽度；(3) Lyα发射在空间上呈延展、不对称且相对rest-optical光有偏移的特征。

Conclusion: 结果支持LRDs的双组分模型：致密rest-optical源嵌入在更延展的宿主星系环境中，星际介质和周际介质气体塑造了Lyα的逃逸和空间再分布。Lyα为理解LRD中致密红组分、宿主星系和周围气体的关系提供了新视角。
...
### [9] [On Cross-Correlating Line Intensity Maps from SPHEREx during Reionization](https://arxiv.org/abs/2606.10005)
...
TL;DR: 本文模拟了SPHEREx在宇宙再电离时期对Lyα、Hα、Hβ、[OII]和[OIII]谱线的强度成图观测，重点通过不同谱线间的互相关分析来消除低红移干扰线的影响。研究发现Hα与[OIII]在z=5时互相关信噪比最高可达99，但大部分信号来自明亮星系而非暗弱星系，且探测主要受仪器噪声限制。


<details>
  <summary>Details</summary>
Motivation: 宇宙再电离时期是星系形成和宇宙演化的关键阶段，但直接探测该时期的暗弱星系非常困难。强度成图技术可以通过统计测量总辐射来探测低于直接探测阈值的星系群体，但面临低红移干扰线的严重污染问题。本研究旨在评估SPHEREx通过多谱线互相关来克服这一挑战的可行性和预期观测效果。

Method: 模拟了包含所有重要辐射源的Lyα、Hα、Hβ、[OII]和[OIII]强度图，对Lyα进行了辐射转移处理，并基于z<5的星系观测构建了简单的尘埃消光模型。通过不同谱线间的互相关分析消除低红移干扰线污染，并模拟了干扰线的建模和明亮干扰星系的掩蔽处理。

Result: 在无尘埃消光条件下，Hα与[OIII]在z=5时互相关总信噪比最高达99；大多数尺度上的主要噪声来源是仪器噪声，仅Lyα和[OII]在最大尺度上受干扰线主导；强度成图可探测质量低于4×10¹⁰ M☉的星系，但可观测信号主要来自明亮星系；在z=5的Hα×[OIII]中仅获得聚类信号的边缘探测。

Conclusion: SPHEREx的多谱线互相关强度成图在宇宙再电离时期具有探测潜力，但受限于仪器灵敏度和明亮星系的主导贡献。更高红移（z>6）或其它谱线的聚类信号探测需要更灵敏的仪器，如Cosmic Dawn Intensity Mapper。
...
### [10] [Individual Star Sampling in Star Formation Simulations: A Semi-Deterministic Model](https://arxiv.org/abs/2606.09999)
...
TL;DR: 本文提出了一种半确定性(SDT)方案来模拟恒星形成过程中的初始质量函数(IMF)采样，通过引入储层粒子(RsvPs)和团簇实时识别算法，相比现有方法能更好地复现观测到的最大恒星质量-星团质量关系，并揭示IMF斜率随恒星形成率的变化。


<details>
  <summary>Details</summary>
Motivation: 现有模拟通常使用固定不变的IMF来表示恒星群体，但恒星质量实际上受局部环境影响，且这些过程在模拟中无法分辨。作者希望开发一种能根据局域环境动态采样IMF的方法。

Method: 提出半确定性(SDT)采样方案：用储层粒子(RsvPs)代表未分辨的分子核和原行星盘，采用实时朋友的朋友(FOF)算法识别星团，新生恒星的瞬时IMF由当前星团质量导出。

Result: SDT方案自然复现了观测到的m⋆,max-Mecl关系，与最优采样理论一致的大质量恒星数量，运行间变化最小；导致大质量恒星出现约0.15 Myr的相干时间延迟，减少泊松噪声带来的大散射，产生初始质量分层；在星系尺度上，低SFR时IMF高质端斜率更陡，斜率与SFR负相关。

Conclusion: SDT方案能更真实地模拟恒星形成过程，并且由于大质量恒星丰度下降，基于Hα的SFR诊断将系统性低估固有恒星形成率。
...
### [11] [Learning the Universe with PRFM-vol: Introducing a new subgrid model for star formation in cosmological simulations](https://arxiv.org/abs/2606.10022)
...
TL;DR: 本文提出PRFM-vol，一种基于压力调节反馈调制（PRFM）理论的宇宙学模拟亚网格恒星形成模型，通过修正有效状态方程并考虑气体、暗物质和恒星的环境密度，在孤立星系测试中准确复现PRFM预测，在宇宙学多尺度模拟中显著提升恒星标高和质量，并揭示了有效状态方程对星系形态（如恒星团块）的关键影响。


<details>
  <summary>Details</summary>
Motivation: 现有宇宙学模拟中的恒星形成亚网格模型物理真实性不足，缺乏对ISM尺度物理过程的充分考虑，尤其未能将压力调节反馈调制（PRFM）理论整合进大规模模拟中。

Method: 1. 基于PRFM理论构建恒星形成率模型，使其依赖于气体、暗物质和恒星的环境密度；2. 采用修正的有效状态方程；3. 先在孤立星系中进行高分辨率验证，再应用于宇宙学多尺度模拟。

Result: 孤立星系模拟准确匹配PRFM预测和TIGRESS标度关系；宇宙学模拟相比TIGRESS/Schmidt模型显著提高了恒星标高和质量；低有效压力导致显著恒星团块形成，高压力则消失；证实团块源于Toomre不稳定性。

Conclusion: PRFM-vol是一种自洽、物理驱动的恒星形成亚网格模型，适用于高分辨率宇宙学模拟；模拟星系形态可有效约束有效状态方程模型。
...
### [12] [MusE GAs FLOw and Wind (MEGAFLOW) XIV: Background-Galaxy Absorption Reveals Kiloparsec-Scale Structure in the Cool Circumgalactic Medium](https://arxiv.org/abs/2606.11089)
...
TL;DR: 本文通过对比背景星系（尺寸几kpc）和类星体（尺寸<0.01 pc）的MgII吸收线堆叠分析，发现中等和平均等值宽度在前者中基本一致，而在后者中显著不同，从而推断CGM中冷云团的相干尺度约为2-7 kpc。


<details>
  <summary>Details</summary>
Motivation: 理解CGM中冷气体云团的相干尺度（特征尺寸），这对于揭示维持多相介质的物理机制至关重要，而此前该尺度尚属未知。

Method: 提出一种几何学直接方法：利用背景扩展星系（尺寸几kpc）和点状类星体（<0.01 pc）分别堆叠测量MgII吸收线，通过比较中等与平均等值宽度（EW）的差异来约束冷云团的相干尺度。

Result: 背景星系堆叠时中等和平均EW基本一致；类星体堆叠时两者显著差异，且随碰撞参数>100 kpc而加大；大星系（半光半径~6.6 kpc）的EW一致，小星系（~1.5 kpc）则出现差异。推断MgII云相干长度为~2-7 kpc。

Conclusion: 通过新的几何度量方法，确定了CGM中冷气体云团的相干尺度为2-7 kpc，并利用玩具模型表明部分覆盖和波束平均能自然解释观测差异。
...
### [13] [The Small-scale Structures in the Wind of Messier 82](https://arxiv.org/abs/2606.11036)
...
TL;DR: 利用HST窄带成像对M82星系南侧风的分辨率观测，揭示了温暖电离气体在秒差距尺度上的纤维状结构，并发现弥漫性成分随高度增加占主导，线比诊断表明星暴光致电离与激波加热共同作用。


<details>
  <summary>Details</summary>
Motivation: 小尺度多相结构对星系风演化至关重要，但温暖电离气体在秒差距尺度上的形态和激发状态尚未得到充分约束。

Method: 使用HST窄带成像（Hα、[OIII]、[SII]、[NII]）观测M82南侧风，开发纤维结构识别管线，构建定量纤维目录，并采用光学线比诊断分析电离机制。

Result: Hα发射延伸至盘上方约2.1 kpc，[OIII]延伸约1.5 kpc；纤维结构典型投影宽度约5.3 pc、长度约9.5 pc；纤维覆盖率和线流贡献随高度下降，外风区域弥漫成分占主导；线比显示中间激发态，星暴光致电离可解释Hα光度，但纤维与弥漫成分的系统性分离及线比随高度变化表明外风弥漫区存在激波等额外加热。

Conclusion: 在高分辨率成像中分离纤维状和弥漫性发射，为连接星系风的形态、激发和多相结构提供了有效途径。
...
### [14] [AGN-driven BBH mergers: Black hole populations and hierarchical growth across the AGN parameter space](https://arxiv.org/abs/2606.10823)
...
TL;DR: 本文提出了一个更新的半解析框架，用于研究活动星系核（AGN）盘中双黑洞（BBH）的形成与演化，系统探索了AGN参数对并合种群的影响，发现AGN盘可以产生重复并合，形成高质量尾端，产生与GW190521和GW231123等事件一致的系统。


<details>
  <summary>Details</summary>
Motivation: AGN盘被认为是双黑洞形成的有效环境，但此前缺乏系统性的参数空间探索和与观测的定量比较。

Method: 构建半解析模型，模拟黑洞捕获、迁移、配对、气体驱动硬化、三体相互作用及并合全过程，并根据低红移AGN观测性质加权构建本征BBH种群。

Result: 低粘滞度盘（α=0.01）更有利于层次化增长；并合效率随爱丁顿比增加而提高；种群呈现高质量端非对称质量比、主质量与有效自旋相关性等特征；可重现GW190521和GW231123等事件。

Conclusion: AGN通道产生的BBH种群的高质量尾端和自旋特征在不同物理模型变体中保持稳健，气体硬化过程是产生这些特征的关键。
...
### [15] [Star-formation variability on the star-forming main sequence during the Epoch of Reionization](https://arxiv.org/abs/2606.10648)
...
TL;DR: 本文利用功率谱密度（PSD）模型分析再电离时期（红移z=3-8）星系恒星形成的变化性，发现~10-30 Myr的特征变化时标与星系动力学和恒星反馈时标一致，且低质量星系恒星形成更剧烈、变化更快。


<details>
  <summary>Details</summary>
Motivation: 理解星系恒星形成主序散度背后的物理机制，特别是在再电离时期高红移星系的恒星形成变异性。

Method: 使用SHO和ExtReg两种PSD模型，结合嵌套采样和神经网络仿真器，对~17000个星系在6个平均时标（10-100 Myr）上的SFR散度数据进行约束分析。

Result: ExtReg模型的调节器分量约束不足；动力学分量和SHO模型支持~10-30 Myr特征时标；低质量星系在~10 Myr时标上的PSD功率更低；低红移可能为双分量PSD，高红移为单分量PSD。

Conclusion: 高红移星系恒星形成主序的10-100 Myr散度主要由短时标变化性主导，与星系动力学时标一致。
...
### [17] [Milky Way's warped disc traced by AGB stars](https://arxiv.org/abs/2606.10235)
...
TL;DR: 本文利用富碳（C-rich）和富氧（O-rich）的渐近巨星分支（AGB）星作为示踪体，揭示了银河系翘曲结构，并发现不同年龄的AGB星所呈现的翘曲幅度存在差异，为恒星年龄与翘曲幅度的关系提供了新约束。


<details>
  <summary>Details</summary>
Motivation: 已有研究表明银河系存在翘曲结构，但主要基于HI气体和造父变星等示踪体。本文旨在利用不同年龄的AGB星（特别是C-rich AGB星）作为新的示踪体，更全面地研究银河系翘曲，并探究恒星年龄与翘曲幅度之间的关系。

Method: 利用Gaia数据，选取两类AGB星：C-rich AGB星（年龄约1 Gyr，主序质量2-2.5 M☉）和O-rich AGB星（年龄100-300 Myr，质量3-5 M☉），通过分析其空间分布来示踪银河系翘曲结构。

Result: O-rich AGB星示踪的向下翘曲与同龄的造父变星结果一致；C-rich AGB星在更广的方位角范围内清晰揭示了银河系翘曲，且其翘曲幅度大于造父变星所测得的幅度。

Conclusion: C-rich和O-rich AGB星为银河系翘曲在中等恒星年龄区间提供了新的约束，揭示了不同年龄恒星示踪的翘曲幅度存在差异，为理解恒星年龄与翘曲幅度的关系提供了新视角。
...
### [18] [The Importance of Galaxy-Wide Star Formation in Driving Winds at z~1](https://arxiv.org/abs/2606.10116)
...
TL;DR: 通过对z~1的86个星暴星系的Mg II谱线分析，发现58%存在星系风，且风速与恒星形成率（SFR）呈现统一关系（log v_wind = 0.16 log SFR + 2.4），而与恒星质量、比SFR或SFR面密度无统一关系，表明星系风主要由全星系尺度的恒星形成活动驱动。


<details>
  <summary>Details</summary>
Motivation: 研究高红移（z~1）星暴星系中星系风的特性及其与局域星暴星系的异同，探究星系风的驱动机制。

Method: 利用Keck深场光谱测量86个z~1星暴星系（M* = 10^9.0-10^11.5 M☉）的Mg II谱线轮廓，分析星系风探测率、风速与星系物理参数的相关性。

Result: 58%样本存在星系风；风速与SFR呈显著正相关（3σ），与局域星暴星系遵循统一关系；未发现风速与恒星质量、sSFR或Σ_SFR的统一关系；全星系尺度的Σ_SFR比致密星暴区更能解释风速变化。

Conclusion: z~1星系中星系风主要由全星系范围的恒星形成活动驱动，而非仅由致密星暴区驱动；全星系恒星形成区产生的风泡合并动量为气体外流提供重要机制。
...
TL;DR: 本文提出了一种基于压强调控、反馈调节（PRFM）理论的星际气体演化亚格子模型，并在Arepo移动网格代码中实现了容积版（PRFM-vol）和积分版（PRFM-int）两种版本。相比IllustrisTNG的经验公式，该模型在高压高密度区域气体消耗时间更短，在高分辨率下与TIGRESS模拟的恒星形成率高度一致，且PRFM-int在所有测试分辨率下均保持稳健，为未来宇宙学应用提供了物理基础坚实的恒星形成框架。


<details>
  <summary>Details</summary>
Motivation: 当前宇宙学模拟中采用的恒星形成经验公式缺乏物理基础，亟需一个基于物理机制的亚格子模型来更真实地描述星际气体演化及恒星形成过程。

Method: 基于PRFM理论，将局部恒星形成率与引力与恒星反馈之间的动态平衡联系起来，开发了容积版（PRFM-vol）和积分版（PRFM-int）两种亚格子模型，并在Arepo移动网格代码中实现。PRFM-vol适用于气体盘标高可分辨的情况，PRFM-int则通过垂直平衡重建中面密度和压强，适用于标高无法分辨的情况。

Result: 在银河系级孤立盘模拟（质量分辨率10^5-10^7 M⊙）中，两种实现的气体消耗时间均比IllustrisTNG经验公式更短；高分辨率下PRFM-vol和PRFM-int与TIGRESS模拟结果高度一致；PRFM-int在所有分辨率下均表现稳健。

Conclusion: PRFM衍生的亚格子模型为恒星形成提供了物理基础坚实且数值稳定的框架，能够适应星系形成模拟中的动态范围，为未来的宇宙学应用铺平了道路。
...
### [20] [Mixed Dark Matter: Limits from the Milky Way Satellite Galaxies](https://arxiv.org/abs/2606.10006)
...
TL;DR: 本文研究了两组分暗物质（混合FDM和混合IDM）模型，利用银河系卫星星系观测数据，对不同暗物质分数下的参数空间给出了新的领先约束，并预测了未来巡天实验的改进潜力。


<details>
  <summary>Details</summary>
Motivation: 标准模型中粒子种类多样，暗示暗区也可能同样复杂。为探索超越单一暗物质组分的可能性，研究了双组分暗物质混合模型。

Method: 利用银河系卫星星系观测数据，要求线性物质功率谱的抑制程度不超过约束参考模型，从而推导出混合FDM和混合IDM（耦合光子、中微子或重子）的参数空间约束，并预测未来LSST灵敏度水平巡天的改进能力。

Result: 在95%置信水平下，FDM质量边界和IDM截面边界随暗物质分数降低而系统性减弱，遵循不同的幂律标度。50%分数时，IDM截面边界减弱约2-6倍，FDM质量边界减弱约1.5倍。未来LSST级巡天可将100%分数边界提升约1.6-14倍（IDM）和约3倍（FDM）。

Conclusion: 混合暗物质模型为解释小尺度结构缺失提供了新视角，但需要自洽的宇宙学模拟来更稳健地刻画粒子物理参数与分数贡献之间的简并性，并将约束扩展到更低分数。
...
### [21] [A universal model for the accretion rates and formation times of dark matter halos](https://arxiv.org/abs/2606.09997)
...
TL;DR: 本文提出了一个通用的六参数函数，能够精确描述不同宇宙学模型和红移范围内暗物质晕的质量吸积率和形成时间，并已在Colossus工具包中公开实现。


<details>
  <summary>Details</summary>
Motivation: 暗晕的形成历史决定了星系在宇宙时间内吸积气体的基线速率，但此前缺乏一个简单且通用的公式来描述质量吸积率。

Method: 在ΛCDM和爱因斯坦-德西特两种极端宇宙学模型下，对暗物质-only和流体动力学模拟中的暗晕进行测量，覆盖z=0-14的红移范围，提出基于峰值高度ν、线性功率谱斜率n_eff和有效线性增长率α_eff的三变量六参数普适函数。

Result: 质量吸积率随质量和红移增加而增加，流体动力学与暗物质-only模拟结果几乎一致；所提六参数函数能准确描述质量吸积率，形成红移的两参数拟合改进了Lacey & Cole的函数。

Conclusion: 该模型在高质量红移和低质量端比现有文献更准确、适用范围更广，拟合函数已在Colossus工具包中公开发布。
...
### [22] [Synergy between the gravitational potential decay rate and other structure growth probes in testing gravity](https://arxiv.org/abs/2606.10597)
...
TL;DR: 本文通过结合引力势衰减率（DR）与结构增长探针（Σ₈和fσ₈），显著提升了对修正引力（MG）参数的约束能力。在唯象参数化中，Σ₀的约束精度因加入DR提升约2倍；在EFT α基中，参数c_M和c_B的约束不确定性比此前研究缩小约2倍。结果证明DR在引力测试中不可或缺。


<details>
  <summary>Details</summary>
Motivation: 现有引力测试中，单一探针对修正引力参数的约束存在简并方向，导致精度有限。本文旨在利用引力势衰减率（DR）与其他互补探针（Σ₈和fσ₈）的协同效应，打破简并，获得更强的约束。

Method: 采用2025年DR断层扫描测量数据，结合CMB透镜断层扫描Σ₈约束、DESI DR1全形分析及DESI本动速度场的fσ₈测量。在两种框架下分析：唯象参数化（μ, η, Σ）和EFT α基（α_M, α_B）。

Result: 唯象参数化：μ₀=0.09±0.35，Σ₀=0.01±0.06，加入DR使Σ₀约束提升约2倍；(μ₀,η₀)联合约束提升约1.5倍。EFT α基：c_M=0.64⁺⁰·³²₋₀·⁷²，c_B=0.31⁺⁰·¹⁹₋₀·²⁹，不确定性比此前研究小约2倍。

Conclusion: DR测量在引力测试中具有不可替代的价值，结合多种互补探针能有效提升对修正引力参数的约束精度。
...
### [23] [The Manticore Project II: Bayesian digital twins of cosmic structure across the SDSS and BOSS volumes](https://arxiv.org/abs/2606.10020)
...
TL;DR: Manticore-Deep 利用BORG算法对五个星系红移巡天数据进行分层贝叶斯推断，以~4 Mpc/h分辨率重建了$(4~h^{-1}\mathrm{Gpc})^{3}$体积内的宇宙大尺度结构，并通过CMB透镜和kSZ效应验证了重建的物理真实性。


<details>
  <summary>Details</summary>
Motivation: 将贝叶斯场级宇宙大尺度结构重建从局域体积扩展到更大尺度（Gpc尺度），实现高分辨率、大体积的联合推断，以更全面地约束初始条件和结构演化。

Method: 采用BORG算法，在单一分层贝叶斯框架内联合约束五个星系红移巡天数据；通过新颖的分块（tiled）推断策略使计算可行；推断初始条件并正向演化至晚期结构，生成密度和速度场后验系综。

Result: 后验实现与LCDM模型统计一致，初始条件高斯且各向同性，重建物质场与Planck PR3 CMB透镜图的交叉相关累积检测显著性达7.4σ，kSZ检测达3.5σ。

Conclusion: 该方法成功将高分辨率贝叶斯场级推断扩展到Gpc尺度，物理真实性通过独立后验预测测试得到验证，为更大尺度宇宙结构研究提供了可靠框架。
...
### [24] [On the potential of pseudo-scalar dark energy](https://arxiv.org/abs/2606.09994)
...
TL;DR: 本文研究了赝标量场作为动态暗能量模型的可行性，结合宇宙膨胀历史与宇宙双折射观测，对多种势能形式（轴子类、二次型、线性型、Ratra-Peebles型）进行了参数约束分析。


<details>
  <summary>Details</summary>
Motivation: 赝标量场是动态暗能量的重要候选者，其与光子的耦合会导致宇宙微波背景极化面旋转（宇宙双折射），现有研究尚未系统结合背景膨胀历史与CB观测对多种势能模型进行联合约束。

Method: 联合宇宙膨胀历史背景观测与宇宙双折射测量数据，对轴子类、二次型、线性型、Ratra-Peebles型等赝标量暗能量模型的势能参数进行约束分析。

Result: 轴子类势能模型仅在大反常系数下可行；二次型、线性型及Ratra-Peebles型势能模型能成功解释暗能量与宇宙双折射，且对称破缺尺度接近大统一理论（GUT）能标。

Conclusion: 多种赝标量暗能量模型在联合观测约束下具有可行性，为动态暗能量与宇宙双折射的统一解释提供了理论支持。
...
### [25] [The dynamics of the Anglerfish cluster](https://arxiv.org/abs/2606.10483)
...
TL;DR: 本文通过多波段观测（X射线、射电、光学）分析了星系团MACS0600的并合动力学，发现一个致密冷核穿越了更大质量的主星系团，并显著扰动了周围簇内介质。


<details>
  <summary>Details</summary>
Motivation: 并合星系团是检验大尺度结构形成历史的关键实验室，但大多数并合团缺乏详细的多波段和动力学重建分析，难以消除投影简并性。本研究旨在对复杂大质量星系团MACS0600进行深入的多波段并合动力学分析。

Method: 利用XMM-Newton和Chandra的高空间分辨率X射线数据（~15角秒）分析形态和热力学性质，结合MeerKAT射电数据研究非热成分，并辅以光学数据进行支持。

Result: 发现X射线形态扭曲、存在多个子结构；射电辐射主体与X射线峰值存在显著偏移；X射线峰值处探测到致密冷核被更热气体包围，存在冷锋；中心区域温度升高且汇聚了大部分弥散射电辐射；光学数据支持冷核与主团之间存在视线方向相对运动。

Conclusion: MACS0600正处于并合过程中，一个致密冷核穿越了更大的主星系团，虽未被完全瓦解，但已显著扰动了周围的簇内介质。
