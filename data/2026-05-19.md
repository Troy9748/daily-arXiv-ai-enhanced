...
### [1] [PEARLS: JWST Counterparts of Micro-Jy Radio Sources in the NEP Time Domain Field. II. All Four Spokes](https://arxiv.org/abs/2605.17040)
...
TL;DR: 利用JWST/NIRCam在北极黄道时域场中为206个射电源（S(3 GHz)≥5微央）识别出4.4微米对应体，其中约79%的射电发射源于恒星形成，约21%可能来自尘埃遮蔽的恒星形成或活动星系核。


<details>
  <summary>Details</summary>
Motivation: 为微焦量级射电源寻找可靠的红外对应体，并探究其射电辐射起源（恒星形成vs活动星系核）。

Method: 在65平方角分天区内，将JWST/NIRCam 4.4微米数据与3 GHz射电巡天进行位置交叉匹配（匹配半径0.3角秒），并采用激进解混算法处理多峰值天体。

Result: 成功匹配206/211个射电源，中值红移1.14，约79%的射电辐射与恒星形成一致，约21%可能来自尘埃遮蔽恒星形成或活动星系核，66%的射电宿主星系显示出活动星系核存在迹象。

Conclusion: JWST/NIRCam能有效识别微焦级射电源的对应体，但需激进解混算法；射电辐射起源需结合时域监测来区分活动星系核与尘埃遮蔽恒星形成的贡献。
...
### [2] [The ${}^{13}\mathrm{CO}(2{-}1)/^{12}\mathrm{CO}(2{-}1)$ Line Ratio from 100 Molecular Clouds in the Large Magellanic Cloud](https://arxiv.org/abs/2605.17024)
...
TL;DR: 通过对大麦哲伦云中100个巨分子云的ALMA观测，测量了¹³CO(2-1)/¹²CO(2-1)线比中位数为0.078，发现线光度呈近线性关系，且活跃恒星形成分子云因内部条件不同而具有更亮的¹³CO发射。


<details>
  <summary>Details</summary>
Motivation: 探究分子云中¹³CO与¹²CO的线比特征及其与恒星形成活动的关系，以理解不同内部条件下分子云的性质差异。

Method: 利用ALMA对大麦哲伦云中100个巨分子云进行观测，测量¹³CO(2-1)/¹²CO(2-1)线比，进行回归分析并引入YSO的远红外光度作为预测变量。

Result: 线比中位数为0.078，68%样本落在0.058-0.107之间；线光度呈近线性关系；引入YSO远红外光度显著改善拟合质量，活跃星形成云具有更亮¹³CO发射。

Conclusion: 活跃恒星形成的分子云与静止分子云内部条件不同，¹³CO线比可作为区分指标。
...
### [3] [Velocity-resolved [O I] 63,145 um, [C II] 158 um, and OH line mapping along the Orion BN/KL explosive outflow and irradiated shocks](https://arxiv.org/abs/2605.18438)
*J. R. Goicoechea,R. Güsten,B. Godard,H. Wiesemeyer,R. Higgins,A. Gusdorf,M. G. Santa-Maria,M. Zannese,K. M. Menten*

Main category: astro-ph.GA

Recommendation: ★★☆☆☆ (25/100) - 与 Zotero 兴趣画像中的 jwst, molecular 等主题/作者匹配。

TL;DR: 本研究利用SOFIA望远镜对Orion BN/KL区域中[O I] 63和145微米精细结构谱线进行首次速度分辨成像，揭示了爆炸性外流中激波激发气体的性质，发现其具有高光度、高质量损失率，并利用磁化激波模型结合紫外辐射解释了观测到的线强比率，支持了J型激波（30-40 km/s）的存在。


<details>
  <summary>Details</summary>
Motivation: 恒星并合产生的爆炸性外流是红外谱线瞬变源，并向早期分子云注入机械能。Orion BN/KL是最邻近的爆炸性外流，但此前缺乏对其速度分辨的[O I]精细结构线观测，限制了对外流物理性质的理解。

Method: 使用SOFIA望远镜获取[O I] 63、145微米以及[C II] 158微米和OH 119微米谱线的高灵敏度速度分辨图；通过线宽和空间分布区分背景云气和外流成分；利用[O I] 63/145和[O I] 63/[C II] 158线强比率诊断气体密度和温度；结合包含紫外辐照的磁化激波模型（J型激波）拟合观测数据。

Result: [O I]外流成分线宽约20-30 km/s，空间分布与JWST观测的H2激波发射一致；OH 119微米线呈现覆盖160 km/s的P-Cygni轮廓；总[O I]线光度高达86.5 L_sun，与H2和CO线光度相当；外流质量损失率为(9.1±2.6)×10^-3 M_sun/yr，质量为3.3-5.9 M_sun；线强比率在谱线翼部极高，与密度10^5-10^6 cm^-3、温度~500 K的激波后气体一致；激波模型支持速度30-40 km/s的J型激波，预激波密度约10^4 cm^-3，并受周围快激波和大质量（原）恒星产生的紫外辐射照射。

Conclusion: [O I]精细结构线是探测爆炸性外流中激波气体的有效探针，结合[C II]和OH线可以全面刻画外流的动力学和物理条件。研究结果支持Orion BN/KL外流由高速J型激波驱动，并受到紫外辐射场的显著影响。
...
### [4] [Constraining the magnetic field strength of a flaring radio core in the compact steep spectrum source 3C 138](https://arxiv.org/abs/2605.18283)
...
TL;DR: 本文研究了CSS类星体3C 138在2022年后进入射电高态并伴随伽马射线爆发的现象。利用韩国KVN等望远镜的多频数据，通过同步自吸收模型估算其核心磁感应强度，发现该源核心处于粒子主导状态且远低于等分磁场，表明激波驱动的粒子注入而非极端相对论性束效应是观测到的多波段活动的关键机制。


<details>
  <summary>Details</summary>
Motivation: 尽管CSS源通常表现出弱的多普勒增亮，但3C 138自2022年起进入射电高态并伴有多次伽马射线爆发，为研究喷流物理条件的变化提供了独特机会。

Method: 利用韩国KVN在22/43/86/129 GHz的甚长基线干涉数据（2024-2025年），校准可见度并用圆形高斯模型解析成分；通过单区SSA模型拟合核心谱以获得转折频率和峰值流量密度，进而估算SSA磁场和等分磁场；并用VLA和ALMA数据约束宽带谱。

Result: KVN SSA核心的转折频率约33 GHz，峰值流量约1.45 Jy；BSA/B_eq比值约0.05，SSA磁场远低于等分磁场。

Conclusion: 3C 138的流量变化由紧凑的粒子主导核心驱动；内喷流中激波驱动的粒子注入可解释核心增亮及通过逆康普顿过程产生X射线/伽马射线辐射，无需极端相对论性束效应。
...
### [5] [Polarization Observations of a Sample of 6.7 GHz Methanol Masers](https://arxiv.org/abs/2605.18275)
...
TL;DR: 本文报告了21个已知恒星形成区域的6.7 GHz甲醇脉泽谱线观测，分析了其偏振特性随时间和谱线的变化，并探测到新源G240.316+0.071，发现偏振变化主要由磁场变化引起。


<details>
  <summary>Details</summary>
Motivation: 研究6.7 GHz甲醇脉泽的偏振特性，探测其随时间的变化规律，以揭示磁场在恒星形成过程中的作用。

Method: 使用Green Bank Telescope在C波段进行全斯托克斯模式观测，通过高斯拟合分析RCP和LCP分量的速度分离来推导塞曼分裂值。

Result: 三分之二的源检测到6.7 GHz甲醇脉泽，含一个新源；线偏振范围为0-15%，圆偏振为0-±9%；多个源观测到塞曼分裂，分裂值随时间变化且与线/圆偏振变化相关。

Conclusion: 偏振特性随谱线和时间的变化很可能由磁场变化引起。
...
### [6] [SDSS-V: Revealing a weak accretion state in X-ray selected red quasars](https://arxiv.org/abs/2605.18161)
...
TL;DR: 本文基于eROSITA/SDSS-V合作数据，构建了首个包含380个红类星体（rQSOs）的大样本X射线目录，发现rQSOs相比蓝类星体（bQSOs） intrinsically X射线更弱，其X射线吸收源于黑洞附近的无尘气体而非经典尘埃环，光学红化则来自更大尺度的宿主星系或盘风中的尘埃气体，表明rQSOs代表一种黑洞吸积受抑制而宿主星系持续增长的独特演化阶段。


<details>
  <summary>Details</summary>
Motivation: 红类星体被认为是活动星系核（AGN）演化早期阶段的短暂阶段，但其内在性质与蓝类星体存在根本差异，此前缺乏大样本、均匀的X射线观测来系统研究其X射线特性。

Method: 从eROSITA/SDSS-V合作中选取380个红类星体，结合X射线成像、光学光谱和多波段测光数据，采用独立成分分析（ICA）以数据驱动、非参数方式重建光学光谱，同时考虑尘埃红化和宿主星系污染。

Result: rQSOs在X射线波段本质较弱，超过50%样本的X射线光度低于10^43.5 erg/s；其α_OX值较低，表明相对于明亮光学连续谱，X射线辐射较弱；X射线光谱测量显示相较于光学红化，气体柱密度较大，暗示X射线吸收来自黑洞附近的无尘气体。

Conclusion: 红类星体代表一个独特的演化阶段，其中黑洞吸积受抑制，而宿主星系继续增长，其X射线弱化和吸收特性源于不同于经典尘埃环的物理机制。
...
### [7] [BISTRO Survey: Gravity-Dominated and Magnetically Regulated Star Formation in M17 SW](https://arxiv.org/abs/2605.17708)
...
Recommendation: ★★☆☆☆ (25/100) - 与 Zotero 兴趣画像中的 dust, polarization, molecular 等主题/作者匹配。

TL;DR: 利用JCMT 850 μm尘埃偏振观测，绘制了M17 SW分子云的高分辨率磁场图。磁场呈弧形结构环绕三个稠密团块。通过Skalidis-Tassis方法测量磁场强度为0.1-2.4 mG（均值0.54 mG）。能量分析显示引力主导（e_G≈10⁻⁷.8 erg cm⁻³），超过磁场（e_B≈10⁻⁸.3）和湍流（e_K≈10⁻⁸.7），但三者量级相近，系统处于近均衡态。磁场上游垂直激波面，内部引力与磁场协同坍缩，但在吸积桥中磁场垂直引力，支撑径向坍缩并引导气流。运动学证据表明物质从C3流向C2。该区域恒星形成受引力全局驱动，但受磁场结构局部调控。


<details>
  <summary>Details</summary>
Motivation: M17 SW是活跃的大质量恒星形成区，但其磁场结构、能量分布及对恒星形成过程的影响尚不清楚。本文旨在通过高分辨率极化观测揭示磁场几何构型与引力、湍流的相互作用机制。

Method: 使用JCMT 850 μm尘埃偏振观测获取14角秒分辨率磁场图；结合氨分子谱线数据；采用Skalidis-Tassis方法计算平面磁场强度以减小角度弥散误差；进行能量密度分析（引力、磁场、湍流）；分析磁场与激波面、引力方向及吸积结构的几何关系。

Result: 磁场呈现弧状结构环绕三个稠密团块（C1/C2/C3）；磁场强度0.1-2.4 mG（均值0.54 mG）；能量密度引力＞磁场＞湍流（量级跨度＜1 dex）；东北边界磁场垂直激波面；云核内部磁场平行引力，吸积桥中磁场垂直引力；运动学证据支持C3→C2的物质输运。

Conclusion: M17 SW的恒星形成由引力全局驱动，但磁场在局部发挥调控作用：在吸积桥中提供径向支撑并引导气流，从而影响大质量团块的形成与物质输运效率。
...
### [8] [Determination of the best dark matter profile for the Milky Way with Gaia DR3 using Bayesian Model Comparison](https://arxiv.org/abs/2605.16994)
...
TL;DR: 基于Gaia数据，通过贝叶斯模型比较分析银河系旋转曲线，发现Einasto轮廓和核心暗物质模型优于NFW轮廓，而MOND拟合效果较差，不同重子模型之间无明显优劣。


<details>
  <summary>Details</summary>
Motivation: 利用最新的Gaia旋转曲线数据，通过贝叶斯模型比较，确定最能描述银河系旋转曲线的暗物质模型。

Method: 采用贝叶斯模型比较分析，使用四组Gaia旋转曲线速度数据，构建三种重子模型和七种暗物质模型（包括Einasto、NFW、核心暗物质模型等），并测试三种不同插值函数的MOND理论。

Result: Einasto轮廓优于NFW轮廓；核心暗物质模型优于尖点NFW模型；MOND拟合效果不如暗物质模型；不同重子模型之间无显著差异。

Conclusion: 在Gaia旋转曲线数据框架下，Einasto轮廓是最优的唯象暗物质模型，MOND的解释力不如暗物质模型。
...
### [9] [The Rapid ASKAP Continuum Survey VII: Spectra and Polarisation In Cutouts of Extragalactic Sources (SPICE-RACS) Second Data Release -- Unveiling the Magnetised Sky](https://arxiv.org/abs/2605.16917)
...
TL;DR: SPICE-RACS DR2发布了基于RACS-low3巡天的第二版数据，覆盖南天至赤纬+49°的87.5%天区，包含约400万个射电源的斯托克斯I、Q、U谱线立方体，以及迄今最大的法拉第旋转测量（RM）目录（约25万到34万个RM），其RM数量是此前所有RM目录总和的约5倍。


<details>
  <summary>Details</summary>
Motivation: 现有法拉第旋转测量（RM）目录的覆盖范围和数量有限，需要更大规模、更高密度的RM巡天数据来推进磁场科学和宇宙学研究。

Method: 利用RACS-low3（799.5–1087.5 MHz）数据，生成约400万射电源的斯托克斯I、Q、U谱线立方体，提取约500万射电成分的谱线，采用去重后以8σ和6σ信噪比阈值检测线性偏振信号，计算法拉第旋转量。

Result: 发布了包含约25万（8σ阈值）到34万（6σ阈值）个RM的目录，RM中位不确定度约2 rad/m²，角分辨率约15角秒，RMS噪声约200μJy/PSF，残余仪器偏振约0.1%，RM网格密度达6.7 deg⁻²，有效分辨率约23角分。

Conclusion: SPICE-RACS DR2是迄今最大的单一RM目录，其数据质量和覆盖范围将推动新一代RM科学研究，并为即将开展的深度偏振巡天（如ASKAP POSSUM）提供理想参考。所有数据已在CSIRO数据平台公开。
...
### [10] [Feedback driven interactions between dark and luminous matter to explain tight galaxy scaling relations](https://arxiv.org/abs/2605.16505)
...
TL;DR: 通过NIHAO宇宙学流体模拟，本文发现星系盘标长与暗物质标度半径之间的紧密相关性可由重子反馈自然产生，无需引入暗物质与重子之间的非引力相互作用。


<details>
  <summary>Details</summary>
Motivation: 检验星系盘标长$R_d$与暗物质标度半径$r_0$之间的经验相关性是否源于重子与暗物质之间的非引力耦合，并探究其物理起源。

Method: 利用31个来自NIHAO宇宙学流体模拟的星系样本（仅包含重力和重子反馈），追踪z=2到z=0的演化，使用贝叶斯层次框架刻画$R_d-r_0$关系的宇宙学演化，并与纯暗物质模拟对比。

Result: NIHAO模拟自然复现了观测到的$R_d-r_0$关系的归一化和小弥散度，但斜率略浅；识别出盘膨胀、收缩和准静态演化三类星系；关系归一化随红移轻微下降（~0.07 dex），斜率从$\alpha\simeq1.05$变平至$\alpha\simeq0.95$，固有弥散度向低红移减弱。

Conclusion: 重子反馈本身即可重塑中心势场，自然建立起发光物质与暗物质之间的观测耦合，无需引入超出引力的暗物质-重子相互作用或修改暗物质模型。
...
### [11] [From inter-filamentary gas to filaments and hubs: gas flows in the Mon R2 hub-filament system](https://arxiv.org/abs/2605.17372)
...
TL;DR: 该研究利用Nobeyama 45米望远镜的$^{13}$CO和C$^{18}$O观测数据，分析了Monoceros R2 Hub-纤维系统(HFS)中低密度和高密度气体的运动学。发现纤维(Fs)中的气体沿纤维方向流向中心的速度远快于纤维间区域(IFs)；同时，至少30%的IFs气体会流向Fs进行物质补充。这表明需同时考虑高低密度气体对中心大质量恒星形成的物质供应。


<details>
  <summary>Details</summary>
Motivation: 以往研究关注了致密纤维中的速度结构，但尚未对低密度纤维间区域的气体运动学进行探究。

Method: 使用Nobeyama 45米望远镜的$^{13}$CO(1-0)和C$^{18}$O(1-0)观测数据，分别追踪低密度和高密度气体；识别速度相干结构并将其分为纤维(Fs)和纤维间区域(IFs)；估算沿纤维方向和平行方向的速率梯度及质量吸积率。

Result: Fs中平行/垂直方向质量吸积率比值为6.8，IFs中为1.5；至少30%的IFs气体质量可能流向Fs。

Conclusion: Fs和IFs中的气体整体上直接流向中心Hub；气体沿纤维方向流动比纤维间更快；IFs气体可补充Fs物质；考虑总气体质量库（包括低密度和高密度）对理解大质量恒星形成至关重要。
...
### [12] [Magnetised CGM Gas at z~1 revealed by SPICE-RACS](https://arxiv.org/abs/2605.16924)
...
TL;DR: 本研究利用SPICE-RACS DR2和DESI DR1数据，构建了包含2483条类星体视线(其中612条含Mg II吸收体)的最大样本，通过扣除银河系贡献后，发现Mg II视线在残余RM色散上比对照样本高出4.13±0.91 rad/m²（4.5σ显著性），表明在红移~1.14处，星系周介质磁场强度已达约0.4-0.8 μG，证明宇宙在z~1时星系晕已建立了显著的磁化环境。


<details>
  <summary>Details</summary>
Motivation: 高红移星系周介质(CGM)的磁场直接观测证据极其有限，缺乏高质量法拉第旋转测量(RM)数据来约束CGM磁场特性及演化。

Method: 结合SPICE-RACS DR2的RM数据和DESI DR1类星体目录，构建最大样本；扣除银河系RM贡献，筛选低银河系HI柱密度和Hα强度的视线；比较含Mg II吸收体与对照样本的残余RM弥散差异。

Result: 在757个类星体的前景清洁样本中，Mg II视线比对照样本显示出4.13±0.91 rad/m²的残余RM色散超出（4.5σ显著性），对应CGM磁场强度约0.4-0.8 μG，投影半径20-150 kpc。

Conclusion: 宇宙在z~1时星系周介质已具备显著的磁化水平，为理解宇宙时间尺度上星系晕磁场的增长和放大提供了新约束。
...
### [13] [Co-evolution of Supermassive Black Holes and their Host L* galaxies: implications for Milky Way and M31](https://arxiv.org/abs/2605.16501)
...
TL;DR: 基于ARTEMIS和EAGLE模拟，研究了L*星系中超大质量黑洞质量离散的起源，发现不同的并合历史导致黑洞质量差异：活跃并合历史产生更大质量黑洞和椭圆星系，宁静历史产生较小黑洞和盘状星系。


<details>
  <summary>Details</summary>
Motivation: 探究L*星系中黑洞质量离散的物理起源，理解不同黑洞质量的星系在演化路径上的差异。

Method: 利用ARTEMIS和EAGLE模拟，按中心黑洞与星系恒星质量比分类为LBH（低比值）和HBH（高比值），追踪两类星系从z~2到低红移的演化。

Result: 在z~2时两类星系性质相似，随后出现分化；LBH星系维持高气体分数和持续恒星形成，HBH星系早期形成恒星、黑洞增长更快并经历反馈淬灭；HBH中黑洞主要通过气体吸积（~90%）增长，LBH中气体吸积（~60%）和黑洞并合（~40%）共同作用。

Conclusion: 并合历史的差异是L*星系黑洞质量离散的主要原因，这可以解释银河系和M31黑洞质量的差异。
...
### [14] [The limits of feedback from active galactic nuclei](https://arxiv.org/abs/2605.16483)
...
TL;DR: 本文利用FLAMINGO模拟研究AGN反馈为何在星系群中显著耗散气体，而在星系团中效果不佳。通过划分三个径向区域（内激波加热区、中间浮力区、外终止激波区），发现加热自限性导致外流具有几乎与晕质量无关的上限熵值（约360 keV cm²），只有当该值高于入流熵时气体才能逃逸，这一条件仅在晕质量小于10^13.7 M⊙时满足，从而解释了群与团的气体含量差异。


<details>
  <summary>Details</summary>
Motivation: 探究AGN反馈在不同质量晕（星系群vs星系团）中产生不同气体耗散效果的根本物理机制，解释为何星系群气体被大量耗尽而星系团仍保有接近宇宙重子份额的气体。

Method: 使用FLAMINGO模拟，划分三个径向区域（内区激波加热、中间区浮力上升、外区终止激波），基于Rankine-Hugoniot关系分析熵上限，并推导维里气体分数的时间演化表达式。

Result: 发现AGN外流具有与晕质量无关的熵上限（~360 keV cm²），反馈强度变化会改变该值（~10^14.0或10^13.5 M⊙对应临界质量）。仅当晕质量M_200m < 10^13.7 M⊙时，外流才能逃逸；更大质量晕中气体被重新吸积，维里气体分数随质量从10^13.0 M⊙开始上升。

Conclusion: 星系群气体被耗散而星系团气体接近宇宙重子分数的现象，根源在于入流熵随晕质量增长的维里关系决定了外流能否逃逸；临界质量以上的晕中，气体再吸积效应主导，使得维里气体分数随质量增加而恢复。
...
### [15] [Circular polarization of the cosmic microwave background induced by the optical Magnus effect on gravitational lensing](https://arxiv.org/abs/2605.16945)
*Yusuke Nishida*

Main category: astro-ph.CO

Recommendation: ★★☆☆☆ (38/100) - 与 Zotero 兴趣画像中的 lensing, polarization 等主题/作者匹配。
...
### [16] [Faster CMB lensing with control variates](https://arxiv.org/abs/2605.18659)
...
TL;DR: 提出一种快速计算CMB透镜功率谱中实现相关偏差的新方法，通过差分两个相关估计（真实掩模模拟与各向同性模拟）将计算成本降低3-5倍，且易于集成到现有分析管线中。


<details>
  <summary>Details</summary>
Motivation: CMB透镜功率谱测量中，实现相关偏差的计算是主要计算瓶颈，现有方法计算成本过高。

Method: 将基于真实掩模的全模拟估计与各向同性模拟的解析偏差估计进行差分，加速偏差计算。

Result: 对于ACT或Simons Observatory级别的噪声，总计算成本降低约5倍；若保持现有各向异性滤波方法不变，则降低约3倍。

Conclusion: 该方法简单有效，可轻松集成到现有CMB透镜分析管线中。
...
### [17] [Gravitational lensing time delay beyond the Shapiro/geometry split](https://arxiv.org/abs/2605.18182)
...
TL;DR: 本文从Schwarzschild-de Sitter度规的精确零测地线出发，推导了强引力透镜时间延迟公式，恢复了标准公式作为小角度展开的首项，并识别了首个修正项——该修正不引入新的宇宙学依赖，仅来自Schwarzschild部分的高阶修正。


<details>
  <summary>Details</summary>
Motivation: 现有强引力透镜时间延迟公式通常被写为几何延迟与Shapiro延迟之和，宇宙学通过角直径距离和红移因子进入。作者希望通过从精确度规出发，严格推导该公式的结构，并检验是否存在高阶修正引入额外的宇宙学依赖。

Method: 从Schwarzschild-de Sitter度规的精确零测地线方程出发，对小角度展开，恢复标准公式作为首项，并识别出首个高阶修正项。

Result: 标准时间延迟公式是精确公式在小角度展开下的首项；首个修正项不引入新的宇宙学常数依赖，仅源于Schwarzschild部分的内禀高阶效应。

Conclusion: 宇宙学常数（暗能量）仅通过未受透镜影响的角直径距离和透镜红移因子进入时间延迟公式，高阶修正不改变这一基本结构。
...
### [18] [Coupled quintessence with a potential from supergravity exhibits sign-changing interaction](https://arxiv.org/abs/2605.17754)
...
TL;DR: 本文测试了超引力（SUGRA）精质场暗能量模型，发现观测数据强烈支持暗能量与暗物质之间存在耦合（>4σ显著性），且能量传递会改变符号导致有效状态方程穿越幻象分割线，该模型在统计上与CPL参数化难以区分。


<details>
  <summary>Details</summary>
Motivation: 探索超引力启发的精质场势能模型在耦合与非耦合情况下的表现，检验其能否解释当前宇宙学观测所暗示的演化暗能量行为。

Method: 使用DESI BAO、DES-Dovekie SNIa和Planck CMB数据，对耦合与非耦合的SUGRA精质场模型进行统计检验和参数约束。

Result: 耦合参数在超过4σ置信水平上偏离零；数据偏好能量传递符号改变的分支；该分支与CPL参数化在统计学上几乎不可区分。

Conclusion: 耦合SUGRA精质场为观测所偏好的演化暗能量提供了一个场论实现基础。
...
### [19] [Constraints on Self-Interacting Fuzzy Dark Matter from the Stellar Kinematics of the Dwarf Galaxy Leo II](https://arxiv.org/abs/2605.17211)
...
TL;DR: 本文通过对矮星系Leo II的Jeans分析，在二维参数空间(m_a, f_a)中对具有自相互作用的模糊暗物质(FDM)模型进行了约束，发现吸引(排斥)自相互作用会改善(恶化)与观测的吻合度，并给出了95%置信水平下的下限约束。


<details>
  <summary>Details</summary>
Motivation: 单参数FDM模型面临来自Lyman-α森林和矮星系观测的日益严格约束，引入FDM自相互作用是缓解这些约束的自然扩展方案。

Method: 利用矮星系Leo II的恒星运动学Jeans分析所推断的暗物质密度轮廓，在二维参数空间(m_a, f_a)中对自相互作用FDM模型进行约束分析。

Result: 吸引(排斥)自相互作用使FDM密度轮廓更集中(弥散)，从而改善(恶化)与Jeans分析结果的吻合度；对于强度f_a^{-1}≲10^{-14}GeV^{-1}的自相互作用，m_a的95%置信水平下限在(1-10)×10^{-22}eV范围内。

Conclusion: 该分析同时约束了(m_a, f_a)两个参数，不依赖宇宙学或星系演化历史的假设，为现有约束提供了互补性探测手段。
...
### [20] [Thermostats, Not Engines: A New Picture of Halo Gas Regulation](https://arxiv.org/abs/2605.16488)
...
TL;DR: 本文提出黑洞反馈通过建立熵天花板调控大质量暗晕气体，使浮力气体无需额外能量即可迁移至维里半径。FLAMINGO模拟支持该机制：各向同性热反馈时维里半径处外流熵与质量无关，而喷流反馈则取决于直接加热气体的立体角。临界暗晕质量约为10^13.5-14太阳质量，超过该质量时维里激波将压倒天花板，预测大质量星系恒星形成将重新活跃，并得到低红移观测证据支持。


<details>
  <summary>Details</summary>
Motivation: 现有黑洞反馈模型难以解释大质量暗晕中气体的行为，特别是为何某些星系在高质量阶段仍能维持恒星形成。作者试图提出一种统一的物理机制——熵天花板——来解释气体在暗晕中的输运和星系恒星形成的演化规律。

Method: 利用FLAMINGO流体模拟，对比各向同性热反馈与喷流反馈两种模式，研究维里半径处外流熵的分布特征，同时结合低红移观测数据验证模型预测。

Result: 各向同性热反馈下，维里半径外流熵与暗晕质量无关；喷流反馈下，熵取决于直接加热气体的立体角。存在临界质量（约10^13.5-14太阳质量），超过该值维里激波压倒熵天花板，大质量星系恒星形成将重新活跃。

Conclusion: 黑洞反馈建立的熵天花板机制可有效解释大质量暗晕中的气体输运行为，并能预测大质量星系恒星形成的再活跃现象，该预测已获得低红移观测数据的初步支持。
