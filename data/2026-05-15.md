...
### [1] [Strong Gravitational Lensing with the James Webb Space Telescope](https://arxiv.org/abs/2605.15189)
*Adi Zitrin*

Main category: astro-ph.CO

Recommendation: ★★☆☆☆ (36/100) - 与 Zotero 兴趣画像中的 lensing, jwst, dark matter 等主题/作者匹配。
...
### [2] [Primordial Black Hole from Tensor-induced Density Fluctuation: First-order Phase Transitions and Domain Walls](https://arxiv.org/abs/2605.15197)
...
TL;DR: 本文提出了一种规范不变且最小化的原初黑洞(PBH)形成机制，基于一阶相变(FOPT)和畴壁(DW)过程中产生的二阶张量扰动，并给出了对FOPT和DW参数的模型无关约束。研究发现小行星质量PBH可构成全部暗物质，对应的引力波信号在LISA、SKA和ET探测器可及范围内。


<details>
  <summary>Details</summary>
Motivation: 现有的PBH形成机制存在规范依赖性等问题，且缺乏对FOPT和DW参数的模型无关约束。本文旨在提出一种更规范、更通用的PBH形成机制，并建立PBH与引力波观测之间的联系，从而约束相关的粒子物理模型。

Method: 基于一阶相变中气泡碰撞和声波产生的二阶张量扰动，以及畴壁湮灭产生的二阶扰动，在微扰论框架下推导曲率扰动。通过PBH质量、丰度与FOPT参数(β/H, α, T₊)及DW参数(α_ann, V_bias, σ)之间的半解析公式，建立PBH与引力波观测之间的联系。

Result: 小行星质量PBH可成为全部暗物质，对应FOPT参数：T₊∈(4×10²,10⁴) GeV, β/H≈6, α>O(1)，引力波峰值Ω_GW^p h²~O(10⁻⁸)在f_p∈(10⁻⁵,10⁻²) Hz；DW参数：σ^{1/3}∈[10⁶,10⁸] TeV, V_bias^{1/4}∈[10⁷,10¹⁰] MeV，引力波峰值Ω_GW^p h²~O(10⁻⁹)在f_p∈(4×10⁻⁴,10⁻¹) Hz。这些信号在LISA、SKA和ET探测器的探测范围内。

Conclusion: 本文提出的规范不变PBH形成机制有效建立了PBH与引力波之间的关联，为利用多信使观测约束早期宇宙相变和畴壁微观物理（无论是可见扇区还是暗扇区模型）提供了新的工具。
...
### [3] [Isocurvature-Free QCD Axion Dark Matter from Inflaton-Driven Early QCD: the Necessity of Inflationary Plateaus](https://arxiv.org/abs/2605.15192)
...
TL;DR: 本文提出了一种通过暴胀子与胶子的直接耦合，在暴胀期间动态提升QCD禁闭标度、暂时使轴子变重并抑制其等曲率扰动，随后禁闭标度弛豫使轴子变轻，利用德西特涨落产生暗物质丰度的机制。通过参数化背景ε(N)∝1/N^p，发现该机制偏好高原型暴胀(p≥2)，并在最小方案中通过暴胀子-胶子耦合实现再加热，在退禁闭晚于CMB窗口时产生可行的轴子暗物质。


<details>
  <summary>Details</summary>
Motivation: 解决轴子等曲率扰动问题，并同时为轴子暗物质产生提供一种新的动力学机制，避免对暴胀势的具体形式做过强假设。

Method: 不指定具体暴胀势，而是参数化背景ε(N)∝1/N^p，其中N为暴胀结束前的e-fold数，p=1对应单项式模型，p=2对应标准高原模型，p≥3对应超平坦高原或山顶模型。通过解析计算分析不同p下禁闭标度的演化行为。

Result: 该机制选择高原型暴胀(p≥2)：单项式模型(p=1)导致禁闭标度增长过快，而高原模型使QCD扇区保持在微扰可控范围内。退禁闭发生在CMB窗口之后时，可获得可行的轴子暗物质产生。早期禁闭扇区将标量谱指数向更大、更蓝的方向移动，为原本被CMB数据排除的模型打开了可行的参数空间。

Conclusion: 暴胀子-胶子直接耦合的动态禁闭机制不仅能有效抑制轴子等曲率扰动并产生暗物质，还能改变CMB可观测量的预测，为某些被观测排除的暴胀模型提供新的生存空间。
...
TL;DR: 本文基于贝叶斯分层模型框架，从KiDS+VIKING-450巡天的测光数据中推断星系红移分布，并将其应用于弱引力透镜分析，发现新方法降低了与Planck宇宙学数据的$S_8$张力的显著性（从$2.3\sigma$降至$1.9\sigma$），得到$S_8=0.756\pm0.039$，并缓解了宇宙学参数冲突。


<details>
  <summary>Details</summary>
Motivation: 测光数据中的断层红移分布是宇宙剪切分析的关键输入，但其精确估计面临挑战。现有方法可能存在系统偏差，导致不同巡天之间的宇宙学参数（如$S_8$）存在张力。本研究旨在开发一种更稳健的贝叶斯方法，以更准确地推断红移分布并缓解这种张力。

Method: 扩展Leistedt等人基于模板的贝叶斯分层模型框架，应用于KiDS+VIKING-450巡天的弱引力透镜数据；通过主成分分析从大超集中提取代表性模板；选取$10^5$个星系子集进行计算可行性测试，并评估子集选择对宇宙学推断的敏感性；边际化红移分布推断结果进行宇宙学参数估计。

Result: 贝叶斯方法得到的聚类参数较以往研究更高，使$S_8=0.756\pm0.039$，将与Planck的张力从$2.3\sigma$降低至$1.9\sigma$；同时得到物质密度$Ω_{\rm{m}}=0.31\pm0.10$；子集选择对宇宙学推断的影响小于统计误差。

Conclusion: 本文提出的基于贝叶斯分层模型的红移分布推断方法有效缓解了KiDS+VIKING-450与Planck之间的$S_8$张力，验证了该框架在弱引力透镜数据分析中的可行性，为未来更精确的宇宙学约束提供了新途径。
...
### [5] [Single field matter bounce with dark energy era: comparison with CMB Planck 2018 data and best fit parameters](https://arxiv.org/abs/2605.14088)
*Rodrigo F. Pinheiro,Nelson Pinto-Neto*

Main category: astro-ph.CO

Recommendation: ★★☆☆☆ (25/100) - 与 Zotero 兴趣画像中的 lensing, dust, polarization 等主题/作者匹配。

TL;DR: 本文利用Planck 2018 CMB数据进行MCMC分析，对比物质反弹模型与标准暴胀ΛCDM模型，发现仅凭Planck数据无法区分两者，表明该模型可作为暴胀的替代方案。


<details>
  <summary>Details</summary>
Motivation: 检验物质反弹模型是否能作为暴胀理论的可行替代方案，通过CMB观测数据约束模型参数并与标准模型对比。

Method: 采用Markov Chain Monte Carlo (MCMC)方法，利用Planck 2018的温度、极化和透镜化数据，对包含指数势标量场的物质反弹模型进行参数约束。

Result: Planck 2018数据单独无法偏向物质反弹模型或标准暴胀ΛCDM模型，两者在数据拟合上表现相当。

Conclusion: 物质反弹模型可作为暴胀理论的一个可行替代方案。
...
### [6] [Massive Galaxies Form Early and Gray: Stellar Assembly and Dust Attenuation at $\mathbf{z>3.5}$ from CAPERS](https://arxiv.org/abs/2605.13966)
...
### [7] [Probing the IMF in the Early Universe -- Direct measurements in the Boötes I UFD with JWST/NIRCam](https://arxiv.org/abs/2605.15069)
...
TL;DR: 利用JWST/NIRCam对Boötes I超暗矮星系进行解析观测，结合机器学习和模拟推断方法，发现其低质量恒星初始质量函数与银河系一致，支持IMF在低金属丰度及高红移环境下的普适性。


<details>
  <summary>Details</summary>
Motivation: 恒星初始质量函数（IMF）在低金属丰度、高红移环境下的依赖关系仍不清楚，超暗矮星系作为高红移星系的局部化石，为约束次太阳质量的IMF提供了独特途径。

Method: 利用JWST/NIRCam解析Boötes I中超过10,000颗恒星（可达到小于0.15太阳质量），采用三种不同的IMF函数形式，结合合成色-星等图的正向建模与基于模拟的推断（SBI）以及机器学习技术。

Result: 单一幂律IMF无法再现观测的光度函数，且偏离经典Salpeter IMF；最佳拟合的断裂幂律和对数正态IMF参数在68%置信水平上与银河系一致；该星系金属丰度低至[Fe/H]≈-2.4。

Conclusion: 通过将Boötes I视为高红移（z≳6）低质量（≲10^5 M☉）星系的局部遗迹，研究结果为IMF在局域及高红移环境下的普适性提供了证据。
...
TL;DR: JWST观测揭示高红移（z~4-6）星系恒星形成具有高度爆发性（log-SFR弥散~0.6 dex），将其引入线强度映射（LIM）后，散粒噪声功率谱需乘以2-7倍的增强因子，这既提升了可探测性又改变了宇宙学应用前景，同时为研究早期恒星形成物理提供了新诊断手段。


<details>
  <summary>Details</summary>
Motivation: 标准LIM预测假设恒星形成呈对数正态分布（弥散~0.3 dex），但JWST最新观测显示z~4-6的恒星形成更具随机性和爆发性（弥散~0.6 dex），且向小质量晕增长，时间相关性约25 Myr。需评估这种爆发性如何影响LIM散粒噪声功率谱和整体观测前景。

Method: 将JWST观测的恒星形成爆发性（SFR相关函数）与各发射线的恒星种群合成核进行卷积，推导出闭合形式的增强因子B_λ，作为确定性散粒噪声的乘性修正。对Hα、[CII]、CO、UV等多条谱线在z~6处计算增强因子，并外推至高红移。

Result: z~6时，Hα的增强因子B_Hα≈7（约为标准预测的5倍），[CII]、CO、UV等长时标示踪子的B~2.5-3.5（约2-3倍），且增强随红移增加而增大。增强效应提升了自功率谱的可探测性，抑制了低红移污染，但削弱了BAO等依赖干净聚类测量的宇宙学应用。不同谱线间散粒噪声的交叉相关可探测时间相干性。

Conclusion: JWST揭示的恒星形成爆发性显著改变了LIM预测图景：单线红移断层扫描可约束爆发性的振幅和质量依赖性，交叉相关则探测时间相干性——将LIM从传统宇宙学工具转变为研究高红移恒星形成物理的新诊断平台。
...
TL;DR: 本文探讨了利用极大望远镜（ELT）及其首光成像仪MICADO系统性地搜索银河系中心致密天体（恒星级黑洞、中子星、白矮星）的可行性与科学意义，旨在首次定量约束"暗星团"的分布与性质。


<details>
  <summary>Details</summary>
Motivation: 银河系中心理论上应存在大量未被观测到的恒星级致密天体（SCOs），形成"暗星团"，但其分布和性质至今缺乏观测约束。这些天体对理解质量 segregation、星团动力学以及未来引力波天文台（如LISA）可探测的极端质量比旋进（EMRIs）率至关重要。

Method: 利用ELT/MICADO的深光度测量、高空间分辨率和精准天体测量能力，通过光变曲线和轨道天体测量信号搜索SCO-恒星双星系统，并直接探测孤立吸积黑洞与银心富气体环境的相互作用。

Result: ELT/MICADO观测有望首次提供对银河系中心暗星团人口的定量约束。

Conclusion: 建立这些约束对于理解银河系中心的动力学演化、致密天体在核星团中的作用以及星系核中引力波源的天体物理背景至关重要。
