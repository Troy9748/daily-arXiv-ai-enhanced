...
### [1] [The Keplerian disk, envelope, and streamers surrounding an early O-type protostar in the Sagittarius C cloud of the Central Molecular Zone](https://arxiv.org/abs/2605.03883)
...
TL;DR: 本文利用ALMA Band 6观测，在银河系中心附近的Sgr C云团中，发现了一个围绕早期O型大质量原恒星的吸积盘。通过动力学模型拟合分子谱线数据，确定中央原恒星质量约为40太阳质量，吸积盘离心半径约1300 au，包层向盘的吸积率约为7×10^{-3}太阳质量/年，并在盘中识别出类似螺旋结构的自由下落流束。


<details>
  <summary>Details</summary>
Motivation: 盘介导的吸积是大质量恒星形成理论的核心，但早期O型原恒星周围的开普勒盘观测非常稀少，常因复杂的周围结构而难以分辨，因此亟需寻找和研究这类吸积盘。

Method: 使用ALMA Band 6（300 au分辨率）对Sgr C云团中的大质量原恒星进行观测，识别谱线并分析复杂有机分子的空间分布；采用内开普勒盘+外自由下落包层的动力学模型，拟合堆叠的CH₃OCHO分子线的三维位置-位置-速度数据。

Result: 中央原恒星质量约40^{+2}_{-3} M⊙，吸积盘离心半径约1300 au，包层向盘的吸积率约7×10^{-3} M⊙ yr⁻¹，并在盘中识别出自由下落流束构成的螺旋结构。

Conclusion: 研究结果突显了吸积盘和流束在CMZ中早期O型星质量累积过程中的关键作用。
...
### [2] [Resolving the Multiphase Outflow, Shock Signatures, and PAHs in the AGN-Starburst Composite ULIRG F10565+2448 with JWST MIRI/MRS](https://arxiv.org/abs/2605.03016)
...
Recommendation: ★★★★☆ (78/100) - 与 Zotero 兴趣画像中的 jwst, molecular, molecular gas 等主题/作者匹配。
...
TL;DR: 本文利用多种模拟（L-GALAXIES、IllustrisTNG、SIMBA、TNG-Cluster）研究高红移（z>3）低质量星系（M⋆<10¹⁰M☉）的淬灭机制，发现环境淬灭（卫星星系被寄主星系的冲压剥离等过程）是其主要机制。这些淬灭星系大多是卫星星系，且往往是暂时性淬灭——约90%会在几亿年内并合，少数会复活。模拟结果与JWST观测基本一致。


<details>
  <summary>Details</summary>
Motivation: JWST观测显示高红移宇宙中存在比现有模型预测更多的淬灭星系，以往研究多聚焦于大质量星系和AGN反馈机制，但对低质量星系淬灭的起源关注不足。

Method: 分析L-GALAXIES、IllustrisTNG、SIMBA、TNG-Cluster四种模拟（共包含z=5时超过50万个星系），考察卫星占比、寄主晕质量、恒星质量、距中心距离等因素与淬灭的关系，并与JWST观测进行对比。

Result: 淬灭星系中卫星星系占绝大多数；卫星淬灭随寄主晕质量增大而增加，随恒星质量和距中心距离增大而减少；与冲压剥离和气体耗尽强相关；L-GALAXIES模拟结果与JWST观测基本一致；约90%的淬灭卫星在几亿年内并合，少数复活。

Conclusion: 高红移低质量淬灭星系可能主要是处于大质量晕附近的环境淬灭系统，往往是暂时性淬灭。未来更多观测样本将能对此环境淬灭假说进行稳健检验。
...
### [4] [The Significant Role of Hydrogen in the Formation of Silicon Carbide in Evolved Stars](https://arxiv.org/abs/2605.03575)
...
TL;DR: 该论文通过实验室模拟，揭示了在富碳恒星大气中，SiC₂是形成碳化硅星尘的关键分子前体，并阐明了分子氢在其中的核心作用。


<details>
  <summary>Details</summary>
Motivation: 富碳恒星中碳化硅尘埃的形成机制尚不清楚，需要识别关键分子前体和反应路径。

Method: 在实验室模拟从原子C、原子Si和H₂出发形成碳化硅尘埃的过程，结合热化学计算和化学动力学建模进行分析。

Result: 明确识别出SiC₂是碳化硅纳米尘埃的关键分子前体；实验得到的纳米尘埃类似物部分氢化；发现分子氢通过与原子碳反应启动烃类形成，进而与原子硅反应生成气相SiC₂。

Conclusion: 分子氢在SiC₂形成中发挥核心作用，有助于深入理解从原子到分子、团簇再到尘埃颗粒的碳化硅尘埃形成过程。
...
### [5] [Systematic underestimation of polarisation angle dispersion and its consequences for magnetic field strength estimates in star-forming regions](https://arxiv.org/abs/2605.03507)
...
TL;DR: 极化尘埃辐射观测是推断磁场结构和通过偏振位置角弥散估计磁场强度的有效工具。本文研究了像素尺寸和波束卷积对常用非锐化掩模法和结构函数法测量角度弥散的影响，发现所有情况下测量值都被低估，导致磁场强度被高估。作者提出了校正因子方法，并应用于JCMT Orion A OMC-1观测，发现该区域的磁场主要在远大于波束的尺度上变化。


<details>
  <summary>Details</summary>
Motivation: 磁流体动力学湍流导致角度弥散具有尺度依赖性，使得测量变得复杂；而现有方法可能系统性地低估角度弥散，从而高估磁场强度，这可能造成对恒星形成过程中磁场作用的过度强调。

Method: 通过参数化、尺度依赖的模拟图研究像素尺寸和波束卷积对非锐化掩模法和结构函数法测量的影响；提出校正因子的计算方法并应用于JCMT Orion A OMC-1实际观测数据。

Result: 所有情况下测量的角度弥散都被低估，在1-3倍波束尺寸的尺度上低估程度为1-10倍；OMC-1磁场主要在远大于14″波束的尺度上变化，未解析弥散程度较低，14″/0.028 pc尺度上的校正因子约为1.6。

Conclusion: 当前利用角度弥散推导的磁场强度被系统性地高估，可能导致对恒星形成中磁场作用的过于磁主导的观点；所提出的校正因子方法可部分弥补这一偏差。
...
### [6] [Not So Isolated: Green Pea Galaxies in Overdense Environments revealed by VLT/MUSE](https://arxiv.org/abs/2605.03676)
...
TL;DR: 本文研究了24个Green Pea星系（z~0.2）的环境触发机制，发现它们存在于高密度环境中，伴星比例高达33%，但未发现紧密相互作用证据。Green Pea星系是均质的年轻（~230 Myr）、低金属丰度、高比星暴星系，而伴星则更年老（~1.6 Gyr）且异质。研究认为Green Pea星系的星暴不是由紧密大并合触发，更可能是高密度区域中由气体吸积维持的瞬态星暴。


<details>
  <summary>Details</summary>
Motivation: 目前关于Green Pea星系星暴是由内部长期过程还是外部环境触发的争议尚未解决，本文旨在通过环境分析确定外部影响来自紧密相互作用还是弥散过程（如气体吸积）。

Method: 利用VLT/MUSE光谱观测24个z~0.2的Green Pea星系，通过谱线特征识别伴星，推导消光、恒星形成率、恒星质量、年龄、金属丰度等物理性质，并估计群动力学质量。

Result: 识别出22个发射线星系，其中11个为伴星（|Δv|≤500 km/s）；伴星比例33^{+11}_{-8}%，数密度超出场约1 dex；伴星通常位于~100 kpc投影距离，无紧密相互作用证据；Green Pea星系年轻均质，伴星年老异质；群动力学质量比总恒星质量高约3 dex。

Conclusion: Green Pea星系的星暴不是由紧密（10-30 kpc）大并合触发，更可能是高密度区域中由气体吸积维持的瞬态事件；但空间分辨率限制无法排除极紧密并合（<10 kpc）。高动力学-恒星质量比暗示系统中存在大量非恒星物质。
...
### [7] [The Computed Microwave Spectrum of the Protonated Fullerene C60H+](https://arxiv.org/abs/2605.04015)
...
TL;DR: 该研究预测了质子化C60（C60H+）的转动光谱，因其具有约3.8 Debye的永久偶极矩，可作为在冷致密分子云核中探测C60的替代方法。


<details>
  <summary>Details</summary>
Motivation: C60是太空中已知的最大分子，但在冷致密分子云核中，由于缺乏紫外泵浦光子流和明亮背景星，常用的振动荧光发射谱和电子吸收谱方法无法有效探测C60及其阳离子。

Method: 通过计算预测质子化C60（C60H+）的转动光谱。质子化将C60的二十面体对称性降低为Cs对称性，使其产生约3.8 Debye的永久偶极矩，从而允许纯转动跃迁。

Result: C60H+是闭壳层体系，具有显著的永久偶极矩，其转动光谱可用于在分子云核中探测C60的存在。

Conclusion: 利用C60H+的转动光谱为在冷致密分子云核中间接探测C60提供了可行方案。
...
### [8] [Interpreting Galaxy Physical Properties Using Stellar Population Synthesis](https://arxiv.org/abs/2605.03500)
...
TL;DR: 本论文主要研究星系形成与演化，重点探讨在仅有有限波段测光数据时，恒星族群合成（SPS）建模的可靠性。


<details>
  <summary>Details</summary>
Motivation: 尽管观测和理论取得了显著进展，星系物理性质之间关系的具体形成机制仍未被完全理解，因此需要评估SPS模型在数据有限情况下的可靠性。

Method: 利用多波段（UV到IR）数据，通过恒星族群合成（SPS）技术生成模型SED库，并采用贝叶斯统计等拟合技术推断星系物理性质。

Result: 论文分为两部分：第一部分详细阐述SPS建模，第二部分探讨在有限测光数据下SPS建模的可靠性问题。

Conclusion: 星系形成与演化的理解依赖于SPS建模，但在有限波段测光条件下，其可靠性需要审慎评估。
...
### [9] [A Multi-parameter Fuzzy Set Framework for Classifying Red, Blue, and Green Valley Galaxies](https://arxiv.org/abs/2605.04028)
...
TL;DR: 本文提出了一种基于数据驱动的模糊集框架，利用SDSS DR18的多观测参量（如颜色、sSFR、D4000）对星系进行红序、蓝云和绿谷分类，通过连续隶属度而非硬边界划分，减少了红星系和绿谷星系的污染，并揭示了更符合物理演化的分布特征。


<details>
  <summary>Details</summary>
Motivation: 传统星系分类方法依赖颜色或恒星质量的硬边界，导致分类不准确且物理意义不明确，尤其是绿谷星系常被污染。本文旨在提出一种更灵活、物理动机更强的模糊分类方法。

Method: 基于SDSS DR18数据，利用高斯混合模型构建(u-r)颜色、sSFR和D4000的双峰分布隶属函数，采用sigmoid函数分配连续隶属度，并通过模糊最小算子融合多观测量的结果，应用于88,579个星系的体积限制样本。

Result: 模糊分类减少了红星系和绿谷星系的污染；红星系呈现单峰低sSFR分布，绿谷星系展现出更清晰的形态演化特征；AGN比例与恒星质量的关系在不同方法间无显著差异；模糊分类的红星系在大尺度上表现出更强的成团性。

Conclusion: 模糊集框架是一种灵活、物理动机明确的星系分类替代方案，比硬边界方法更准确且更具可解释性，有助于更深入地理解星系群体及其演化过程。
...
### [10] [Microlensing time-scales and flux magnification probabilities of a sample of 204 lensed quasars](https://arxiv.org/abs/2605.03905)
...
TL;DR: 本文对204个已知引力透镜类星体系统进行了微透镜统计分析，考虑了真实光学深度和透镜星系引力效应，发现平均源穿越时间约2.59年，爱因斯坦半径穿越时间约11.29年，并估算了类星体源的半光半径约5.4光天及微透镜质量分数下限≥0.15。


<details>
  <summary>Details</summary>
Motivation: 微透镜既是宇宙学与天体物理学的有用工具，也是哈勃常数测定等研究中的不确定性来源。此前研究多以孤立质量的爱因斯坦环穿越时间为参考尺度，本文旨在将微透镜统计分析扩展到所有已知的透镜类星体系统，并考虑更现实的物理条件。

Method: 对204个透镜系统进行自动透镜建模，考虑类星体大小和透镜星系速度的最新观测结果，为每个像计算微透镜放大率图和直方图，采用薄盘源尺度和不同微透镜质量分数进行匹配分析。

Result: 平均源穿越时间2.59±0.07年，平均爱因斯坦半径穿越时间11.29±0.05年。微透镜质量分数α=0.2时模型与实验放大率直方图匹配良好。估算半光半径5.4±2.7光天，微透镜质量分数下限α≥0.15。单个像发生高放大率事件的平均概率约9%。

Conclusion: 通过大规模系统分析，得到了较精确的穿越时间统计结果，约束了类星体源大小和微透镜质量分数，并识别出具有高放大率概率和短穿越时间的候选图像组。
...
### [11] [ArkenstoneBH. A model for high-specific energy black hole feedback in cosmological simulations](https://arxiv.org/abs/2605.03154)
...
TL;DR: 本文介绍了Arkenstone框架的黑洞反馈模型（Arkenstone BH），该模型扩展了原有的多相流子网格框架，专门用于模拟高比能黑洞反馈对星系演化的影响。在孤立星系模拟中，该模型成功展示了高能反馈如何抑制恒星形成，通过抵销从星系周介质流入星际介质的气体来实现。


<details>
  <summary>Details</summary>
Motivation: AGN反馈是星系演化的关键环节，但由于其高比能、多相性以及模拟分辨率有限，难以准确建模。现有模拟框架需要更好的方法来处理高能黑洞反馈。

Method: 将Arkenstone子网格框架扩展至黑洞反馈场景（Arkenstone BH），重点处理热的高比能外流相；冷云多相结构将在后续工作中实现。在孤立星系模拟中验证框架性能。

Result: Arkenstone BH能够捕捉与冷稠密气体弱相互作用的高比能反馈，有效抑制星系的恒星形成，机制是抵销星系周介质向星际介质的气体流入。

Conclusion: 该工作是"Learning the Universe"合作项目的一部分，为后续包含多相冷云结构的完整模型奠定了基础，有助于更准确地理解AGN反馈在星系演化中的作用。
...
### [12] [Peering down the barrel with DESI DR2: 10 000+ inflows at $z$ < 0.6 reveal how galaxies accrete cold gas](https://arxiv.org/abs/2605.02999)
...
TL;DR: 利用DESI数据对1560万个星系进行Na I D吸收线搜索，发现了5万多个具有证据的星系吸收系统，揭示了星系在z<0.6时期通过多种路径（低速内流、卫星吸积等）获取气体的过程。


<details>
  <summary>Details</summary>
Motivation: 目前对星系如何获取气体（重子循环）的直接观测约束极其有限，亟需大规模统计样本来揭示气体吸积机制。

Method: 使用DESI DR2数据对z<0.6的1560万个星系进行down-the-barrel Na I D吸收线搜索，通过贝叶斯证据比评估吸收成分是否来自星际介质而非星系系统成分，构建了50088个（中等证据）和27420个（强证据）星系目录。

Result: 吸收成分速度分布广泛：约50%在v_flow < -50 km/s（内流），30%在系统速度±50 km/s内，20%在v_flow > 50 km/s（外流）。在侧向星系中发现大量低速（~20 km/s）内流吸收体，与模拟预测的径向内流一致。早型星系中内流速度与恒星速度弥散的相关性强于与恒星质量的相关性。

Conclusion: 首次在统计显著的大样本中揭示了z<0.6时期星系吸积气体的多种路径，包括径向内流和卫星吸积等机制。
...
### [13] [Built to Rest: The Evolving Star-Forming Main Sequence Requires Episodic Quiescence or Late Assembly](https://arxiv.org/abs/2605.02997)
...
TL;DR: 本文构建了一个解析模型，研究星系沿恒星形成主序的演化历史，发现低质量早期形成星系若始终保持在主序上，最终会演化为今天的大质量星系，而银河系质量的星系必须经历显著的恒星形成抑制阶段，否则会变得过于庞大。


<details>
  <summary>Details</summary>
Motivation: 现有观测已证实星系恒星形成主序存在于高红移，但星系在其上停留多久、离开后多久返回等问题尚不明确，因此需要构建模型来预测典型的恒星形成历史。

Method: 构建解析模型，沿恒星形成主序随时间演化星系性质，并与观测数据重建的恒星形成历史和平均年龄进行对比验证。

Result: 模型预测与观测吻合良好；大质量星系自然呈现更年老、更尖锐的恒星形成历史；低质量早期形成星系若一直留在主序上，今天会变成约10^11太阳质量的大质量星系；中等质量星系的前身必须晚形成或经历显著抑制阶段；银河系质量的星系必须经历接近静止的恒星形成抑制阶段。

Conclusion: 星系必须经历离开恒星形成主序的抑制阶段，否则会变得过于庞大；对于银河系，其大量年老恒星的存在直接意味着其在大部分历史中已偏离了主序。
...
### [14] [\textit{Euclid} preparation. Baryon acoustic oscillations extraction techniques: comparison and optimisation](https://arxiv.org/abs/2605.03446)
...
TL;DR: 本文首次端到端验证了欧几里得卫星重子声学振荡（BAO）分析流程，涵盖密度场重建、两点相关函数测量和宇宙学参数推断。基于八组欧几里得模拟星表，在红移0.9至1.8范围内评估了两种标准BAO重建方法（RecSym和RecIso）。通过引入基于仿真器的模型评估器（Bora.jl）与哈密顿蒙特卡洛采样器（NUTS），实现了超过500倍的速度提升，并采用半解析协方差估计器（BeXiCov+WinCov）仅用八个模拟实现即可获得稳健误差估计。两种重建方法在所有红移和分析选择下均得到无偏BAO测量，重建后{Ω_m, H_0 r_s}的质量因子提升约3倍，相当于有效巡天体积扩大三倍。BAO单独约束达到Ω_m约10%精度、H_0 r_s约3%精度。


<details>
  <summary>Details</summary>
Motivation: 首次端到端验证欧几里得卫星BAO分析流程的准确性、鲁棒性和可扩展性，为首次数据发布（DR1）的宇宙学分析奠定基础。

Method: 使用八组欧几里得Flagship I快照的模拟星表，评估基于Zel'dovich近似的RecSym和RecIso两种标准BAO重建方法。引入Bora.jl仿真器模型评估器+NUTS采样器（速度提升500倍），以及BeXiCov+WinCov半解析协方差估计器（仅需8个模拟实现即可获得稳健误差估计）。

Result: 两种重建方法在所有红移和分析选择下均得到无偏BAO测量。重建后{Ω_m, H_0 r_s}质量因子提升约3倍，相当于有效巡天体积扩大三倍。BAO单独约束达到Ω_m约10%精度、H_0 r_s约3%精度。RecSym和RecIso结果在不确定度内一致，但RecSym对协方差变化敏感度更低。

Conclusion: 欧几里得BAO分析流程在准确性、鲁棒性和可扩展性方面得到验证，为DR1及未来的宇宙学分析提供了坚实基础。
...
### [15] [New constraints on primordial non-Gaussianity from large-scale cross-correlations of CMB lensing and the cosmic infrared background](https://arxiv.org/abs/2605.03783)
*Joseph Thornton,Fiona McCarthy,Carmen Embil Villagra,Blake D. Sherwin*

Main category: astro-ph.CO

Recommendation: ★★★★☆ (75/100) - 与 Zotero 兴趣画像中的 lensing, dust 等主题/作者匹配。

TL;DR: 本文通过宇宙红外背景（CIB）与普朗克CMB透镜图的互相关分析，利用尺度依赖偏置效应，对局域型原初非高斯性参数f_NL^local给出了新约束，结果为f_NL^local=43±23，精度与现有最严格约束相当。


<details>
  <summary>Details</summary>
Motivation: 现有CIB分析受大尺度银河尘埃污染限制，对f_NL^local的约束力有限；本文旨在通过改进的尘埃清理CIB地图和互相关方法，提升约束精度，避免系统偏差。

Method: 采用新设计的无偏大尺度尘埃清理CIB地图，与普朗克CMB透镜图进行互相关分析，利用尺度依赖偏置效应约束局域型原初非高斯性参数。

Result: 得到f_NL^local=43±23，约束精度较以往CIB分析提升约2倍，与现有交叉相关方法的最严格约束精度相当。

Conclusion: 一致性检验和零检验表明结果对模型假设和残留尘埃污染具有鲁棒性，该方法有效提升了CIB对原初非高斯性的约束能力。
...
### [16] [Thermal activation rate of dilute axion stars close to the maximum mass](https://arxiv.org/abs/2605.03771)
...
TL;DR: 本文利用瞬子理论计算了具有吸引自相互作用的亚稳态自引力玻色-爱因斯坦凝聚体（如稀释轴子星）的热激活速率。在最大质量附近给出了显式解析结果，发现亚稳态寿命极长（与粒子数N呈指数关系），因此在实际中可视为稳定平衡态。


<details>
  <summary>Details</summary>
Motivation: 为了理解亚稳态自引力玻色-爱因斯坦凝聚体（如轴子星）的热激活衰变速率，判断其在物理和天体物理中是否可被视为稳定状态。

Method: 采用瞬子理论计算热激活速率，在接近最大质量处利用鞍-结分岔的标准形式给出显式解析结果。

Result: 亚稳态寿命极长，与粒子数N呈指数关系：t_life ~ e^N t_D。对于QCD轴子星（N~10^57, t_D~10小时）和超轻轴子暗物质晕量子核（N~10^96, t_D~100 Myr）均如此。

Conclusion: 亚稳态自引力玻色-爱因斯坦凝聚体在实际中可视为稳定平衡态，该结果与实验室BEC、球状星团、自引力布朗粒子、BMF模型及细菌种群等领域的类似结果具有可比性。
