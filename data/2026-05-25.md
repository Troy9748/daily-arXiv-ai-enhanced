...
### [1] [ALMA CO-CAVITY I. Resolved Molecular Gas in Void Galaxies](https://arxiv.org/abs/2605.23399)
...
### [2] [A comparison between Galactic magnetic field models and polarized synchrotron emission with C-BASS at 4.76 GHz and S-PASS at 2.3 GHz](https://arxiv.org/abs/2605.23489)
...
TL;DR: 本文通过比较银河系磁场模型与S-PASS和C-BASS巡天的极化同步辐射观测，发现现有模型在极化强度形态上表现不佳，并指出局部结构（如北银极 Spur/Loop I 和 Fan 区域）对精确建模至关重要。


<details>
  <summary>Details</summary>
Motivation: 评估当代银河系磁场模型能否准确再现极化同步辐射观测，以改进对微波波段极化发射的建模。

Method: 结合S-PASS和C-BASS巡天数据，排除大法拉第旋转像素并施加小角度法拉第修正；采用模板拟合方法评估各模型在极化强度和极化角方面的表现。

Result: 大多数GMF模型在极化角上匹配较好，但难以复现极化强度的形态；模型与数据在大尺度极化角上存在明显相关性，但在极化强度上不成立。

Conclusion: 局部前景区（如北银极Spur/Loop I和Fan区域）对极化同步辐射有显著贡献，须纳入模型才能精确描述微波波段的极化发射。
...
### [3] [No Blue without Red: Evolutionary Properties of Super-Early Galaxies](https://arxiv.org/abs/2605.22914)
*A. Ferrara,G. Rodighiero,S. Carniani,Z. Zhang,M. Kohandel,B. Das*

Main category: astro-ph.GA

Recommendation: ★★★★☆ (73/100) - 与 Zotero 兴趣画像中的 jwst, alma, dust 等主题/作者匹配。

TL;DR: 本文利用无衰减模型（AFM）解释了JWST发现的z>10超早期星系紫外特性的多样性，认为辐射驱动外流将尘埃重新分布到星系外围，揭示了星系从“红怪物”（尘埃遮蔽）向“蓝怪物”（紫外明亮）的演化序列，并指出EGS-z11-R0正处于遮蔽阶段。


<details>
  <summary>Details</summary>
Motivation: JWST观测到z>10的超早期星系在紫外特性上呈现显著多样性（从极蓝的贫尘系统到显著红化的源），需要统一的物理机制来解释这一现象。

Method: 基于无衰减模型（AFM），对32个光谱确认的超早期星系样本进行建模，推导其暗物质晕质量、恒星形成效率、金属丰度和外流范围等关键物理性质。

Result: 这些星系位于质量约10^10.7 M⊙的大质量暗晕中，恒星形成效率中等（0.01-0.05），常达到超爱丁顿条件并触发强大外流；紧凑源（r_e<150 pc）难以用AFM解释，可能由活动星系核主导。

Conclusion: 提出超早期星系从尘埃遮蔽的“红怪物”相演变为紫外明亮的“蓝怪物”相的统一演化序列，辐射驱动外流在宇宙黎明时期的星系演化中起关键作用。
...
### [4] [Suppression of star formation at the centre of barred AGN galaxies](https://arxiv.org/abs/2605.23269)
...
### [5] [Astrophysical Parameters of 5056 Open Star Clusters from Bayesian Nested Sampling with PARSEC Isochrones](https://arxiv.org/abs/2605.23802)
...
TL;DR: 本文基于Gaia DR3数据，利用贝叶斯嵌套采样和PARSEC恒星等时线，统一推导了5,056个疏散星团的基本天体物理参数（年龄、金属丰度、日心距离和颜色超），其中3,766个高质量星团（74.5%）的拟合质量指数≥0.80，并公开提供了星表数据。


<details>
  <summary>Details</summary>
Motivation: 现有疏散星团星表缺乏统一、无人工干预的参数推导方法，亟需一个基于同质化方法、参数自洽的大样本星团星表，以减少不同研究之间的系统偏差。

Method: 从Unified Cluster Catalogue (UCC)中选取5,056个疏散星团，利用Gaia DR3的色-星等图，结合PARSEC恒星等时线进行贝叶斯嵌套采样。将初始金属丰度Z_ini作为自由参数，采用基于物理的先验（Gaia DR3天体测量提供的视差距离、Gaia XP光谱的分光光度金属丰度约束、Schlegel-Finkbeiner-Davis尘埃图的星际消光）来减少简并性，全程无人工干预。

Result: 获得5,056个星团的参数，其中3,766个（74.5%）满足拟合质量标准η_fit ≥ 0.80。高质量子样本的年龄范围为0.003-5.5 Gyr，日心距离88-19,011 pc，金属丰度-1.17 ≤ [Fe/H] ≤ +0.42 dex，消光A_G最高达7.37 mag。星表通过CDS/VizieR公开，后验链数据存档于Zenodo。

Conclusion: 本文提供了一份同质化的疏散星团参数星表，验证了全自动贝叶斯嵌套采样方法在大样本星团参数推导中的有效性，为银河系结构和演化研究提供了重要数据资源。
...
### [6] [Examining extinction distributions for type Ia supernovae in simulated 3D galaxies](https://arxiv.org/abs/2605.23512)
...
TL;DR: 本文通过辐射转移模拟发现，标准单参数指数模型无法准确描述Ia超新星消光分布，而双参数分布（如Weibull或指数化指数分布）能更精确地拟合不同环境下的消光特征，并影响宇宙学分析中本征颜色的推断。


<details>
  <summary>Details</summary>
Motivation: 现有消光概率密度函数（PDF）模型过于简单（如单参数指数模型），可能对超新星模拟和宇宙学测量产生负面影响，亟需更精确的消光参数化方法。

Method: 使用SKIRT辐射转移代码模拟不同星系环境中的Ia超新星消光，生成合成消光分布；分别用单参数指数PDF和多种双参数PDF（如Weibull、指数化指数）进行拟合；并通过2D KS检验和文献数据对比分析。

Result: 标准指数PDF低估低消光事件、高估高消光事件，无法区分不同环境下的消光差异；双参数PDF在所有模拟环境中拟合更准确；双参数消光PDF导致本征颜色更红，均值偏移约2σ。

Conclusion: 双参数Weibull或指数化指数PDF是描述Ia超新星消光的最佳参数化方案，能更好区分宿主星系形态与尘埃质量的影响，并显著改变宇宙学分析中对本征颜色的推断。
...
### [7] [Cosmic web stripping and starvation of low-mass filament galaxies in TNG50](https://arxiv.org/abs/2605.23457)
...
TL;DR: 本文利用TNG50-1模拟研究低质量星系在宇宙纤维与场环境中的性质差异。虽然整体性质相似，但纤维星系表现出更小、更不对称的冷气体盘，这由两种机制驱动：早期进入纤维的星系因宇宙网潮汐场导致气体吸积几何改变或吸积抑制；晚期落入纤维的星系则经历类似冲压剥离的宇宙网剥离过程。


<details>
  <summary>Details</summary>
Motivation: 探究宇宙纤维环境对低质量星系（8 ≤ log(M_star/M_sun) ≤ 10）性质的影响，特别是在排除群和团环境影响后，纤维结构如何塑造星系演化。

Method: 使用TNG50-1流体动力学模拟，控制恒星质量与暗物质晕质量，比较纤维星系与场星系的积分性质（恒星质量组装、暗物质晕组装、熄灭比例）和空间分辨的冷气体盘性质。

Result: 纤维星系与场星系的积分性质相似，但纤维星系的冷气体盘更小、更不对称。早期进入纤维的星系经历潮汐场导致的吸积几何改变或吸积抑制（饥饿演化）；晚期落入纤维的星系经历宇宙网剥离（类似冲压剥离的快速气体移除）。

Conclusion: 空间分辨的气体性质对纤维驱动的环境机制敏感，纤维环境通过潮汐场和动力学剥离两种途径影响低质量星系的冷气体盘结构。
...
TL;DR: 本文利用LoTSS DR2巡天数据与DESI星系团目录交叉匹配，研究FRI/FRII射电星系形态二分法是否与星系团环境有关。结果发现FRII在星系团中出现频率更低，尤其在高射电光度下差异更显著，但一旦进入星系团，两者所处环境相似，说明星系团尺度环境并非二分法的主因。


<details>
  <summary>Details</summary>
Motivation: Fanaroff-Riley I/II射电星系形态二分法的起源尚不明确，本研究旨在探究星系团尺度环境是否在这一分类中发挥作用。

Method: 使用LoTSS DR2中z<0.4的形态分类目录，构建了光度L_{144}>4×10²⁴ W Hz⁻¹的体限样本和光度-红移配对样本，与DESI Legacy Imaging Survey星系团交叉匹配。关联条件为|Δz|<0.01且投影距离<2R_{500}。

Result: 体限样本中48.6%的FRI和30.6%的FRII与星系团关联；配对样本中对应比例为45.6%和32.6%。高光度L_{144}>10²⁶ W Hz⁻¹时差异更显著，体限样本中55.6% vs 19.0%，配对样本中50.0% vs 6.7%。但两者在团内占据相似环境（射电光度、恒星质量与团富度/M_{500}的关系相似，径向分布在0.5R_{500}处峰值）。大多数与团关联的源是BCG。

Conclusion: FRII在星系团中较少出现（尤其在高射电光度时），支持致密团内气体破坏或减速喷流、抑制稳定FRII结构的假说。但一旦进入团内，FRI和FRII所处大尺度环境相似，说明星系团尺度属性本身不太可能是FRI/FRII二分法的主要驱动因素。
...
### [9] [The FAST Hundred-Deg$^2$ HI Deep (HD$^2$) Survey: Early Results from the Pilot Survey](https://arxiv.org/abs/2605.23686)
...
TL;DR: 本文介绍了利用FAST望远镜开展的百平方度HI深度巡天（HD²）的初步结果。在10平方度的先导巡天中，识别出339个HI源，探测密度约为每平方度34个，是宽场巡天的6倍。光学对应体识别匹配率超过90%，并为DESI BGS样本中近50%的星系提供了HI探测。结果表明完整HD²巡天是可行的，将构建高完备度的HI普查。


<details>
  <summary>Details</summary>
Motivation: 利用FAST望远镜的高灵敏度优势，开展大天区、高深度的中性氢（HI）巡天，以构建高完备度的HI星系样本，进而研究星系的冷气体标度关系。

Method: 利用FAST望远镜进行先导巡天观测，在10平方度的天区内实现7.3分钟/波束的积分时间和0.45 mJy/beam的灵敏度（4.8 km/s分辨率）。通过DESI光谱数据识别光学对应体，并分析HI探测星系的光学性质及气体分数标度关系。

Result: 识别出339个HI源（z<0.09），探测密度~34个/平方度，是宽场巡天的6倍。光学对应体匹配率和正确率超过90%（r<19.5 mag）。在r<17.8 mag且0.01<z<0.05约束下，DESI BGS样本中近50%的星系有HI探测。气体分数与恒星质量、恒星质量面密度、NUV-r、比恒星形成率的标度关系与ALFALFA、DINGO、xGASS等巡天一致。

Conclusion: 先导巡天结果验证了完整HD²巡天的可行性，该巡天将在连续天区内构建高完备度的HI星系样本，为研究不同尺度下星系的冷气体标度关系提供重要数据支撑。
...
### [10] [Atomic gas properties at the positions of supernovae Type Ia, II, and Ib/c](https://arxiv.org/abs/2605.23579)
...
TL;DR: 本文基于133颗超新星的样本，分析了其所在位置的原子气体分布，发现II型超新星在原子气体分布上的偏移最大，但所有类型超新星的位置分布与原子气体密度并无确定性关联，无法证实Ib/c和II型超新星与最密集的原子气体有关。


<details>
  <summary>Details</summary>
Motivation: 理解哪些恒星最终爆炸成为哪种类型的超新星，对于测量它们在金属生产和反馈停止星形成中的贡献至关重要。此前关于超新星环境气体的研究受限于小样本量（<10），因此需要更大样本的统计分析来约束超新星的性质。

Method: 选取了133颗超新星（29颗Ia型、77颗II型、27颗Ib/c型），这些超新星所在的星系已有原子气体数据。通过分析HI图上比超新星所在像素更暗的像素比例以及这些像素贡献的HI流量比例，来检验超新星位置是否追踪原子气体分布的增强区域。

Result: 所有类型的超新星位置分布偏离完全随机分布。II型超新星在HI分布上的偏移最大，倾向于更高浓度的原子气体，且偏离了与宿主恒星表面密度的比例关系。但三种超新星类型的分布之间仍相互一致。结果受到调查规模和HI观测分辨率不均匀的限制。

Conclusion: 无法确定Ib/c型核坍缩超新星以及II型超新星与其宿主中密度最高的原子气体有关，这与GRB和Ic-BL超新星的情况不同。II型和Ib/c型超新星的前身星仍可能与宿主中的当前恒星形成有关，而GRB和Ic-BL超新星的前身星需要更特殊的条件（如低金属丰度）才能形成。
...
Recommendation: ★★★★☆ (78/100) - 与 Zotero 兴趣画像中的 euclid, lensing 等主题/作者匹配。

TL;DR: cloelike 是一个用于 ESA Euclid 任务的 Python 包，提供模块化、可组合的高斯似然类，涵盖弱引力透镜、星系聚类、星系-星系透镜、光谱功率谱和 BAO 等主要宇宙学观测数据。


<details>
  <summary>Details</summary>
Motivation: 为 Euclid 空间任务提供统一的、模块化的高斯似然计算工具，支持社区验证和结果可重复性。

Method: 实现模块化、可组合的高斯似然类，支持谐波角功率谱和实空间两点相关函数，涵盖 WL、GCph、GGL 及其联合探测组合 (3x2pt, 2x2pt)，以及光谱全形功率谱多极子和 BAO。

Result: 已积极用于 Euclid 联盟内部分析，并开源发布以支持社区验证和可重复性。

Conclusion: cloelike 是 CLOE 生态系统的核心组件，通过接口连接理论预测和官方数据产品，为 Euclid 任务提供标准化的似然分析框架。
...
### [12] [cloelib: A Flexible Python Library for Computing Cosmological Observables in the Euclid Era](https://arxiv.org/abs/2605.23839)
...
TL;DR: cloelib 是一个为 Euclid 任务开发的 Python 库，用于计算宇宙学观测数据，支持自动微分、模块化架构，并计划作为 Euclid 首次宇宙学发布的参考理论计算基础设施。


<details>
  <summary>Details</summary>
Motivation: 随着 Euclid 等巡天项目推动宇宙学进入高精度时代，需要灵活、高效、可微分的软件来支持下一代推断流程。

Method: 采用模块化架构，与现有 Boltzmann 求解器无缝接口，并集成 JAX 自动微分实现基于梯度的方法；定义了背景演化、扰动和非线性结构形成的统一协议。

Result: 支持多种宇宙学观测数据，包括光电和光谱大尺度结构探针，以及与 CMB 和星系团的互相关。

Conclusion: cloelib 将作为 Euclid 首次宇宙学发布的参考理论计算基础设施，连接传统数值宇宙学与现代优化技术及机器学习推断方法。
...
### [13] [Dissecting the Perseus-Pisces supercluster observed with CFHT-MegaCam: Exploring late-type galaxy shape alignments within the local cosmic web](https://arxiv.org/abs/2605.23338)
...
TL;DR: 本研究利用CFHT深度r波段巡天数据，分析了Perseus-Pisces超星系团中2004个星系的固有取向对齐信号，发现早型和晚型星系在1 Mpc/h内均存在正向对齐，且形态依赖性显著：晚型星系主导形状-形状相关性，早型星系则主导位置-形状相关性，表明两者存在不同的对齐机制。


<details>
  <summary>Details</summary>
Motivation: 星系形状的固有对齐是弱引力透镜测量的主要系统误差，同时也能揭示星系在宇宙网中获得取向的物理机制。以往研究依赖大统计样本，本文旨在单一近邻超星系结构中探测该信号，并延伸至次内落主导的外围区域。

Method: 利用CFHT r波段深度成像（UNIONS巡天），覆盖367平方度，表面亮度达28 mag/arcsec²。针对2004个log M*/M☉ > 8.6的星系，按形态和恒星质量分层，在共动坐标下计算三个等光深半径处的相关函数。

Result: 早型和晚型星系在1 Mpc/h内均检测到正向固有对齐信号，包括旋涡星系的清晰信号。形状-形状相关性强于位置-形状相关性。晚型星系主导形状-形状信号，偏好纤维状结构且呈高椭率边缘取向；早型星系则聚集在群和星系团中心，位置-形状相关性无显著超额。

Conclusion: 形态分离表明存在不同的对齐机制：早型星系在致密环境中受潮汐拉伸，晚型星系在纤维结构中受潮汐转矩。这些局域宇宙约束对Euclid、DESI和LSST的固有对齐建模至关重要，凸显了蓝色旋涡星系对低红移宇宙剪切污染的贡献。
...
### [14] [A No-Go Theorem for the Mass-Radius Relation of Solitons](https://arxiv.org/abs/2605.22901)
...
TL;DR: 本论文证明了一个关于孤子（稳定局域场构型）质径关系的不可行定理：在d维空间中，典型非拓扑、非相对论、球对称孤子的质径指数Γ不能落在[0, d]范围内。该定理适用于任意自相互作用，并排除了超轻或流体状暗物质模型作为晕核自然解释的可能性，因为观测到的暗物质晕核Γ≈1.7（d=3时）正好位于排除区间内。


<details>
  <summary>Details</summary>
Motivation: 作者希望从理论上严格证明特定类型的孤子（如暗物质晕核候选者）在质径关系上存在一个不可能区间，从而排除某些暗物质模型作为晕核自然解释的可能性。

Method: 作者通过考虑梯度能、自相互作用和引力三种力的相对强度，在排除精细调谐参数区域（三种力强度相当）的"典型性"假设下，对任意自相互作用的非拓扑、非相对论、球对称孤子进行理论分析，推导出质径指数Γ的排除范围[0, d]。

Result: 证明了典型孤子的质径指数Γ不能落在[0, d]区间内；该结论同样适用于自引力、非相对论、正压流体的致密天体；观测到的暗物质晕核Γ≈1.7（d=3时）正好位于排除区间内。

Conclusion: 超轻暗物质或流体状暗物质模型作为晕核的自然解释基本上被排除，除非其他天体物理效应不可忽略。
...
### [15] [The $τ$ of Neutral Hydrogen: Increased CMB Optical Depth at Long Wavelengths](https://arxiv.org/abs/2605.22934)
...
TL;DR: 本文提出利用长波（>21cm）CMB光子被中性氢吸收产生的额外光学深度，通过交叉关联射电波段与毫米波段CMB涨落，来测量宇宙历史中中性氢分数与自旋温度之比（x_HI/T_s），这比直接探测21cm信号更容易实现。


<details>
  <summary>Details</summary>
Motivation: 长波CMB光子被中性氢吸收会产生额外的团簇抑制效应，这一光学深度敏感于自旋温度和中性氢分数，但目前缺乏对其有效的观测手段。本文旨在探索利用交叉关联方法间接测量这一信号。

Method: 理论分析长波CMB光子被中性氢吸收的光学深度随频率的依赖关系，提出将射电波段（对应21cm暗夜）的CMB涨落与已充分表征的毫米波段CMB涨落进行交叉关联，以提取吸收信号。

Result: 过量光学深度在100 MHz附近达到百分之几的水平；射电波段与毫米波段CMB涨落的交叉关联探测可能比直接探测21cm本征涨落更容易实现。

Conclusion: 测量CMB涨落幅度随射电波长的变化，提供了一种直接测量不同红移处x_HI/T_s组合的新途径。
