...
TL;DR: 本文提出利用谱线强度映射（LIM）探测活动星系核（AGN），通过[Ne V]发射线与星系巡天交叉关联，可检测到低于直接探测阈值的AGN群体，并预测了两种未来仪器的信噪比表现。


<details>
  <summary>Details</summary>
Motivation: 现有AGN探测方法对暗弱、尘埃遮蔽的AGN不敏感，需要一种能有效追踪被遮蔽AGN且不受其他天体污染的新探测手段。

Method: 利用[Ne V]谱线（电离势97.1 eV）作为AGN活动的纯净示踪剂，通过LIM技术进行强度映射，与星系红移巡天交叉关联，并使用Fisher矩阵形式预测信噪比。

Result: CDIM类仪器可在z=2-3对AGN平均强度与偏置因子的乘积给出强约束，红移空间畸变可分解二者；对低于5σ直接探测阈值（z=3时对应L_bol~5×10^43 erg/s）的AGN群体仍保持灵敏度，约10%的来自亚阈值源；PRIMA类仪器信噪比略低，但14.3μm谱线不受尘埃消光影响，提供互补探测。

Conclusion: AGN谱线强度映射可应用于追踪AGN总光度历史、约束暗弱光度下黑洞-晕关系、区分超大质量黑洞种子形成机制等科学问题。
...
### [2] [The Multi-phase Biconical Outflow in the local IR-Luminous Merger IRASF01364-1042](https://arxiv.org/abs/2606.06005)
...
### [3] [Strong [OIII]$+$H$β$ emitters dominated the ionizing budget at $z\sim7$](https://arxiv.org/abs/2606.05323)
*Isak Wold,Sangeeta Malhotra,James Rhoads*

Main category: astro-ph.GA

Recommendation: ★★★★☆ (73/100) - 与 Zotero 兴趣画像中的 jwst, lensing, dust 等主题/作者匹配。

TL;DR: 本研究利用JWST超深场数据与引力透镜效应，在红移~7处筛选出强[OIII]+Hβ发射线星系，发现这些年轻、贫金属、低尘埃的星系贡献了再电离所需电离光子预算的约70%，证实已知星系群体是再电离的重要动力来源。


<details>
  <summary>Details</summary>
Motivation: 量化红移~7处电离光子的产生速率，评估已知星系群体在再电离过程中的贡献。

Method: 利用UNCOVER超深F410M成像与引力透镜效应，结合NIRSpec光谱确认，通过强[OIII]+Hβ发射线筛选年轻、贫金属、低尘埃的星系候选体，并测量其物理性质与电离光子逃逸分数。

Result: 高等值宽发射线星系占UV选择总体的56±12%；电离光子产生率log(ṅion)=50.63±0.05，贡献了再电离所需总预算的约70%。

Conclusion: [OIII]+Hβ选择是一种强有力的、不受尘埃影响的探测手段，表明已知的星系群体在再电离过程中发挥着主导作用。
...
### [4] [Mapping Active Star-Formation in Serpens and the Aquila Rift](https://arxiv.org/abs/2606.05668)
...
TL;DR: 本文利用DECam在蛇夫-天鹰裂隙进行高灵敏度HH天体巡测，新发现88个HH天体，使该区域已知总数增加三倍以上。结合尘埃图和Gaia数据，识别出五个活跃恒星形成云，并发现HH天体可有效示踪反馈驱动的星形成区域，即使在缺乏天文学家表征的年轻恒星时也能指示星形成位置。


<details>
  <summary>Details</summary>
Motivation: 蛇夫-天鹰裂隙是恒星形成活跃区，但此前HH天体巡测不够全面，限制了对其星形成结构和动力学过程的理解。

Method: 使用DECam在4米望远镜上进行高灵敏度巡测，结合尘埃图、Gaia天体测量数据和前身星识别，对HH天体及其所在分子云进行空间和运动学分析。

Result: 新发现88个HH天体，识别出5个独立的活跃恒星形成云（蛇夫分子云、LDN 673、西天鹰裂隙、东天鹰裂隙、蛇夫近距云），空间分布揭示反馈驱动的空腔结构。

Conclusion: HH天体即使在没有已表征的年轻恒星时也能有效指示星形成位置，可作为未来YSO巡测的有力工具；蛇夫近距云可能属于Local Bubble，与Scutum North Association相关。
...
### [5] [Little red dots as a cosmological probe: constraining $H_0$ with quasi-periodic pulsations](https://arxiv.org/abs/2606.05281)
...
TL;DR: 本文基于“小红点”（LRDs）的脉动现象，推导了周期-光度-温度关系，并提出利用该关系约束哈勃常数的新方法，初步给出H0=120.7 km/s/Mpc，预测10年观测可将不确定度降至3-20%。


<details>
  <summary>Details</summary>
Motivation: JWST发现的高红移“小红点”可能是早期超大质量黑洞被致密气体包层包裹所致，其中RXJ2211-RX1表现出类似恒星脉动的准周期光变，这为利用脉动关系测量哈勃常数提供了新的可能性。

Method: 基于流体静力学包层模型，推导了理想化的周期-光度-有效温度关系；利用该关系结合稀疏观测数据初步估算H0；并通过前瞻性分析评估不同基线长度对H0不确定度的影响。

Result: 初步得到H0=120.7_{-46.5}^{+47.0} km/s/Mpc；连续监测10年可将不确定度降至3-20%，但系统误差尚待完整评估。

Conclusion: 该脉动方法有望成为早期宇宙中测量光度距离的独立探针，为哈勃常数测定提供新途径。
...
### [6] [A New Member of the Fast and Furious Family: A Relativistic and Time-Variable UV Outflow in a Luminous Quasar](https://arxiv.org/abs/2606.06226)
...
TL;DR: 在类星体SDSS J231854.31+243954.2中首次探测到紫外波段的最快外流，速度达-77,000至-90,000 km/s（约0.3c），其动能光度比下限超过反馈阈值，对宿主星系具有显著反馈效应。


<details>
  <summary>Details</summary>
Motivation: 类星体外流对星系演化具有重要反馈作用，但极高速（接近0.3倍光速）的紫外吸收外流极为罕见，此前仅在一个源（PDS 456）中被观测到。发现并研究这类极端外流对于理解类星体反馈机制和星系演化模型至关重要。

Method: 利用Gemini GNIRS光谱测量Hα红移（z=2.6781），通过二十年的光变曲线分析g波段0.5星等峰值变化，在三个历元（覆盖约2.2个静止系年）监测C IV吸收线的增强，结合维里估计计算黑洞质量、爱丁顿光度及外流参数。

Result: 发现速度-77,000至-90,000 km/s的C IV和Si IV吸收外流；黑洞质量1.65×10^9 M⊙，爱丁顿比0.45；外流质量损失率>0.82 M⊙/yr，动能光度比L_kin/L_bol≥0.75%，超过反馈阈值；与PDS 456对比暗示真实值可能高出两个数量级。

Conclusion: 该发现对类星体外流模型提出挑战：模型需解释如何将气体加速至0.3c的同时保持C IV和Si IV离子存在，或如何在已加速的气体中形成这些离子。该外流可能对宿主星系产生显著反馈，其强度可能远超当前保守估计。
...
### [7] [The quenching time and timescale distribution of z~2 quiescent galaxies from precise colour distribution analysis](https://arxiv.org/abs/2606.05971)
...
TL;DR: 本文利用模拟推断方法，通过宽波段测光颜色分布直接约束红移1.7-2之间大质量星系的形成时间、熄灭时间和熄灭时标，发现星系必须快速熄灭（中位时标约182百万年）才能实现精确的双峰颜色分布。


<details>
  <summary>Details</summary>
Motivation: 理解星系何时以及如何熄灭其恒星形成活动，对于揭示主导的物理过程至关重要。

Method: 采用基于模拟的推断方法，利用宽波段测光颜色分布，通过双幂律恒星形成历史模型来约束星系的形成时间、熄灭时间和熄灭时标。

Result: 熄灭星系比例为0.24±0.02；熄灭时标分布众数为97_{-25}^{+31}百万年，中位数为182±16百万年，最长达约700百万年；完全静止所需中位时间约400百万年。

Conclusion: 大质量星系在红移约2.6之后快速熄灭，结合化石记录与高红移直接数密度测量，表明红移>3.5观测到的熄灭星系存在显著的重启和/或并合事件。
...
### [8] [A Decade to Map the Diffuse Universe: FRB-QSO Pairs with HST/COS Spectroscopy](https://arxiv.org/abs/2606.05310)
...
TL;DR: 本文预测到2035年，下一代干涉仪将探测到数千个快速射电暴（FRB）与紫外亮类星体（QSO）的投影对，利用HST/COS联合分析可约束宇宙弥散介质的电离分数、重子质量、磁场结构和湍流，并绘制多相银河系及M31晕的三维地图。


<details>
  <summary>Details</summary>
Motivation: 目前仅约100个角秒级定位的FRB（z<1），无法进行统计性FRB-QSO成对巡天。联合分析FRB与邻近QSO的视线方向可互补各自探测能力，但现有样本量不足。

Method: 利用最新的FRB定位预测和紫外亮QSO星表，估算下一代干涉仪到2035年在20000平方度天区内可获得的FRB-QSO成对数量（θ<10'达数千对，θ<1'约100对）。

Result: 到2035年，将获得数千对θ<10'的FRB-QSO对，其中约100对θ<1'，覆盖20000平方度共同天区。

Conclusion: HST/COS是唯一能执行该计划的仪器，2030年代是在HWO定义下一时代之前开展此项研究的唯一窗口期。
...
### [9] [Metal-Poor Gas Accretion Drives Giant Clump Formation at 0.6 < z < 2.6](https://arxiv.org/abs/2606.05284)
...
TL;DR: 本研究分析了约300个高红移（0.6<z<2.6）恒星形成星系的化学丰度，发现团块星系通常比质量-金属丰度关系预测的金属丰度更低，而非团块星系则更高。团块星系中金属丰度低于预期的宿主星系，其团块更年轻、恒星形成率更高，且多数团块星系并非并合事件，表明其团块性质与富金属贫乏气体吸积事件有关。


<details>
  <summary>Details</summary>
Motivation: 千秒差距尺度团块的物理性质对理解高红移恒星形成星系的组装机制至关重要。前期研究表明团块星系存在金属丰度偏移，本文旨在通过大样本光谱观测系统探究这一现象。

Method: 利用LEGA-C、MOSDEF和CANUCS近红外光谱观测，分析了约300个0.6<z<2.6的恒星形成星系。通过质量-金属丰度关系对比团块与非团块星系的化学丰度，并利用CANUCS子样本研究团块恒星性质与宿主星系整体金属丰度的关系，最后使用Gini-M20形态参数计算并合统计量。

Result: 团块星系整体上质量-金属丰度关系偏低，非团块星系偏高。金属丰度低于预期的星系中，团块更年轻、恒星形成率更高，但质量无显著差异。多数团块星系不被归类为并合事件。

Conclusion: 团块星系的特征与金属贫乏气体吸积事件相关，这些事件触发恒星形成并稀释金属丰度，而非主要由并合过程驱动。
