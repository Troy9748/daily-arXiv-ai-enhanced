...
### [1] [Euclid preparation: Testing multi-field inflation with galaxy power spectrum and bispectrum](https://arxiv.org/abs/2605.21436)
...
TL;DR: 本文通过欧几里得巡天模拟数据验证了功率谱与双谱联合分析在探测原初非高斯性（PNG）中的有效性，发现联合分析相比单独使用功率谱可将f_NL约束精度提升29-46%，且引入物理先验可有效消除偏差。


<details>
  <summary>Details</summary>
Motivation: 原初非高斯性是探测宇宙结构起源的有力探针，即将开展的第四代巡天（如欧几里得）将高信噪比测量星系2点和3点相关函数。为了充分利用这些数据，需要建立稳健的联合分析方法，为未来实际数据分析做准备。

Method: 基于Abacus-PNG N体模拟生成的欧几里得类模拟数据，采用晕占据分布（HOD）模型，构建红移空间功率谱（单圈P_ℓ）和双谱（树级B_ℓ）分析管道。系统测试了PNG偏差参数化方案、先验设置和尺度截断等分析选择，并进行了无PNG的零检验。

Result: 在固定尺度截断下，双谱单独相比功率谱可将f_NL约束误差降低约29-46%，功率谱与双谱联合分析进一步收紧约8-13%。从z=0.8到1.7累积增益约2.3倍。最佳结果在z=1.7：先验无关设置下f_NL b_φ达1.9σ显著性，基于先验设置下f_NL达2.35σ显著性。

Conclusion: 功率谱与双谱的联合分析为检验多场暴胀模型提供了有力前景，但需要在完整的欧几里得巡天几何和观测系统误差下进行端到端验证。
...
### [2] [CMB Acoustic Power Spectra in STVG-MOG](https://arxiv.org/abs/2605.20231)
*John W. Moffat*

Main category: astro-ph.CO

Recommendation: ★★★★☆ (68/100) - 与 Zotero 兴趣画像中的 dust, polarization, dark matter 等主题/作者匹配。

TL;DR: 本文提出了一种标量-张量-矢量引力（STVG-MOG）的宇宙学实现，其中复合前标量扰动动力学与ΛCDM模型退化，无需引入粒子暗物质。早期宇宙中非相对论性激发的矢量场φμ表现出无碰撞、零压、密度∝a⁻³的特性。傅里叶尺度上有效引力耦合G_eff≈G_N，使得引力势演化与标准宇宙模型一致，从而保持第三声学峰的高度。该框架中，冷暗物质的作用被引力场本身的自由度替代，并通过CLASS代码获得了与声学功率谱数据的拟合。


<details>
  <summary>Details</summary>
Motivation: 传统ΛCDM模型需要引入粒子暗物质来解释宇宙大尺度结构，但至今未直接探测到暗物质粒子。本文旨在探索是否可以通过修改引力理论（STVG-MOG）来替代暗物质的作用，同时保持与宇宙微波背景（CMB）声学峰观测的一致。

Method: 在STVG-MOG框架中引入一个矢量场φ_μ，其非相对论性激发在早期宇宙中表现为类似冷暗物质的组分（ρ_φ∝a⁻³，声速为零）。通过调节矢量场的参数，使得在声学峰对应的傅里叶尺度上有效引力常数G_eff≈G_N，从而保证引力势演化与ΛCDM一致。利用玻尔兹曼代码CLASS进行数值计算，并与CMB声学功率谱数据拟合。

Result: 发现STVG-MOG框架可以产生与ΛCDM相同的温度极化功率谱，第三声学峰高度得以保持。矢量场的贡献在引力势演化中替代了冷暗物质的作用，且通过CLASS代码获得了与观测数据的良好拟合。

Conclusion: STVG-MOG理论中的矢量场自由度可以在早期宇宙中有效替代粒子暗物质的作用，与ΛCDM在声学峰尺度上观测一致。这种"矢量场尘埃"虽在动力学上等价于冷暗物质，但本质属于引力修正而非粒子物理范畴，为暗物质问题提供了一种替代解释。
...
### [3] [Effective Field Theory for a Baryon-Correlated Dark Matter Profile](https://arxiv.org/abs/2605.20217)
*Kento Kamada*
...
TL;DR: 本文提出一个经验定律（暗物质能量密度与重子引力势相关），并通过5D时空的有效场论为其提供理论基础，成功解释了小尺度宇宙学难题（核尖-核饼问题、旋转曲线多样性、Tully-Fisher关系）。


<details>
  <summary>Details</summary>
Motivation: 标准ΛCDM模型在大尺度上成功，但在小尺度上面临核尖-核饼问题、星系旋转曲线多样性和Tully-Fisher关系等挑战，需要新理论框架来解释这些现象。

Method: 构建包含标量、矢量和张量中间介子的有效场论（EFT），将介子耦合比例调整为4:6:3且质量简并，以抵消对重子的第五力效应；进一步从5D时空出发，将重子源视为5D零流体，从单一5D对称张量场衍生出三种介子，并将χ场限制在4D膜上。

Result: 该框架产生的相互作用能量密度ρ_int ∝ Φ_b^2，实现了重子相关的暗物质密度剖面；χ场在宇宙学尺度表现为标准冷暗物质，在星系尺度由相互作用能主导；导出了μ = K M_b^{-3/2}，为Tully-Fisher关系提供了理论基础。

Conclusion: 该5D有效场论框架能够统一解释小尺度宇宙学难题，并为暗物质与重子物质之间的关联提供了理论依据。
...
### [4] [Probing late-time deviations from $Λ$CDM with a quadratic dark energy expansion](https://arxiv.org/abs/2605.20748)
...
TL;DR: 本文研究了一种二次暗能量膨胀（QDEE）模型，作为标准ΛCDM宇宙学框架的现象学扩展。通过结合Planck 2018、ACT DR6、SPT-3G、DESI DR2和Pantheon Plus等多种宇宙学数据集，作者发现QDEE模型能将哈勃常数推向更高值，部分缓解了与局域测量的张力，且贝叶斯模型比较显示该模型优于标准ΛCDM。


<details>
  <summary>Details</summary>
Motivation: 标准ΛCDM模型在解释哈勃常数局域测量与早期宇宙观测之间存在张力，作者希望引入一种现象学扩展模型（QDEE），通过允许暗能量在晚期有轻微偏离常数的行为，来缓解这一张力。

Method: 采用二次暗能量膨胀（QDEE）模型，引入额外自由度允许晚期暗能量偏离常数；使用Planck 2018 CMB、ACT DR6、SPT-3G、DESI DR2和Pantheon Plus等综合数据集进行约束；通过贝叶斯模型比较和后验预测检验评估模型。

Result: QDEE模型将哈勃常数推断值推向更高，部分缓解了与局域测量的张力；贝叶斯模型比较显示该框架在多种数据集组合下优于标准ΛCDM；后验预测检验表明模型预测与观测数据一致。

Conclusion: QDEE模型作为一种现象学扩展，在保持与早期宇宙约束一致的同时，能够缓解哈勃常数张力，且统计证据支持其优于标准ΛCDM模型。
...
### [5] [Cosmological perturbations with $f(R)$ gravity scalarons : Galaxy power spectra and the scalaron mass](https://arxiv.org/abs/2605.20227)
...
TL;DR: 本文研究了f(R)引力标量场模型下的宇宙学微扰，分析了从早期广义相对论(GR)相到晚期非GR相的过渡时期，发现物质功率谱与ΛCDM模型存在偏差，并报告了标量场质量在星系和黑洞尺度上的GR极限。


<details>
  <summary>Details</summary>
Motivation: 旨在研究f(R)引力理论中从早期GR相到晚期非GR相过渡时期的宇宙学微扰，并通过物质功率谱的偏差来检验修改引力理论，为未来星系巡天任务提供可观测信号。

Method: 利用接近ΛCDM理论的f(R)引力拉格朗日量，结合星系偏差、速度弥散和修改引力参数，生成星系功率谱多极矩（单极和四极），分析标量场质量随宇宙时间的演化及其GR极限。

Result: f(R)理论中的单极和四极物质功率在小尺度（大k）上均高于ΛCDM；四极功率谱在大尺度上（k≈0.02以下）仍保持较高水平。标量场质量随宇宙时间增加而减小，导致偏离GR的宇宙学演化。

Conclusion: f(R)引力产生的功率谱特征可作为DESI、EUCLID、4MOST和PFS等未来星系巡天任务检验引力的可观测信号；标量场质量的演化是导致GR偏离的关键因素。
...
### [6] [Negative neutrino mass or negative dark energy?](https://arxiv.org/abs/2605.21456)
...
TL;DR: 本文系统探索了多种暗能量模型，发现符号切换宇宙学常数模型（Λ_sCDM）能通过修改特定红移范围的宇宙膨胀历史，有效缓解DESI和CMB数据中中微子质量总和与振荡实验下限之间的张力，使有效中微子质量从负值恢复到正值。


<details>
  <summary>Details</summary>
Motivation: DESI和CMB的联合宇宙学分析发现，在ΛCDM模型假设下，推断的中微子质量总和与振荡实验允许的最小值之间存在张力（中微子质量出现负值），需要探索替代的暗能量模型来解决这一矛盾。

Method: 系统比较了多种暗能量模型，重点分析那些能提供负暗能量密度以模拟负有效中微子质量效应的模型。使用DESI DR2 BAO、CMB和DES-Dovekie超新星数据，对ΛCDM和Λ_sCDM等模型进行约束和比较。

Result: Λ_sCDM模型将有效中微子质量从ΛCDM下的∑mν,eff=-0.075+0.039-0.053 eV（68%）改善为∑mν,eff=0.055±0.050 eV（68%），暗能量转变红移的95%下限为z†>2.4。

Conclusion: 虽然Λ_sCDM模型在整体统计支持上并非最强，但研究表明，特定符号和红移结构的晚期膨胀历史贡献是缓解中微子质量张力的可行途径。
...
### [7] [The Splashback Mass Function of Galaxy Clusters from Photometric Data](https://arxiv.org/abs/2605.20393)
...
TL;DR: 本文提出了一种纯测光方法，通过Sloan数字巡天数据测量星系团的splashback半径和质量，构建了首个基于测光数据的splashback质量函数，验证了其在无光谱或剪切数据下进行宇宙学研究的可行性。


<details>
  <summary>Details</summary>
Motivation: Splashback半径定义了星系团的物理边界，能避免传统晕定义中的伪演化问题，但此前缺乏纯测光手段来测量单个团的splashback半径和质量函数。

Method: 基于径向和测光红移信息开发概率性成员判定方法，通过自适应概率截断优化核心显著性；对499个星系团建模累积星系数轮廓提取splashback半径，并重新标定M_sp-R_sp标度关系；将该关系应用于超过1.5万个redMaPPer星系团以推导splashback质量，构建质量函数。

Result: 测得splashback半径与R_200m的中值比为~1.1；M_sp-R_sp标度关系斜率比常密度预期更浅，且无明显红移演化；构建的splashback质量函数在高质端与模拟预测一致，低质端偏差源于光学星表完备性限制。

Conclusion: 研究表明，基于splashback的团大小、质量和丰度可仅通过测光数据稳健测量，为无光谱或剪切数据的宇宙学研究开辟了新途径。
...
### [8] [Detection of a dark matter subhalo in the strongly lensed system PJ011646](https://arxiv.org/abs/2605.21212)
...
### [9] [Difference Between Half-mass Radius and Half-light Radius of Galaxies at 0.2 $< z <$ 2.5 Revealed by JWST/NIRCam Data](https://arxiv.org/abs/2605.20887)
...
TL;DR: 利用JWST观测数据，研究14,333个星系（质量>10^9 M☉，红移0.2<z<2.5）的光学半径与质量半径的差异，发现两者差异与星系质量、光学半径和颜色相关，且对不同类型星系（静止星系和恒星形成星系）的演化趋势有不同影响。


<details>
  <summary>Details</summary>
Motivation: 探究星系的光学半光半径与质量半质量半径之间的差异，以及这种差异如何随星系性质（质量、颜色、红移）变化，从而更准确理解星系的物理结构和演化过程。

Method: 基于JWST在CANDELS天区的观测数据，测量14,333个星系（M_* > 10^9 M☉，0.2 < z < 2.5）的半光半径和半质量半径，分析两者比值与星系质量、颜色、红移的关系，并拟合星系大小与恒星质量之间的线性关系。

Result: 1) 半光半径大于半质量半径，对静止星系和恒星形成星系均成立；2) 两者差异与星系质量、半光半径及U-V颜色显著相关；3) 恒星形成星系在z>1.7时r_e,mass/r_e,light比值显著增加，而静止星系无明显增加；4) 半光半径的质量-大小关系斜率比半质量半径大0.1-0.3 dex；5) 静止星系的半质量半径从z~2.5到z~0.2增加了约3-5倍，恒星形成星系增加了约2倍。

Conclusion: 基于半光半径得出的星系演化见解在使用半质量半径时仍然有效，但具体的斜率存在一定差异，表明需要谨慎对待基于单一半径指标的演化结论。
...
### [10] [Spider-Webb: enhanced star formation in low-mass galaxies within the Spiderweb protocluster revealed by JWST Pa$β$ narrow-band imaging](https://arxiv.org/abs/2605.20590)
...
Recommendation: ★★★★☆ (74/100) - 与 Zotero 兴趣画像中的 jwst, dust 等主题/作者匹配。

TL;DR: 利用JWST窄带成像研究蜘蛛网原星系团（z=2.16）中Pα发射体的恒星形成活动，发现低质量星系（M<10⁹M☉）的恒星形成率显著高于场星系，而中等质量和大质量星系无明显差异，表明环境对恒星形成的增强效应具有强质量依赖性。


<details>
  <summary>Details</summary>
Motivation: 揭示环境在星系演化中的作用，探究原星系团中不同质量星系的恒星形成活动是否受环境因素调控。

Method: 利用詹姆斯·韦伯太空望远镜（JWST）/NIRCam窄带成像观测Pα发射线，推导恒星形成率（SFR），并比较蜘蛛网原星系团与场星系的SFR差异。

Result: 低质量（M<10⁹M☉）PBEs在蜘蛛网原星系团中恒星形成增强，且不依赖尘埃消光校正；中高质量（M>10⁹M☉）PBEs与场星系无显著差异；增强的低质量星系并未明显集中在星系团核心区域。

Conclusion: 低质量星系的星暴活动可能由星系并合/相互作用或高效气体供应等环境过程驱动。恒星形成增强在正午宇宙（cosmic noon）中依赖于星系质量和原星系团的动力学状态，且中等质量区间的行为在不同原星系团间并不一致。
...
### [11] [Formation of intermediate-mass black holes in young massive clusters detected with JWST: analytic mass estimates](https://arxiv.org/abs/2605.20381)
...
TL;DR: 本文利用Fokker-Planck模型结合解析框架，估算了JWST观测到的高红移致密星团（如Cosmic Gems弧和Firefly Sparkle）通过失控恒星碰撞形成中等质量黑洞（IMBH）的质量，得到典型IMBH质量范围为~10² M⊙到~4×10³ M⊙，形成效率约百分之几。极端致密的Cosmic Gems星团可形成1600-2700 M⊙的黑洞种子，低金属丰度是保留种子质量的关键。


<details>
  <summary>Details</summary>
Motivation: JWST揭示了一批高红移致密恒星系统，其质量（10⁵-10⁷ M⊙）和尺寸（0.4-15 pc）非常适合通过碰撞机制形成中等质量黑洞（IMBH），但直接N体模拟不可行，因此需要建立理论模型来估算这些环境中IMBH的形成质量。

Method: 采用Fokker-Planck模型结合失控碰撞的解析框架，并考虑恒星风质量损失，该模型已通过紧凑星团的直接N体模拟验证。将其应用于JWST观测到的高红移大质量星团样本。

Result: 典型IMBH质量范围为~10² M⊙到~4×10³ M⊙，形成效率约百分之几。Cosmic Gems星团的极端致密性（R_h ~ 1 pc）可形成1600-2700 M⊙的高质量黑洞种子。低金属丰度（Z ≲ 0.02 Z⊙）对保留种子质量至关重要。结果与直接N体模拟预期一致。

Conclusion: 这些致密、低金属丰度的星团是高质量黑洞种子的可行工厂，能够生长为早期宇宙中观测到的超大质量黑洞。
...
### [12] [B-Fields and Star Formation across Scales with TRAO (B-FROST): CO Abundances, Dynamics and Relative Orientations in the Translucent High Latitude Cloud MBM12](https://arxiv.org/abs/2605.21101)
...
Recommendation: ★★★★☆ (73/100) - 与 Zotero 兴趣画像中的 herschel, dust, molecular 等主题/作者匹配。

TL;DR: 本研究利用TRAO望远镜观测了高纬度分子云MBM12，结合Herschel和Planck数据，研究了其低恒星形成效率的原因。发现该云团存在大范围变化的维里参数（α_vir=3-60），在0.1 pc尺度上最小；外部压力比高α_vir结构大一个数量级；柱密度结构与磁场方向在N(H₂)=4.5×10²¹ cm⁻²处从平行转为垂直。这是首次对MBM12进行综合化学、动力学和磁场分析。


<details>
  <summary>Details</summary>
Motivation: 银河系平均恒星形成效率仅为百分之几，而高纬度分子云MBM12的恒星形成活动尤其低，本研究旨在探究其低恒星形成效率的原因。

Method: 结合Herschel校准的N(H₂)估计、¹²CO和¹³CO (J=1-0)观测（2.5°×3°，48角秒分辨率），计算N(CO)、X(CO)和[CO/H₂]；利用树状图计算多尺度维里参数α_vir和质量-尺寸标度律；从Planck尘埃极化数据推导相对取向直方图。

Result: 识别出四个主要速度区域，H₂柱密度范围2×10²⁰ cm⁻²至1.3×10²² cm⁻²；平均X(CO)接近银河系平均值；层次结构遵循分段幂律质量-尺寸关系M=AR^α；α_vir范围3-60，最小值为0.1 pc尺度；低α_vir结构的标度因子A比高α_vir结构大三倍；柱密度结构与磁场方向在N(H₂)=4.5×10²¹ cm⁻²处从平行转为垂直。

Conclusion: 首次对MBM12进行了综合化学、动力学和磁场分析。尺度相关的质量-尺寸和维里分析可以进一步约束外部压力在调节恒星形成效率中的作用，外部压力对低α_vir结构的影响比高α_vir结构大一个数量级。
...
### [13] [Euclid: Early Release Observations -- The formation of peanutty dwarf galaxies in Perseus](https://arxiv.org/abs/2605.21342)
...
TL;DR: 利用欧几里得卫星对英仙座星系团的早期观测数据，通过累积光分数方法识别出13个具有盒状等照面的矮星系，并发现其盒状形态与有效半径之间存在反相关关系，模拟表明这些星系是受到潮汐作用改造的、具有花生状结构的三轴体，其观测形态取决于视线方向。


<details>
  <summary>Details</summary>
Motivation: 盒状等照面在巨椭圆星系中研究较多，但在矮星系中却知之甚少。本研究旨在识别并表征英仙座星系团中具有盒状/花生状形态的矮星系，探究其等照面形状的起源。

Method: 基于欧几里得早期释放观测数据，开发了一种累积光分数方法来稳健测量等照面形状参数c4，特别适用于低表面亮度环境。从约1100个编录矮星系中筛选出约190个早型星系进行可靠测量，并通过与N体模拟的潮汐改造矮星系对比来解读观测趋势。

Result: 识别出13个具有显著盒状等照面（c4 < -0.0175）的矮星系。这些星系没有可见的薄盘，位于星系团红序上，在英仙座内无空间集中。发现c4与有效半径之间存在显著反相关：更大星系表现出更盒状的等照面，该反相关在模拟中也被复现。

Conclusion: 英仙座中的盒状矮星系是中等旋转前身星经潮汐作用改造后的残留体，盒状等照面追踪了内部由盒状轨道主导的花生状结构。尺寸-形状相关性源于观测几何效应：正面朝向揭示拉长的三轴结构（大且盒状），侧向则呈现更圆且致密的形态。样本代表了一组经定向选择的潮汐改造花生状矮星系。
...
### [14] [On the correlation between globular clusters and the distribution of dark matter in galaxy clusters: the case of Abell 2744](https://arxiv.org/abs/2605.20352)
*Marta Reina-Campos,Joshua S. Speagle,William E. Harris*

Main category: astro-ph.GA

Recommendation: ★★★★☆ (67/100) - 与 Zotero 兴趣画像中的 dark matter, lensing 等主题/作者匹配。

TL;DR: 本文提出了一种基于空间泊松点过程的统计方法，用于评估球状星团（GCs）对星系团质量分布的示踪能力，并将其应用于Abell 2744星系团。结果表明，明亮GCs的空间分布与弱透镜导出的质量图高度相关（Spearman系数>0.7），尤其是亮蓝色GCs与弱透镜质量图最为吻合，可作为独立补充的质量示踪工具。


<details>
  <summary>Details</summary>
Motivation: 球状星团分布在星系团内部，可能是潜在的质量示踪物，但需要一种统计方法来量化其与不同星系成分（如质量分布）的关联程度。

Method: 假设GCs在星系团周围服从非齐次空间泊松点过程，构建统计方法；应用于Abell 2744星系团，将GCs空间分布与不同成分（如质量图、其他星系）进行相关分析。

Result: 明亮GCs的空间分布大致示踪了Abell 2744中的三个主要相互作用团块；GCs与预测质量图的相关性优于其他任何星系成分（Spearman系数>0.7）；亮蓝色GCs与弱透镜质量图最为一致。

Conclusion: 该方法可以区分GCs与哪种图最吻合；明亮蓝色GCs能提供与弱透镜相似细节水平的独立质量分布信息；该方法已公开，可应用于不同宇宙时期的星系团，为质量分布研究提供独立手段。
...
### [15] [Robustness Analysis of USmorph: II. Optimizing Feature Extraction, Dimensionality Reduction, and Clustering for Unsupervised Galaxy Morphology Classification](https://arxiv.org/abs/2605.20871)
*Guanwen Fang,Xiaolei Yin,Yirui Zheng,Zesen Lin,Shiwei Zhu,Jie Song,Chichun Zhou,Xu Kong*

Main category: astro-ph.GA

Recommendation: ★★★☆☆ (56/100) - 与 Zotero 兴趣画像中的 csst 等主题/作者匹配。

TL;DR: 本文对混合框架USmorph中的无监督机器学习模块进行了系统性鲁棒性分析，通过对比多种特征提取、降维和聚类方法，提出基于Bagging的多聚类投票方案，显著提升聚类稳定性，验证了该方法在星系形态自动分类中的技术稳健性和科学可信度。


<details>
  <summary>Details</summary>
Motivation: 大规模星系图像需要自动化的形态分类方法，而现有无监督学习模块的鲁棒性尚未得到系统评估，亟需验证其在未来大型巡天项目（如CSST）中的可靠性。

Method: 对比五种预训练模型进行特征提取，选择ImageNet预训练AlexNet；采用UMAP降维；提出Bagging多聚类投票方案提升稳定性；比较五种聚类算法的收敛性、可扩展性和质量。

Result: Bagging投票方案结合K-means、Birch和Agg聚类效果最佳；K=16时分类粒度与人工验证效率最优；t-SNE显示清晰紧凑的簇边界；形态分类结果与星系演化理论一致。

Conclusion: USmorph具备技术鲁棒性和科学可信度，可作为CSST等未来大规模巡天项目中可靠的星系自动形态分类方法。
...
### [16] [Revisiting the mass metallicity relation and the fundamental metallicity relation of dwarf galaxies at cosmic noon with NIRISS](https://arxiv.org/abs/2605.20810)
...
TL;DR: 本文利用JWST/NIRISS数据将恒星质量-金属丰度关系（MZR）延伸至极低质量星系（log M*/M⊙ = 6.3-10.2），发现MZR斜率在近四个数量级内保持恒定（β≈0.24），并指出在低质量端，气体分数比外流金属负载因子更主导MZR斜率的调控。研究未发现基本金属丰度关系（FMR）的稳健证据。


<details>
  <summary>Details</summary>
Motivation: 将z=1.1-3.4处的高红移恒星质量-金属丰度关系（MZR）拓展至极低质量星系（矮星系）区域，并检验基本金属丰度关系（FMR）在高红移矮星系中的存在性。

Method: 使用NGDEEP项目的JWST/NIRISS无缝光谱数据，分析183个log(M*/M⊙)=6.3-10.2的星系，结合GLASS-JWST样本；通过分析外流金属负载因子（ζ_out）与气体分数（μ_gas）的作用机制解析MZR斜率；测试SFR是否对MZR有额外依赖（FMR检验）。

Result: MZR斜率β≈0.24±0.03，在近四个数量级质量范围内保持恒定；在M*≲10^8 M⊙时，ζ_out对MZR斜率的调控作用弱于μ_gas；未发现FMR的稳健证据，SFR未显著降低金属丰度弥散或改变MZR关系。

Conclusion: MZR在极低质量端依然成立且斜率恒定，气体分数主导低质量端MZR调控；当前NIRISS数据深度下FMR证据不充分，需更大样本和更深观测。
...
### [17] [The TNG50-SKIRT Atlas: Multi-wavelength nonparametric galaxy morphology](https://arxiv.org/abs/2605.20886)
...
TL;DR: 本文利用TNG50-SKIRT星系图谱，研究了从紫外到近红外波段下星系非参数形态指标的波长依赖性，发现形态指标随波长显著变化，且盘主导星系比 bulge主导星系的依赖性更强，尘埃消光的影响对整体星系群较小但对个别星系显著。


<details>
  <summary>Details</summary>
Motivation: 星系形态是描述星系演化的基本属性，但观测形态可能随观测波长而变化，因此需要系统研究波长对非参数形态指标的影响以及尘埃消光效应的作用。

Method: 利用TNG50-SKIRT Atlas（包含1154个星系从紫外到近红外的合成图像），使用StatMorph代码计算四个非参数形态指标，并与SINGS巡天的观测数据进行比较。

Result: 光学波段测得的形态参数与恒星质量的相关性、Gini-M20、浓度-Gini和浓度-M20平面与观测数据一致；非参数形态指标随波长显著变化，盘主导星系变化更强；模拟结果与SINGS巡天观测一致；尘埃消光对整体星系影响适中，但对个别星系影响显著。

Conclusion: 波长依赖性是星系形态研究中的重要因素，盘主导星系的形态随波长变化更为明显，尘埃消光的影响不可忽视，尤其是在个体星系层面。
...
### [18] [Evolution of compressed clouds formed by filament coalescence. I. Oblique collisions](https://arxiv.org/abs/2605.20835)
...
TL;DR: 本研究通过三维磁流体动力学模拟，探讨了斜向碰撞的磁化纤维丝如何形成"枢纽"结构，并揭示了碰撞角度、速度等条件对引力坍缩和恒星形成的影响。


<details>
  <summary>Details</summary>
Motivation: 理解hub-filament系统的形成与演化对揭示星团和大质量恒星形成过程至关重要，但完整图景尚不清晰，因此需探索纤维-纤维碰撞作为枢纽形成机制的可能性。

Method: 采用三维理想磁流体动力学模拟，以两个相同的有限长磁化纤维丝为初始模型，改变其长轴间的碰撞角度、垂直于长轴的碰撞速度及初始线质量，研究斜向碰撞的基本演化过程。

Result: 碰撞角度从正交减小到平行时，压缩云更易引力坍缩；压缩云的不稳定性可由碰撞后能量平衡解释——若引力能绝对值超过动能、热能和磁能之和则坍缩，否则膨胀；给出了可形成hub-filament的碰撞速度上限及利于大质量恒星形成的碰撞条件。

Conclusion: 纤维-纤维碰撞是枢纽结构形成的重要机制，碰撞角度和速度等参数决定了后续引力坍缩的可能性及大质量恒星形成的条件。
...
### [19] [A systematic study of CO/SiO absorption features in early-type galaxies using AKARI/IRC near-infrared spectra](https://arxiv.org/abs/2605.20686)
...
TL;DR: 本文利用AKARI近红外光谱和2MASS、WISE等多波段测光数据，分析了30个早期型星系（ETGs）中尘埃的起源。研究发现尘埃质量与SiO/CO吸收线等值宽度正相关，支持尘埃主要来自演化恒星质量损失（内部产生）的假说；同时多环芳烃（PAHs）可能与外部合并事件及星系核活动相关。


<details>
  <summary>Details</summary>
Motivation: 早期型星系中尘埃的起源长期以来存在争议，主要候选来源包括演化恒星的质量损失、星系合并以及星际介质中的尘埃增长。本研究旨在通过观测证据厘清哪种机制是主导因素。

Method: 对30个ETGs进行AKARI近红外光谱分析，追踪代表老年恒星光球的SiO和CO吸收特征；结合2MASS、WISE和AKARI的近红外到远红外测光数据，推导尘埃质量；分析尘埃质量与吸收线特征、X射线亮度以及PAH特征之间的相关性。

Result: 尘埃质量与SiO和CO吸收特征的等值宽度之和呈正相关，与X射线亮度无反相关；PAH强度与SiO/CO等值宽度无关，但与热尘埃和暖尘埃成分的光度相关。

Conclusion: 早期型星系中相当一部分尘埃可能源自演化恒星的质量损失（内部产生），且与X射线等离子体相互作用较弱；PAHs可能具有外部起源（如星系合并残留物），并受星系核活动加热。
...
### [20] [Inferring Globular Cluster Initial Mass Function from Stellar Streams](https://arxiv.org/abs/2605.20322)
...
TL;DR: 利用Gaia数据，通过冷暗物质模拟与观测结合，发现恒星流可编码球状星团初始质量函数信息，恢复出斜率α=1.3±0.05的幂律分布。


<details>
  <summary>Details</summary>
Motivation: 现有恒星流主要用于揭示银河系暗物质晕结构，但其是否包含球状星团初始质量函数的信息尚不清楚，本文旨在探索这一新角度。

Method: 结合冷暗物质模拟（模拟嵌入球状星团的演化与瓦解）与恒星流及球状星团的观测数据，通过流质量和角动量测量推断初始星团质量函数。

Result: 初始质量越大的星团产生越大的恒星流，但沉积到流中的质量比例更小；恢复的初始质量函数为斜率α=1.3±0.05的递减幂律（≥1000 M⊙）。

Conclusion: 恒星流可作为探测球状星团早期质量分布的新探针，为理解星团形成和瓦解历史提供新途径。
...
### [21] [Dynamical Cluster Assembly Framework (D-CAF): The Link Between Star Cluster Formation and Expansion Rates](https://arxiv.org/abs/2605.20489)
...
TL;DR: D-CAF框架连接了恒星形成历史与年轻星团的动力学演化，发现气体在恒星形成过程中持续收缩，显著影响星团的存活和膨胀，且膨胀特征仍保留着形成时的动力学信息。


<details>
  <summary>Details</summary>
Motivation: 现有模型通常将恒星形成后的动力学演化与形成过程割裂，缺乏一个统一的框架来连接嵌入式恒星形成历史与年轻星团系统的动力学演化。

Method: 基于AMUSE开发D-CAF框架，从MHD模拟中提取气体演化，结合直接N-body模拟构建受控网格，模拟恒星在演化背景势中的逐步形成过程。

Result: 气体在恒星形成过程中持续收缩，提高了嵌入式星族的中心集中度和速度尺度；气体收缩缩短了恒星穿越时间，使气体移除更接近绝热过程；星协的膨胀率受气体排出前的速度尺度限制，膨胀效率取决于气体排出时标。

Conclusion: 常用膨胀诊断工具在全运动学信息可用时可直接追踪年轻星系统的物理膨胀率，为利用恒星运动学约束嵌入式恒星形成的动力学条件提供了可能。
