...
### [1] [$Ab$ $initio$ modeling of Galactic dust polarized CMB foreground](https://arxiv.org/abs/2605.06896)
*Alexei G. Kritsuk,Ka Wai Ho,Ka Ho Yuen,Raphael Flauger*

Main category: astro-ph.GA

Recommendation: ★★☆☆☆ (38/100) - 与 Zotero 兴趣画像中的 dust, polarized, polarization 等主题/作者匹配。
...
### [2] [Potamides: Mapping Dark Matter Halo Shapes from Stellar Stream Tracks in the Local Universe](https://arxiv.org/abs/2605.07809)
...
TL;DR: 本文提出了一种名为Potamides的方法，利用河外星系恒星流的曲率来约束暗物质晕的形状和方向。通过对15条恒星流的分析，发现具有边缘缠绕环或尖锐转折点的流能提供最强约束，而大圆状流则信息有限。所有样本均支持球状暗物质晕。该方法为未来大规模巡天提供了快速检验暗物质和重子物理的统计工具。


<details>
  <summary>Details</summary>
Motivation: 宇宙学模拟预言暗物质晕的形状受重子物理和暗物质性质影响，但目前对单个星系晕的扁平度和取向的观测约束非常有限，亟需新的观测手段来验证理论预测。

Method: 提出了Potamides方法，通过分析河外星系恒星流的曲率来推断宿主暗物质晕的投影轴比和方向。将该方法应用于Stellar Stream Legacy Survey中的15条恒星流。

Result: 1）部分恒星流能够排除大范围的晕扁平度和取向参数；2）边缘缠绕的环状流或具有尖锐转折点的流约束能力最强，而大圆状流几乎无法提供约束；3）所有样本在给定扁平化方向下均支持球状暗物质晕。

Conclusion: 恒星流形态能够为单个外部星系提供暗物质晕形状约束。随着Euclid、Rubin、Roman、ARRAKIHS等未来巡天项目发现大量恒星流，这种基于曲率的技术将能够快速进行统计检验，揭示暗物质和重子物理的本质。
...
### [3] [Space as a spectroscopic laboratory: High-resolution spectroscopy of the [$^{13}$C II] hyperfine structure with SOFIA/upGREAT](https://arxiv.org/abs/2605.07995)
...
TL;DR: 本文利用SOFIA/upGREAT望远镜的高分辨率观测，首次通过天文学方法实验测定了[$^{13}$C II] 158 μm三条超精细结构跃迁的磁偶极常数，并提高了其中心频率的精度，展示了天文观测在原子物理参数测量上的应用潜力。


<details>
  <summary>Details</summary>
Motivation: [$^{12}$C II] 158 μm发射线是星际介质的关键冷却线，但其谱线轮廓常受光学深度影响。通过与丰度较低的[$^{13}$CII]同位素比较可重建本征线形，但[$^{13}$C II]的超精细结构跃迁中两条弱组分的频率此前仅来自量子力学计算，缺乏实验测量，其磁偶极超精细结构常数从未被实验测定。

Method: 利用SOFIA搭载的升级版upGREAT接收机的高光谱分辨率观测，同时探测到[$^{13}$C II]的三条超精细结构跃迁。通过对天文数据的分析，首次实验测定了[$^{13}$C II]基项$2s^2\,2p\,{}^2P^\circ$的磁偶极超精细结构常数$A_{1/2}^{\rm hf}=810.71(11)$ MHz和$A_{3/2}^{\rm hf}=162.18(5)$ MHz。结合实验室测定的中心频率，确定了三条hfs线的静止频率。并以[$^{12}$C II]为参考，提高了[$^{13}$C II]中心频率的精度。

Result: 首次通过天文观测实验测定了[$^{13}$C II]的磁偶极超精细结构常数，精度可与实验室测量相媲美；同时提高了[$^{13}$C II]中心频率的测量精度。

Conclusion: 高光谱分辨率的天文观测能够用于约束基本原子性质，超精细结构常数测量精度可与实验室测量媲美。该方法可推广到其他难以通过实验室获取数据的原子和分子跃迁研究中。
...
### [4] [First Interstellar Detection of Methyl Carbamate: A New Observational Anchor for Glycine Chemistry](https://arxiv.org/abs/2605.07159)
...
TL;DR: 利用ALMA望远镜在热分子核G358.93-0.03 MM1中首次稳健探测到氨基甲酸甲酯（甲基氨基甲酸酯），未检测到甘氨酸等其他C2H5O2N异构体，表明该异构体家族主要由动力学化学过程而非热力学平衡塑造，氨基甲酸甲酯可作为甘氨酸化学的新观测锚点。


<details>
  <summary>Details</summary>
Motivation: 甘氨酸作为最简单的氨基酸，在星际介质中历经数十年高灵敏度搜寻仍未被探测到，亟需替代方案来约束其天体化学起源。研究其更广泛的C2H5O2N异构体家族并识别可探测成员，可作为甘氨酸相关化学的观测锚点。

Method: 利用ALMA 1毫米波段对热分子核G358.93-0.03 MM1进行观测；识别10条未混叠的氨基甲酸甲酯转动跃迁；对其它具有转动光谱数据的C2H5O2N异构体（包括甘氨酸）进行搜寻并给出柱密度上限；比较观测丰度模式与最小能量原理（MEP）预测。

Result: 氨基甲酸甲酯柱密度为(4.21±0.84)×10^15 cm^-2，激发温度204±10 K；未检测到甘氨酸等其他异构体；观测丰度模式与最小能量原理预测严重偏离；氨基甲酸甲酯丰度与其前体甲醇和甲酰胺在多种天体环境中呈现相关性。

Conclusion: C2H5O2N异构体家族主要由动力学化学过程而非热力学平衡主导；支持氨基甲酸甲酯通过颗粒表面CH3O + NH2CO自由基-自由基复合生成的机制；该探测建立了氨基甲酸甲酯作为甘氨酸化学的新观测锚点，为星形成区中氨基酸相关分子的形成路径提供了关键约束。
...
TL;DR: 本文利用ALMA高分辨率数据，对18个邻近超亮红外星系的HCN/HCO⁺流量比进行空间和光谱分辨分析，发现其升高特征可分为球壳状、光谱紧凑和填充三种形态，分别对应空间分辨外流、活动星系核/未分辨外流和受限外流等物理起源。


<details>
  <summary>Details</summary>
Motivation: 传统空间积分或速度积分分析无法区分HCN/HCO⁺流量比升高的物理起源，需要更高分辨率的数据来揭示其本质。

Method: 利用ALMA≤0.2角秒分辨率数据，对18个邻近ULIRGs的HCN和HCO⁺分子谱线(J=2-1,3-2,4-3)进行空间和光谱分辨分析，将PPV空间中流量比升高区域的几何形态进行分类。

Result: 发现三种形态：球壳状(空间分辨外流)、光谱紧凑(AGN/未分辨外流)和填充(AGN/受限外流)。空间分辨外流起源在7个ULIRGs中观测到，AGN/未分辨外流在9个ULIRGs中观测到。

Conclusion: 空间和光谱分辨的HCN/HCO⁺流量比分析能提供比传统积分分析更丰富的物理起源信息，有助于揭示ULIRG核区的物理过程。
...
### [6] [Hidden Monsters with SPHEREx I: A goldmine for heavily reddened quasars at cosmic noon](https://arxiv.org/abs/2605.06791)
...
TL;DR: 本文通过扩大样本量，确认了77颗新的大幅度红化类星体（HRQs），发现其热尘埃含量低于蓝类星体，支持HRQs是强反馈开始清除中心区域遮蔽物质的“吹出阶段”的演化图景。


<details>
  <summary>Details</summary>
Motivation: 以往对HRQs的研究受限于样本数量小，难以进行可靠的统计分析。本研究旨在扩大最亮HRQs样本，实现群体层面的研究，并将其置于大质量星系和黑洞形成的演化序列中。

Method: 收集最亮HRQ候选体的多波段宽频测光数据，以(J-K)_AB > 1.6的红色近红外颜色选择AGN；利用SPHEREx分光光度测量确认HRQ并确定红移；进行详细的SED拟合，并与未遮蔽的Quaia类星体和热尘埃遮蔽星系（Hot DOGs）等对照组进行比较。

Result: 确认了77颗新HRQs（红移1.5–3.9），使已知z>1.5的HRQ数量翻倍以上，包含首批7颗z>3的HRQ。76%的HRQ检测到与散射类星体辐射一致的UV过剩。HRQ在相同光度下比蓝类星体更缺乏热尘埃，6μm连续谱光度在固定3000Å光度下系统性更暗。

Conclusion: HRQs代表一种“吹出阶段”，其中强反馈开始清除中心区域的遮蔽物质，从而将其置于大质量星系和黑洞演化的序列中。
...
TL;DR: 本文利用COLIBRE宇宙学流体模拟与SKIRT辐射传输代码，预测了z=7-15的紫外光度函数，发现模拟预测的亮端星系亮度低于JWST观测值，尤其在z=15时差距达2.5星等，表明需要额外物理机制（如top-heavy IMF）来解释早期宇宙的明亮星系。


<details>
  <summary>Details</summary>
Motivation: JWST已能探测宇宙最早期的星系，但现有模拟是否能够重现观测到的紫外光度函数尚不明确，需要系统比较模拟预测与JWST观测数据，检验当前星系形成模型的合理性。

Method: 使用COLIBRE宇宙学流体模拟生成星系样本，再通过SKIRT辐射传输代码计算星系紫外光度（考虑星际介质和尘埃消光），将结果与JWST等观测的紫外光度函数进行对比分析。

Result: COLIBRE模拟的尘埃消光后紫外光度函数在亮端系统性低于观测：z=7时暗约1星等，z=15时暗约2.5星等。考虑观测不确定性可部分缓解但不完全消除差异。忽略尘埃消光后，z≤12亮端吻合，但z=15仍偏暗。

Conclusion: 当前模拟在最高红移（z=15）无法产生足够亮的星系，需要引入top-heavy IMF等额外物理机制。紫外光度密度从z=7到z=15下降约300倍，且高红移处暗端斜率更陡、特征光度更暗。
...
### [8] [Complex organic molecules and cosmic ray ionisation rate towards the massive protostar Cepheus A HW2](https://arxiv.org/abs/2605.06767)
...
TL;DR: 该研究利用Onsala 20米望远镜对高质量原恒星Cepheus A HW2进行高灵敏度宽带谱线巡测，通过观测离子和复杂有机分子（COMs）的丰度，结合RADEX分析和Nautilus化学模型，估算了宇宙线电离率（CRIR），发现与COMs发射相关的运动学成分的CRIR存在局部增强（-11 km/s处为6.8×10⁻¹⁷ s⁻¹）。


<details>
  <summary>Details</summary>
Motivation: 宇宙线是星形成区分子化学的重要驱动力，实验室实验表明宇宙线能刺激甲醇等复杂有机分子释放。此前观测主要针对冷、低质量核，因此研究宇宙线在高质量星形成环境中如何影响COM形成具有重要意义。

Method: 使用Onsala 20米望远镜对Cepheus A HW2进行高灵敏度宽带谱线巡测；通过转动图、单跃迁和RADEX网格搜索估算柱密度和相对丰度；利用不同离子作为探针通过解析化学估算电子分数和CRIR；使用Nautilus气-尘化学码进行模拟验证。

Result: 识别出两个主要速度成分（-11 km/s和-5 km/s）；估算了多种COMs的丰度（如CH₃OH: 1.6×10⁻⁹）；氘丰度比在0.002-0.3范围；RADEX约束分子氢体积密度为2.6×10⁵ cm⁻³；CRIR在-11 km/s处为6.8×10⁻¹⁷ s⁻¹，在-5 km/s处≤9.2×10⁻¹⁹ s⁻¹；Nautilus模型成功再现了观测丰度。

Conclusion: 与大多数COMs发射相关的运动学成分的宇宙线电离率存在局部增强，表明高质量星形成环境中宇宙线对分子化学有显著影响。
...
TL;DR: 本文利用模拟研究矮星系中旋转速度与速度色散比(V/σ)随质量的变化，发现V/σ随质量增加，HI气体和年轻恒星更倾向于旋转支持，而年老恒星更倾向于色散支持，并讨论了观测分辨率对V/σ测定的影响。


<details>
  <summary>Details</summary>
Motivation: 理解矮星系的形成机制，探究V/σ是否依赖于星系质量以及环境因素，并通过模拟验证观测研究中的争议。

Method: 使用Marvelous Massive Dwarfs模拟套件，选取67个孤立矮星系（恒星质量10^6-10^9 M⊙），生成不同视角下的线速度图和速度色散图，分析HI气体、年轻恒星（<1 Gyr）和年老恒星（>1 Gyr）的V/σ差异，并量化空间分辨率的影响。

Result: V/σ随质量增加；HI气体和年轻恒星的V/σ≈1-13（旋转支持为主），年老恒星V/σ≈0.2-5（色散支持为主）；观测分辨率低估了年老恒星的本征V/σ；HI全球V/σ与HI线轮廓形状存在相关性，但整体高于先前研究结果。

Conclusion: 结果支持年轻恒星由动态冷的星际介质气体形成并随时间经历动力学加热的图景；未来需使用不同运动学示踪物进一步研究V/σ与矮星系形成。
...
### [10] [CMB Limits on the Absorption of Light Vector and Axial-Vector Dark Matter](https://arxiv.org/abs/2605.08080)
*Gabriele Montefalcone,Nicola Bellomo,Kimberly K. Boddy*

Main category: astro-ph.CO

Recommendation: ★★☆☆☆ (31/100) - 与 Zotero 兴趣画像中的 dark matter, lensing, polarization 等主题/作者匹配。

TL;DR: 本文首次研究了轻子型亚MeV自旋1暗物质通过非弹性散射或氢原子吸收在 primordial 等离子体中注入能量对CMB各向异性的影响，并利用Planck 2018数据给出了暗物质-电子耦合的上限。


<details>
  <summary>Details</summary>
Motivation: 探索轻子型亚MeV自旋1暗物质通过非弹性散射和氢原子吸收过程对CMB各向异性的影响，填补该领域的研究空白。

Method: 利用Planck 2018温度、极化和引力透镜数据，对暗物质质量在100 eV至100 keV范围内，分析非弹性散射和氢原子吸收两种能量注入通道对CMB的影响，并推导矢量及轴矢量暗物质-电子耦合的上限。

Result: 在keV质量以上，非弹性散射主导约束；在低质量区域，氢原子吸收为主通道。所得到约束弱于现有实验室和天体物理极限。

Conclusion: 虽然约束较弱，但提供了一种稳健且独立的宇宙学探针，用于探测轻子型暗物质相互作用。
