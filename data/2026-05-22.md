...
### [1] [MICONIC: The multiphase circumnuclear region of Centaurus A as seen with JWST/MIRI MRS observations. I. Spectral inventory and properties of the warm molecular disk](https://arxiv.org/abs/2605.22497)
...
### [2] [Silicate cosmic dust grain collisions in the interstellar medium: A molecular dynamics study](https://arxiv.org/abs/2605.22225)
*C. J. Esmerian,S. R. Hashemi,W. M. C. Sameera,W. Vlemmings,S. Andersson,T. J. L. C. Bakx,K. K. Knudsen,S. Aalto,G. Nyman*

Main category: astro-ph.GA

Recommendation: ★★☆☆☆ (38/100) - 与 Zotero 兴趣画像中的 dust, molecular 等主题/作者匹配。
...
### [3] [JWST Advanced Deep Extragalactic Survey (JADES) Data Release 5: stellar population catalogue for galaxies in GOODS-N and GOODS-S](https://arxiv.org/abs/2605.21599)
...
TL;DR: 本文介绍了JWST高级深空外星系巡天（JADES）第五次数据发布中的星系星族目录，使用Prospector框架对约50万个源进行贝叶斯推断，采用非参数星族形成历史模型和演化的主序先验，获得了恒星质量、SFR、尘埃消光等物理参数的后验分布，并公开释放了该目录。


<details>
  <summary>Details</summary>
Motivation: 利用JWST的深场多波段数据，构建一个均匀、物理自洽的星系星族参数目录，以支持对星系增长、熄灭及恒星质量累积的统计研究。

Method: 使用Prospector框架对JADES DR5中约50万个源进行SED建模，采用非参数SFH、星云发射、尘埃消光、金属丰度和中红外AGN与尘埃发射模型，并引入演化的恒星形成主序（SFMS）先验来约束SFH的形状。

Result: 获得了约35万个红移z=1-9的星系的后验分布，包括恒星质量、SFR、SFH、尘埃消光、金属丰度和AGN贡献等参数，验证了目录与光谱红移的一致性。

Conclusion: 该目录提供了一个均匀的星族参数集合，适用于星系演化的统计研究，并已作为JADES DR5的一部分公开释放。
...
### [4] [Mass Segregation in the CMZoom Survey](https://arxiv.org/abs/2605.22754)
...
TL;DR: 本文利用最小生成树（MST）方法，基于CMZoom巡天的1.3 mm尘埃连续谱目录，分析了银河系中央分子区（CMZ）中685个致密毫米连续谱源的空间分布和质量 segregation。研究发现，这些致密源的分离距离和质量更符合热碎裂尺度（~0.1 pc），并在17个云中检测到5个云存在质量 segregation，但与其他活跃恒星形成云的相关性尚不明确。


<details>
  <summary>Details</summary>
Motivation: 为了表征银河系中央分子区（CMZ）中致密毫米连续谱源的空间分布和质量 segregation，并探究其与热碎裂和湍流碎裂的关系，以及其与恒星形成演化阶段的相关性。

Method: 采用最小生成树（MST）方法分析685个致密源的分布特征；计算热和湍流金斯长度及质量，比较源间距与质量的碎裂尺度一致性；构建质量 segregation 比率（Λ_MSR）进行分类。

Result: 致密源间距和质量更符合热碎裂尺度（~0.1 pc）；17个云中5个显示质量 segregation（Λ_MSR > 1.5），其余显示逆 segregation 或无 segregation；活跃恒星形成云中质量 segregation 与演化阶段相关性不明确。

Conclusion: CMZ中致密源主要由热碎裂主导，质量 segregation 在部分云中存在，但与恒星形成演化阶段的关系仍需更多数据验证。
...
### [5] [Dwarf Galaxy Constraints on Interacting Fermionic Dark Matter](https://arxiv.org/abs/2605.22685)
...
TL;DR: 本文通过经典矮椭球星系数据分析，发现相互作用与非相互作用的简并费米子暗物质模型给出的费米子质量、中心密度等后验分布相似，当前数据无法区分两者，排除了显著偏离非相互作用极限的可能性。


<details>
  <summary>Details</summary>
Motivation: 利用本地群矮星系恒星运动学数据检验暗物质模型，特别是研究简并费米子暗物质中相互作用项的影响力。

Method: 采用两种模型（标准非相互作用费米气体和具有唯象状态方程的相互作用简并费米子流体），通过麦克斯韦构造处理力学不稳定分支，求解非相对论流体静力学方程，利用球面金斯方程计算视线速度色散轮廓，并对八个经典银河系矮椭球星系进行MCMC拟合。

Result: 数据支持的暗物质费米子质量范围为100-300 eV；相互作用与非相互作用状态方程给出的费米子质量、中心密度和恒星各向异性后验分布大致相似。

Conclusion: 当前数据未能强烈偏好相互作用状态方程，排除了显著偏离非相互作用的自由简并费米气体极限的情况。
...
### [6] [Probing the ion-neutral drift velocity towards the L1544 prestellar core: Detection of ambipolar diffusion using N$_2$D$^+$ and para-NH$_2$D](https://arxiv.org/abs/2605.22541)
...
TL;DR: 本文在典型前恒星核L1544中观测到离子（N₂D⁺）与中性粒子（para-NH₂D）之间的速度差异（约0.05 km/s），结合尘埃生长模型解释为磁致扩散（ambipolar diffusion）的首个观测证据，并强调尘埃生长在调节磁致电阻率及核塌缩动力学中的关键作用。


<details>
  <summary>Details</summary>
Motivation: 在恒星形成过程中，磁场与物质的耦合依赖于部分电离星际介质中离子与中性粒子的相互作用。当前恒星核致密环境中电离度下降时，磁场与物质可能退耦，导致离子与中性粒子下落速度不同（磁致扩散）。然而，磁致扩散引发的引力塌缩起始过程从未被直接观测到。本研究旨在寻找前恒星核内磁致扩散的观测特征。

Method: 选取典型前恒星核L1544，观测氘代离子N₂D⁺与中性粒子para-NH₂D（两者同为高密度示踪物），比较两者的速度质心和谱线宽度分布图，并将观测结果与包含尘埃生长的磁致电阻率自洽模型预测进行对比。

Result: 在核心区域发现离子与中性粒子平均速度差约0.05 km/s，未检测到显著的谱线宽度差异。通过与自洽模型对比，将速度差异解读为磁致扩散的信号。

Conclusion: 结果强调前恒星核阶段尘埃生长在调节磁致电阻率中的重要性，进而影响致密核向原恒星塌缩的动力学演化。提出测量离子-中性粒子漂移速度可为了解前恒星核内总磁场强度和尘埃粒径分布提供新约束。
...
### [7] [An AGN in the Antennae galaxies ?](https://arxiv.org/abs/2605.21879)
...
TL;DR: 利用ALMA观测数据，在Antennae合并星系中发现了一个紧致、时变（特征时标13±3天）的100GHz射电源S4，它很可能是一个被遮蔽的活动星系核（AGN），而另一个紧致源S3则可能是年轻星团或AGN。


<details>
  <summary>Details</summary>
Motivation: 由于Antennae星系在光学、红外和X射线波段均未显示AGN迹象，而射电时变性是探测小尺度高能现象（如AGN）的强有力工具，因此作者利用ALMA高分辨率时域观测来寻找其中潜在的AGN活动。

Method: 使用ALMA在100 GHz对Antennae星系进行为期2.5个月的监测，通过功率谱分析两个紧致射电源（S3和S4）的时间变异性，并结合其空间分辨特征（S3在10 pc尺度上勉强可分辨，S4在4 pc尺度上仍未分辨）进行物理起源判别。

Result: 发现S4存在显著时变性，特征时标为13±3天，对应辐射区尺寸小于0.01 pc；S3则未表现出显著时变性。通过流量比较，S3可能为年轻大质量星团或AGN，而S4更可能是一个Compton厚AGN。

Conclusion: 该研究首次在Antennae星系中通过射电时变性发现了一个潜在的Compton厚AGN（S4），证明了高分辨率时域射电观测在探测光学/X射线暗弱的隐藏AGN方面的独特能力。
...
### [8] [(LRDs)$^2$: The Low-ReDshift Little Red Dots Survey. II. DESI DR1 Sample](https://arxiv.org/abs/2605.21574)
...
TL;DR: 本文在DESI DR1中系统筛选出27个低红移（z=0.2-0.9）"小红点"（LRDs），通过近红外光谱证实它们与高红移LRDs具有一致特性：紧凑形态、V型UV-光学连续谱、宽巴默发射线、低金属丰度等。研究还发现低红移LRDs存在广泛温度范围（~2000-4700 K），普遍存在电离外流（78%），且其宽线光度与L_5100的关系偏离本地1型AGN，表明无法直接使用本地黑洞质量标定。


<details>
  <summary>Details</summary>
Motivation: 此前JWST在高红移（z>4）发现大量LRDs，但其低红移对应体尚未被系统研究。本文旨在填补这一空白，通过低红移LRDs验证其与高红移LRDs是否为同一物理过程，并作为近邻实验室探究LRDs的本质。

Method: 从DESI DR1光谱数据中系统筛选低红移LRDs，对18个候选体进行近红外光谱后随观测，分析其SED形态、发射线特征、BPT诊断图、宽线光度关系、电离外流及光学/红外光变等。

Result: 识别出27个低红移LRDs（z=0.2-0.9），数密度下限7.5×10^{-10} cMpc^{-3}。低红移LRDs与高红移对应体在形态、光谱形状、发射线特征等方面高度一致，包括极端巴默递减（Hα/Hβ中位数~16）、频繁吸收线（67%）、低金属丰度等。但光学-近红外连续谱温度范围更广（2000-4700 K），部分源有更冷更大的包层。宽线光度关系偏离本地1型AGN标定，78%的源存在[O III]电离外流，一个源（J1717+3807）表现出长期光变。

Conclusion: 低红移和高红移LRDs的物理性质高度一致，表明两者由相同物理过程驱动。低红移LRDs可作为研究LRDs本质的近邻实验室，并追踪该种群从宇宙黎明至今的演化。
...
### [9] [From protogalaxy through thick and thin: Why did the Milky Way evolve in three kinematic phases?](https://arxiv.org/abs/2605.22806)
...
TL;DR: 本文通过FIRE-2模拟发现，银河系结构演化经历了三个运动学阶段：早期无序的原星系、旋转的厚盘阶段、以及冷薄盘阶段，并揭示了各阶段形成的物理机制。


<details>
  <summary>Details</summary>
Motivation: 理解银河系结构从无序到有序演化的物理驱动机制，特别是三个运动学阶段（无序原星系→旋转厚盘→冷薄盘）的形成条件和转变原因。

Method: 使用FIRE-2宇宙学zoom-in模拟，对多个银河系质量星系进行高分辨率数值模拟，分析气体冷却速率、恒星形成率、质心运动、角动量演化等物理量在各阶段的变化。

Result: 1) 无序阶段：冷气体转恒星速率低、恒星形成爆发性、重子物质在势阱中"晃动"；2) 厚盘阶段：冷气体转恒星速率最高、恒星形成仍爆发性、质心运动稳定；3) 薄盘阶段：内CGM virial化、恒星形成平稳、冷气体转恒星速率中等。并发现厚盘形成只需质心运动稳定，薄盘需气体慢速吸积使角动量混合一致。

Conclusion: 银河系三阶段演化是自然过程，合并事件并非驱动力；厚盘形成条件较宽松，薄盘形成需更严格的气体吸积条件。
...
TL;DR: 本文利用ASKAP的RACS巡天数据对G4Jy-3CRE星表（南天最亮射电源）进行了新的连续谱成像，在264个源中识别出173个具有喷流形态的源（其中37个此前未显示喷流活动），并新确定了6个光学对应体。


<details>
  <summary>Details</summary>
Motivation: 为南天提供与北天3CR星表等效的亮射电源样本，并利用更高分辨率和灵敏度的ASKAP数据对这些源进行详细形态学分类和光学对应体识别。

Method: 使用RACS三次数据释放（low1、mid、high，覆盖600-1800 MHz）对G4Jy-3CRE中的264个源进行ASKAP连续谱成像，通过形态学分析分类FRI/FRII射电星系。

Result: 173个源（66%）呈现喷流形态，其中37个为首次发现；成功对红移z≤1.35的源进行了FRI/FRII分类；新识别了6个光学对应体。

Conclusion: ASKAP的高分辨率和灵敏度显著提升了南天亮射电源的形态分类能力，发现了更多射电喷流活动，并完善了光学对应关系。
...
### [11] [Advancing the detection of low surface brightness galaxies. I. ATTILA: multi-tAsking deTecTIon tool for Lsb gAlaxies](https://arxiv.org/abs/2605.21598)
...
TL;DR: 本文介绍了一个名为ATTILA的新型自动检测工具，用于改进超暗星系（UDGs）和低表面亮度（LSB）星系的探测与表征。在Hydra I星系团中，通过VST VEGAS巡天数据，成功识别了24个新的UDGs（使总数翻倍至48个）以及92个额外的LSB星系，检测率超过80%。


<details>
  <summary>Details</summary>
Motivation: 超暗星系（UDGs）处于低表面亮度星系尺寸-光度分布的极端端，其探测和表征需要能够处理低信噪比和严重源混合的深度成像及可靠的源检测技术。现有方法在自动化检测低亮度、弥散源方面存在不足。

Method: 基于Python开发了ATTILA自动化检测工具，利用VST VEGAS巡天的深g和r波段成像数据，覆盖Hydra I星系团中心区域及三个新增天区。采用瓦片处理、源检测和迭代去混合相结合的方法识别源，通过表面亮度轮廓分析和Sérsic模型拟合获取结构参数，并使用早型星系颜色-星等关系确定团成员资格。

Result: 在Hydra I星系团中识别了24个新的UDGs（使已知种群翻倍至48个），以及92个额外的LSB星系。在实际数据中，ATTILA恢复了超过80%的先前已知LSB星系，相比标准方法显著提高了自动化检测率。

Conclusion: 通过改进对暗弱和弥散源的恢复，同时减轻混合和污染效应，ATTILA能够更完整地普查LSB星系种群（包括UDGs），其性能与暗物质晕质量定标关系预期一致。
...
### [12] [A Magnetized Black Hole Envelope Model for Little Red Dots](https://arxiv.org/abs/2605.21589)
...
TL;DR: 本文提出磁化黑洞包层模型解释“小红点”（LRD）类活动星系核的观测特征，包括宽发射线的多普勒分量、X射线不探测等，并指出传统维里质量估计方法可能失效。


<details>
  <summary>Details</summary>
Motivation: 观测发现了一类独特的活动星系核——“小红点”（LRDs），但其物理本质尚不清晰。现有理论认为它们由快速吸积的大质量黑洞驱动，但具体辐射机制和谱线特征缺乏合理解释，需要构建自洽的动力学与光谱模型。

Method: 假设球对称自由落体吸积到旋转磁化黑洞包层上，构建磁化BHE模型；将宽发射线多普勒分量归因于BHE磁层内共转的等离子体团块，并引入电子散射展宽拟合线型；评估吸积激波后区和磁加热日冕两个潜在X射线源的辐射。

Result: 模型能重现高达数千km/s的多普勒分量；发现传统维里质量估计可能产生错误结果；预测X射线光度在较宽黑洞质量范围和吸积率下≲10^41 erg/s，与当前观测上限一致。

Conclusion: 磁化黑洞包层模型能够自洽解释LRD的主要观测特征，为理解这类特殊AGN的物理本质提供了新框架，同时对黑洞质量测量方法提出了警示。
...
### [13] [Blue-tilted Runnings and the JWST Early Galaxy Tension](https://arxiv.org/abs/2605.22161)
...
TL;DR: 本文提出通过具有正runing的蓝倾谱标量扰动谱(α_s≈0.2, β_s≈0.2)来解释JWST观测到的高红移大质量星系过剩现象，联合CMB分析可在1σ置信度内解决该张力，且该谱也与早期原初黑洞形成一致。


<details>
  <summary>Details</summary>
Motivation: JWST观测发现高红移(z≈6.5-9)处大质量星系(约10^10 M⊙)数量远超标准ΛCDM模型预测，存在显著张力，亟需理论解释。

Method: 引入具正runing的蓝倾标量扰动谱，联合CMB观测数据进行统计分析，检验该谱对高红移星系丰度的修正效果。

Result: 当α_s≈0.2且β_s≈0.2时，该模型可在1σ置信度内消除JWST观测与ΛCDM预测之间的张力。

Conclusion: 蓝倾谱+正runing的扰动谱可有效解释JWST观测到的高红移大质量星系过剩，且与CMB观测相容，同时与小尺度原初黑洞形成理论自洽。
...
TL;DR: 本文研究了宇宙空洞与CMB透镜效应的交叉关联信号，使用Roman模拟星表量化了方法选择和宇宙学参数敏感性，预测Roman与下一代CMB巡天（如CMB-S4）结合可实现高达31σ的探测信噪比。


<details>
  <summary>Details</summary>
Motivation: 宇宙空洞在CMB上产生的次级透镜各向异性信号已被测量，但方法论选择对其测量的影响以及宇宙学参数依赖性尚不完全清楚，限制了其宇宙学约束能力。

Method: 利用经过验证的Roman模拟星表，比较了不同空洞定义（2D vs 3D）、叠加方式（重缩放vs非重缩放）、CMB滤波方案和噪声水平对信号的影响，并预测了Roman与Planck、SO、CMB-S4等巡天结合的信噪比。

Result: 空洞×CMB透镜信号对模拟星表选择不敏感；2D空洞加重缩放轮廓信噪比最高；Roman+Planck达13σ（2D）/8σ（3D），Roman+SO达22σ/13σ，Roman+CMB-S4达31σ/18σ。

Conclusion: Roman与下一代大尺度结构和CMB巡天结合，有望实现首个来自空洞×CMB透镜的直接宇宙学约束。
...
### [15] [The impact of evolving dark energy on the Weyl potential measured from the Dark Energy Survey Year 3 data](https://arxiv.org/abs/2605.22599)
...
TL;DR: DES数据显示Weyl势演化比GR预测慢，DESI偏好的动态暗能量（含phantom crossing）可将与GR的张力降至1.6-2.2σ，但这源于暗能量背景演化而非Weyl势不确定性增加。


<details>
  <summary>Details</summary>
Motivation: DES Year 3数据发现Weyl势演化比广义相对论（GR）在ΛCDM背景下预测的更慢，这构成了对GR的潜在挑战。本文旨在检验动态暗能量模型能否缓解这一张力。

Method: 采用w₀wₐCDM模型（动态暗能量），对比ΛCDM模型，分析DES Year 3数据中Weyl势的演化，并考虑CMB透镜处理的不同方式。

Result: w₀wₐCDM模型将与GR的张力降低至1.6-2.2σ（取决于CMB透镜处理方式），且这种降低源于动态暗能量背景对GR理论预测的影响，而非Weyl势不确定性的增加。

Conclusion: 动态暗能量可部分缓解DES数据与GR之间的张力，但仍需更多数据来确认是否完全由暗能量演化解释，或需要修改引力/暗区相互作用来解释，同时后者可能稳定DESI所指示的phantom crossing。
...
### [16] [Machine Learning Techniques for Astrophysics and Cosmology: Lyman-$α$ forest](https://arxiv.org/abs/2605.22489)
...
TL;DR: 本文综述了机器学习技术如何革命性地推动Lyman-α森林分析，克服了传统方法在计算和方法上的局限，成为下一代天体物理和宇宙学分析的核心工具。


<details>
  <summary>Details</summary>
Motivation: Lyman-α森林是探测宇宙再电离、热历史、中微子质量、暗物质性质等多种物理过程的重要探针，但传统分析方法存在计算复杂性和方法局限性，亟需新的技术突破。

Method: 系统综述了机器学习在Lyman-α森林分析中的多种应用：自动表征吸收系统、重建类星体连续谱、加速流体动力学仿真、发展基于仿真的分析和场级推断方法，以及三维物质密度场重建技术。

Result: ML驱动的分析管线能够有效克服传统方法的计算瓶颈，提升分析精度和效率，适应不断增长的数据量和精度要求。

Conclusion: 随着当前和未来巡天项目持续增加Lyman-α森林观测数据的体量与精度，机器学习驱动的分析管线已成为下一代天体物理和宇宙学分析不可或缺的组成部分。
...
TL;DR: 本文利用CMB、BAO、超新星等宇宙学数据，在CPL动力学暗能量背景下，研究了晚期宇宙中有效引力耦合偏离牛顿常数的可能性。通过引入两个红移区间（0≤z<1和1≤z≤3）的修正引力参数，发现修正引力效应需在~10 Mpc尺度以下才能抑制低红移结构增长，并同时满足CMB约束。数据表明，CPL背景+标准引力比ΛCDM更受偏好，而引入修正引力后这种偏好进一步增强至中强水平，且CPL参数略微向quintom区域移动。


<details>
  <summary>Details</summary>
Motivation: 宇宙学数据暗示驱动宇宙加速膨胀的组分可能不是严格意义上的宇宙学常数，而是动态的暗能量。为了解释这一现象，需要在晚期宇宙中探索有效引力耦合偏离广义相对论的可能性，并确定这种偏离发生的尺度范围。

Method: 在CPL参数化动态暗能量背景下，引入两个红移区间（0≤z<1和1≤z≤3）的修正引力参数，通过设定一个共动尺度λc来参数化有效引力耦合偏离牛顿常数的行为。利用Planck PR4、DESI DR2、Pantheon+（或DES-Dovekie）以及红移空间畸变数据，进行贝叶斯统计分析。

Result: 修正引力效应必须在λc ~ O(10) Mpc尺度以下才能实现低红移结构增长的抑制并符合CMB约束（主要是低ℓ的晚期ISW效应和高ℓ的透镜效应）。CPL背景+标准引力比ΛCDM更受偏好，加入修正引力后偏好增强至中强水平，CPL参数略微向quintom区域移动。

Conclusion: 动态暗能量模型（CPL）比ΛCDM更能解释当前宇宙学数据，而引入修正引力效应能进一步增强这种偏好。修正引力必须发生在较小尺度（~10 Mpc）上才能与观测一致，这为未来高精度宇宙学探测提供了可检验的预测。
...
### [18] [Still non-accelerating: age-bias correction in supernova cosmology is robust to host-progenitor age mapping](https://arxiv.org/abs/2605.21586)
...
TL;DR: 本文质疑Wiseman等人(2026)关于Ia型超新星前身星年龄偏差对宇宙学推断影响可忽略的结论，指出其样本红移范围过宽导致年龄-哈勃残差斜率被严重低估，且Pantheon+宿主质量校正模型与实测尘埃消光曲线不符。当年龄偏差与斜率效应一致结合时，最终校正结果与Son等人(2025)基本一致。


<details>
  <summary>Details</summary>
Motivation: 重新审视Wiseman等人(2026)声称前身星年龄偏差对Ia型超新星宇宙学推断影响可忽略的结论，证明其分析存在方法论缺陷。

Method: 通过分析样本红移范围对年龄-哈勃残差斜率的影响、评估Pantheon+宿主质量校正的尘埃模型兼容性，以及检验年龄偏差校正对不同延迟时间分布选择的稳健性。

Result: Wiseman等人(2026)的年龄-HR斜率被严重低估，Pantheon+校正中尘埃模型与实测不符，但年龄偏差校正在不同延迟时间分布下保持稳健，最终宇宙学修正与Son等人(2025)基本一致。

Conclusion: Wiseman等人(2026)降低前身星年龄演化的结论必然伴随更陡的年龄-HR斜率，两者一致结合后，最终的宇宙学影响与先前研究无异。
