...
### [1] [Bar-driven gas redistribution suppresses star formation in spiral galaxies: Evidence from dust lanes in NGC 3351](https://arxiv.org/abs/2605.27663)
...
### [2] [MAGAZ3NE: Spatially Resolved Ages and Chemical Abundances of Ultra-Massive Quiescent Galaxies at z $\sim$ 3.5 using JWST/NIRSpec IFU](https://arxiv.org/abs/2605.27555)
...
TL;DR: 利用JWST/NIRSPec IFU光谱，首次在红移z~3.5的三个超大规模（>10^11 M☉）、紧凑（Re≤2 kpc）静止星系中实现了恒星年龄、[Fe/H]和[α/Fe]的空间分辨测量。结果显示星系中心年龄一致年轻（≈0.6-0.7 Gyr）且α元素丰度较高（≈0.2-0.5），表明近期快速增丰后迅速淬灭。其中两个星系显示出正的年龄梯度和负的[α/Fe]梯度，符合并合驱动淬灭；第三个星系年龄分布平坦，符合均匀淬灭。研究证实z~3.5时淬灭路径已多样化，并强调了α元素增强处理对解释早期淬灭系统星族历史的必要性。


<details>
  <summary>Details</summary>
Motivation: 已有研究表明高红移大质量静止星系形成迅速，但对其淬灭机制（如并合驱动vs.均匀淬灭）缺乏空间分辨的化学丰度约束。本文旨在通过JWST/NIRSpec IFU首次获得z~3.5超大规模静止星系的空间分辨α元素增强信息，在晚期并合等过程抹除化学信号之前直接检验淬灭机制。

Method: 利用JWST/NIRSpec IFU对三个z~3.5、log(M*/M⊙)>11、Re≤2 kpc的超大质量静止星系进行积分场光谱观测，通过星族合成模型拟合光谱，获得空间分辨的恒星年龄、[Fe/H]和[α/Fe]分布，并比较了使用α增强模板与太阳比例模板的金属丰度结果差异。

Result: 三个星系中心区域均显示年轻年龄（0.6-0.7 Gyr）和高[α/Fe]（0.2-0.5），表明淬灭前经历过快速增强的星暴活动。其中两个星系显示正年龄梯度和负[α/Fe]梯度，符合并合驱动淬灭的预期；第三个星系年龄分布平坦。使用α增强模型与太阳比例模型给出的[Fe/H]存在0.2-0.4 dex的系统差异。

Conclusion: 在宇宙诞生仅20亿年时（z~3.5），超大质量静止星系的淬灭路径已呈现多样化，其中并合驱动淬灭在一部分星系中已经发生。同时，α元素增强的显式处理对准确解释早期淬灭系统的恒星形成历史至关重要。
...
### [3] [TRINITY: A coupled model of winds, radiation, and photoionised gas in molecular clouds. I. Methods and validation](https://arxiv.org/abs/2605.27517)
...
TL;DR: TRINITY是一个1D薄壳代码，用于模拟恒星反馈驱动分子云演化的过程，发现光致电离压力可使云壳半径扩大约17%，云的结构（均匀、幂律、Bonnor-Ebert球）显著影响云是否膨胀、再塌缩或逃逸LyC光子。


<details>
  <summary>Details</summary>
Motivation: 理解大质量恒星形成后分子云的命运（分散、再塌缩、或泄漏LyC光子）取决于前超新星阶段多种反馈机制（风、辐射压、光致电离压）与云壳的耦合方式，现有模型缺乏高效且可解释的框架来全面刻画这些过程。

Method: 开发TRINITY 1D薄壳代码，整合风、超新星、直接和尘埃再处理辐射压、光致电离压（P_HII）和重力；采用相位感知策略（能量驱动时取热泡与光致电离压较大者，动量驱动时取P_HII加冲压）；支持均匀、分段幂律和Bonnor-Ebert球初始云结构；在10^5-10^6.5 M⊙、核心密度10^3-10^4 cm^-3、星形成效率0.01-0.30范围内验证。

Result: 光致电离压使云壳半径在10 Myr时增大约17%；高效率时能量驱动阶段<1 Myr，辐射压始终次主导，P_HII在动量驱动阶段仍重要；均匀或浅梯度云再塌缩，ρ∝r^-2陡梯度云持续膨胀，Bonnor-Ebert球比均匀云晚约55%分散。

Conclusion: 光致电离压（P_HII）和云结构在恒星种群固定时仍显著塑造反馈驱动的膨胀演化；TRINITY为映射云参数空间中的反馈主导机制和分辨HII区提供了高效、可解释的框架。
...
### [4] [Double Dots: Compact Pairs Mark Little Red Dots and High-Redshift Broad-line AGNs](https://arxiv.org/abs/2605.27903)
...
TL;DR: 利用JWST对A2744星系团场的观测，发现大量间距<0.25"的致密双源（"Double Dots"），其中多数对应"小红点"（LRDs）或高红移宽线AGN。分析31对双源，中值间距0.15"，且多为物理对。约67%的已知LRDs和两个高红移BLAGNs属于这类双源。伴星会污染光谱能量分布，掩盖LRD特征"V形"谱。二维光谱分析显示BLAGN活动可来源于双源中的任一成员。研究表明致密双源是高红移宽线星系的极佳示踪物，可能源于并合驱动吸积。


<details>
  <summary>Details</summary>
Motivation: 探究JWST在透镜星系团A2744场中发现的大量间距极小的致密双源（"Double Dots"）是否与"小红点"（LRDs）及高红移宽线AGN存在物理关联，并评估伴星对LRD光谱特征识别的影响。

Method: 利用JWST高分辨率成像识别A2744场中间距<0.25"的致密双源；通过统计背景比较验证其物理关联性；对已知LRDs和BLAGNs进行交叉匹配；采用2D光谱分析探究双源中BLAGN活动的来源。

Result: 31对双源的中值间距为0.15"，统计检验表明多数为物理对；24个已知LRD中至少16个（~67%）和两个高红移BLAGN均属于此类双源；伴星会污染光谱导致LRD特征"V形"谱被掩盖；BLAGN活动可来源于双源中的任一成员。

Conclusion: 致密双源是高红移宽线星系的有效示踪物，其形成机制很可能与并合驱动的吸积过程有关。
...
### [5] [Transient axion streams from disrupted miniclusters](https://arxiv.org/abs/2605.28005)
...
TL;DR: 本文通过模拟暗物质轴子迷你晕被恒星潮汐瓦解后形成的流结构，发现这些流在银河系时间尺度上密度可下降达10^9倍，因此在太阳附近难以形成持久的致密流，可观测的流主要来自近期和邻近的罕见瓦解事件；尽管流非常稀薄，但其谱线宽度远窄于当前实验的腔体带宽。


<details>
  <summary>Details</summary>
Motivation: 理解轴子迷你晕在银河系中被恒星潮汐瓦解后形成的流结构的演化过程，评估其对局部暗物质分布的贡献以及可探测性。

Method: 结合大规模蒙特卡洛模拟恒星多次飞掠过程与示踪粒子重构剥离碎屑的方法，追踪相空间演化，覆盖广泛的银心半径范围。

Result: 剥离碎屑的动能通常超过其自引力束缚能，后续演化由各向异性自由膨胀和轨道剪切主导；流密度在银河系时间尺度上可下降约10^9倍，太阳附近稳态致密流丰度被强烈抑制，仅少数样本在10年曝光时间内有非零相遇概率。

Conclusion: 可观测的轴子流主要来自罕见的近期和邻近瓦解事件，而非长寿命的致密子结构；尽管流高度稀释，但其动力学温度极低，产生的谱线宽度（~10^-7到10^1 Hz）远窄于当前实验腔体带宽，多普勒漂移也远低于腔体响应宽度。
...
### [6] [Reconstructing the Globular Cluster Initial Mass Function from Present-Day Globular Cluster Systems](https://arxiv.org/abs/2605.27509)
...
TL;DR: 本文通过反演动力学质量损失，重建了不同宿主星系质量下球状星团初始质量函数（GCIMF）的高质量端，发现GCIMF呈现幂律分布且斜率与星系质量强相关，表明GCIMF依赖于星系性质。


<details>
  <summary>Details</summary>
Motivation: 当前球状星团质量函数的近普适性拐点质量（~2×10^5 M⊙）是一个重要的观测特征，但其起源存在争议（动力学演化 vs. 形成物理印记）。本文旨在区分这两种解释，重建初始质量函数以揭示其本质。

Method: 基于环境依赖的质量损失模型（经N体模拟标定），通过反演当前球状星团系统的动力学演化，重建GCIMF的高质量端。该方法不预设GCIMF的函数形式，适用于不同宿主星系质量（10^9-10^12 M⊙），结合个体测量和统计样本。

Result: 恢复的GCIMF系统性地向高质量端偏移，高质量端呈现幂律行为。幂律斜率在不同星系中变化，与宿主星系质量强相关：更大质量的星系具有更陡的高质量端斜率。

Conclusion: GCIMF的斜率依赖于星系性质，这为当前球状星团系统与其高红移前身之间提供了直接的经验联系。研究支持GCIMF受形成环境调控的观点。
...
### [7] [Fewer simulations, sharper covariances: Reducing mock covariance noise with Zeldovich approximation control variates](https://arxiv.org/abs/2605.28817)
...
TL;DR: 本文提出了一种控制变量法，通过将模拟与Zeldovich近似配对，显著降低了功率谱协方差矩阵估计的方差，在大尺度上效率提升约10倍。


<details>
  <summary>Details</summary>
Motivation: 大尺度结构模拟中，功率谱协方差矩阵的精确估计计算成本极高，尤其是在大尺度上样本方差主导，需要大量模拟才能获得可靠估计。

Method: 将每个模拟与共享相同初始条件的廉价Zeldovich近似实现配对，利用Zeldovich场的已知统计特性去除相关样本方差。在Gaussian disconnected近似下，导出了最优控制变量系数β和相关函数ρ的完全解析表达式。

Result: 对于DESI LRG的掩蔽红移空间对数正态模拟，在大尺度(k≲0.05 h Mpc⁻¹)上，协方差矩阵估计方差降低约一个数量级；更高k值时收敛速度加速2-3倍。

Conclusion: 该方法简单易实现，可显著降低当前及未来大尺度结构巡天（DESI、Euclid、LSST、PFS、SPHEREx）协方差估计的计算成本。
...
### [8] [Exploring non-Poisson satellite occupation in HOD models and its impact on 2- and 3-point galaxy clustering](https://arxiv.org/abs/2605.28644)
*Antoine Rocher*
...
TL;DR: 本文在HOD框架中引入Conway-Maxwell-Poisson分布替代传统泊松假设，通过单参数ν描述卫星星系数的子/超泊松行为，发现非泊松卫星分布显著影响小尺度成团统计（聚类投影函数变化达10%，CiC统计量变化达30%），但对大尺度（k_max<0.3）功率谱和双谱的宇宙学约束影响有限（<2%）。


<details>
  <summary>Details</summary>
Motivation: 标准HOD模型假设固定质量暗晕中卫星星系数服从泊松分布，这一假设缺乏严格验证。本文旨在检验该假设并量化非泊松卫星分布对星系成团统计量的影响。

Method: 引入CMP分布作为泊松模型的最小扩展（额外参数ν），推导λ的解析近似并开发平滑连接小/大λ区域的数值方案（精度~5%）；利用HODDIES软件包生成模拟星系目录，分析投影聚类、多极矩、计数-圆柱统计和树级双谱。

Result: 小尺度投影聚类变化达10%，单极子和四极子变化5%；CiC统计量变化达30%；树级双谱（Sugiyama基）在k_max<0.3时变化<2%。

Conclusion: 非泊松卫星统计对小尺度分析重要，但对k_max<0.3的功率谱和双谱测量约束的宇宙学参数影响有限。
