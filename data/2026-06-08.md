...
### [1] [Dust in the very metal-poor galaxy Sextans A with JWST. I: Characterizing the evolved stellar population of Sextans A based on JWST observations and stellar evolution models](https://arxiv.org/abs/2606.07421)
...
### [2] [Polarized and unpolarized synchrotron emission from dark matter in extragalactic targets](https://arxiv.org/abs/2606.07446)
*Javier Reynoso-Cordova,Catherine Gibson,Stefano Profumo*

Main category: astro-ph.GA

Recommendation: ★★☆☆☆ (36/100) - 与 Zotero 兴趣画像中的 polarized, dark matter 等主题/作者匹配。

TL;DR: 本文利用Planck卫星30、44、70 GHz频段数据，对五个河外目标（M31、大麦哲伦云、天龙座和玉夫座矮椭球星系、后发座星系团）的暗物质湮灭截面和衰变率给出了95%置信水平的上限。通过DRAGON和HERMES数值模拟，对比了总强度和偏振同步辐射两种探针，发现30 GHz通道约束最强，e⁺e⁻通道比b\bar{b}通道更严格，且大多数目标两者效果相当，但大麦哲伦云中法拉第退偏振使总强度更优。


<details>
  <summary>Details</summary>
Motivation: 探索利用微波偏振测量作为独立互补的探针，从河外目标的同步辐射中约束暗物质性质，弥补传统总强度方法可能存在的系统误差和模型依赖。

Method: 对五个河外目标，使用DRAGON数值求解暗物质产生电子/正电子的扩散-损失方程，再用HERMES沿视线积分计算总强度和偏振同步辐射图，结合Planck 30/44/70 GHz数据，针对每个目标建立特定的磁场、气体和辐射场环境。

Result: 30 GHz通道给出最强约束；e⁺e⁻湮灭/衰变通道上限比b\bar{b}更严格；大多数目标总强度和偏振极限相当；大麦哲伦云中法拉第退偏振使偏振信号弱于总强度；结果对通量估计方法和坐标不确定性稳健。

Conclusion: 微波偏振测量可作为河外目标暗物质同步辐射的有效独立探针，与总强度方法互补，有助于提高暗物质搜寻的可靠性和灵敏度。
...
### [3] [JWST reveals how black holes are fed: kiloparsec-scale multiphase filaments feed sub-kiloparsec circumnuclear disks](https://arxiv.org/abs/2606.06620)
...
Recommendation: ★★☆☆☆ (36/100) - 与 Zotero 兴趣画像中的 jwst, molecular, molecular gas 等主题/作者匹配。

TL;DR: 本文利用JWST/NIRSpec对半人马座星系团中央星系NGC 4696进行观测，发现其中性气体漩涡是一个旋转的多相环核盘(CND)，连接了千秒差距尺度的冷却流与黑洞吸积，结合磁流体动力学模拟，揭示了多相纤维丝将气体从星系团尺度输送到黑洞附近的共同反馈机制。


<details>
  <summary>Details</summary>
Motivation: 长期以来，星系团核心中千秒差距尺度的冷却流如何与<100 pc尺度的黑洞吸积相连接一直缺乏直接观测证据，本研究旨在填补这一缺失环节。

Method: 利用JWST的NIRSpec仪器对NGC 4696进行高空间分辨率（10 pc）光谱成像观测，覆盖中心618 pc×618 pc区域，并结合定制的磁流体动力学(MHD)模拟进行对比。

Result: 发现中心S形电离气体漩涡是一个旋转的多相环核盘(CND)，它在物理和运动学上与更大尺度的纤维丝网络相连，且观测形态和运动学与MHD模拟结果高度吻合。

Conclusion: 多相纤维丝将气体从星系团尺度输送到黑洞附近的环核盘，后者介导黑洞吸积，从而闭合了AGN反馈回路。NGC 4696与NGC 1275的相似性表明这是一种普遍的射电模式AGN反馈机制。
...
### [4] [The Combined Dense Gas and Excitation Atlas of the Milky Way I. Mass Distribution and Ripples in the Molecular Layer](https://arxiv.org/abs/2606.06590)
*Peter J Barnes*

Main category: astro-ph.GA

Recommendation: ★★☆☆☆ (33/100) - 与 Zotero 兴趣画像中的 molecular, dust, molecular gas 等主题/作者匹配。

TL;DR: 本文介绍了银河系联合稠密气体与激发巡天（CODEX）项目，整合了五个已有巡天数据，提出了新的螺旋臂模型，并确认了分子层中存在波纹结构，同时对比了分子气体与尘埃消光的分布差异。


<details>
  <summary>Details</summary>
Motivation: 整合已有巡天数据，统一银河系尺度与旋转参数，以更好地描述银河系结构，并解决局部云团在本地静止标准中的运动学不一致问题。

Method: 整合NANTEN+CHaMP、ThrUMMS、FUGIN、MWISP五个巡天数据，统一采用最新R₀和Θ₀参数，增加局部云团修正公式，分析组合的经度-速度图，构建新的Ragged螺旋臂模型，并对比分子气体与尘埃消光数据。

Result: 构建了新的Ragged螺旋臂模型，更好地追踪了12CO的投影结构；首次在第四象限发现的分子层波纹在第一象限FUGIN数据中得到确认，幅度分别约为40 pc和23 pc，波长均约4 kpc；第四象限和内第一象限的分子气体与尘埃消光分布匹配良好，但外第一象限匹配较差；整体上气体与尘埃的波纹幅度和符号不一致。

Conclusion: CODEX巡天有效整合多源数据，揭示银河系分子气体结构的新特征，波纹结构在多个象限普遍存在，尘埃消光对致密分子气体不敏感导致其与气体分布存在差异。
...
### [5] [Simple cyanides and formylium ions isotopologues in early star-forming molecular cores](https://arxiv.org/abs/2606.07343)
*R. D. Taboada,S. Paron,N. C. Martinez,M. E. Ortega*

Main category: astro-ph.GA

Recommendation: ★★☆☆☆ (32/100) - 与 Zotero 兴趣画像中的 molecular, alma 等主题/作者匹配。

TL;DR: 本研究利用ALMA数据分析了37个大质量红外暗分子核中氰基分子和甲酰离子同位素体的化学行为，发现HN¹³C、H¹³CO⁺、HC¹⁷O⁺丰度与温度正相关，HC₃N丰度在40-100K范围内与温度无关，并首次将层次聚类方法应用于天体化学中的丰度比分析，识别出与热条件一致的化学核群，同时提供了28个核的HC¹⁷O⁺探测结果。


<details>
  <summary>Details</summary>
Motivation: 理解恒星形成早期阶段的化学过程对于研究星际介质中最复杂化学的开端至关重要，但目前对大质量红外暗核中简单氰基分子和甲酰离子同位素体的化学行为尚缺乏系统研究。

Method: 利用ALMA Band 7档案数据对嵌入ATLASGAL团块中的37个早期分子核进行观测，获得HC₃N、H¹³CN、HN¹³C、H¹³CO⁺和HC¹⁷O⁺的丰度；采用多种统计方法（包括层次聚类）分析分子丰度、比值与温度之间的相关性。

Result: HN¹³C、H¹³CO⁺和HC¹⁷O⁺丰度与动力学温度正相关；HC₃N丰度在40-100K范围内与温度无关；首次将层次聚类基于丰度比应用于天体化学，识别出与热条件一致的化学核群；在28个核中探测到HC¹⁷O⁺，显著扩展了现有文献。

Conclusion: 温度驱动了年轻大质量核中的化学调控，HC₃N在40-100K达到气相生成与颗粒表面消耗的化学稳态，H¹³CO⁺跃迁的光深可能高于普遍假设，这些结果对理解恒星形成早期阶段的化学状态具有重要意义。
...
### [6] [A Population of Red Galaxies with Very Strong Emission Lines at $z > 5$ Revealed by the NIRCam Medium Bands: ''Classic'' LRDs, Dusty Star-Forming Galaxies, and a Missing Population of LRDs](https://arxiv.org/abs/2606.06585)
...
Recommendation: ★★☆☆☆ (32/100) - 与 Zotero 兴趣画像中的 dusty, dsfgs, jwst 等主题/作者匹配。

TL;DR: 本文利用JWST/NIRCam中波段成像，在红移4.9-8.9范围内识别出具有强发射线的星系（ELGs），特别关注了其中偏离颜色-发射线强度关系的"红色发射线星系"（REGs），并指出其中一类致密REGs可能被传统的"小红点"（LRDs）选择标准遗漏。


<details>
  <summary>Details</summary>
Motivation: 中波段成像在识别高等效宽度发射线星系（ELGs）方面效率很高，但传统选择标准可能遗漏某些特殊类型星系，因此需要更全面的识别方法。

Method: 利用CANUCS、Technicolor和JUMPS巡天的NIRCam中波段成像数据，在红移4.9-8.9范围内识别ELGs，分析连续谱颜色与发射线强度的相关性，并针对偏离该关系的REGs进行分类研究。

Result: 发现ELGs的连续谱颜色与发射线强度存在强相关；识别出26个偏离该关系的REGs，并将其分为三类：经典LRDs、扩展REGs（可能是尘埃恒星形成星系）和致密REGs（未被传统LRD标准识别）。

Conclusion: 致密REGs很可能是被经典LRD选择标准遗漏的对象，中波段选择方法可以提供更完整的样本。
...
### [7] [JWST absorption line spectroscopy with SPURS: ISM covering fractions and kinematics in individual galaxies at $z=5-9$](https://arxiv.org/abs/2606.06609)
...
### [8] [ALMAGAL IX. The chemical complexity of AG318.9477-00.1960: A line-identification template for ALMAGAL](https://arxiv.org/abs/2606.07388)
...
TL;DR: 本文对ALMAGAL样本中的高质量核AG318-c9进行详细分子谱线分析，利用ALMA Band 6观测数据，通过MADCUBA软件进行谱线识别和LTE建模，探测了乙二醇、乙醇醛、甲酸甲酯等复杂有机分子，并绘制了其空间分布图。


<details>
  <summary>Details</summary>
Motivation: 探究高质量恒星形成区中最富含化学物质的冷核之一AG318-c9的分子组成，并评估特定复杂有机分子能否用于示踪热分子核的最内部区域。

Method: 利用ALMA Band 6 (~217-221 GHz)观测数据，使用MADCUBA软件进行谱线识别和局部热力学平衡(LTE)建模，推导柱密度、激发温度、速度、线宽及相对H2丰度，并进行像素级空间分辨制图。

Result: 获得了所有探测到的分子的物理参数（柱密度、激发温度、速度、线宽和丰度），并与已知热分子核G31.41+0.31进行了化学清单对比；生成了乙二醇、乙醇醛和甲酸甲酯的空间分辨柱密度和激发温度图及径向轮廓。

Conclusion: 乙二醇、乙醇醛和甲酸甲酯等复杂有机分子可用于示踪热分子核的最内部区域，且AG318-c9的化学丰富度与已知热分子核G31具有可比性。
...
### [9] [Origin of monolithic high-z galaxies and UV luminosity of mergering high-z galaxies in the cosmological model with non-standard spectrum of density perturbations](https://arxiv.org/abs/2606.07357)
...
TL;DR: JWST发现高红移星系数量超ΛCDM预期。本文提出一种在功率谱特定质量尺度存在"凸起"的模型，但需引入小尺度截断以避免过早再电离矛盾。通过EPS理论和N体模拟验证了并合率分布，计算了紫外光度函数，预测了更多单次大尺度塌缩形成的早期星系以及首批直接出现在≥10⁸M⊙暗晕中的"第四族星"。


<details>
  <summary>Details</summary>
Motivation: JWST观测到红移z≥10的星系数量远超标准ΛCDM模型预言，需要解释这一偏差。

Method: 在扰动功率谱中引入特定质量尺度（约10¹⁰M⊙）的凸起，同时在小尺度设置截断（通过降低原初扰动振幅或考虑温暗物质）；利用延伸Press-Schechter理论推导并合率分布，并用N体模拟验证；基于并合触发星暴的假设计算紫外光度函数。

Result: 带小尺度截断的凸起模型避免了过早再电离问题，紫外光度函数与观测数据吻合；模型中通过单次大尺度塌缩（ monolithic ）形成的早期星系比例显著增加；首批恒星直接出现在≥10⁸M⊙的暗晕中。

Conclusion: 带小尺度截断的功率谱凸起模型能同时解释JWST观测到的高红移星系过剩和宇宙再电离历史约束；这种模型中诞生了化学组成原初、演化路径不同的"第四族星"。
...
### [10] [Star formation at z$\sim$0.9 from the OTELO survey: A comprehensive view combining deep optical spectroscopy and infrared data](https://arxiv.org/abs/2606.06644)
...
TL;DR: 本研究利用OTELO巡天数据，通过光学光谱和远红外测光量化了z~0.9星系样本的恒星形成率，发现远红外示踪剂揭示的恒星形成率密度是仅使用发射线源的三倍，表明红外示踪剂在探测尘埃遮蔽的恒星形成中至关重要。


<details>
  <summary>Details</summary>
Motivation: 为了量化z~0.9星系中由光学光谱数据和远红外测光揭示的恒星形成率，并比较光学示踪剂与远红外发射所估计的恒星形成活动差异。

Method: 利用OTELO多波段星表构建远红外源样本，识别并分离活动星系核；分析其余星系的光谱能量分布，获取恒星质量、尘埃消光、光度及基于红外光度的恒星形成率；同时研究无显著远红外发射的Hβ和[OIII]发射线星系，对多种校准器（尤其是Hβ）得到的恒星形成率进行比较分析。

Result: 远红外示踪剂揭示了大量隐藏的恒星形成，其得到的恒星形成率密度是仅使用发射线源的三倍。光学发射线在低质量星系中有效示踪恒星形成，而远红外示踪剂在富含尘埃的大质量星系中提供更可靠的测量。

Conclusion: 通过同时考虑光学可见和被遮蔽的恒星形成，本研究提供了z~0.9恒星形成主序的更全面视图，强调了红外示踪剂在星系演化研究中的重要性。
...
### [11] [EPISODE II: Variability in the CO and H$_2$O rovibrational absorption lines in a periodically variable protostar EC 53](https://arxiv.org/abs/2606.06628)
...
TL;DR: 通过两期JWST光谱和IGRINS高分辨率光谱，研究年轻原恒星EC 53在爆发和静默期的CO、H₂O吸收特征变化，发现CO基带和H₂O弯曲模式在爆发期间减弱约2倍，归因于连续谱稀释而非吸收气体变化，并引入“相对遮蔽”方法量化了不同区域吸积率的变化差异。


<details>
  <summary>Details</summary>
Motivation: 探索年轻原恒星EC 53在爆发期和静默期吸积过程的变化机制，利用其周期变光特性研究内盘物质的传输和吸积行为。

Method: 使用两期JWST NIRSpec/MIRI光谱和IGRINS高分辨率光谱，结合LTE slab模型拟合CO和H₂O吸收特征，并引入“相对遮蔽”方法量化连续谱稀释效应。

Result: CO基带和H₂O弯曲模式在爆发期间减弱约2倍；CO泛频和基带的热连续谱比值分别为2.9±0.2和1.71±0.11，对应的吸积率比值约为3.6和2.0。

Conclusion: 不同温度（半径）的内盘区域在爆发周期中响应不同，静默期内盘物质积累，爆发时物质更高效地通过最内盘传输到原恒星上。
...
### [12] [The quasi-star model for Little Red Dots: potential and challenges](https://arxiv.org/abs/2606.06575)
...
TL;DR: 本文研究“准恒星”模型能否解释韦伯望远镜发现的“小红点”（LRDs）。使用Cloudy辐射传输代码建模发现，准恒星模型能重现LRD的UV-NIR连续谱和氢发射线，但无法解释氦线和热尘埃，说明不同LRD模型之间存在简并性。


<details>
  <summary>Details</summary>
Motivation: 小红点（LRDs）是JWST发现的一类具有独特光谱特征的天体，它们可能是早期宇宙中超大质量黑洞种子向经典活动星系核演化过程的中间阶段。验证准恒星模型能否解释LRD的观测特征，对于理解早期宇宙黑洞增长机制至关重要。

Method: 使用辐射传输代码Cloudy进行建模。模型包含：一个吸积的SMBH（~10^5-6 M⊙），周围被对流层包裹产生黑体谱（T~5000 K, L~10^44.4 erg/s）；该黑体辐射被致密气体壳层（ΔR~1000 AU, nH~10^11 cm^-3）再处理；再经过准恒星周围的弥散团块介质进一步处理。将模型拟合到JWST/NIRSpec观测的LRD光谱。

Result: 准恒星模型可重现LRD的UV-NIR连续谱形状、巴尔默跳变特征及氢发射线光度。但模型无法天然解释宽氦发射线和热尘埃的存在，需要额外组件。这表明不同LRD模型之间存在显著简并性。

Conclusion: 部分LRD可以被建模为准恒星，但不同模型间存在简并性，这对理解早期宇宙黑洞增长机制有重要影响。
...
### [13] [Angular momentum evolution in the CIELO simulations. I. Temporal evolution of gas-stellar misalignments and baryonic merger timing](https://arxiv.org/abs/2606.07160)
...
TL;DR: 利用CIELO模拟追踪了44个星系从z=3.5到z=0的动力学演化，发现近80%的星系在z=0时处于对齐状态，但86%经历过至少一次不对齐阶段。气体-恒星动力学不对齐的根本驱动因素是气体库竞争，而非并合本身。


<details>
  <summary>Details</summary>
Motivation: 虽然局域宇宙中星系气体-恒星动力学不对齐现象已有充分观测，但其从对齐到不对齐再到反向旋转的连续演化路径尚不清楚，特别是气体吸积通道和并合如何驱动角动量重定向缺乏系统性研究。

Method: 分析了CIELO模拟中44个中心星系从z=3.5到z=0的演化，利用恒星形成气体与恒星角动量矢量间的内禀角psi定义动力学阶段，追踪吸积气体来源并评估配对统计对比。

Result: 86%的模拟星系经历过至少一次不对齐阶段；不对齐阶段中19%持续>2Gyr；psi突变与吸积气体主导先前恒星形成气体库且高度倾斜的时刻吻合；过渡期吸积/原有气体质量比中位数从0.57升至2.14，角偏移从21.2度增至64.6度；并合作为条件性触发因素，但银河系最终动力学命运取决于新吸积气体如何与原有气体耦合、替换或混合。

Conclusion: 气体-恒星动力学不对齐的根本驱动因素是气体库竞争（reservoir competition），并合仅作为条件性触发因素，不能独立决定星系的动力学命运。
...
### [14] [The accretion history of the Milky Way V. The kinematics of most globular clusters trace the merger epochs](https://arxiv.org/abs/2606.06594)
...
TL;DR: 本文通过N体模拟分析球状星团与银河系吸积事件（GSE）的关联，发现盘状起源的GCs会因轨道能量损失而难以示踪吸积事件，而晕族和合并过程中形成的GCs则能保留轨道能量，支持了基于能量-角动量和年龄-金属丰度关系的观测关联。


<details>
  <summary>Details</summary>
Motivation: 此前研究通过能量-角动量平面和年龄-金属丰度关系将球状星团与银河系吸积事件（如GSE）关联起来，但近期模拟表明与GSE盘相关的GCs可能因轨道能量损失而不再可靠示踪该吸积事件。本文旨在更全面地模拟GCs在不同起源（盘、晕、合并期间形成）下的轨道演化，检验它们是否仍能可靠示踪GSE吸积事件。

Method: 采用三个不同初始质量、质量比和气体含量的N体模拟，模拟银河系与GSE的并合过程。除了考虑GSE盘原初星系的GCs外，还纳入其晕中的GCs，以及在富含气体的模型中考虑并合期间形成的GCs。通过追踪这些GCs在能量-角动量平面上的位置演化，评估它们对GSE吸积事件的示踪能力。

Result: 盘起源的GCs在多次穿越致密盘介质过程中损失了大量轨道能量，很多可能已被潮汐激波摧毁；而晕族GCs和并合期间形成的GCs则基本保留了轨道能量，甚至在并合完成后9 Gyr内仍与GSE恒星的能量相当。通过更真实的GC种群和GSE建模，发现大多数与GSE关联的GCs在能量-角动量平面上仍可与银河系吸积事件相关联。

Conclusion: 本文支持此前基于能量-角动量和年龄-金属丰度关系将GCs与GSE吸积事件进行关联的观测结论，同时指出盘起源的GCs因能量损失不适合作为吸积事件示踪物，而晕族和并合形成的新GCs是更可靠的示踪者。
...
### [15] [Lyman-$α$ forest constraints on pure and mixed fuzzy dark matter](https://arxiv.org/abs/2606.06969)
...
TL;DR: 该研究利用Lyman-α森林1D通量功率谱数据（z=5.0, 4.6, 4.2）约束模糊暗物质（FDM）模型，通过两步神经网络模拟器分析纯FDM和混合FDM模型。结果给出纯FDM质量下限为1.9×10⁻²¹ eV，混合FDM在不同质量下的暗物质分数上限。


<details>
  <summary>Details</summary>
Motivation: 模糊暗物质（FDM）作为一种超轻标量场，能够抑制小尺度结构增长，需要通过Lyman-α森林测量对其进行严格检验和约束。

Method: 使用改进初始条件的宇宙学流体动力学模拟，构建两阶段神经网络模拟器：第一阶段预测冷暗物质（CDM）的1D通量功率谱，第二阶段预测混合FDM（MFDM）相对于CDM的效应。该方法提高了对弱FDM效应的灵敏度，确保正确CDM极限，并实现较宽FDM质量与分数范围的鲁棒插值。

Result: 纯FDM模型：m_FDM > 1.9×10⁻²¹ eV（95%置信水平）；混合FDM模型：log10(m_FDM/eV) = -23.0, -22.0, -21.0时，f_FDM上限分别为0.07, 0.12, 0.65（95%置信水平）；当log10(m_FDM/eV) ≳ -20时，数据无法给出有效上限。

Conclusion: 高红移Lyman-α森林数据对FDM模型具有强约束能力，纯FDM质量下限与混合FDM分数上限均得到显著改进，为FDM模型提供了严格的观测限制。
...
### [16] [Halo mass functions in mixed cold and fuzzy dark matter models](https://arxiv.org/abs/2606.06599)
...
TL;DR: 本文通过数值模拟研究了混合冷暗物质与模糊暗物质（MDM）宇宙学对暗物质晕质量函数（HMF）的影响，开发了一种基于网格的晕寻找方法，并提出一个现象学模型，可在约0.1-0.2 dex精度内将CDM的HMF映射到MDM结果，无需针对每个参数进行专用模拟。


<details>
  <summary>Details</summary>
Motivation: 探索混合冷暗物质与模糊暗物质（MDM）宇宙学对暗物质晕质量函数的影响，旨在理解超轻类轴子成分如何改变小尺度结构形成，并为无需大量模拟即可预测MDM宇宙结构形成提供高效方法。

Method: 使用AxiREPO框架进行数值模拟，考虑质量m=10^{-24.5}eV的超轻类轴子成分（占比f≤0.3）；开发基于网格的晕寻找流程，将粒子CDM与波状FDM结合为统一密度场；提出一个现象学模型，利用依赖于红移和FDM占比的抑制函数将CDM的HMF映射到MDM结果。

Result: FDM示踪大尺度CDM分布，但通过波干涉效应抑制小尺度结构，导致低质量晕数量减少，HMF出现红移和FDM占比依赖的偏移；增加FDM占比使HMF系统性下降并改变高质量端斜率；现象学模型在z≤4、f≤0.3范围内以约0.1-0.2 dex精度复现模拟HMF。

Conclusion: 该研究提供了一种计算高效的MDM宇宙结构形成预测方法，无需对每个参数组合进行专用模拟，为探索MDM对宇宙结构形成的影响建立了框架。
...
### [17] [The colour variability of low-z SNe Ia is entirely explained by dust](https://arxiv.org/abs/2606.06593)
...
TL;DR: 本文通过贝叶斯层次模型Simple-BayeSN分析ZTF DR2和Foundation DR1两个低红移数据集，发现传统颜色截断会产生选择效应，在消除该效应后，SN Ia颜色-星等相关性完全可由尘埃效应解释，无需引入本征颜色相关性。


<details>
  <summary>Details</summary>
Motivation: SN Ia的本征颜色变化与尘埃导致的红化之间的相对重要性尚不明确，这对理解其环境依赖性和传统的Tripp线性校正用于宇宙学推断的有效性具有重要影响。

Method: 使用贝叶斯层次模型Simple-BayeSN分析ZTF DR2和Foundation DR1两个低红移同质数据集，通过模拟和真实数据验证传统颜色截断产生的选择效应。

Result: 传统颜色截断会产生先前未被认识到的选择效应，消除该效应后，颜色-星等相关性完全可由尘埃效应解释，无需本征颜色相关性，且该结果对宿主星系质量分裂和投影距离具有鲁棒性。

Conclusion: 传统Tripp线性校正在经验上仍然有效，但其物理来源应归因于尘埃而非本征颜色变化。
...
### [18] [Impact of fiducial cosmology in model-agnostic cosmological inference with the BAO feature](https://arxiv.org/abs/2606.06591)
...
TL;DR: 本文提出了一个名为"Zel'dovich涂抹近似"的模型无关宇宙学推断框架，用于分析重子声学振荡(BAO)特征。该框架不依赖特定宇宙学模型，并考虑了由参考宇宙学选择引起的几何畸变。通过对模拟星系样本的测试，证明其能产生无偏的宇宙学约束，为传统模板方法与新框架的性能比较奠定了基础。


<details>
  <summary>Details</summary>
Motivation: 传统基于模板的BAO分析方法依赖于特定宇宙学模型假设，作者希望开发一个模型无关的框架来避免这种依赖，同时解决参考宇宙学选择导致的几何畸变问题。

Method: 框架包括三个关键组件：线性理论相关函数的最优基、描述大尺度流引起的BAO涂抹效应的高斯核、以及红移空间尺度依赖偏差和模式耦合的模型。此外，还引入了对参考宇宙学几何畸变的修正。使用AbacusSummit模拟的两个类DESI和类Euclid巡天河样本进行验证。

Result: 完整的Zel'dovich涂抹框架在半真实模拟样本上产生了无偏的宇宙学约束，成功处理了几何畸变效应。该框架已达到"数据就绪"状态。

Conclusion: 本文完成了模型无关的BAO推断框架的几何畸变修正，验证了其在模拟数据上的有效性，并规划了与传统模板方法的详细性能比较路线图。该框架可直接应用于实际巡天数据。
...
### [19] [From Cosmic Web to Supernova Remnants: Modeling FRB DM to Trace Baryons across Multiple Scales](https://arxiv.org/abs/2606.06963)
*Zhao Joseph Zhang*
...
TL;DR: 本论文利用宇宙学模拟、zoom-in星系模拟和超新星遗迹模拟，系统研究了快速射电暴（FRB）色散测量（DM）从宇宙网到源区尺度的多成分贡献，揭示了IGM、CGM、宿主星系及源区环境对DM信号的综合影响。


<details>
  <summary>Details</summary>
Motivation: FRB的DM信号包含来自IGM、CGM、宿主星系和源区环境的多重贡献，准确解耦这些成分对于利用FRB探测电离重子分布至关重要。

Method: 采用CROCODILE模拟套件研究DM-红移关系及重子分布；使用一维流体动力学模拟研究嵌入SNR中的年轻磁星模型。

Result: 弥散重子分数分别为0.865和0.856；宿主星系DM从矮星系的<100 pc cm⁻³到星系团环境的>1300 pc cm⁻³；源区DM贡献数十至数百pc cm⁻³，多数模型在70年内对GHz射电透明；11 M⊙单星模型最佳复现FRB 20121102的RM演化。

Conclusion: FRB色散测量必须被解释为跨越多种物理尺度的多成分信号，连接宇宙网、气体晕、宿主星系和致密天体环境。
