...
### [1] [Capturing statistical isotropy violation with rotational averages](https://arxiv.org/abs/2605.04983)
...
TL;DR: 本文提出了一种几何实空间框架来量化宇宙微波背景（CMB）中的统计各向同性破坏（nSI），通过旋转平均角相关函数补充了传统的BipoSH谐波空间方法，并展示了其在特定场景下的优势。


<details>
  <summary>Details</summary>
Motivation: 高精度宇宙学观测揭示了CMB中多个统计各向同性破坏的异常现象，现有研究主要基于谐波空间（如BipoSH形式体系），需要一种互补的实空间几何框架来更直观地量化和解释nSI。

Method: 提出几何实空间框架，通过对角相关函数进行所有旋转配置下的加权平均（使用Wigner矩阵），系统分离出CMB天图中的nSI分量。以偶极调制模型为例进行数值实现，并与谐波空间结果进行对比验证。

Result: 数值实现显示旋转平均度量与其谐波空间对应量一致；该方法可直接从给定多极矩的关联函数提取nSI信息，避免了计算高内部秩的BipoSH系数；实空间分析可规避部分天区效应带来的复杂性。

Conclusion: 该实空间方法在特定场景下具优势，可拓展至原初非高斯性、CMB偏振、弱引力透镜以及球面上一般随机场的研究。
...
TL;DR: 提出了一种改进的星-星系分离方法和最优加权方案，用于减少星系样本中的恒星污染对宇宙学约束的偏差。


<details>
  <summary>Details</summary>
Motivation: 大尺度结构巡天中星系样本受恒星污染会严重干扰宇宙学参数约束，尤其对于星系聚类和星系-星系透镜探测，污染是加性系统误差的主要来源。

Method: 利用测光星系样本在颜色空间中与全测光数据集不同的分布特性，为每个红移bin优化星-星系颜色切割；结合DES Y3 MagLim透镜样本，通过交叉匹配DECaLS DR9获取强迫NIR unWISE测光，定义红移bin优化的颜色切割，并设计最优加权方案在图像层面同时减轻加性和乘性污染。

Result: 在DES Y3透镜样本中识别并去除了残差恒星污染，污染率随红移bin变化明显（1.3-5.5%），且在天区覆盖上存在空间变化。

Conclusion: 该方法作为形态学星-星系分离器的补充手段，可有效处理PSF和混合系统误差，提升宇宙学分析的精度。
...
### [3] [Little red dots as obscured little blue dots: relative abundances, luminosities, and black-hole masses](https://arxiv.org/abs/2605.05074)
*Piero Madau,Roberto Maiolino*

Main category: astro-ph.GA

Recommendation: ★★★★☆ (77/100) - 与 Zotero 兴趣画像中的 dust, jwst, dusty 等主题/作者匹配。

TL;DR: 本文通过前向建模，验证了“小红点”（LRDs）很可能是“小蓝点”（LBDs）的高倾角、尘埃红化对应体。模型成功复现了LRD的光度函数，并预测LRD/BLAGN比例随光度强烈变化。结果支持基于视角的统一样本，并指出UV选中的LRD宿主有更大质量黑洞，这是尘埃消光的选择性效应所致。


<details>
  <summary>Details</summary>
Motivation: 检验“小红点”（LRDs）是否是“小蓝点”（LBDs，即更蓝的致密宽发射线活动星系核）的高倾角、尘埃红化对应体，通过建模它们的相对数密度和光度来验证这一统一假说。

Method: 以z>4的宽发射线活动星系核（BLAGNs）的紫外光度函数为母分布，在超爱丁顿统一框架下前向模拟吸积率、各向异性辐射、视角和尘埃消光效应，对比预测与JWST观测的LRD光度函数。

Result: 模型成功复现了LRD光度函数；LRD/BLAGN比例从M_1500=-21时的3%上升到M_1500=-19时的峰值20%；在光学波段LRD比例更高（M_4500=-20时达26%）；最佳拟合给出平均每团消光<A_V>=2.8 mag和平均尘埃覆盖因子<C_dust>=0.23。

Conclusion: 结果支持基于视角的小点统一模型，UV选中的LRD宿主有更大质量黑洞并非因为它们属于不同的母体，而是尘埃消光优先从UV样本中剔除了低质量消光系统所致。LRD光度函数可作为宇宙黎明期快速吸积婴儿黑洞的关键人口统计检验。
...
### [4] [The galaxy-halo connection and the dynamical evolution of a giant disc in a massive node of the Cosmic Web at z~3](https://arxiv.org/abs/2605.04144)
*G. Quadri,S. Cantalupo,C. Bacchini,A. Pensabene,A. Lupi,G. Pezzulli,W. Wang,M. Galbiati,T. Lazeyras,N. Ledos,H. Mao,A. Travascio*

Main category: astro-ph.GA

Recommendation: ★★★★☆ (76/100) - 与 Zotero 兴趣画像中的 alma, jwst, dark matter 等主题/作者匹配。
...
### [5] [Near-infrared diagnostic diagrams of the gas ionization sources in nearby galaxies: a JWST NIRSpec view](https://arxiv.org/abs/2605.04925)
...
TL;DR: 本文利用JWST-NIRSpec数据，针对9个局域U/LIRGs星系，提出了两种新的近红外诊断图（[C i]/Pay与H2/PAH 3.3 μm），用于区分AGN、恒星形成和激波激发源，并通过线宽分析（W80）进一步验证了不同激发机制的动力学特征。


<details>
  <summary>Details</summary>
Motivation: 传统光学诊断图难以区分埋藏AGN和 obscured年轻星团，因此需要开发新的近红外诊断工具。

Method: 分析9个z<0.1的U/LIRGs星系核周区域的高分辨JWST-NIRSpec光谱，使用[Fe ii]/PaB vs H2/Bry图及新提出的[C i]/Pay vs H2/PAH 3.3 μm图，结合光致电离模型和线宽（W80）分析。

Result: 提出了两种新的近红外诊断图，可直接关联辐射场硬度与气体激发机制；W80分析显示星暴激发区最低，AGN区中等，激波激发区最高。

Conclusion: H2发射由多种激发机制共同产生，新诊断图能有效区分不同类型活动星系核和恒星形成区。
...
### [6] [Compact, AGN-hosting Dwarf Galaxies with "Little Red Dots"-like SEDs in the Local Universe](https://arxiv.org/abs/2605.04685)
...
TL;DR: 通过对本地矮星系中的活动星系核（ADGs）进行聚类分析，发现约一半的ADGs具有与JWST在红移>5发现的“小红点”（LRDs）相似的“V型”SED和紧凑形态，但两者在有效半径和电离状态上存在显著差异，表明它们可能遵循不同的形成路径。


<details>
  <summary>Details</summary>
Motivation: 寻找本地宇宙中与JWST发现的高红移“小红点”（LRDs）相对应的天体，以评估矮星系中的活动星系核（ADGs）是否可作为早期超大质量黑洞的类比。

Method: 对本地AGN宿主矮星系样本应用K-means聚类分析，基于SED形状和形态学尺寸进行分类，并比较其与高红移LRDs的物理性质。

Result: 将ADGs分为四组，约一半的ADGs显示“V型”SED和紧凑形态，但本地“V型”紧凑ADGs比高红移LRDs更演化，具有更大的有效半径和不同的电离状态。

Conclusion: 本地紧凑ADGs与高红移LRDs可能遵循不同的形成路径，凸显了黑洞-星系共演化在宇宙时间尺度上的复杂性。
...
### [7] [Contribution of interstellar objects to local dark matter density](https://arxiv.org/abs/2605.04801)
...
TL;DR: 本文发现星际彗星（如3I/ATLAS）作为未被计入的重子物质，可能对银河系质量预算有显著贡献，进而影响非重子暗物质密度的估算。研究显示，星际物体在厚盘中的分布可能使局部暗物质晕密度降至0.24 GeV/cm³。


<details>
  <summary>Details</summary>
Motivation: 近期发现的三颗星际彗星表明银河系中存在尚未被计入的重子物质（星际物体，ISO），而这一贡献会影响非重子暗物质晕的质量估算。作者试图在考虑ISO对银河系旋转曲线贡献后，重新估算非重子暗物质的质量密度。

Method: 利用3I/ATLAS的观测数据（核半径、发生率、运动学年龄）构建ISO分布模型：厚盘（0.8 kpc），径向尺度长度≈7 kpc，并结合GAIA DR3旋转速度测量数据进行拟合，从而估算ISO总质量及其对暗物质密度的影响。

Result: ISO对重子质量预算的贡献可达5×10¹⁰ M☉，导致局部暗物质晕密度降至0.24 GeV/cm³。

Conclusion: 尽管该场景需要过高比例的星际物体释放，但鉴于我们对银河系中不可探测重子物质的认知不足，局部暗物质晕密度可能被高估了。
...
### [8] [Matter Clustering in Astrid: Reduced Baryonic Suppression from Realistic Black Hole Dynamics](https://arxiv.org/abs/2605.04176)
...
TL;DR: Astrid-DMO模拟发现，AGN反馈对物质功率谱的抑制远弱于其他模拟，原因在于黑洞重定位方案人为增强了AGN反馈效率；更真实的动力学摩擦模型会削弱这种抑制，使得调和LSS与CMB的ΛCDM宇宙学更加困难。


<details>
  <summary>Details</summary>
Motivation: 探究AGN重子反馈对物质功率谱的抑制效应及其对S8张力和精确宇宙学的影响，特别关注不同模拟间反馈强度差异的物理原因。

Method: 将大体积Astrid流体模拟与其暗物质only对应版本Astrid-DMO进行功率谱比值分析，并辅以小体积受控实验，对比黑洞重定位方案与动力学摩擦模型对反馈效率的影响。

Result: Astrid在z=0时无明显抑制，z=0.2时仅轻度抑制，弱于其他先进模拟；黑洞重定位方案使z=1.5时动能反馈增强约2倍；动力学摩擦模型则减弱抑制效应。

Conclusion: AGN反馈难以单独调和CMB与LSS宇宙学；需考虑非重子新物理（如晚期物质 clustering抑制机制）或能高效排气但不破坏其他星系性质的新机制。
...
### [9] [The MeerKAT Massive Distant Clusters Survey: a search for diffuse radio emission in 30 massive SZ-selected clusters at $z > 1$](https://arxiv.org/abs/2605.04645)
...
TL;DR: 对30个红移z>1的大质量星系团的观测发现，27%的团内存在弥散射电晕，检出率低于中红移但高于理论预测（<10%）。探测到的射电晕与已知的射电功率-质量关系一致，未探测到的则位于该关系下包络。未发现射电遗迹或迷你晕。


<details>
  <summary>Details</summary>
Motivation: 此前对红移z>1的大质量星系团中弥散射电辐射的统计研究几乎空白，缺乏对射电晕演化及射电功率-质量关系在高红移下是否成立的高红移约束。

Method: 利用MeerKAT 1.28 GHz深场观测，对30个均匀选择的z>1的SZ效应选大质量星系团进行弥散射电辐射搜索，并分析射电功率与团块质量M_500c的标度关系。

Result: 8个团（27%）探测到射电晕，21个团（70%）未探测到，1个因数据质量被排除。检出率低于中红移但高于理论模型预测的≲10%。探测到的射电晕符合P-M关系，未探测到的位于下包络。未发现射电遗迹或迷你晕。

Conclusion: MeerKAT能探测最遥远星系团的非热过程，但需要更深的低频巡天来揭示暗弱弥散射电辐射，并检验P-M关系在宇宙时间上的持续性。
...
### [10] [Investigating the Effects of Bars on Star Formation and Nuclear Activity of Galaxies Using DESI Survey Data](https://arxiv.org/abs/2605.04537)
...
TL;DR: 基于DESI DR1和Galaxy Zoo DESI数据，分析了33,201个盘状星系中棒状结构、恒星形成与活动星系核活动之间的关系。发现强棒多存在于红色、大质量星系，而弱棒常见于蓝色、低质量星系。强棒在低质量星系中促进中心恒星形成，但在大质量星系中抑制比恒星形成率。棒状星系中AGN活动更普遍，但棒的结构参数与AGN活动相关性较弱，表明二者关系是间接的。


<details>
  <summary>Details</summary>
Motivation: 探究星系棒状结构如何影响星系演化，特别是其与恒星形成和活动星系核活动之间的复杂关系。棒状结构被认为能够驱动气体向内输运，但其在不同质量星系中的作用差异尚不明确，需要大样本统计研究。

Method: 利用DESI DR1的33,201个盘状星系样本（0.01<z<0.05），结合Galaxy Zoo DESI的形态分类，识别出3,508个强棒和8,335个弱棒星系。通过分析颜色-质量空间分布、恒星形成率、金属丰度和AGN活动比例等，对比不同棒状结构的星系性质差异。

Result: 1) 弱棒星系偏蓝、低质量；强棒星系偏红、大质量且金属丰度更高。2) 强棒在低质量星系中增强中心恒星形成，但在大质量星系中降低比恒星形成率。3) 棒状星系中AGN活动比例更高，强棒星系中强AGN比例最高。4) 棒的结构参数与AGN活动相关性较弱。

Conclusion: 支持棒状结构通过角动量输运驱动气体向内流动的模型，从而促进中心恒星形成并为超大质量黑洞提供燃料。但棒与AGN活动的联系是间接的，受多种因素共同调节。棒在低质量星系中触发恒星形成，在大质量星系中则通过加速气体消耗促进淬灭，体现其双重作用。
