...
### [1] [DREAMS. JWST Spectroscopy of a $z=8.3$ Galaxy with an ALMA Dust Continuum Detection: Early Dust, Very High $T_{\rm dust}$, and a Multi-wavelength [OIII] Ratio Discrepancy](https://arxiv.org/abs/2605.14922)
...
### [2] [Discovery of a Compact Hub-Filament System in G286.21+0.17 with JWST and ALMA: Insights into Protocluster Formation and Competitive Accretion](https://arxiv.org/abs/2605.14693)
...
### [3] [Ionization Structure and Metal Enrichment of the Galactic Center Minispiral Observed with JWST](https://arxiv.org/abs/2605.13995)
...
TL;DR: 利用JWST/MIRI MRS数据对银心Sgr A*周围0.1秒差距内电离气体结构进行高分辨率研究，揭示了Mini旋臂的形态运动学特征及多个致密气体结构，并通过精细谱线测量约束了辐射场、金属丰度和尘埃特性。


<details>
  <summary>Details</summary>
Motivation: Sgr A*作为最近的静止超大质量黑洞，其邻近性为研究黑洞周围燃料供应提供了独特机会，但目前对其中红外电离气体结构的空间和光谱信息了解不足。

Method: 利用JWST/MIRI MRS仪器提供的广泛空间和光谱信息，解析银河系中心0.1秒差距内的中红外电离气体结构，并通过精细结构谱线（覆盖电离能7-55 eV）测量约束入射辐射场、金属丰度（Ne、Ar、S、Ni、Fe）及尘埃消耗/破坏程度。

Result: 揭示出Mini旋臂的Bar和Northern Arm的形态与运动学特征；发现多个致密气体结构（如X7）位于Sgr A*约0.05秒差距内，具有≥600 km/s的蓝移径向速度；Mini旋臂金属丰度约1-2.5倍太阳丰度，具有Wolf-Rayet恒星驱动的电离辐射场，以及显著的Ni和Fe尘埃破坏；能量≥41 eV的通量增加表明致密气体结构受到额外硬电离辐射源影响。

Conclusion: Mini旋臂气体由Wolf-Rayet恒星主导电离，而致密气体结构可能受到局部快速辐射激波（来自恒星风、假想的Sgr A*外流或与周围介质相互作用）产生的额外硬电离辐射源的影响。
...
### [4] [An Updated Characterization of Luminous Lyα emitters at the End of Reionization](https://arxiv.org/abs/2605.14313)
...
TL;DR: 利用JWST/NIRCam中带测光，解析了14个z≈6明亮莱曼α发射体（LAEs）的物理性质：它们本质上是极年轻（<10 Myr）、低质量（M*<10⁸ M⊙）、无尘埃的矮星暴星系，莱曼α逃逸分数中位数>40%，类似于莱曼连续谱泄露者，是再电离末期的高效电离引擎。


<details>
  <summary>Details</summary>
Motivation: 已有研究对高红移明亮LAEs的质量估算存在争议（受强星云发射线与巴尔末断裂的简并影响），需要更精确的测光数据来区分这些效应，从而揭示其真实物理本质。

Method: 结合地面Magellan/M2FS光谱与JWST/NIRCam宽/中带测光（特别是F410M中带），通过SED拟合约束恒星质量、年龄、尘埃等参数，同时分析莱曼α逃逸分数、电离光子效率及空间偏移。

Result: 这些LAEs是低质量（半数<10⁸ M⊙）、超年轻（<10 Myr）、无尘埃的矮星暴；莱曼α逃逸分数中位数>40%，随UV连续谱变蓝而升高；Lyα与恒星成分的空间偏移表明内部尘埃是抑制Lyα辐射转移的主导因素，而非中性氢。

Conclusion: 明亮LAEs的强Lyα发射源于剧烈星暴活动与高逃逸分数共同作用；这些极端矮星系类似莱曼连续谱泄露者，是再电离末期的高效电离引擎。
...
### [5] [A new sample of Little Red Dots at $z<0.45$ in DESI DR1: Broad Balmer lines, low ionization spectrum and no variability](https://arxiv.org/abs/2605.14233)
...
TL;DR: 本文利用DESI巡天数据在低红移（z=0.2-0.45）发现了8个“小红点”（LRDs），其光谱特征与高红移LRDs相似，但数密度比高红移时代低约10,000倍，表明LRD活动在宇宙晚期可能因金属丰度增高而减弱。


<details>
  <summary>Details</summary>
Motivation: JWST在高红移（z≳4）发现大量紧凑型宽线发射体“小红点”（LRDs），可能代表高红移时期超大质量黑洞演化的主要通道，但低红移（z≲2）LRDs一直未被有效探测，限制了对其详细研究。

Method: 基于DESI巡天数据，利用发射线特性（而非传统的光度SED）搜索低红移LRDs，结合时间域巡天的密集光变曲线分析变异性。

Result: 发现8个z=0.2-0.45的LRDs，具有宽Balmer线、陡峭Balmer递减、紧凑形态等特征；多数源在4-17年（静止系）内变异性弱至无（0.0-0.1 mag）；数密度约1.6×10⁻⁹ Mpc⁻³，比高红移时代低约10,000倍。

Conclusion: 低红移缺乏明亮红色LRDs，可能由于宇宙晚期金属丰度更高导致LRD活动的气体供应率降低，表明LRD是主要在高红移存在的特殊黑洞演化阶段。
...
### [6] [Lyman-alpha Radiation Pressure in Dense Star Clusters: Implications for Star Formation and Winds at Cosmic Dawn](https://arxiv.org/abs/2605.13982)
...
TL;DR: 本研究通过模拟发现，在宇宙黎明时期的高密度、低金属丰度星系中，Lyα辐射压力对恒星形成效率影响温和（~10%），但能迅速驱动动态显著的星风，且Lyα在所有低尘埃丰度环境下都主导紫外和红外辐射压力。


<details>
  <summary>Details</summary>
Motivation: JWST观测揭示宇宙黎明时期的星系可能将恒星形成集中在高密度的致密星团中，其低金属丰度和高柱密度环境使Lyα辐射压力可能对星团形成和演化至关重要，但目前对此缺乏定量理解。

Method: 对辐射流体动力学模拟的快照进行后处理，使用COLT蒙特卡洛代码模拟不同尘埃丰度（0-0.1太阳尘埃丰度）下的Lyα辐射压力效应。

Result: 1) Lyα对气体-恒星转换效率影响温和（~10%），尘埃丰度≥0.01太阳值时效率≥60%，无尘环境下仍≥25%；2) 致密丝状结构保持亚爱丁顿状态，而低密度气体体积呈现超爱丁顿状态（部分f_Edd≥10），可在<4Myr内驱动星风；3) Lyα力倍增因子对尘埃高度敏感（无尘环境≤500，0.1太阳尘埃≤3）；4) Lyα在所有≤0.1太阳尘埃丰度环境下主导UV和IR辐射压力3-500倍。

Conclusion: Lyα辐射压力强化了宇宙黎明星系中局部高效、爆发式恒星形成并伴随快速外流的图像。
...
### [7] [Observation of spontaneous N-bearing PAH formation using ion trap: a new formation pathway in the interstellar medium](https://arxiv.org/abs/2605.14424)
...
TL;DR: 本研究通过离子阱技术和电子结构计算，揭示了嘧啶阳离子与乙炔之间多个无势垒反应，生成了此前未报道的内环氮掺杂多环芳烃，这对理解土卫六大气和星际介质中的天体化学具有重要意义。


<details>
  <summary>Details</summary>
Motivation: 氮掺杂多环芳烃（N-PAHs）是星际介质和富氮行星大气中复杂有机分子的关键前体，但其形成路径尚不清楚，预测丰度与观测值之间存在差异，表明存在未知的形成机制。

Method: 采用离子阱技术结合电子结构计算，研究气相嘧啶阳离子（C₄H₄N₂⁺）与乙炔（C₂H₂）之间的反应。

Result: 发现了多个无势垒反应通道，生成了此前未见报道的内环N-PAHs（C₈H₇N₂⁺）。

Conclusion: 涉及双氮取代芳香杂环的反应测量对土卫六大气和星际介质的天体化学具有重要启示。
