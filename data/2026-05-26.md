...
### [1] [A Redshift-based Red Selection of Dusty Star-forming Galaxies](https://arxiv.org/abs/2605.24264)
...
### [2] [Interstellar extinction, polarization efficiency, and grain alignment in the direction towards bright-rimmed clouds and cometary globules](https://arxiv.org/abs/2605.26030)
*Saikhom Pravash*
...
TL;DR: 本文利用背景星光偏振观测，研究了九个明亮边缘云和彗状云的尘埃偏振效率与对齐机制，发现偏振效率随消光增加而下降，且在某些云中随尘埃温度略有升高，支持辐射转矩（RAT）对齐理论。


<details>
  <summary>Details</summary>
Motivation: 星际磁场可通过非球形尘埃颗粒的偏振来探测，但颗粒对齐机制尚不完全清楚。本研究旨在通过光学偏振观测，探究明亮边缘云（BRCs）和彗状云（CGs）外围弥散包层中尘埃颗粒的对齐机制，特别是验证辐射转矩（RAT）理论。

Method: 利用背景恒星的偏振观测数据，结合Gaia EDR3和StarHorse 2星表的距离与消光数据，分析偏振度、偏振位置角及消光随恒星距离的变化，检测偏振尘埃层，并估计各云的偏振效率。

Result: 在某些云中发现了消光和偏振度在特定距离处的离散增强，表明存在偏振尘埃层。偏振效率随消光增加而下降；在某些磁场有序性较高的云中，偏振效率随尘埃温度略有升高，支持RAT对齐；而在其他云中，偏振效率下降可能由磁场方向波动引起。

Conclusion: 研究结果部分支持辐射转矩（RAT）对齐理论，同时指出磁场方向波动也是影响偏振效率的重要因素。
...
### [3] [Exploring biases in derived stellar parameters and the ionizing photon production efficiency](https://arxiv.org/abs/2605.25130)
...
TL;DR: 研究發現，對再電離時代星系的參數推導（尤其是xi ion）高度依賴於所選用的數據類型（光譜或測光）和SED擬合模型，不同方法可導致參數差異超過1個量級，且xi ion的紅移演化趨勢也取決於方法的一致性。


<details>
  <summary>Details</summary>
Motivation: JWST旨在約束再電離時代的時間尺度和方式，但推導星系電離參數（xi ion）時面臨數據和模型帶來的偏差，需要系統評估這些偏差的影響。

Method: 選取z=3的再電離類比星系，使用三種不同技術（光譜與測光數據）和三種SED擬合代碼（每種包含多種星族合成、星際介質和塵埃模型），比較各方法對恆星參數和xi ion的影響。

Result: 中位恆星質量變化超0.6 dex，SFR變化超0.9 dex；單個源xi ion在光譜與測光推導間差異超1.1 dex，不同模型擬合差異超0.5 dex；xi ion的紅移演化趨勢依賴於所選方法的一致性。

Conclusion: 模型、方法和數據選擇對xi ion參數的影響不可否認，即使對同質星系群也是如此；一致的方法才能得到可靠的紅移演化趨勢。
...
### [4] [Dark matter-deficient twins: FCC 224 and FCC 240 as possible analogues of NGC 1052-DF2 and DF4](https://arxiv.org/abs/2605.24099)
...
TL;DR: 本研究报告了在Fornax星系团外围发现的一对超弥散星系FCC 224和FCC 240，它们与NGC 1052群中DF2和DF4的特征高度相似：缺乏暗物质、拥有异常明亮的球状星团、恒星年龄约100亿年，且内部动力学可由恒星自身解释。尽管它们之间的间距更小、相对速度更低，但仍支持“子弹矮星系”碰撞模型对其形成机制的解释。


<details>
  <summary>Details</summary>
Motivation: 为验证“子弹矮星系”（bullet-dwarf）模型——即高速碰撞的矮星系可产生缺乏暗物质且拥有超亮球状星团的恒星系统——寻找类似的天体系统，并加深对此类特殊星系形成与演化机制的理解。

Method: 利用甚大望远镜（VLT）上的MUSE积分场光谱仪对FCC 224和FCC 240进行深度观测，表征其恒星族群、内部运动学以及球状星团系统，并与NGC 1052群中的DF2和DF4进行对比分析。

Result: 两个星系均具有低速弥散，内部动力学更倾向于由恒星自身解释而非暗物质晕；均拥有异常明亮的球状星团，其光度函数与NGC 1052对高度一致；两个星系共时形成，年龄约100亿年；但FCC 224与FCC 240的投影间距仅75 kpc，相对速度仅16 km/s，远小于DF2-DF4系统。

Conclusion: FCC 224和FCC 240可能是与DF2/DF4同类的星系，但其更紧密的构型暗示了同一形成模型的不同现代表现，为“子弹矮星系”形成模型的进一步观测约束提供了重要参考。
...
### [5] [The AGORA High-resolution Galaxy Simulations Comparison Project. XI: Solving the Non-Spherical Morphology and Evolution of Dark Matter Halos with Haskap Pie](https://arxiv.org/abs/2605.24097)
...
TL;DR: 本文提出了一种将暗物质晕视为非球状物体的新算法，利用Haskap Pie的束缚粒子搜索技术，在AGORA Collaboration的流体动力学模拟中揭示了形态学趋势，发现高质量比并合对形态测量影响显著，且自旋和轴比在4>z>2达到峰值后下降。


<details>
  <summary>Details</summary>
Motivation: 现有暗物质晕研究方法大多假设球对称性，而实际暗物质晕本质上是非球状的，需要一种不依赖球对称假设的算法来更准确地刻画晕的形态和动力学特征。

Method: 采用基于Haskap Pie的束缚粒子搜索技术，将暗物质晕作为非球状物体进行求解和追踪，并在AGORA Collaboration的CosmoRun流体动力学模拟中比较不同代码的结果。

Result: 高质量比并合对形态测量响应显著；不同代码间的最大差异源于时间偏差和并合前的动力学状态；晕自旋和长短轴比在4>z>2达到峰值后下降；低红移低质量晕的超密度呈现质量和红移依赖性。

Conclusion: 该方法提供了非球对称视角下的新洞察，揭示了其他工作因依赖球对称假设而未能充分复现的趋势。
...
### [6] [Changing-look Active Galactic Nuclei from SDSS, LAMOST and DESI Survey](https://arxiv.org/abs/2605.24429)
...
TL;DR: 通过交叉匹配SDSS、LAMOST和DESI光谱数据，新发现40个光学变脸AGN和12个重复变脸AGN，支持变脸行为与吸积率波动或盘不稳定性等重复物理过程相关。


<details>
  <summary>Details</summary>
Motivation: 目前已有超1000个光学变脸AGN被报道，但其物理起源仍不明确，且重复变脸AGN十分稀少。扩大变脸AGN样本、特别是重复变脸AGN样本，对于约束其内在机制至关重要。

Method: 交叉匹配SDSS与LAMOST的光谱观测，并利用DESI光谱作为第三历元，系统地搜寻变脸AGN及重复变脸行为。

Result: 识别出45个变脸AGN（其中40个为新源），以turn-off事件为主（43个turn-off vs 2个turn-on）；发现12个重复变脸AGN，检测率较高；重复变脸AGN在log MBH - log(Lbol/LEdd)平面上呈现出高-低-高吸积态演化；首次转变时间尺度上限约10年，第二次约4年。

Conclusion: 变脸AGN的转变可能源于重复的物理过程（如吸积率波动或盘不稳定性），重复变脸AGN的吸积态演化进一步支持变脸行为与吸积功率的重复变化密切相关。
...
### [7] [The 3D Structure and Kinematics of the Local Disk-Halo Interface: Intermediate-velocity Clouds are the Minority of High-altitude Clouds in the Solar Neighborhood](https://arxiv.org/abs/2605.24342)
...
TL;DR: 本研究利用3D尘埃图在太阳邻域1.25 kpc范围内识别出1,695个尘埃云，并通过HI 21 cm发射线测量其运动学，构建了519个高置信度云样本。首次在3D中识别出中间速度云（IVCs），发现北银冠云数量是南银冠的2.9倍，且IVCs仅占高海拔云的18%，表明低速结构在盘-晕界面研究中至关重要。


<details>
  <summary>Details</summary>
Motivation: 历史上对银河系盘-晕界面的研究基于径向速度识别出与盘旋转不符的入流和出流气体（IVCs和HVCs），但缺乏三维空间上的系统性搜索。本研究旨在利用3D尘埃图技术，首次在局部盘-晕界面进行三维空间搜索，以更全面地识别和分析异常速度云。

Method: 对太阳1.25 kpc范围内的3D尘埃图应用拓扑结构发现方法，识别出1,695个尘埃云；通过评估这些云与HI 21 cm发射线的形态相似性来测量云的运动学，最终构建519个具有高置信度距离、3D形态和径向速度的云样本。

Result: 首次在3D中识别出多个IVCs（嵌入著名的Intermediate Velocity Arch复合体中），并直接测量其距离、大小、密度、质量、压力和尘气比。发现北银冠云数量是南银冠的2.9倍（|z|≥480 pc），且IVCs仅占高海拔云的18%。

Conclusion: 低速结构在局部盘-晕界面中占据主导地位（高海拔云中82%为低速云），在评估盘-晕界面和建模反馈驱动的喷泉流时必须充分考虑低速结构的重要性。
...
### [8] [Beyond the [OIII]$λ$4363 auroral line: [NeIII]$λ$3868 as a direct tracer of electron temperature](https://arxiv.org/abs/2605.25691)
...
TL;DR: 本文提出了一种利用[NeIII]λ3868和[OIII]λ4959,5007亮线比值(O3Ne3)来估算电子温度的新方法，解决了在低温(高金属丰度)环境下[OIII]λ4363极弱难以探测的问题，适用于6000-13500K的温度范围。


<details>
  <summary>Details</summary>
Motivation: 传统的极光线下无法探测低温(高金属丰度)环境中的电子温度，因为[OIII]λ4363在这些条件下极其微弱，即使深场观测也难以获取。

Method: 通过详细的光电离模型，建立O3Ne3比值（[OIII]λ4959,5007/[NeIII]λ3868）与O++主导区域电子温度之间的紧密关系，并结合O3O2比值（电离度示踪）进行验证。

Result: 在覆盖z~0-9的大样本星系和河外HII区中，基于O3Ne3比值得到的电子温度估计与基于[OIII]λ4363的传统方法在不确定度内一致。

Conclusion: 该关系可用于低温(高金属)区间[6000, 13500K]的电子温度估算，有效弥补了传统方法在该温度范围失效的不足。
...
### [9] [The Gas-Phase Mass-Metallicity Relation of Dwarf Galaxies Across Large-Scale Environments Using the CAVITY Parent Sample](https://arxiv.org/abs/2605.25557)
...
TL;DR: 本文利用CAVITY巡天样本，系统研究了矮星系在不同宇宙环境（空洞、纤维状结构、星系团）中的气体相位金属丰度-恒星质量关系（MZR）。研究发现MZR斜率随环境变化：空洞中较陡（0.28），星系团中较平缓（0.17），且纤维状结构中孤立与非孤立矮星表现不同。结果表明环境效应是造成矮星系MZR散度的重要因素。


<details>
  <summary>Details</summary>
Motivation: 矮星系的MZR存在明显的斜率断裂和大散度，暗示其化学增丰的物理过程与大质量星系不同。由于矮星系引力势阱浅，易受内部和外部环境影响，但单一MZR是否能描述不同大尺度环境中的矮星系尚不明确。作者旨在探究环境对矮星系化学增丰的影响，解释MZR散度的来源。

Method: 使用CAVITY巡天父样本，选取恒星质量8.9 < log(M_star/M_sun) < 9.5的矮星系。利用SDSS光学光谱，通过pyPipe3D管线测量发射线流量，推导MZR和SFR。样本包括353个空洞矮星系、311个纤维状结构矮星系和22个星系团矮星系。按局域环境进一步分为孤立和非孤立子样本进行分析。

Result: MZR斜率存在系统性变化：空洞中最陡（0.28±0.03），纤维状结构中居中，星系团中最平坦（0.17±0.08）。空洞中孤立与非孤立矮星无显著差异。纤维状结构中，孤立矮星性质与空洞矮星类似，而非孤立矮星的MZR斜率更平坦，与星系团矮星相似。

Conclusion: 矮星系的MZR斜率存在大尺度和局域尺度的环境依赖性。支持"预加工"框架：在宇宙纤维状结构内，局域环境已开始影响低质量星系的化学增丰和恒星形成，环境效应是矮星系MZR散度的重要来源。
...
TL;DR: 利用IllustrisTNG模拟，发现致密矮星系（CDs）的前身星在进入星系团前会经历一个短暂的超扩散星系（UDG）阶段，通过星暴过程在剥离过程中新生恒星核，而非经典潮汐剥离理论所认为的暴露已有恒星核。


<details>
  <summary>Details</summary>
Motivation: 观测发现星系团中UDGs和CDs存在空间反相关，暗示两者可能存在演化联系，经典假说认为CDs是UDGs经潮汐剥离后暴露的恒星核残余。本研究旨在利用数值模拟检验这一假说。

Method: 使用IllustrisTNG（TNG100）宇宙学模拟，分析7个星系团中117个CDs的演化历史，追踪其前身星是否经历UDG阶段，并研究其结构、气体含量、恒星形成率等物理性质的变化。

Result: 模拟重现了观测到的空间反相关；发现8个CDs的前身星在进入星系团前经历短暂UDG阶段（气体分数>0.8）；UDG前身星无显著恒星核；CDs的致密核是在剥离过程中通过星暴新生而成；峰值恒星形成率是星系全生命周期的最高值。

Conclusion: 发现一种由星系团环境驱动的UDG到CD的转变路径，其机制与经典潮汐剥离假说根本不同：致密核是新生的而非暴露的已有核，气体丰度是该转变通道的关键前提条件。
...
### [11] [The Sensitivity of Substructure Lensing to SIDM Core-collapse Model Variation](https://arxiv.org/abs/2605.24174)
...
TL;DR: 本研究系统测试了自相互作用暗物质（SIDM）模型中核心坍缩子晕的不同建模方法对引力透镜效应的影响，发现不同建模方式在小尺度上会对有效偏转场的两点相关函数产生显著差异。


<details>
  <summary>Details</summary>
Motivation: SIDM产生的致密晕在透镜观测中会留下独特印记，但子晕核心坍缩的时间线难以精确建模，这阻碍了准确的子结构透镜预测。

Method: 测试了多种核心坍缩晕建模方法（平滑演化vs瞬时坍缩、概率坍缩vs个体演化、初始/最终密度分布参数变化），并评估每种变化对有效偏转场散度和旋度的两点相关函数的影响。

Result: 两点相关函数在小尺度上对大多数建模变化敏感，但这些差异的可探测性取决于具体的观测手段。

Conclusion: 核心坍缩子晕的建模方式对透镜预测有显著影响，未来透镜分析需谨慎考虑建模不确定性。
...
### [12] [Early- and Late-Time Modifications to $Λ$CDM: Implications for the Hubble Tension](https://arxiv.org/abs/2605.26050)
...
TL;DR: 本文提出一个包含冷暗物质衰变为暗辐射（早期）和非标准暗能量状态方程（晚期）的扩展ΛCDM模型。结合Planck 2018+ACT DR6+DESI DR2+CMB透镜数据，该模型将哈勃常数提升至H₀=69.83±0.98 km/s/Mpc，将H₀ tension降至约2.2σ（与SH0ES）和约2.9σ（与H0DN）。加入SH0ES和Pantheon+后，H₀=70.20±0.66 km/s/Mpc。贝叶斯证据表明模型拟合质量与ΛCDM相当，且早期和晚期联合修正比单独修正更有效缓解H₀ tension。


<details>
  <summary>Details</summary>
Motivation: ΛCDM模型存在日益显著的H₀ tension（局域测量与CMB推算的不一致），亟需新物理来解释。现有方案多从早期（如暗物质衰变）或晚期（如暗能量演化）单独修改，但单一修正效果有限。本文探索早期暗物质衰变与晚期暗能量演化联合效应的可行性。

Method: 在ΛCDM基础上引入两个扩展：1）冷暗物质在辐射-物质相等时期附近衰变为不可见暗辐射；2）暗能量具有非标准状态方程w₀。联合分析Planck 2018、ACT DR6、DESI DR2、CMB透镜等多组宇宙学观测数据，运用贝叶斯推断进行参数估计和模型比较。

Result: 联合数据集下H₀=69.83±0.98 km/s/Mpc，与SH0ES的tension降至~2.2σ，与H0DN降至~2.9σ。加入SH0ES和Pantheon+后H₀=70.20±0.66 km/s/Mpc。贝叶斯证据显示模型拟合质量与ΛCDM统计相当。

Conclusion: 早期暗物质衰变（缩小声学视界）和晚期暗能量演化（改变晚期膨胀率）的协同效应比单一修正更能有效缓解H₀ tension。该模型在统计上不劣于ΛCDM，为缓解哈勃常数危机提供了一个可行的新物理框架。
...
### [13] [Constraining the Potential Index $n$ of the Early Dark Energy Model Using Cosmic Birefringence from Planck and ACT](https://arxiv.org/abs/2605.24341)
*Kedi Zhang,Lu Yin*
...
TL;DR: 本文系统分析了轴子类早期暗能量（EDE）模型中势能指数n=2、3、∞对宇宙双折射和哈勃张力的影响，发现n=3模型在Planck和ACT数据下拟合最优，耦合参数一致且能同时解释两个宇宙学异常。


<details>
  <summary>Details</summary>
Motivation: 标准ΛCDM模型面临宇宙双折射和哈勃张力两大挑战，早期暗能量（EDE）模型有望统一解释这两种异常，但此前研究主要集中在n=3情形，势能指数n对双折射信号的影响尚未被充分探索。

Method: 对n=2、3、∞三种轴子类EDE模型进行统计分析，使用Planck-EB和ACT-EB的EB交叉极化数据，比较各模型的耦合参数gM_pl、最小χ²值及拟合优度。

Result: n=2模型拟合极差，耦合值极端（Planck: 69.912, ACT: -40.726），χ²_min很大（144.52和86.93）；n=3模型拟合最优，χ²_min分别为65.70和48.08，耦合参数一致且稳定（gM_pl ≈ -0.210和-0.158）；n=∞模型表现居中。

Conclusion: n=3是同时解决哈勃张力和宇宙双折射问题的最优理论选择，具有理论自洽性和观测一致性。
...
### [14] [Unifying Early and Late Dark Energy: Dynamical Requirements and Obstructions](https://arxiv.org/abs/2605.26116)
...
TL;DR: 本文研究单一标量场能否同时解释早期暗能量（缓解哈勃张力）和晚期暗能量（宇宙加速膨胀），发现需要具有“陡-更陡-缓”三级坡度的势能函数，且该结论在存在共形耦合时依然成立，对统一模型构建施加了严格约束。


<details>
  <summary>Details</summary>
Motivation: 探究单一标量场是否能够同时解释早期暗能量（缓解哈勃张力）和晚期宇宙加速膨胀，尝试构建统一的暗能量模型。

Method: 采用自下而上方法，先识别统一场景必须经历的动力学阶段序列及对能量密度和状态方程的要求；再采用自上而下方法，将这些要求转化为对具有追踪动力学特征的标量场相空间结构的约束。

Result: 满足所有要求需要势能函数具有三种不同斜率，形成“陡-更陡-缓”的层级结构；该结论在引入与暗物质的共形耦合后仍保持不变。

Conclusion: 这些结果对构建早期与晚期暗能量统一模型施加了严格约束，并讨论了模型构建的启示。
