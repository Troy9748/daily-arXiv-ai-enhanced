...
### [1] [From THESAN-ZOOM to JWST: Predicting ionizing photon escape and the rise of UV-bright reionization sources](https://arxiv.org/abs/2606.14671)
...
Recommendation: ★★★★★ (100/100) - 与 Zotero 兴趣画像中的 jwst, sources, galaxies, sfr, field 等主题/作者匹配。

TL;DR: 本文利用THESAN-ZOOM宇宙学辐射-流体动力学模拟中的35,000多个星系样本，训练随机森林回归模型来预测莱曼连续谱（LyC）光子的逃逸分数（f_esc）和逃逸率（ṅ_ion,esc）。研究发现SFR比值和气体-恒星质量比是f_esc的最强诊断指标，而紫外绝对星等（M_UV）主导ṅ_ion,esc的预测。结合观测的紫外光度函数，重建的再电离历史与观测约束一致，表明再电离主要在z≈8之后由明亮紫外星系快速完成。


<details>
  <summary>Details</summary>
Motivation: 理解宇宙再电离的来源和演化是天体物理学的核心挑战，而早期星系中电离LyC光子的逃逸是一个主要的不确定因素。现有观测难以直接测量LyC光子逃逸，因此需要寻找可靠的间接诊断指标来预测逃逸分数和逃逸率。

Method: 使用THESAN-ZOOM宇宙学辐射-流体动力学模拟中的超过35,000个星系样本，训练随机森林回归模型。构建了四个模型：两个使用全部模拟诊断指标分别预测f_esc和ṅ_ion,esc，两个限制为仅使用JWST测光巡天可观测的指标。

Result: 1) SFR_10/SFR_100比值和气体-恒星质量比是f_esc的最强诊断指标；2) M_UV主导ṅ_ion,esc的预测；3) 利用M_UV的强预测能力，结合观测的紫外光度函数和ṅ_ion,esc-M_UV关系重建的再电离历史与观测约束一致，避免了近期报告的离子化光子预算危机；4) 再电离的主体发生在z≈8之后，由紫外明亮星系（M_UV<-17）主导。

Conclusion: 研究成功建立了预测LyC光子逃逸的机器学习模型，揭示了爆发性恒星形成驱动的气体清除与电离光子逃逸之间的强关联。再电离主要由z≈8之后的明亮紫外星系快速完成，这解决了理论预测与观测之间可能存在的光子预算矛盾。
...
### [2] [A new CIGALE module for modeling AGN emission lines](https://arxiv.org/abs/2606.14643)
*Hao Zhang,Patrice Theule,Veronique Buat,Denis Burgarella,Estelle Pons,Mederic Boquien*

Main category: astro-ph.GA

Recommendation: ★★★★★ (100/100) - 与 Zotero 兴趣画像中的 emission, model, line, new, galaxies 等主题/作者匹配。

TL;DR: 本文开发了CIGALE的新模块[nebular_AGN]，用于模拟和拟合高红移AGN的宽线区和窄线区发射线，并估计其物理性质。


<details>
  <summary>Details</summary>
Motivation: 高红移AGN的不断发现对光谱分析工具提出了更高要求，现有工具在发射线诊断方面存在不足。

Method: 使用Cloudy构建数据库，基于CIGALE中的X射线和吸积盘连续谱生成辐射场，结合AGN几何与尘埃设置定义气体云物理参数。

Result: 模块能近似重现类星体发射线轮廓，覆盖关键发射线比率，金属丰度与经验公式一致，证实尘埃消光律在SED拟合中重要。

Conclusion: [nebular_AGN]模块是一个高效灵活的AGN发射线模拟与拟合工具，可用于估算AGN物理性质。
...
### [3] [GATOS N: Extended circumnuclear dust emission in nearby Seyfert galaxies surveyed by JWST/MIRI](https://arxiv.org/abs/2606.14477)
...
Recommendation: ★★★★★ (100/100) - 与 Zotero 兴趣画像中的 galaxies, jwst, dust, emission, survey 等主题/作者匹配。

TL;DR: 本文介绍了JWST MIRIM对塞弗特星系的巡天观测，旨在研究AGN加热的尘埃在中心几百秒差距内的分布，并寻找尘埃核外流的特征。研究发现，AGN加热的尘埃广泛存在于塞弗特星系中，延伸至核外数百秒差距，且与窄线区电离气体存在强空间相关性。


<details>
  <summary>Details</summary>
Motivation: 利用JWST MIRIM的高角分辨率和深灵敏度，研究邻近宇宙中活动星系核（AGN）周围亚角秒尺度上的尘埃结构和热性质，特别是AGN加热的尘埃在核周几百秒差距内的分布，以及寻找尘埃核外流的证据。

Method: 使用JWST MIRIM在5-25 μm波段对塞弗特星系进行Cycle 1巡天观测，采用特定的数据归约和分析技术来分离核区和延展发射，并将观测到的中红外结构与恒星星族、电离气体、分子气体及吸收尘埃等其他相进行对比。

Result: 在塞弗特星系中普遍存在可分辨的AGN加热尘埃，延伸至核外数百秒差距，且通常比更广泛的星暴加热的环核尘埃盘具有更高的面亮度。在扣除发射线污染后，中红外尘埃发射与窄线区（NLR）的AGN电离气体之间存在强空间相关性。

Conclusion: AGN加热的尘埃在塞弗特星系中广泛存在，且与NLR电离气体密切相关，这为理解AGN反馈机制和尘埃在星系核区的分布提供了重要观测依据。
...
### [4] [Lensed hot stars with HST in the 2030s](https://arxiv.org/abs/2606.14392)
*J. M. Diego*

Main category: astro-ph.GA

Recommendation: ★★★★★ (100/100) - 与 Zotero 兴趣画像中的 lens, star, stars, lensed, evolution 等主题/作者匹配。

TL;DR: 本文论证了哈勃太空望远镜（HST）在紫外和光学波段仍具有不可替代的科学价值，特别是在研究红移z>0.5的透镜热星方面，其优势将持续到2040年代宜居世界天文台发射之前。


<details>
  <summary>Details</summary>
Motivation: 响应"为2030年代哈勃科学制定路线图"的号召，强调在罗曼望远镜即将发射、韦伯望远镜主导红外的背景下，HST在紫外和光学波段的独特能力仍值得长期投入。

Method: 通过对比HST、韦伯（JWST）、罗曼（Roman）三者在紫外、光学、红外波段的性能差异（空间分辨率、像素尺寸、波段覆盖），论证HST在紫外/光学波段的不可替代性，并聚焦透镜热星这一典型案例。

Result: HST在光学波段对点源的分辨率优于罗曼；罗曼虽灵敏度更高但像素尺寸约为HST的3倍，导致点扩散函数欠采样；HST是唯一能进行高质量紫外成像的望远镜。

Conclusion: HST在紫外和光学波段的独特能力使其成为特定科学目标（如z>0.5透镜热星研究）的最佳平台，应继续支持其运行至2040年代。
...
### [5] [Uncovering DM distribution by combining stellar kinematics and integrated HI spectra: method validation](https://arxiv.org/abs/2606.14271)
*Yu Lei,Meng Yang,Ling Zhu*

Main category: astro-ph.GA

Recommendation: ★★★★★ (100/100) - 与 Zotero 兴趣画像中的 stellar, distribution, over, galaxies, galaxy 等主题/作者匹配。

TL;DR: 本文提出一种联合建模恒星运动学（IFU数据）和HI气体运动学（单条积分谱线）的方法，用于确定星系中的暗物质分布。通过在TNG50模拟中生成的58个星系（116组模拟观测）验证，模型在2-20 kpc范围内恢复暗物质质量的系统偏差小于20%，且加入HI数据后外围区域（20 kpc）暗物质质量的不确定性从约85%降至约30%。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅利用IFU恒星运动学数据约束暗物质分布时，外围区域约束能力不足，不确定性较大。引入HI气体运动学信息可增强对外围暗物质分布的约束，从而更准确地揭示星系暗物质分布。

Method: 采用三轴轨道叠加Schwarzschild模型描述恒星运动学，并结合理想化盘模型描述HI气体动力学，两者共享同一引力势。引力势由恒星质量、广义NFW暗物质晕和中心黑洞组成。在TNG50模拟生成的58个星系上，分别构建侧向和端向两种视角的模拟观测数据（共116组）进行验证。

Result: 模型在2-20 kpc范围内恢复总质量、恒星质量和暗物质质量的中位系统偏差小于20%。加入HI数据后，5 kpc内暗物质质量统计不确定性与仅用IFU数据相当；但20 kpc处相对不确定性从约85%降至约30%。gNFW模型定义的暗物质密度斜率被系统性低估，而引入2-20 kpc间的密度斜率则能被统计上较好地恢复。

Conclusion: 该方法能可靠揭示暗物质分布，尤其在外围区域的约束能力显著提升，有望应用于大量观测星系样本的暗物质研究。
...
### [6] [Identifying Warped Galaxies in Pan-STARRS and Euclid using Deep Convolutional Neural Network](https://arxiv.org/abs/2606.14152)
*Saranya Suguna,Arunima Banerjee*

Main category: astro-ph.GA

Recommendation: ★★★★★ (100/100) - 与 Zotero 兴趣画像中的 galaxies, star, two, galaxy, formation 等主题/作者匹配。

TL;DR: 本文利用深度学习（Zoobot convNext-nano模型）在Pan-STARRS数据上以83%准确率分类翘曲与非翘曲盘状星系，识别出2088个翘曲星系，并通过Euclid数据验证了模型泛化能力。翘曲星系通常具有更低轴比、更高不对称性、更蓝的颜色和更年轻的恒星族。


<details>
  <summary>Details</summary>
Motivation: 翘曲星系盘常见但难以检测，传统方法受限于星系外围的暗淡信号。随着深空巡天数据量增长，需要高效自动化的方法识别这些细微形态特征。

Method: 基于Pan-STARRS EGIPS目录中1000张侧向星系FITS图像，微调Zoobot convNext-nano模型进行翘曲/非翘曲二分类；使用LayerCAM进行可解释性分析；将模型迁移至Euclid Q1数据进行泛化验证。

Result: 模型分类准确率83%；在高置信度阈值（≥0.85）下识别出2088个翘曲星系和1398个非翘曲星系；在Euclid数据上成功泛化。翘曲星系结构上轴比更低、不对称性更高，颜色更蓝、恒星族更年轻、恒星形成活动更强。

Conclusion: 深度学习能有效识别星系盘中翘曲等细微形态特征，为当前及未来大规模成像巡天中研究星系结构性质提供了有力工具。
...
### [7] [The S-PLUS Fornax Project (S+FP): Fornax-like clusters in cosmological hydrodynamical simulations](https://arxiv.org/abs/2606.14109)
...
Recommendation: ★★★★★ (100/100) - 与 Zotero 兴趣画像中的 simulations, dynamical, galaxies, alma, galaxy 等主题/作者匹配。

TL;DR: 本文利用EAGLE和IllustrisTNG模拟生成Fornax星系团的合成测光目录，并与S-PLUS观测数据对比，验证了模拟在重现星系颜色-星等关系、空间分布等特征方面的可靠性。


<details>
  <summary>Details</summary>
Motivation: 利用模拟解释Fornax星系团中星系的演化过程，并为S-PLUS观测数据提供可对比的理论基准。

Method: 从EAGLE和IllustrisTNG模拟中选取Fornax类似体，使用SKIRT辐射传输代码生成12个S-PLUS波段的合成测光数据，并与观测数据进行对比分析。

Result: 合成测光与S-PLUS观测吻合良好；找到了与Fornax空间分布相似的模拟候选系统；模拟再现了颜色-星等关系和空间子结构；模拟星系在团外围更蓝。

Conclusion: 所选模拟标准与合成测光方法有效，当前宇宙学模拟可可靠地再现Fornax星系团的关键特征。
...
### [8] [GA-NIFS: The interplay between feedback and star formation at 3 < z < 9 probed by JWST/NIRSpec IFU](https://arxiv.org/abs/2606.13819)
...
Recommendation: ★★★★★ (100/100) - 与 Zotero 兴趣画像中的 jwst, formation, star, star formation, feedback 等主题/作者匹配。

TL;DR: 基于JWST/NIRSpec IFU对6个高红移星系（z=3-9）的空间分辨研究，发现大质量星系中AGN驱动的离子外流与过去100-300 Myr内的核区淬火事件相关，而高红移星暴星系中驱动的外流则伴随连续增长，表明AGN驱动外流在早期宇宙中已对星系演化起关键作用。


<details>
  <summary>Details</summary>
Motivation: 理解星暴和活动星系核（AGN）驱动的反馈如何影响星系增长、恒星形成淬灭以及星系与中心黑洞的共同演化，尤其是在早期宇宙epoch中这些过程的机制尚不清晰。

Method: 使用JWST/NIRSpec IFU对6个z=3-9的星系（包含星暴主导和AGN主导系统）进行低分辨率（R~100）和高分辨率（R~2700）的空间分辨观测，结合之前R~2700数据中识别的离子外流，进行空间分辨的星族分析与外流性质的关联分析。

Result: 大质量星系（M*>5×10¹⁰ M☉）中强AGN驱动离子外流对应核区（r<3 kpc）过去100-300 Myr的淬火证据；高红移（z>5）低质量（M*<10¹⁰ M☉）星暴星系中外流驱动但连续增长无淬火；弱外流的大质量星系无淬火历史；AGN宿主GS20936显示10-30 Myr前的再活化阶段，可能与并合有关。

Conclusion: AGN驱动的外流在早期宇宙中已经在大质量星系的恒星形成历史演化中发挥关键作用，能够有效触发淬火事件。
...
### [9] [A novel data-driven approach to extract stellar population properties from galaxy spectra using absorption indices](https://arxiv.org/abs/2606.13791)
*Zahra Sharbaf,Ignacio Ferreras,Anna R. Gallazzi,Stefano Zibetti,Daniele Mattolini,Laura Scholz-Díaz*

Main category: astro-ph.GA

Recommendation: ★★★★★ (100/100) - 与 Zotero 兴趣画像中的 galaxy, stellar, data, population, properties 等主题/作者匹配。

TL;DR: 本文使用PCA方法分析恒星种群模型，通过六个光谱指数构建潜在空间，成功打破年龄-金属度简并，并利用巴尔默指数差异识别近期爆发行为。


<details>
  <summary>Details</summary>
Motivation: 在复杂机器学习方法难以解释的背景下，PCA作为一种简单可解释的工具，被用于分析恒星种群模型，以理解星系光谱数据的统计特征。

Method: 对六个主要光谱指数进行PCA分析，构建三维潜在空间，并比较SDSS和LEGA-C星系光谱在该空间中的分布。

Result: 前三主成分成功打破年龄-金属度简并；最后一个特征向量揭示了Hγ_A和Hδ_A之间的竞争关系，可用于识别近期爆发行为。

Conclusion: 基于纯数据的PCA方法与传统SPS模型拟合技术结果一致，为跨巡天的恒星种群研究提供了可靠基础。
...
### [10] [Reconstructing the orbits of Milky Way dwarf galaxies: An LMC perspective](https://arxiv.org/abs/2606.13787)
...
Recommendation: ★★★★★ (100/100) - 与 Zotero 兴趣画像中的 galaxies, milky, way, galaxy, formation 等主题/作者匹配。

TL;DR: 本研究基于精确的六维相空间坐标和六种考虑银河系与大麦哲伦云相互作用的时变引力势模型，对72个银河系矮卫星的轨道进行了积分，提供了迄今最大的轨道参数目录，并识别了10个可能的大麦哲伦云卫星，以及Grus II和Tucana IV可能近期被大麦哲伦云捕获的银河系卫星。


<details>
  <summary>Details</summary>
Motivation: 理解银河系矮卫星的轨道历史对于揭示其演化过程以及将当前性质置于动力学背景中至关重要。

Method: 基于文献中精确的六维相空间坐标，利用六种考虑银河系与大麦哲伦云相互作用的时变引力势模型，对72个矮星系进行轨道积分，分析轨道参数、束缚状态、下落时间等。

Result: 提供了迄今最大的银河系矮星系轨道参数目录；识别出10个可能的大麦哲伦云卫星；发现小麦哲伦云最近一次绕大麦哲伦云的近心点（~8 kpc，~1.7亿年前）与直接碰撞模型预测一致；发现小麦哲伦云过往近心点与两个麦哲伦云的恒星形成率峰值存在时间关联；识别出Grus II和Tucana IV可能为近期被大麦哲伦云捕获的银河系卫星。

Conclusion: 多数矮卫星在过去50亿年内位于银河系维里半径内；大麦哲伦云对其卫星系统有显著动力学影响；小麦哲伦云的轨道演化与其恒星形成历史存在因果联系；银河系与麦哲伦云之间的卫星交换过程正在发生。
...
### [11] [The host halo masses of AGNs and quasars at $z \sim 3-7$ with TNG-Cluster, FLAMINGO and other cosmological galaxy simulations](https://arxiv.org/abs/2606.13784)
...
Recommendation: ★★★★★ (100/100) - 与 Zotero 兴趣画像中的 galaxy, mass, simulations, halo, formation 等主题/作者匹配。

TL;DR: 通过多个宇宙学流体模拟（TNG300、TNG-Cluster、FLAMINGO等）研究发现，AGN的辐射光度与暗物质晕质量之间存在高度非线性的关系，且在晕质量超过约10¹² M☉时出现变平或反转。类星体通常不居住在最大质量的暗物质晕中，这一结果与观测估计基本一致。


<details>
  <summary>Details</summary>
Motivation: 类星体是否居住在一个较窄的暗物质晕质量范围内（约10¹²⁻¹³ M☉）？随着大尺度流体模拟的发展，现在可以直接用自洽的星系形成模型来量化AGN光度与晕质量之间的关系，以检验这一传统认知。

Method: 利用TNG300、TNG-Cluster、FLAMINGO L1_m8、L2p8_m9等大尺度模拟，以及Illustris、EAGLE、TNG100、Simba等小尺度模拟，分析和统计AGN的辐射光度与暗物质晕质量之间的中位数关系和弥散分布。

Result: AGN光度与晕质量的关系高度非线性：在晕质量约10¹² M☉以下，更大晕质量对应更亮AGN；但超过该质量后，光度趋势变平（FLAMINGO）或反转（TNG300+TNG-Cluster）。类星体的典型晕质量约为10¹²⁻¹².⁵ M☉（z=3-6）。在固定晕质量下AGN光度弥散可达3个数量级，而固定光度下晕质量弥散仅约1个数量级。

Conclusion: 高晕质量处存在AGN淬灭现象，导致最明亮的AGN并不居住在最大质量的暗物质晕中。暗晕增长与瞬时黑洞吸积之间的耦合较弱。模拟结果与观测估计的类星体晕质量基本一致。
...
Recommendation: ★★★★★ (100/100) - 与 Zotero 兴趣画像中的 galaxy, mass, jwst, data, galaxies 等主题/作者匹配。

TL;DR: 本文利用JWST NIRCam数据对约4,100个恒星形成星系（0.6<z≤4）进行形态学研究，测量了尺寸-质量-波长关系，发现临界质量~10^9.5 M⊙作为星系形态由弥散向致密转变的分界点，并发布了包含约41,000个星系的形态测量数据。


<details>
  <summary>Details</summary>
Motivation: 利用JWST的高分辨率和多波段覆盖能力，系统研究星系尺寸随恒星质量、红移和静止框架波长的变化，建立星系尺寸-波长关系的新参数化方法。

Method: 使用GALFIT对CANUCS和Technicolor巡天中19个NIRCam中宽带和宽波段图像进行形态参数测量，分析约4,100个恒星形成星系（0.6<z≤4，log(M*/M⊙)介于8.5-11.5）的尺寸-质量-波长关系。

Result: 发现尺寸-质量关系的斜率存在静止框架波长梯度，在~10^9.5 M⊙处存在临界交叉质量，该质量被提议为星系形态从弥散转变为致密的分界点。

Conclusion: 提出了一个新的星系形态转变特征质量，并发布了包含约41,000个星系在最多29个JWST+HST滤光片中的结构参数数据产品。
...
### [13] [Turbulence in the multi-phase circumgalactic medium: bridging TNG50 and idealized kpc-scale simulations](https://arxiv.org/abs/2606.13777)
*Jonas Biba,Dylan Nelson*

Main category: astro-ph.GA

Recommendation: ★★★★★ (100/100) - 与 Zotero 兴趣画像中的 medium, galactic, simulations, galaxies, galaxy 等主题/作者匹配。

TL;DR: 该研究利用TNG50宇宙学模拟和理想化湍流盒模拟，分析了银河系质量星系周围介质(CGM)的湍流特性，发现其以亚音速流动为主，并研究了湍流如何通过冷却和混合时间尺度竞争影响冷气体的形成与演化。


<details>
  <summary>Details</summary>
Motivation: 理解CGM动力学如何受辐射冷却和湍流相互作用的影响，特别是湍流如何触发密度涨落并导致热不稳定性，以及冷热气体之间的混合过程。

Method: 首先分析TNG50宇宙学磁流体模拟中银河系质量星系CGM的湍流特性，然后利用这些结果设置理想化湍流盒模拟的初始条件和驱动参数，在AREPO代码中实现湍流驱动算法，并进行包含金属线辐射冷却的参数化模拟。

Result: CGM中以亚音速流动为主(马赫数0.1-0.7)，压缩性与螺线管分量大致平衡(压缩比0.3-0.6)；冷气体(~10^4 K)的丰度和演化强烈依赖于湍流强度和冷却效率。

Conclusion: 湍流通过种子密度涨落触发热不稳定性并促进冷热相之间的混合，冷气体的形成取决于冷却时间尺度和混合时间尺度的竞争关系。
...
### [14] [On the survival of dark matter spikes: Stellar and compact-object perturbations](https://arxiv.org/abs/2606.13761)
*Theophanes K. Karydas,Francesca Scarcella,Bradley J. Kavanagh,Gianfranco Bertone*

Main category: astro-ph.GA

Recommendation: ★★★★★ (100/100) - 与 Zotero 兴趣画像中的 stellar, dark matter, dark, matter, mass 等主题/作者匹配。

TL;DR: 本文以银河系中心为观测约束案例，研究暗物质密度尖峰在恒星和恒星级黑洞引力扰动下的演化。发现恒星团散射在远大于引力波信号产生区域处消耗暗物质，而近星体（S2、S38）影响很小。恒星级黑洞并合事件仅将中心密度降至初始值的约82%，表明暗物质超密结构基本保持完整。


<details>
  <summary>Details</summary>
Motivation: 建立对超大质量黑洞附近暗物质分布的现实预期，对于评估暗物质环境对引力波信号的影响至关重要。

Method: 以银河系中心为观测约束案例，通过模拟核星团恒星散射、已知近星体（S2、S38）的引力扰动以及过去100亿年间恒星级黑洞（约10太阳质量）的极端质量比旋进事件累积效应，研究暗物质密度尖峰的演化。

Result: 恒星团散射在~0.1 pc处消耗暗物质；S2和S38在~0.001 pc处影响可忽略；恒星级黑洞并合仅将中心(~10⁻⁵ pc内)密度降至初始值的约82%。

Conclusion: 至少就银河系中心而言，在恒星扰动和合理的恒星级黑洞并合历史下，中心黑洞周围的暗物质超密结构预计基本保持完整。
...
Recommendation: ★★★★★ (100/100) - 与 Zotero 兴趣画像中的 mass, star, star-forming, redshift, strong 等主题/作者匹配。

TL;DR: 该论文比较了在再电离纪元21厘米信号分析中两种前景处理策略（避免法和移除法），发现两者都会在宇宙学参数推断中引入系统性偏差，其中避免法得到的后验分布更宽，而移除法在晚期再电离的中性分数推断中存在持续偏差。


<details>
  <summary>Details</summary>
Motivation: 评估两种前景处理策略（前景避免vs前景移除）对再电离纪元21厘米信号参数推断的影响及其系统性偏差。

Method: 比较两种方法：(i)前景避免法——限制在傅里叶空间的EoR窗口；(ii)前景移除法——通过高斯过程回归利用频谱平滑性分离前景。在相同尺度上评估参数后验分布。

Result: 两种方法在最小恒星形成晕质量和电离逃逸分数等参数上引入约1σ的系统偏差；避免法后验更宽；中性分数在晚期存在持续偏差；多红移推断易受未被充分移除的污染箱影响。

Conclusion: 两种策略在相同尺度上可恢复相似的功率谱和后验，但均存在系统性偏差；排除受污染的红移箱可减少偏差，但需要独立的质量指标来识别。
...
### [16] [Impact of non-Gaussian likelihood on cosmological constraints from the thermal Sunyaev--Zel'dovich power spectrum: a simulation-based inference analysis](https://arxiv.org/abs/2606.14622)
*Licong Xu,Íñigo Zubeldia,James Alvey,Boris Bolliet,Anthony Challinor*

Main category: astro-ph.CO

Recommendation: ★★★★★ (100/100) - 与 Zotero 兴趣画像中的 analysis, galaxy, eld, mass, massive 等主题/作者匹配。

TL;DR: 本文利用模拟推理（SBI）检验了Planck tSZ功率谱分析中高斯似然假设的准确性，发现高斯似然能给出无偏的宇宙学约束，而SBI可作为一种验证非高斯似然的工具。


<details>
  <summary>Details</summary>
Motivation: tSZ功率谱信号受稀有、大质量低红移星系团的显著贡献，统计量非高斯，传统高斯似然在大尺度上可能不准确，需验证其可靠性。

Method: 使用基于晕模型的全球康普顿-y图模拟，训练神经后验估计器和似然估计器，并与标准高斯似然的约束结果进行比较，仅使用多极矩ℓ<1000的数据。

Result: 高斯似然假设给出无偏的宇宙学约束；SBI推断显示残差前景幅度的后验分布有轻微展宽。

Conclusion: 对于Planck级别的tSZ分析，高斯似然假设足够精确；SBI是超越解析近似的有效验证工具。
...
### [17] [Optimal marked statistics from the Effective Field Theory of Large Scale Structure](https://arxiv.org/abs/2606.14332)
*Marco Marinucci,Michele Liguori,Federico Semenzato,Benjamin D. Wandelt*

Main category: astro-ph.CO

Recommendation: ★★★★★ (100/100) - 与 Zotero 兴趣画像中的 field, eld, structure, large, density 等主题/作者匹配。

TL;DR: 本文在有效场论框架下提出了一种评估标记密度场功率谱信息含量的方法，通过优化标记函数来增强对宇宙学参数的约束能力。结果表明，优化后的标记可将Ω_m、σ_8和h的参数误差分别降低约1.30-2.3倍、1.3-8.3倍和1.2-2.5倍，展示了标记统计作为超越标准两点分析工具的巨大潜力。


<details>
  <summary>Details</summary>
Motivation: 传统的大尺度结构两点统计（如功率谱）存在信息损失，作者希望探索标记密度场（marked density field）的功率谱能否提取更多宇宙学信息，并在有效场论框架下给出其微扰理论的适用范围。

Method: 基于有效场论（EFT）框架，建立标记密度场功率谱的物理有效理论；通过优化标记函数（mark）的形式，最大化对宇宙学参数（Ω_m, σ_8, h）的约束力；对小尺度逆项（counterterms）进行边缘化处理，评估参数不确定性的降低幅度。

Result: 优化后的标记函数将参数误差条改善约1.30-2.3倍（Ω_m）、1.3-8.3倍（σ_8）和1.2-2.5倍（h），具体数值依赖于红移和光滑尺度。优化的标记函数具有简洁的函数形式，可基于其对密度场的响应进行物理解释。

Conclusion: 标记统计是超越标准两点分析、提取额外宇宙学信息的有效工具，为非线性密度场变换的进一步探索开辟了新途径。
...
Recommendation: ★★★★★ (100/100) - 与 Zotero 兴趣画像中的 dark, energy, data, these, early 等主题/作者匹配。

TL;DR: 研究含四次多项式非最小耦合的标量-张量（quintessence）暗能量模型，在DESI数据和引力局域测试约束下，参数空间允许区域非常狭窄。


<details>
  <summary>Details</summary>
Motivation: 验证含多项式非最小耦合的quintessence模型在DESI最新约束和引力局域测试下的可行性，寻找同时满足所有约束的参数空间区域。

Method: 系统探索由二次和四次非最小耦合参数及势能斜率构成的参数空间，将模型预测与DESI的暗能量状态方程约束、太阳系测试及牛顿常数时间变化约束进行对比。

Result: 找到了同时满足所有约束的参数带，但该参数带非常狭窄。

Conclusion: 含四次多项式非最小耦合的线性势quintessence模型在理论上面临严峻的参数微调问题，可生存的参数空间极其有限。
...
### [19] [Controlling instrumental systematics for the BICEP inflation survey](https://arxiv.org/abs/2606.13942)
...
Recommendation: ★★★★★ (100/100) - 与 Zotero 兴趣画像中的 survey, lens, lensing, gravitational, data 等主题/作者匹配。

TL;DR: BICEP实验利用南极观测的CMB极化数据约束暴胀引力波，计划将张量标量比r的测量精度提升至σ(r)~0.005（2024年数据）乃至σ(r)~0.001（2034年），并通过校准程序、系统误差分析与模拟，确保主要系统误差控制在σ(r)的20%以内。


<details>
  <summary>Details</summary>
Motivation: 通过观测CMB极化信号约束暴胀引力波，是检验早期宇宙暴胀理论的关键途径。

Method: 利用南极BICEP望远镜进行长达20年的CMB极化观测；通过校准程序表征接收器的光学、光谱和极化响应；采用端到端模拟评估各类系统误差对宇宙学参数的影响，重点分析波束系统误差。

Result: 预计2024年数据可将张量标量比r的测量精度提升至σ(r)~0.005（含去透镜处理），长期目标（2034年）达到σ(r)~0.001；主要系统误差源被控制在σ(r)的20%以内。

Conclusion: BICEP实验通过严格的校准与系统误差控制策略，有望在未来十年内将暴胀引力波约束提升一个数量级，为下一代CMB实验奠定方法论基础。
...
### [20] [The baryonic Tully-Fisher relation as an independent direct probe of cosmology and of the nature of dark matter](https://arxiv.org/abs/2606.13936)
*Francesco Sinigaglia*

Main category: astro-ph.CO

Recommendation: ★★★★★ (100/100) - 与 Zotero 兴趣画像中的 dark matter, dark, matter, galaxies, galaxy 等主题/作者匹配。

TL;DR: 本文首次证明重子塔利-费舍关系（BTFR）不仅是一个星系标度关系和距离指示器，还能直接敏感于宇宙学参数（Ω_m, σ_8）、暗物质粒子质量（M_wdm）以及天体物理反馈参数，通过模拟推断和深度学习，可作为一种全新的宇宙学与基础物理探针。


<details>
  <summary>Details</summary>
Motivation: BTFR长期以来被视为星系形成与演化的基准标度关系及校准的距离指示器，但尚未被探索作为宇宙学和基础物理的直接探针。本文旨在挖掘BTFR在约束宇宙学参数、暗物质属性及反馈机制方面的潜力。

Method: 基于DREAMS宇宙磁流体动力学模拟套件，进行模拟推断（simulation-based inference）；训练深度神经网络（正规化流，normalizing flows）来估计后验分布，输入为BTFR测量数据，输出为Ω_m、σ_8、M_wdm及三个天体物理自由参数。

Result: 该方法能无偏恢复Ω_m和σ_8，精度分别约2.6%和3.9%；对温暗物质粒子质量M_wdm的约束精度约30-35%；能有效约束超新星反馈参数，但对AGN反馈参数约束不佳。

Conclusion: BTFR除了作为距离指示器和约束重子循环及反馈机制的工具外，还可作为宇宙学和基础物理的直接独立探针，为未来平方千米阵列（SKA）等观测开辟了新的研究前景。
