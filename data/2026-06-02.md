...
### [1] [A study of the Physical Properties and Star Formation Activity of a Large Sample of Molecular Clouds: I Distances](https://arxiv.org/abs/2606.02028)
...
Recommendation: ★★★★☆ (75/100) - 与 Zotero 兴趣画像中的 molecular, dust 等主题/作者匹配。

TL;DR: 本文利用MWISP CO巡天数据与三维尘埃消光图匹配，提出了三种独立方法，为1,573个分子云提供了距离测量（150 pc至3000 pc），其中90%为首次测量，并构建了包含物理性质的公开星表。


<details>
  <summary>Details</summary>
Motivation: 分子云的精确距离对于确定其物理性质、理解恒星形成以及追踪银河系旋臂结构至关重要，但现有大部分分子云缺乏可靠的距离数据。

Method: 提出了三种独立方法，将MWISP 12CO线的速度积分强度图与基于Gaia、Pan-STARRS 1和2MASS的三维尘埃消光图进行匹配，从而确定分子云距离。

Result: 获得了1,573个分子云的稳健距离（约150-3000 pc），90%为首次测量，统计和系统不确定性分别约为20%和10%，并推导了质量、尺寸等物理性质。

Conclusion: 公开的分子云距离星表为检验分子云标度关系以及研究不同银河系环境下云条件如何影响恒星形成提供了基础。
...
### [2] [It's Not Just Star Formation: A trend of low dark matter densities in the Andromeda dwarf galaxy system](https://arxiv.org/abs/2606.00221)
...
TL;DR: 通过动力学建模分析了7个M31矮椭球星系，发现其中5个的暗物质中心密度低于ΛCDM预测，可能挑战标准宇宙学模型。


<details>
  <summary>Details</summary>
Motivation: M31矮椭球星系的暗物质密度普遍低于ΛCDM模拟预测，但目前仅成功建模了4个，需要更多样本以理解局域群矮椭球星系的演化规律。

Method: 更新7个M31矮椭球星系的运动学数据，使用GravSphere工具进行动力学Jeans建模，估算150 pc处的中心暗物质密度，并通过丰度匹配更新暗物质晕质量M200。

Result: Andromeda III和V的中心暗物质密度符合ΛCDM预期，与银河系矮椭球星系类似；其余5个星系密度异常偏低，进一步证实了M31卫星系中存在的低密度趋势。

Conclusion: 恒星形成导致的"暗物质加热"不能完全解释低密度现象；这些星系可能处于更极端的轨道上。若观测数据与轨道预测不符，可能需引入超越标准宇宙学模型的新物理。
...
### [3] [Subarcsecond Multi-line Observations of NH$_3$ with VLA toward the Class 0 Source IRAS 16293-2422](https://arxiv.org/abs/2606.00212)
*Yoshihide Yamato,Yuri Aikawa,Kenji Furuya,John J. Tobin,Jes K. Jørgensen*

Main category: astro-ph.GA

Recommendation: ★★★★☆ (67/100) - 与 Zotero 兴趣画像中的 alma, molecular 等主题/作者匹配。

TL;DR: 利用VLA对原恒星系统IRAS 16293-2422进行了亚角秒分辨率的NH₃分子谱线观测，检测了17条反转跃迁线（E_u约23K-1580K），通过双组分模型约束了旋转温度和柱密度，发现高温NH₃气体在源A中源于局部激波加热，在源B中则归因于吸积加热。


<details>
  <summary>Details</summary>
Motivation: 氨（NH₃）是星际氮化学的关键挥发物，但以往对原恒星区域的高分辨率NH₃观测有限，特别是缺乏覆盖宽能级范围的综合观测数据来揭示其在不同温度区域的空间分布和加热机制。

Method: 使用VLA对IRAS 16293-2422进行亚角秒（~0.5″）分辨率的NH₃分子线观测，覆盖17条反转跃迁线（E_u约23K-1580K）；采用双组分模型拟合旋转温度和柱密度；结合ALMA高分辨率数据对比分析加热机制。

Result: 高E_u（≥1000K）谱线呈致密分布集中在原恒星附近，低E_u（≤150K）谱线呈现延展分布；源A和源B的暖组分旋转温度约200-300K；源A高温NH₃气体源于局部激波加热，源B源于吸积加热。

Conclusion: 高能级NH₃谱线可选择性地示踪原恒星内部热区域；不同原恒星中NH₃的加热机制存在差异（激波加热 vs 吸积加热）；结合文献数据讨论了NH₃与主要冰分子的丰度比及相关化学过程。
...
### [4] [Mergers Matter: Gravothermal Collapse in Dwarf Halos with Self-Interacting Dark Matter](https://arxiv.org/abs/2606.02566)
...
TL;DR: 研究了自相互作用暗物质（SIDM）模型中，不同并合历史对矮星系晕核心演化的影响，发现剧烈并合会阻止核坍缩，并可能产生暗物质缺失的星系。


<details>
  <summary>Details</summary>
Motivation: 检验大截面SIDM模型在矮星系尺度上的可观测性，特别是并合历史如何影响暗物质晕的核心密度分布。

Method: 对6个~10^10 M⊙的暗物质-only zoom-in模拟，采用σ/m=70 cm²/g的SIDM截面，分析不同并合历史下的核心演化。

Result: 3个并合历史较平静的晕发生核坍缩；有持续并合的晕未坍缩；其中2个非坍缩晕的中心密度远低于引力热流体模型预测。

Conclusion: 并合历史是理解SIDM中矮星系晕中心密度分布的关键，并合可注入能量阻止坍缩，产生暗物质缺失星系和多样化的旋转曲线。
...
### [5] [The magnetic field of the Milky Way: an observational perspective](https://arxiv.org/abs/2606.02149)
*Marijke Haverkorn*

Main category: astro-ph.GA

Recommendation: ★★★☆☆ (60/100) - 与 Zotero 兴趣画像中的 dust, polarization 等主题/作者匹配。

TL;DR: 这篇论文综述了银河系大尺度磁场的最新观测进展，描述了磁场在星系盘和晕中的不同构型、湍流特性以及未来观测数据增长的前景。


<details>
  <summary>Details</summary>
Motivation: 磁场是银河系生态系统中重要且神秘的组成部分，对气体动力学、恒星形成、能量收支、星际物体演化及宇宙线传播等关键过程有重要影响，因此需要系统综述其观测进展。

Method: 采用综述方法，从观测视角总结银河系大尺度磁场（主要在弥散介质中）的测量与表征，涵盖星系尺度、小尺度湍流场等多层次分析。

Result: 1）星系尺度上，磁场大致沿旋臂分布，并包含垂直于盘面的分量，盘与晕的磁场构型不同，符合不同发电机模态；2）小尺度上，磁场表现为湍流、各向异性和间歇性，高阶统计量正在替代传统的功率谱和高斯描述；3）磁场取向和强度与多相星际气体的冷热成分及星际尘埃分布相关。

Conclusion: 未来几年，旋转测量网格、法拉第断层成像和星际星光偏振等观测数据将大幅增加，有望推动银河系磁场表征与理解取得显著进展。
...
### [6] [Magneto-Gravitational Regulated Streamer Accretion onto a Class 0 Protostellar System](https://arxiv.org/abs/2606.01707)
...
Recommendation: ★★★☆☆ (56/100) - 与 Zotero 兴趣画像中的 submillimeter, alma 等主题/作者匹配。

TL;DR: 利用ALMA高分辨率观测年轻三合星系统HOPS-182，发现一条延伸数千天文单位的吸积流管，气体沿该流管向恒星系统输送物质。流管形状与磁场线一致，且磁场足够强以引导和约束下落气体并有效移除角动量，表明大部分物质可能通过磁结构化的吸积流管落入原恒星系统。
...
Method: 利用ALMA高分辨率观测年轻三合星原恒星系统HOPS-182，分析吸积流管的形态、气体速度分布，并比较引力与磁张力的强度，测量气体旋转与局部磁场的相对关系。

Result: 发现一条延伸数千天文单位的吸积流管，气体速度向恒星方向增加（符合引力自由落体），流管形状与磁场线紧密吻合，且磁场足够强以帮助约束和引导下落气体并有效移除角动量。

Conclusion: 相当大比例的物质可能通过伸长的、磁结构化的吸积流管落入年轻原恒星系统。
...
### [7] [Non-detection of HC(S)SH: Estimating Upper Limits and Constraining Chemistry](https://arxiv.org/abs/2606.00569)
*Dipen Sahu,Ankan Das,Prasanta Gorai,Victor M. Rivilla,Sheng-Yuan Liu,Bhalamurugan Sivaraman,Paola Caselli*

Main category: astro-ph.GA

Recommendation: ★★★☆☆ (56/100) - 与 Zotero 兴趣画像中的 alma 等主题/作者匹配。

TL;DR: 本文重新分析了ALMA数据，发现此前报道的二硫代甲酸（t-HC(S)SH）在NGC 1333 IRAS 4A2中的探测并无可靠证据，所有谱线均可由已知丰度分子解释，并给出了严格的上限约束。


<details>
  <summary>Details</summary>
Motivation: 理解星际硫化学并解决“缺失硫”问题，需要探明星际中的含硫分子。此前有研究声称在NGC 1333 IRAS 4A2中探测到了二硫代甲酸，但该结果可能存在问题，需要独立验证。

Method: 独立重新分析相同的ALMA数据集，结合全面的光谱建模和化学模型，对每条跃迁进行细致识别，并与已知分子谱线进行比对，推导柱密度和丰度的上限。

Result: 未发现t-HC(S)SH的可信证据：所有声称的跃迁均可由已知丰度分子完全解释，不存在未被混合的独特谱线。柱密度上限为N_t-HC(S)SH <= 4×10^14 cm^-2，丰度上限<= 1×10^-10（相对于H2）。

Conclusion: 该探测很可能是谱线混合和不一致的建模假设导致的误判。非探测结果与热内核环境中复杂双硫取代分子稀少的化学预期一致，并建立了未来寻找含硫分子的严谨分析框架。
...
### [8] [OCEANS of Absorption: High-resolution NIRSpec Spectroscopy Reveals Diverse Balmer-line Absorption in Little Red Dots](https://arxiv.org/abs/2606.00258)
...
TL;DR: 本文通过对10个小红点（LRDs）的高分辨率光谱分析，发现其中4个存在巴默线吸收特征，吸收率高于以往低分辨率巡天结果。研究揭示了吸收气体的速度偏移、等效宽度等性质，并提出了部分覆盖吸收气体的径向分布模型来解释多样化的吸收特征。


<details>
  <summary>Details</summary>
Motivation: JWST深场图像中发现的小红点（LRDs）引起了广泛关注，但此前低分辨率光谱研究对其吸收特征的认识有限，需要高分辨率光谱来深入探究其物理性质。

Method: 利用OCEANS项目对CEERS/EGS天区中10个红移3<z<7的LRDs进行高分辨率后续光谱观测，分析其Hα谱线轮廓、巴默线吸收特征、速度偏移和等效宽度等，并与模型进行比较。

Result: 4/10的LRDs检测到巴默线吸收，吸收率高于此前NIRSpec巡天报告的比例；7个LRDs的Hα谱线最佳拟合为指数翼轮廓；吸收体多呈蓝移，中值速度偏移-49 km/s，等效宽度5.3 Å；发现一个LRD在Hα和Hβ之间存在统计显著的速度偏移差异。

Conclusion: LRDs中吸收氢的高出现率意味着致密、激发态n=2氢的几乎普遍存在；多样化的吸收特征可用径向分布的部分覆盖吸收气体模型有效解释。
...
### [9] [The cosmic ray ionization rate from H3+ observations can be overestimated due to neglect of time-dependent chemistry](https://arxiv.org/abs/2606.00901)
...
TL;DR: 本文通过3D MHD模拟发现，使用H₃⁺观测推断宇宙线电离率时，若假设稳态化学会导致推断值比真实值高约2-5倍（中位数约3倍），并报告了修正后的平均电离率ζ_H₂ = 2×10⁻¹⁷ s⁻¹。


<details>
  <summary>Details</summary>
Motivation: 宇宙线电离率是星际介质物理、化学和热演化的关键参数，目前测量扩散分子云中CRIR的主要技术依赖于H₃⁺观测，但以往分析均假设稳态化学，忽略了时间依赖化学对推断结果的影响。

Method: 采用3D MHD模拟结合耦合化学和驱动湍流，通过后处理模拟以不同CRIR值获得H₂和H₃⁺的稳态丰度，并与时间依赖化学的丰度进行比较，确定最佳拟合CRIR值。

Result: 时间依赖化学模拟中H₂和H₃⁺丰度均高于稳态情况（尤其在低密度区域）；稳态假设下推断的CRIR比真实值高约2-5倍（中位数约3倍）；该偏差随磁场增强、FUV辐射减弱和湍流增强而增大；修正后的平均CRIR为ζ_H₂ = 2×10⁻¹⁷ s⁻¹。

Conclusion: 稳态化学假设会显著高估CRIR，考虑时间依赖化学后，H₃⁺观测得出的CRIR在柱密度N=(2-6)×10²¹ cm⁻²范围内为恒定值。
...
### [10] [Vision-Language Model Ensembles Achieve Human-Expert Accuracy for Galaxy Merger Classification](https://arxiv.org/abs/2606.00415)
*Marco Chiaberge,Elias Stengel-Eskin,Massimo Stiavelli,Colin Norman*

Main category: astro-ph.GA

Recommendation: ★★★☆☆ (42/100) - 与 Zotero 兴趣画像中的 polarized 等主题/作者匹配。

TL;DR: 本文展示了使用贝叶斯框架集成15个视觉-语言模型（VLM）分类器，在星系合并形态分类任务上达到了与人类专家相当的准确率（83.3%），比简单多数投票提升17.1个百分点，错误数量（5个）接近人类分类器（6个），证明了VLM可作为可扩展、可重复的替代方案。


<details>
  <summary>Details</summary>
Motivation: 传统星系合并形态分类依赖人类专家，耗时且主观性强，难以应用于大规模巡天数据，因此需要一种可扩展、可重复且准确的自动化替代方法。

Method: 使用4种VLM架构（Gemma-4 E2B/E4B、Qwen2.5-VL、Qwen3-VL）共15种配置，结合4种提示工程策略，并通过贝叶斯统计框架对各个模型进行加权集成，在41个模拟星系图像上验证。

Result: VLM集成模型在置信分类中达到83.3%准确率，仅5个误分类（2假阳性、3假阴性），人类分类器为6个误分类（5假阳性、1假阴性）。贝叶斯加权比多数投票准确率提升17.1个百分点，灵敏度提升29.2个百分点。种群合并分数恢复至0.66σ置信区间内。

Conclusion: VLM集成可作为人类分类器的可扩展、可重复替代方案，适用于当前及未来大型星系巡天的合并分数贝叶斯概率框架分析。
...
### [11] [Accurate inner stellar density slopes from projected surface densities in galaxies](https://arxiv.org/abs/2606.01956)
...
TL;DR: 本文提出了一种无需反投影即可从恒星表面密度径向导数直接推断三维恒星密度中心斜率的方法，并应用于六个超暗弱矮星系，发现它们均存在恒星核（中心斜率为零）。


<details>
  <summary>Details</summary>
Motivation: 暗物质研究需要约束矮星系三维恒星密度内斜率以检验CDM模型，但传统反投影方法在噪声干扰下具有模糊性，因此需要一种更直接、观测成本更低的方法。

Method: 推导了从表面密度径向导数直接获取中心斜率rho'(0)的解析表达式（Eq.9），利用所有投影分布具有相同中心函数形式的性质（Eq.20），通过大半径约束外推导数至中心。

Result: 对六个超暗弱矮星系的应用表明，所有星系表面密度形状一致，推断出其恒星分布存在核（rho'(0)≈0），且该方法能诊断rho'(0)>0的情况。

Conclusion: 该方法避免了反投影的歧义性，为利用测光数据约束矮星系中心恒星密度结构提供了高效工具，有助于检验黑洞、MOND或偏离CDM的动力学效应。
...
### [12] [Radio Continuum Emission from Evolving Star-Forming Galaxies -- I. Correlations Involving the Total Synchrotron Luminosity](https://arxiv.org/abs/2606.01957)
*Sukanta Ghosh,Luke Chamandy,Charles Jose,Anvar Shukurov,Luiz Felippe S. Rodrigues,Fatemeh Tabatabaei*

Main category: astro-ph.GA

Recommendation: ★★☆☆☆ (36/100) - 与 Zotero 兴趣画像中的 ska 等主题/作者匹配。

TL;DR: 本文利用半解析星系形成模型GALFORM结合发电机模拟代码MAGNETIZER，模拟了恒星形成星系的同步辐射发射，发现同步辐射光度与恒星形成率和星系旋转速度存在强正相关，并与低红移观测数据吻合，但高红移存在差异。


<details>
  <summary>Details</summary>
Motivation: 理解恒星形成星系在MHz至30 GHz频段的同步辐射连续谱发射机制，为下一代射电巡天（如SKA）提供理论模型支持。

Method: 将半解析星系形成模型GALFORM与发电机模拟代码MAGNETIZER结合，假设宇宙线与磁场局域能量均分，计算每个模拟星系在不同频率下的同步辐射光度，并分析与恒星形成率（SFR）和旋转速度（V_rot）的相关性。

Result: 在z≲3范围内，同步辐射光度与SFR和V_rot呈强正相关；低红移时湍流磁场主导同步辐射，高红移（z≳1）时大尺度磁场贡献显著增加；低红移模型与观测数据吻合，但高红移时模型SFR系统性低于观测推断值。

Conclusion: 该模型能有效预测低红移的同步辐射性质，但高红移存在偏差，需进一步改进；模型对下一代射电巡天（如SKA）的数据解释具有重要价值。
...
### [13] [Revisiting ram pressure stripping in Wolf-Lundmark-Melotte: No evidence for stripped HI with LGLBS](https://arxiv.org/abs/2606.02353)
...
TL;DR: 对WLM矮星系的HI 21厘米观测显示，此前MeerKAT数据中发现的星系外气体可能源于观测误差，而非真实存在的冲压剥离现象。


<details>
  <summary>Details</summary>
Motivation: 验证WLM矮星系是否存在冲压剥离效应，并检验此前MeerKAT观测结果的可信度。

Method: 利用Local Group L-Band巡天的HI 21厘米观测数据对WLM进行再分析，并评估对之前探测到的星系外气体的灵敏度。

Result: 新观测未发现此前声称的星系外原子气体发射，且证明新数据对该信号足够灵敏，暗示此前探测为观测误差所致。

Conclusion: WLM不存在冲压剥离气体，因而无需假设该区域极高的星系际介质密度。
...
### [14] [Understanding the Broad-line Region of Active Galactic Nuclei with Photoionization. II. Slim disks, Self-shadowing, and BLR sizes](https://arxiv.org/abs/2606.00992)
...
TL;DR: 本文通过分析吸积盘结构、宽线区云团性质及光致电离过程，发现高吸积率的活动星系核（AGN）在宽线区半径-光度关系上出现偏离，主要由内盘自遮光效应和吸积率依赖的宽线区气体密度增加共同导致。


<details>
  <summary>Details</summary>
Motivation: 高吸积率AGN的宽线区滞后时间低于标准R-L关系预测，其物理起源尚存争议，需探究吸积流结构与宽线区云性质如何影响发射权重半径。

Method: 采用解析薄盘光谱能量分布（SED）与二维轴对称网格上的光致电离计算，模拟吸积率变化下的各向异性辐射、自遮光效应及宽线区气体密度随吸积率和黑洞质量的变化。

Result: 自遮光效应可较好再现高吸积率下R-L关系的平坦化趋势，但仅此不足以解释低质量高吸积率系统的滞后偏差；引入吸积率依赖的宽线区气体密度增强（每十倍吸积率增加3-5倍）后，理论与观测吻合更好。

Conclusion: 自洽模拟吸积盘SED、宽线区光照与光致电离以及气体密度变化，可完整解释AGN在宽线区大小-光学光度平面上的分布，为吸积流结构与宽线区观测之间建立了物理联系。
...
### [15] [Spectroscopic characterization of Young Stellar Populations and their Feedback in NGC 5253](https://arxiv.org/abs/2606.00332)
...
TL;DR: 本文利用HST/COS的远紫外光谱数据分析了近邻金属贫乏蓝致密矮星系NGC 5253中的年轻大质量星团，并与VLT/MUSE光学观测结合，研究了星团对其周围电离气体外流的影响。研究发现，光谱测定的年龄优先于测光方法，且外流速度与星团年龄、质量及超新星驱动的机械光度之间存在显著相关性。尽管理论预测低金属丰度环境下机械反馈存在延迟，但在NGC 5253（Z~0.3 Z⊙）中，超新星反馈在>5 Myr时已明显显现，未观察到明显延迟。


<details>
  <summary>Details</summary>
Motivation: 探究低金属丰度环境中年轻大质量星团对其周围星际介质（ISM）的反馈机制，特别是超新星驱动的气体外流与星团物理参数之间的关系。

Method: 使用HST/COS进行远紫外光谱观测，结合VLT/MUSE光学数据，通过全光谱拟合分析星团参数（年龄、金属丰度、质量、红化），并测量电离气体外流速度，进行统计分析。

Result: 光谱年龄比测光方法更年轻；外流速度约125-300 km/s；外流速度与星团年龄、质量及超新星机械光度呈强显著相关；在Z~0.3 Z⊙环境下，超新星反馈在>5 Myr时已显现，无明显延迟。

Conclusion: 在NGC 5253的金属丰度水平下，超新星反馈机制并未表现出理论预测的延迟效应，但更极端的低金属丰度环境可能仍存在机械反馈的抑制与延迟。
...
### [16] [Ultraviolet diversity of Little Red Dots as a probe for direct-collapse black hole ages](https://arxiv.org/abs/2606.00205)
...
TL;DR: 本文利用MELIORA宇宙学流体动力学模拟，研究了红移z>8.5的小红点（LRDs）中PopIII恒星和直接塌缩黑洞（DCBHs）的贡献。发现宿主星系的紫外辐射在DCBH形成后约3000万年内快速演化，从初始的黑洞主导系统转变为恒星主导系统。UV明亮的LRDs倾向于拥有更年长的DCBHs、更高的金属丰度、更低的黑洞-恒星质量比和更低的爱丁顿比。LRD相位的短暂性对其宇宙学出现时间施加强约束。


<details>
  <summary>Details</summary>
Motivation: LRDs被提议为嵌有直接塌缩黑洞的候选星系，但其宿主星系的紫外辐射贡献高度不确定且多样化，亟需理论模拟来厘清。

Method: 使用MELIORA套件的大尺度宇宙学流体动力学模拟，分析z>8.5的LRD候选体中PopIII恒星和吸积DCBHs在静止框架0.2-0.6 μm波段的贡献。

Result: 宿主星系UV辐射在DCBH形成后约30 Myr内快速演化，经历从黑洞主导到恒星主导的转变；UV明亮的LRDs对应更年长DCBH、更高金属丰度、更低BH-恒星质量比和更低爱丁顿比。

Conclusion: DCBH年龄可通过LRDs的UV-光学光谱中宿主星系贡献来约束，支持DCBH演化序列系统性地编码在发射线性质、气相金属丰度和吸积状态中的观点。
...
### [17] [CSST large-scale structure analysis pipeline: IV. Cosmic Voids Identified from Galaxy Group Samples as Probes of the Large-scale Structure](https://arxiv.org/abs/2606.01907)
...
Recommendation: ★★★★☆ (78/100) - 与 Zotero 兴趣画像中的 csst, dark matter 等主题/作者匹配。

TL;DR: 本文提出利用星系群中的宇宙空洞（group-voids）研究大尺度结构。基于CSST的模拟数据，对比了群空洞与暗物质晕空洞的统计性质，发现当光谱红移完备度≥40%时，群空洞能很好地再现晕空洞的统计结果，即使在低至30%的完备度下，通过引入红移误差项仍可可靠描述。


<details>
  <summary>Details</summary>
Motivation: 星系群与暗晕直接关联，相比基于单个星系的建模更简单，因此作者希望利用星系群中的空洞（而非传统基于单个星系的空洞）来研究宇宙大尺度结构。

Method: 使用CSST的模拟星系红移巡天数据，分别构建了100%和约30%光谱红移完备度的理想与保守星系群目录，并识别其中的空洞。采用最亮中心星系（BCG）作为群中心以提高空洞中心精度，在五个红移区间（z=0~1.0）测量空洞尺寸函数和空洞密度轮廓。

Result: 当光谱红移完备度≥40%时，群空洞统计与晕空洞统计高度一致；即便完备度降至30%，引入红移误差项后仍可可靠描述。群空洞是探测大尺度结构的有效工具。

Conclusion: 群空洞可作为传统空洞研究的有力补充，特别适用于基于模拟器的研究方法，为宇宙大尺度结构探测提供了新途径。
...
### [18] [Decaying Dark Matter as a Possible Solution for Cosmological Tensions](https://arxiv.org/abs/2606.00529)
...
TL;DR: 本文探讨了衰变暗物质（DDM）模型如何通过抑制晚期物质涨落来缓解宇宙结构生长参数$S_8$的测量张力，发现慢速暗物质衰变产生的尺度依赖抑制效应与DES数据一致，并能自然地将结构振幅预测值拉向弱引力透镜调查所偏好的较低值。


<details>
  <summary>Details</summary>
Motivation: 宇宙大尺度结构测量揭示了早期与晚期宇宙学探针之间持续存在的张力，最显著的是$S_8$参数的长期不一致性。本文旨在探索衰变暗物质模型能否缓解这一张力。

Method: 考虑一种中微子亲和衰变通道：重暗物质粒子$χ$缓慢衰变为标准模型中微子和一个轻的非费米子$φ$（$χ\rightarrow ν+ φ$）。利用DES第一年红移分布，构建基线物质功率谱，计算$Λ$CDM和DDM场景下的星系-星系、剪切-剪切和星系-剪切角功率谱。

Result: 慢速暗物质衰变产生尺度依赖的团块抑制效应，与DES测量结果一致，并能自然地将预测的结构振幅移向弱引力透镜调查所偏好的较低值。

Conclusion: 衰变暗物质是解决$S_8$张力的一个具有说服力和物理动机的框架。
...
### [19] [Generative Diffusion Priors for 3D Mapping of the Dark Universe](https://arxiv.org/abs/2606.00803)
...
TL;DR: 本文利用扩散模型先验结合物理前向模型，从弱引力透镜观测中三维重建暗物质分布，在模拟数据上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 弱透镜观测重建暗物质3D分布是高度病态的反问题，现有方法难以捕捉宇宙网的非高斯纤维状结构，需要更强的先验假设。

Method: 构建Conicus3D数据集，基于高分辨率宇宙学模拟学习扩散模型先验，结合可微物理前向模型与plug-and-play后验采样方案，实现3D弱透镜重建。

Result: 在模拟现代弱透镜巡天的数据上，2D和3D重建精度均显著优于基线方法；后验样本统计特性与模拟高度一致，且对宇宙学参数偏移具有鲁棒性。

Conclusion: 数据驱动的扩散模型先验能有效捕获暗物质结构的非线性统计特征，为弱透镜三维重建提供了更准确、更鲁棒的新范式。
...
### [20] [Optimal Transport Reconstruction of Biased Tracers in Primordial Non-Gaussian Fields](https://arxiv.org/abs/2606.01529)
...
TL;DR: 本文展示了最优输运方法可扩展至重建局部原始非高斯场，通过利用偏倚示踪物与“尘埃”分量中特征尺度依赖性的差异，实现更精确的初始条件重建。


<details>
  <summary>Details</summary>
Motivation: 现有最优输运方法在高斯位移场假设下表现良好，但无法处理当前宇宙学中关注的局部原始非高斯场。作者希望将该方法扩展至非高斯初始条件的重建。

Method: 利用偏倚示踪物在大尺度聚类中的特征信号（与f_NL和b_φ相关）与未被偏倚子集选中的“尘埃”分量具有不同振幅的尺度依赖性这一事实，构建更真实的尘埃模型来改进最优输运重建。

Result: 最优输运重建的质量随着尘埃模型的逐步真实化而提升。

Conclusion: 该方法为重建非高斯初始条件提供了一条可行路径，特别是对当前宇宙学感兴趣的局部原始非高斯场类型。
...
### [21] [A short review on Quintom dark energy theory](https://arxiv.org/abs/2606.01360)
...
TL;DR: 本文综述了Quintom暗能量理论，讨论了动态暗能量的No-Go定理、能够穿越w=-1的状态方程模型、Quintom物质驱动的反弹宇宙和涌现宇宙，以及通过CMB极化旋转角研究暗能量性质的可行性。


<details>
  <summary>Details</summary>
Motivation: 暗能量是宇宙加速膨胀的主要驱动力，但其本质尚不明确。Quintom模型能够实现状态方程穿越w=-1，这对理解暗能量动力学行为具有重要意义。作者希望通过综述该领域进展，为未来研究提供理论基础。

Method: 采用文献综述的方法，首先回顾动态暗能量的No-Go定理及其局限性；其次列举能够实现EoS穿越w=-1的Quintom模型实例；然后探讨Quintom物质在反弹宇宙和涌现宇宙中的应用；最后讨论通过测量CMB极化旋转角来探测暗能量性质的观测方法。

Result: 文章总结了Quintom暗能量理论的核心进展：证明了可以规避No-Go定理构建穿越w=-1的模型；Quintom物质可以支撑非奇异的反弹宇宙和涌现宇宙宇宙学；CMB极化旋转角有望成为探测暗能量性质和Quintom效应的观测窗口。

Conclusion: Quintom暗能量理论为理解宇宙的早期演化和当前加速膨胀提供了统一框架，但需要进一步的观测数据（如CMB极化测量）来验证和约束这些模型。
...
### [22] [Calibrating Self-Weighted Shear Estimation via Field Distortion](https://arxiv.org/abs/2606.00553)
...
TL;DR: 本文提出了一种自加权剪切估计方法（SWSE），结合场畸变测试（FD test），在不引入系统误差的前提下实现高效、稳健的弱引力透镜剪切测量，计算效率显著优于传统PDF-SYM方法。


<details>
  <summary>Details</summary>
Motivation: 为了在大规模星系巡天中实现精确的宇宙剪切测量，同时克服PDF-SYM方法因迭代导致计算开销大的瓶颈，需要一种高效且无系统误差的替代方案。

Method: 提出自加权剪切估计（SWSE）方法，通过自加权机制自然抑制形状噪声并平衡亮暗星系贡献；再结合场畸变测试（FD test）来校准SWSE固有的乘性和加性偏差。

Result: 在模拟和实际巡天数据上，FD测试成功校准了SWSE的固有偏差，其在星系-星系透镜测量中达到了与PDF-SYM相当的统计精度。

Conclusion: FD测试与SWSE的组合是一种稳健且高效的剪切估计方法，适用于未来大规模弱引力透镜巡天。
...
### [23] [The sound of dynamical dark energy and modified gravity](https://arxiv.org/abs/2606.00411)
...
TL;DR: 本文通过引入暗能量声速作为自由参数，研究了修改引力场景下暗能量动力学与引力偏离的相关性，发现CMB+BAO+SN数据组合支持在z<2时以超过95%置信度偏离广义相对论，但该信号在加入弱引力透镜数据后被削弱。


<details>
  <summary>Details</summary>
Motivation: 不同的候选模型都能再现暗能量动力学信号，但可以通过其扰动行为（由有效声速控制）来区分。为了探索暗能量声速与扰动行为之间的关联，作者测试了修改引力场景。

Method: 在广义相对论和修改引力框架下，分析暗能量声速变化对多种宇宙学量的影响；使用Planck PR4 CMB、Pantheon+超新星、DESI DR2 BAO和DES-Y3弱引力透镜数据，对暗能量和修改引力参数进行约束。

Result: CMB+BAO+SN组合在修改引力场景下，支持z<2时暗能量动力学偏离GR，置信度>95%；动态或超光速声速不降低显著性，但假设宇宙学常数时信号消失；加入弱引力透镜和CMB透镜数据后，约束显著偏向GR。

Conclusion: 本研究提供的框架可以将暗能量声速作为物理上有意义的自由参数来探索修改引力模型。
...
### [24] [Redshift Duality with Pantheon+SH0ES in a Planck-anchored Flat $Λ$CDM Framework: Implications for Hubble Tension and Observational Inference](https://arxiv.org/abs/2606.02097)
*Tae-Kyoung Lee*
...
TL;DR: 该论文测试了一种“红移对偶性”假设，即在标准度规膨胀红移之外，还存在由于光子能量沿传播路径转化为有效质量而产生的量子贡献。将该混合模型应用于Pantheon+SH0ES超新星数据，发现度规膨胀的哈勃常数与Planck基线值（67.4 km/s/Mpc）高度一致（偏差<0.33σ），且相比标准ΛCDM模型，该模型在不同红移区间恢复了哈勃常数的恒定性，并有望缓解高红移星系相关异常。


<details>
  <summary>Details</summary>
Motivation: 标准ΛCDM模型在解释哈勃常数测量（Hubble tension）以及高红移星系观测时存在不一致性，因此需要探索新的红移机制来调和这些矛盾。

Method: 提出一个混合红移模型，将标准度规膨胀红移与光子沿视线方向量子效应（能量转化为有效质量）的红移贡献相结合；利用Pantheon+SHOES超新星数据拟合该模型，并进行红移分箱分析。

Result: 混合模型恢复的度规膨胀哈勃常数与Planck CMB结果一致（67.4 km/s/Mpc，偏差<0.33σ）；相比ΛCDM模型在不同红移区间出现哈勃参量漂移，该模型在各红移箱中保持H_Λ的恒定性；该框架下反推的宇宙学物理量显示出可缓解高红移星系异常的校正趋势。

Conclusion: 红移对偶性概念值得在观测处理和推演中进一步考虑，且该模型与Planck锚定的平坦ΛCDM基线保持一致。
...
### [25] [21cmEMUv3: a hybrid diffusion-LSTM emulator of 21cmFAST summary observables](https://arxiv.org/abs/2606.00219)
...
TL;DR: 本文提出21cmEMUv3，基于21cmFASTv3仿真训练的仿真器，可高精度快速模拟宇宙黎明和再电离时代的7种观测信号，利用扩散模型和LSTM网络实现亚百分比精度，并首次结合流体模拟先验重新解释HERA观测上限，为SKA提供预测。


<details>
  <summary>Details</summary>
Motivation: 宇宙黎明和再电离时代的观测数据快速增加，亟需快速且鲁棒的理论解释框架。传统仿真计算成本高，机器学习/仿真器方法可大幅加速推断流程。

Method: 基于21cmFASTv3仿真训练仿真器21cmEMUv3，以σ₈和10个天体物理参数为条件，输出7种观测摘要量。圆柱形21cm功率谱采用基于分数的扩散模型仿真，其余6种（球平均21cm功率谱、中性分数、自旋温度、全球21cm信号、UV光度函数、汤姆逊散射光深）使用LSTM网络仿真，均达到亚百分比精度。

Result: 所有7种观测摘要量均达到亚百分比中位精度。利用仿真器重新解释HERA的21cm功率谱上限，发现软波段X射线光度与恒星形成率之比与高质量X射线双星在低金属丰度环境的预测一致，以95%置信度排除了低于10³⁹·² erg s⁻¹M⊙⁻¹yr的值。为SKA不同阵列配置生成21cm功率谱探测预测。

Conclusion: 21cmEMUv3为宇宙黎明和再电离时代提供了高效精确的理论解释工具，代码已开源。首次结合流体模拟先验约束了再电离时期天体物理参数，并为未来SKA观测提供了重要预测。
...
### [26] [Interacting dark energy constraints from Fermi GRBs and Pantheon+ SNe Ia with full GRB covariance](https://arxiv.org/abs/2606.00549)
...
TL;DR: 本研究利用15年的Fermi/GBM长伽马暴数据和Pantheon+ Ia型超新星数据，检验相互作用暗能量（IDE）模型是否优于标准ΛCDM模型。结果表明，在考虑完整协方差的情况下，IDE模型并未显著提升拟合优度，贝叶斯信息准则更倾向于简单的ΛCDM模型，当前数据无法为相互作用暗能量提供证据支持。


<details>
  <summary>Details</summary>
Motivation: 标准ΛCDM模型面临哈勃张力等理论和观测问题，需要探索包括相互作用暗能量（IDE）在内的扩展模型。伽马暴（GRB）可扩展距离测量至高红移，但其宇宙学应用受限于光度关系校准、协方差处理和固有散射等系统误差。

Method: 使用15年Fermi/GBM长GRB观测数据和Pantheon+ SNe Ia数据，比较四种平坦宇宙学模型：ΛCDM、wCDM、IDE-ρ_de和IDE-ρ_c。通过传播Amati关系校准协方差构建GRB完整协方差矩阵，并将GRB固有散射作为 nuisance参数采样，同时采用对角协方差进行鲁棒性检验。

Result: 完整GRB协方差下，GOLD和FULL样本在ΛCDM模型中都给出H₀≈72.8 km s⁻¹ Mpc⁻¹。IDE模型未能显著提升拟合优度以补偿额外参数，贝叶斯信息准则（BIC）更倾向ΛCDM。对角协方差检验得到相同模型选择结论，但改变了GRB固有散射的拟合值。

Conclusion: 对于所考虑的两种相互作用形式，在当前GRB系统误差水平下，GRB和Pantheon+数据无法为相互作用暗能量提供证据。当前GRB主要提供高红移哈勃图扩展，用以检验膨胀历史的形状。
