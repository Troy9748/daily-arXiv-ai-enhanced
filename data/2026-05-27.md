...
### [1] [NISER-IUCAA New Simulations of JWST GAlaxies and Quasars(NINJA): Properties of galaxies at $5 \leq z \leq 10$](https://arxiv.org/abs/2605.26211)
...
### [2] [Survival of very small carbonaceous dust grains in the inner-CGM of NGC 891 from JWST/MIRI MRS](https://arxiv.org/abs/2605.26864)
...
### [3] [First Light and Reionization Epoch Simulations (FLARES) XXII: UV-dust spatial offsets at the Epoch of Reionisation](https://arxiv.org/abs/2605.27370)
...
### [4] [How galaxies acquire their stellar mass at high redshift: High star formation efficiencies and the relative roles of dust and initial mass function](https://arxiv.org/abs/2605.26209)
...
### [5] [Magnetic field Topology and Star Formation in the Cepheus B Filamentary cloud under External Feedback](https://arxiv.org/abs/2605.26810)
...
TL;DR: 本文基于JCMT望远镜的亚毫米偏振与$^{13}$CO谱线观测，研究了Cep B分子云中 filaments 的磁场结构、重力不稳定性与恒星形成活动的关系。


<details>
  <summary>Details</summary>
Motivation: 探究Cep B分子云中细丝的磁场形态、引力收缩与恒星形成之间的关系，理解磁场如何调控细丝的演化和碎裂过程。

Method: 使用SCUBA-2/POL-2获取850 μm尘埃偏振图及HARP获取$^{13}$CO (J=3--2)谱线数据，分析细丝的磁场方向、线质量、质量-磁通比以及核心间距。

Result: 细丝呈NW-SE方向，磁场在头部呈弓形弯曲、沿脊柱方向排列；细丝热超临界但整体磁亚临界；核心间距与香肠不稳定性碎裂尺度一致，也存在较小间隔的非均匀分布。

Conclusion: Cep B中引力驱动碎裂，磁场调控演化，外部反馈塑造形态与恒星形成活动。
...
### [6] [Digging into the Massive Protostar S255IR NIRS3: A Study of Nitrogen-Bearing Molecules and Their Prebiotic Chemistry](https://arxiv.org/abs/2605.27177)
...
TL;DR: 本文利用ALMA望远镜在S255IR NIRS3高质量原恒星中识别出多种含氮复杂分子（如甲基氰、乙基氰等），通过LTE模型推导其柱密度和激发温度（175-220 K），并发现氰化物分子间存在强正相关性，化学模型预测与观测丰度高度吻合。


<details>
  <summary>Details</summary>
Motivation: 研究复杂含氮分子对理解恒星形成区的物理和化学演化至关重要。

Method: 使用ALMA band 4观测数据，通过LTE光谱建模推导柱密度和激发温度，计算相对丰度，进行Pearson相关性分析，并与三阶段升温化学模型比较。

Result: 检测到CH₃CN、C₂H₅CN、C₂H₃CN、NH₂CN、NH₂CHO及HC₃N振动激发态，激发温度175-220 K；氰化物家族分子间存在强正相关性（r>0.7）；观测丰度与化学模型预测偏差在0.67-1.28倍以内。

Conclusion: 这些含氮分子源自源区温暖内部区域（≥100 K），氰化物分子间可能具有化学关联，观测与模型一致性支持当前对气体-颗粒化学过程的理解。
...
### [7] [The first near-infrared high-resolution échelle spectroscopy of the outflow in NGC 4151. A study of the clouds covering the Eye of Sauron](https://arxiv.org/abs/2605.27363)
...
TL;DR: 本文利用IRTF/iSHELL和TNG/GIANO-B高分辨率光谱仪，对赛弗特星系NGC 4151（"索伦之眼"）的He I λ10830Å吸收特征进行了前所未有的高分辨率观测（λ/Δλ>50,000），结合Swift-XRT X射线数据，发现该吸收特征的变化与X射线电离连续谱变化相关，表明尘埃团块状外流响应X射线通量变化，但外流动能不足以触发显著的AGN反馈（动能率/辐射光度~0.001%）。


<details>
  <summary>Details</summary>
Motivation: 过去研究发现NGC 4151存在可变的He I吸收特征，可能指示中央引擎的遮蔽事件，但缺乏高分辨率光谱来解析吸收槽的精细结构及其与X射线电离连续谱变化的联系。

Method: 利用IRTF/iSHELL和TNG/GIANO-B光谱仪获取超高分辨率（λ/Δλ>50,000）近红外光谱，结合Swift-XRT X射线数据，在~700天时间跨度内分析He I吸收特征的结构变化与X射线通量变化的相关性。

Result: 发现He I吸收特征变化与X射线电离连续谱变化存在潜在驱动关系；推导出外流物理参数：质量损失率≲10⁻² M☉/yr，外流半径~3 pc，最大速度~1000 km/s；外流动能率仅占辐射光度的~0.001%，不足以触发显著AGN反馈。

Conclusion: NGC 4151中尘埃团块状外流响应电离X射线通量变化，导致遮蔽现象，这种机制与类星体中的宽吸收线和NGC 5548等多相外流赛弗特星系类似，但该外流能量不足以产生显著反馈效应。
...
### [8] [Looking into the faintEst WIth MUSE (LEWIS): Exploring the nature of ultra-diffuse galaxies in the Hydra-I cluster. VI. A star-forming UDG in Hydra I: a rare UDG or a transition phase?](https://arxiv.org/abs/2605.27109)
...
TL;DR: 本文通过MUSE积分场光谱数据，分析了Hydra I星系团中一个富含气体的超弥散星系UDG 6。研究发现该星系具有规则拉长的形态、丰富的尘埃、贫金属电离气体以及老至中等年龄的恒星成分，并存在局部星暴活动和潮汐特征。研究认为UDG 6可能起源于一个“膨胀矮星系”，其恒星成分被拉伸到更大半径，环境相互作用（如潮汐作用）触发了近期恒星形成，但未破坏气体的整体旋转。


<details>
  <summary>Details</summary>
Motivation: 揭示气体丰富且正在形成恒星的超弥散星系（UDG）的形成历史，并将其与主流UDG形成模型进行比较。

Method: 使用MUSE积分场光谱数据，结合光谱拟合、Voronoi镶嵌算法以及多波段测光分析，研究UDG 6的形态、结构、运动学和星族性质。

Result: UDG 6确认属于Hydra I星系团；具有规则拉长的形状、丰富尘埃、贫金属电离气体和老至中等年龄恒星成分；存在局部星暴活动及潮汐弧形特征。

Conclusion: UDG 6可能源自“膨胀矮星系”，恒星被拉伸到更大半径被动演化；环境过程（如潮汐相互作用）触发了近期恒星形成，但未破坏气体的整体旋转。
...
### [9] [For modified gravity, it's the LITTLE THINGS that matter](https://arxiv.org/abs/2605.27217)
*Geoff Beck*
...
TL;DR: 本研究利用LITTLE THINGS和SPARC两个矮星系样本，通过哈密顿蒙特卡洛采样和ELPD差异比较，发现暗物质晕模型以超过4σ的置信度优于MOND理论，但多数星系偏好并不显著，且MOND仍与大部分SPARC星系兼容；MOG理论则被以超过8σ的置信度排除。


<details>
  <summary>Details</summary>
Motivation: 矮星系是检验暗物质模型的关键场所（如核尖-核芯问题），需要系统比较MOND、暗物质晕和MOG等不同理论在矮星系中的表现。

Method: 选取LITTLE THINGS和SPARC两个矮星系样本，采用最新的哈密顿蒙特卡洛采样方法进行参数推断，并通过ELPD（期望对数点密度）差异进行稳健的模型比较。

Result: 暗物质核芯晕模型以>4σ优于MOND，但这一偏好主要来自19个SPARC星系中的7个和18个LITTLE THINGS星系中的11个，仅有一个星系偏好MOND；MOND外部场效应证据较弱；大部分SPARC星系仍与普适MOND标度兼容；LITTLE THINGS中自由MOND模型以~8σ优于普适MOND值，但物理意义存疑；MOG被以>8σ排除。

Conclusion: 暗物质晕模型在统计上优于MOND，但证据并非压倒性，MOND仍未完全被排除；而MOG的普适旋转曲线模型则被强烈否定。
...
### [10] [Bar-halo interaction: the role of orbital anisotropy](https://arxiv.org/abs/2605.26225)
...
TL;DR: 本文通过受控测试粒子模拟，系统研究了银河系棒旋转对晕族恒星动力学响应的影响，发现晕的速度各向异性（尤其是轨道各向异性）对共振结构形成、角动量交换及力矩有显著调控作用，径向偏好的晕表现出更强响应。


<details>
  <summary>Details</summary>
Motivation: 探究在旋转棒势下，晕的轨道各向异性如何影响共振结构形成，以及角动量交换和力矩受晕密度轮廓以外的因素（各向异性结构）的调控机制。

Method: 在固定银河系类势场中，使用受控测试粒子模拟，系统变化晕成分的速度各向异性和净旋转，同时固定晕密度轮廓、全局势和棒属性。

Result: 棒诱导共振在能量-角动量空间产生显著子结构；子结构的形态、强度和密度对比度敏感依赖于晕轨道各向异性；径向偏好的晕在所有主共振上表现更强响应；角动量交换和力矩不仅由密度轮廓决定，更关键地由轨道各向异性结构调控。

Conclusion: 晕各向异性对解释银河系恒星晕中的相空间子结构至关重要，对理解盘星系中暗物质晕-棒耦合具有重要启示。
...
### [11] [Unveiling the dark matter nature with reionization relics](https://arxiv.org/abs/2605.26518)
...
TL;DR: 本文提出了一种利用"再电离遗迹"作为大尺度探针来约束温暗物质（WDM）的新方法。通过分析Lyman-α森林和21厘米强度映射中的额外大尺度涨落，结合DESI和SKA等下一代巡天数据，可将WDM质量下限约束从目前的9.7 keV提升至39 keV。


<details>
  <summary>Details</summary>
Motivation: 暗物质占宇宙约四分之一，但其物理本质未知。温暗物质（WDM）因具有不可忽略的速度弥散而抑制小尺度宇宙结构形成，现有约束主要依赖小尺度探针（如Lyα森林、卫星星系等），缺乏大尺度探针。本文旨在提出一种基于再电离遗迹的大尺度新探针。

Method: 利用"再电离遗迹"——即由于再电离斑块性在Lyα森林不透明度和21厘米强度映射中留下的额外大尺度涨落。这些遗迹的强度受WDM影响，通过小尺度气体演化及WDM驱动的再电离历史变化两方面起作用。采用DESI能力的协方差模型对Lyα森林进行预测，并结合SKA的21厘米观测。

Result: 3 keV WDM相比冷暗物质，Lyα（21厘米）功率谱在k=0.05 Mpc⁻¹、z=4（z=5.5）处差异约19%（19%）。单独使用DESI可约束m_WDM>5.0 keV（95%），结合SKA可提升至m_WDM>7.1 keV。下一代巡天可将目前最佳下限从9.7 keV提升至39 keV。

Conclusion: 再电离遗迹可作为有效的大尺度探针约束WDM，结合Lyα森林和21厘米观测可显著提升约束能力，为理解暗物质本质提供新途径。
...
### [12] [Constraints on Dynamical Dark Energy from Multiple Probes in the Full Dark Energy Survey](https://arxiv.org/abs/2605.27221)
...
TL;DR: 本文利用暗能量巡天（DES）六年的完整数据，结合超新星、重子声学振荡、弱引力透镜和星系聚类等多种探针，对暗能量状态方程参数（w₀, wₐ）进行了最严格测量，发现与宇宙学常数模型存在2.2σ到3.2σ的偏离，支持近期多项研究中暗能量可能演化的弱偏好。


<details>
  <summary>Details</summary>
Motivation: 验证暗能量是否随时间演化（即动态暗能量），检验宇宙学常数ΛCDM模型是否成立，并实现DES最初构想的多种探针联合的暗能量研究计划。

Method: 使用DES六年完整数据，结合时间相关状态方程w(a)=w₀+wₐ(1-a)，联合分析Ia型超新星、重子声学振荡、弱引力透镜和星系聚类（3×2pt）等多种几何与增长探针，并分别加入DESI DR2 BAO数据和原初CMB信息进行组合分析。

Result: DES单独组合给出w₀=-0.84⁺⁰·¹⁰₋₀·₁₀、wₐ=-0.44⁺⁰·⁶⁰₋₀·₅₅（2.2σ偏离）；加入DESI BAO后w₀=-0.84⁺⁰·⁰⁶₋₀·₀₇、wₐ=-0.53⁺⁰·³³₋₀·₂₈（2.3σ偏离）；再加入CMB后w₀=-0.82⁺⁰·⁰⁵₋₀·⁰⁵、wₐ=-0.63⁺⁰·²¹₋₀·₁₈（3.0σ偏离）。排除任一探针，偏离范围保持在2.3-3.2σ，最佳拟合参数一致为w₀>-1、wₐ<0。

Conclusion: 研究结果支持暗能量演化的弱偏好，实现了DES多探针联合的暗能量研究目标；3×2pt探针的加入显著增强了约束能力；排除超新星数据仍得到2.6σ偏离，为独立于超新星标定的交叉检验提供了证据。
...
### [13] [Gaussian Process Reconstruction of Cosmological Parameters with Gravitational Wave Sirens using Machine Learning](https://arxiv.org/abs/2605.27357)
...
TL;DR: 本文利用高斯过程回归（GPR）对LISA和爱因斯坦望远镜（ET）的模拟引力波标准汽笛目录进行模型无关重建，发现背景距离测量虽能忠实恢复宇宙膨胀历史，但无法有效区分不同宇宙学模型；而导数敏感诊断量（如$q(z)$、$\mathcal{O}_m(z)$等）可在特定红移窗口（ET: $z\simeq1.6-1.8$；LISA: $z\simeq2.6-2.9$）最大化区分能力。


<details>
  <summary>Details</summary>
Motivation: 未来引力波标准汽笛目录将探测传统电磁观测难以触及的红移范围的晚期宇宙膨胀历史，需要评估这种背景距离信息在区分不同宇宙学模型方面的有效性。

Method: 提出基于高斯过程回归（GPR）的模型无关重建框架，分析模拟的LISA和ET标准汽笛目录，涵盖六种宇宙学背景模型（$\Lambda$CDM、CPL、CPL+$\Lambda$、相互作用暗物质、相互作用暗能量、轴子早期暗能量）。恢复共动距离及其导数，利用完整GP协方差（包括导数交叉协方差）稳健计算$H(z)$、$q(z)$、$\mathcal{O}_m(z)$、$w_{\rm total}(z)$和$\kappa(z)$等诊断量，并采用逐点边缘Hellinger距离评估模型区分度。

Result: 引力波标准汽笛能忠实恢复扩张历史，但背景测量本身无法提供模型间的决定性统计分离；导数敏感诊断量可在特定红移窗口（ET: $z\simeq1.6-1.8$；LISA: $z\simeq2.6-2.9$）最大化区分能力。

Conclusion: 该贝叶斯GPR管道提供了一种原则性的非参数方法，可精确定位最有价值的宇宙学信息所在之处，为引力波宇宙学中的模型区分提供指导。
