...
### [1] [Investigating the role of turbulence in the interstellar medium in $z\sim3$ dusty star-forming galaxies using kpc-resolution ALMA dust and gas maps](https://arxiv.org/abs/2606.11444)
...
### [2] [Spatially Resolved Nebular-Stellar Reddening with JWST/NIRISS](https://arxiv.org/abs/2606.12249)
...
TL;DR: 利用JWST/NIRISS数据对z=1.0-2.4星系进行亚kpc尺度尘埃消光分析，发现巴尔默递减与局部关系一致，尘埃几何随红移无明显演化，并建立了差分红化与SFR面密度的经验校准。


<details>
  <summary>Details</summary>
Motivation: 精确测定星系内尘埃消光对推导恒星形成率等关键物理量至关重要，而高红移星系中尘埃衰减的特性及其演化尚需进一步研究。

Method: 使用GLASS-JWST ERS巡天的NIRISS数据，对99个星系（质量范围7.0<logM*<10.5，红移1.0<z<2.4）进行多区域拟合，提取Hα和Hβ发射线图，分析巴尔默递减、星云与连续谱红化关系，并建立差分红化与SFR面密度的经验校准。

Result: 未发现低于Case B复合固有极限的巴尔默递减比值；局部巴尔默递减与恒星质量关系得到重现且无红移演化；星云与连续谱消光关系E(B-V)_SED=(0.46±0.02)E(B-V)_neb与局域星暴星系一致；差分红化与SFR和恒星质量呈显著统计关系。

Conclusion: 在z≲2.4范围内星系内尘埃几何无显著演化，建立的空间分辨差分红化与SFR面密度经验校准可用于未来Roman等更高红移观测中巴尔默线不可及情况下的尘埃消光校正。
...
TL;DR: 利用JWST数据对593个z~3.5以内的旋涡星系螺距角进行测量，发现平均螺距角无显著红移演化，但在最重星系中随时间略微收紧；红移z>1.25时螺距角与内部性质无关，z<1.25时与恒星质量等呈弱负相关，表明z~1处存在螺旋结构驱动机制的转变。


<details>
  <summary>Details</summary>
Motivation: 早期宇宙旋涡星系的性质研究不足，其本质和演化知之甚少，亟需系统测量不同宇宙时期的旋涡结构。

Method: 使用JWST数据，通过微调Zoobot深度学习模型识别旋涡星系，利用SpArcFiRe识别旋臂并测量螺距角，样本包含593个恒星质量>10^10 M⊙、红移z~3.5以内的旋涡星系。

Result: 全样本平均螺距角无显著红移演化；最重星系(log(M*/M⊙)=11-12)螺距角随时间略微减小；z>1.25时螺距角与星系性质无相关，z<1.25时与恒星质量、核球质量、盘质量呈弱负相关，与sSFR在z<0.75时呈正相关；螺距角与潮汐强度无关。

Conclusion: z~1处存在转变：高红移时旋涡结构主要由局部过程驱动，与整体性质无关；低红移时旋臂受全局引力势调控，符合密度波理论预测。
...
### [4] [Geometry and Kinematics of Molecular Cloud Substructures in the Second Galactic Quadrant](https://arxiv.org/abs/2606.11733)
*Wen Ge,Fujun Du*
...
TL;DR: 本文利用MWISP巡天数据构建了分子云亚结构的 unbiased 样本，发现其形态在银道面方向存在轻微但系统的 elongation，并研究了亚结构主轴、速度梯度方向和磁场方向之间的相对取向关系，发现速度梯度垂直于主轴、磁场平行于主轴，且速度梯度-磁场 alignment 随尺度增大而减弱。


<details>
  <summary>Details</summary>
Motivation: 分子云内部亚结构的几何和运动学特征对理解分子云的形成、演化以及恒星形成过程至关重要。然而，以往研究多聚焦于致密核或巨分子云尺度，缺乏对中间尺度(~0.3-30 pc)亚结构的系统观测分析。本文旨在填补这一空白，系统揭示中等尺度分子云亚结构的形态学特征及其与磁场、速度场的耦合关系。

Method: 利用MWISP巡天数据构建 unbiased 分子云亚结构目录，以20%峰值积分强度等值线定义空间连通区域作为亚结构。通过投影尺度比 R=Δb/(Δl·cos b) 量化形态学特征（该比值本质上是 elongated filament 相对于银道面夹角的正切值）。结合Planck磁场数据，分析亚结构主轴、速度梯度方向和磁场方向之间的相对取向关系，并研究其随物理尺度的变化。

Result: 样本的中位R值为0.96，表明亚结构倾向于沿银经方向 elongation，且该趋势在大尺度上更显著。在~0.3-30 pc尺度范围内，速度梯度方向垂直于亚结构主轴，磁场方向平行于主轴。速度梯度与磁场之间的 alignment 随物理尺度增大而逐渐减弱。

Conclusion: 分子云亚结构的观测各向异性可能源于大尺度银河动力学、各向异性气体运动和磁场的共同作用，这些效应的相对重要性随尺度变化。研究结果揭示了与致密核尺度(~0.05 pc)和GMC尺度(~10-100 pc)不同的中间尺度取向规律，为理解分子云的多尺度结构和演化提供了新视角。
...
### [5] [Imprints of the Neutral Interstellar Medium on Polarized Synchrotron Emission and Faraday Rotation](https://arxiv.org/abs/2606.11359)
*Minjie Lei,S. E. Clark,Mehrnoosh Tahani,A. Bracco,Yik Ki Ma,Amit Seta,Jennifer West,E. Carretti*

Main category: astro-ph.GA

Recommendation: ★★★★☆ (69/100) - 与 Zotero 兴趣画像中的 polarized 等主题/作者匹配。

TL;DR: 本研究通过多频段（~300 MHz - 23 GHz）偏振射电观测与HI结构对比，发现中性气体主导的ISM区域可直接贡献大量弥漫同步辐射和法拉第旋转，建立了新的观测约束。


<details>
  <summary>Details</summary>
Motivation: 星际介质（ISM）是多相复杂介质，目前难以区分不同相中的气体分布和磁场结构，而法拉第层析成像的解读受限于对ISM各组分的理解不足。

Method: 对高银纬（|b|>30度）弥漫天空进行多频段（~300 MHz-23 GHz）偏振同步辐射与HI结构对比分析，利用HI视线结构复杂度测量，研究法拉第深度谱与中性气体结构的关系。

Result: 在具有多个HI速度分量的高复杂度区域观测到增强的去极化现象；法拉第深度谱的一阶和二阶矩与中性气体结构相关联；中性气体主导区域可能直接贡献大量弥漫同步辐射和法拉第旋转。

Conclusion: 这些发现为将多相示踪物综合到统一框架中的银河磁场模型建立了新的观测约束。
...
### [6] [Between Degeneracy and Evolution: UV-to-optical Insights into the BH$^*$ Model in Little Red Dots](https://arxiv.org/abs/2606.12355)
...
TL;DR: 本研究对66个小红点(LRDs)进行系统性的UV-光学连续谱拟合分析，发现大多数LRDs的光学波段由恒星和/或AGN发射主导，仅有少数(~6%)符合黑洞星(BH*)模型，但当采用倾向BH*的先验时，比例可升至~40%，表明存在强简并性。


<details>
  <summary>Details</summary>
Motivation: LRDs是一类性质多样的天体，虽有多种物理解释模型，但缺乏结合不同模型、覆盖宽波段范围、对LRD群体进行系统性贝叶斯分析的研究。

Method: 利用JWST/NIRSpec PRISM光谱对66个红移2<z<6的LRDs进行一致的UV-光学连续谱拟合，采用改进版Bagpipes工具，包含受Balmer吸收影响的黑体发射、尘埃衰减的恒星与星云发射以及AGN成分，使用宽先验不预设物理场景。

Result: ~6%的LRDs(共52个统计稳健样本)最佳拟合为光学BH*+UV宿主星系；~8%的黑体主导光学连续谱但缺少恒星成分或有AGN UV泄漏；大多数LRDs光学波段由恒星/AGN主导。采用倾向BH*的先验时，BH*比例升至~40%，但许多仍需要恒星主导光学连续谱。

Conclusion: BH*模型与替代场景存在强简并性，BH*模型可能有局限性，或者存在演化序列：随着宿主增长，BB贡献减小、温度降低、恒星质量增加，更明显的"V"形对应LRD演化后期。
...
### [7] [Phase-dependent magnetic coherence in the turbulent interstellar medium](https://arxiv.org/abs/2606.11323)
...
TL;DR: 该研究通过高分辨率（2048³）数值模拟，重现并解释了观测中尘埃极化分数与冷中性介质（CNM）质量分数正相关的现象，证明这种相关性源于CNM云沿磁场排列且磁无序度低于暖中性介质（WNM），揭示了星际介质中磁场结构的相依赖性。


<details>
  <summary>Details</summary>
Motivation: 观测显示尘埃极化分数与冷中性介质（CNM）质量分数正相关，但与总中性氢柱密度无关，暗示了相依赖的磁场几何结构，但其物理起源尚不清楚。

Method: 采用2048³分辨率的湍流、磁化、多相星际介质数值模拟，构建合成中性氢和尘埃极化图，并与观测结果直接比较。

Result: 正相关性在视线方向包含少于约20个独立CNM云时最显著；CNM云沿局部磁场伸长，单位柱密度的磁无序度低于WNM；模拟与观测的磁无序度差异源于每单位路径长度与每单位质量的量化方式不同。

Conclusion: CNM结构拥有相对有序的磁场，导致在弥漫星际介质中CNM主导的视线方向产生更高的极化分数，支持了磁场结构具有相依赖性的图景。
...
### [8] [JADES: the mass-metallicity relation at $z=1-10$. New calibrations, extremely metal-poor galaxies, and chemical diversity](https://arxiv.org/abs/2606.11345)
...
TL;DR: 利用JWST/NIRSpec光谱对约1500个z=1-10的恒星形成星系进行堆叠分析，建立了基于强线的金属丰度校准，获得了质量-金属丰度关系(MZR)，并识别出50个极贫金属星系(EMPG)候选体，其中两个为"小红点"AGN候选体。


<details>
  <summary>Details</summary>
Motivation: 现有高红移星系金属丰度测量依赖个别的极光发射线探测，可能存在观测偏差。需要更大样本的堆叠分析来建立更可靠的强线校准，并探索极贫金属星系的性质。

Method: 堆叠约1500个中分辨率JWST/NIRSpec光谱，探测[OIII]λ4363极光线，建立12+log(O/H)=7.0-8.7范围内的强线校准；分析质量-金属丰度关系；筛选极贫金属星系候选体。

Result: 在固定金属丰度下，堆叠样本的[OIII]λ5007/Hβ和[OIII]λ5007/[OII]比值低于依赖个别极光线探测的校准，表明存在观测偏差。获得了z=1-10的MZR，发现从z~0到z~4-10金属丰度下降但斜率不变。识别出50个EMPG候选体(金属丰度仅为太阳的1-4%)，其中两个为包含宽Hα和强Lyα发射的"小红点"。

Conclusion: 极贫金属星系的MZR具有大散射，低金属丰度对应更低的sSFR，与局域基本金属丰度关系预期相反，支持随机恒星形成历史（涉及气体消耗/抛射和贫金属内流）强烈影响低质量星系金属丰度的观点。
...
### [9] [Morphology, sizes, and scatter in a large sample of distant quiescent galaxies](https://arxiv.org/abs/2606.11328)
...
TL;DR: 该研究基于JWST数据构建了z>3的137个静止星系样本，通过多波段Sérsic轮廓建模发现：星系大小随红移增加而减小（延伸至z_spec=4.9），随观测波长增加而减小（大质量椭圆星系在近红外波段更致密），尺寸-质量关系的本征散射与红移和滤光片无关。贝叶斯分析表明，仅恒星质量和红移即可预测静止星系大小，但存在较大散射，Sérsic分布常用参数无法解释该散射，需引入其他物理量。


<details>
  <summary>Details</summary>
Motivation: 已有研究表明静止星系在淬灭后随时间持续增长，但z>3高红移区域的大样本多波段研究尚不充分，需要系统分析静止星系尺寸的多波段依赖性和内在散射的起源。

Method: 在825平方角分的JWST深场中筛选出137个z>3（3<z<7）的静止星系候选体，利用5个波段（UV到近红外）进行Sérsic轮廓拟合，提取尺寸、浓度和椭圆度参数，并进行前向/后向贝叶斯拟合分析以评估各参数对尺寸预测的显著性。

Result: 尺寸-质量关系与先前研究一致；尺寸随红移增大而减小（最高至z_spec=4.9）；尺寸随观测波长增大而减小（波长梯度随恒星质量增大而减小）；Sérsic指数不随波长显著变化；贝叶斯分析显示仅恒星质量和红移可预测尺寸，但散射较大，轴比、Sérsic指数、UVJ颜色和环境无显著二次依赖。

Conclusion: 静止星系在z>3的尺寸主要由恒星质量和红移决定，但存在较大本征散射，表明Sérsic分布的常用参数不足以解释该散射，需要引入其他物理量以打破演化路径的简并性。
...
### [10] [Investigating the young stellar populations and hierarchies in nearby galaxies with the UVIT. II. Presenting the properties of ~25,000 UV-detected star-forming clumps](https://arxiv.org/abs/2606.12254)
...
TL;DR: 本文利用UVIT望远镜的远紫外和近紫外观测数据，在17个形态各异的近邻星系中构建了约25,000个年轻恒星形成团块（SFCs）的星表，并研究了其年龄分布与星系形态的关系，验证了内向外盘面形成、旋臂引力不稳定性等已知现象。


<details>
  <summary>Details</summary>
Motivation: 通过研究星系中年轻恒星群体，加深对星系近期恒星形成及其演化的理解。

Method: 利用UVIT的远紫外和近紫外观测（分辨率~1.5"，视场28'），在17个近邻星系中采用均匀的SFC检测标准，结合6"分辨率的消光图进行尘埃衰减校正，并通过与Starburst99恒星群体模型比较UV颜色-星等图来估计SFC年龄。

Result: 构建了约25,000个年轻（<400 Myr）SFC的星表，发现了与内向外盘面形成、局部引力不稳定性导致絮状旋臂、矮星系中恒星形成随机性等一致的年龄趋势。

Conclusion: 该SFC星表补充了现有光学识别的星团星表，有助于理解不同星系形态、质量和环境下的恒星形成过程，并已公开数据。
...
### [11] [Quenching of Star Formation in Massive Galaxies](https://arxiv.org/abs/2606.12156)
...
TL;DR: 本文综合评述了巨星系 quenching（恒星形成终止）的机制，区分了快速（黑洞反馈）和缓慢（气体耗尽、加热等）两种主要模式，并总结了观测特征与演化路径。


<details>
  <summary>Details</summary>
Motivation: 恒星形成终止（quenching）是巨星系演化中的关键转变阶段，理解其触发与维持机制对于揭示宇宙中质量最大的星系如何形成和演化至关重要。

Method: 综合评述了当前对 quenching 机制的理解，包括对静态星系的识别方法（经验颜色选择 vs. 特定恒星形成率阈值）、物理性质（恒星族群、化学丰度、气体尘埃储库）以及观测特征的整合分析。

Result: (1) 静态星系可通过颜色选择识别，但 sSFR 阈值更稳健；(2) 早期静态星系形成快速、高金属丰度、α元素丰度异常；(3) 静态星系中气体和尘埃呈多样多相态储库和流出；(4) 原位过程建立星系中心密度，并合（尤其是小并合）在后继续改造星系。

Conclusion: 巨星系的淬灭存在两种主要模式：一是由超大质量黑洞反馈驱动的快速早期关闭，二是通过气体耗尽、维里加热或预防性反馈实现的渐进过程。这两种路径构成了可检验的理论框架，有待未来对恒星、气体、尘埃和动力学的进一步观测来完善。
...
### [12] [pop-cosmos: Disentangling galaxy properties from observables using data-driven approaches](https://arxiv.org/abs/2606.11308)
...
TL;DR: 本文利用pop-cosmos生成式星系种群模型和β-VAE，发现星系静止光学SED仅需5个独立自由度（恒星质量、近期恒星形成、尘埃、以及两个电离态自由度）即可描述，而非传统模型假设的3-4个。恒星金属丰度和年龄并非主驱动因素，其光谱效应分散在其他维度中。该方法打破了星暴-尘埃-金属丰度简并，比传统线比诊断更清晰地恢复气体物理条件。


<details>
  <summary>Details</summary>
Motivation: 观测中塑造星系光谱的物理过程存在强简并，掩盖了哪些过程是独立作用的。作者希望利用生成式模型来解耦这些简并，明确光谱中真正独立的物理自由度数量。

Method: 使用pop-cosmos生成式星系种群模型，结合β-变分自编码器（β-VAE）将16参数的恒星种群合成（SPS）描述压缩为解耦的潜在表示，并通过互信息（MI）评估潜在维度的独立性。

Result: 发现5个独立维度足以描述星系静止光学SED：恒星质量、近期恒星形成、尘埃，以及两个气体电离态自由度（而非标准星云模型假设的3-4个）。恒星金属丰度和年龄并非主要驱动力，其光谱效应分散在其他维度中。

Conclusion: 该分解打破了星暴-尘埃-金属丰度简并，比传统线比诊断能更干净地恢复气体物理条件，提供了更优的星系物理诊断方法。
...
TL;DR: 本文利用Gaia卫星的恒星速度数据和传统气体速度数据，发现两者的银河系旋转曲线存在显著差异：恒星速度得到的旋转曲线更低且随半径下降，而气体速度的旋转曲线不下降。这一差异导致基于气体速度估算的暗物质质量分数可能被高估。


<details>
  <summary>Details</summary>
Motivation: 传统上利用星际气体的旋转速度来估算银河系的质量，结果发现可见物质不足以解释如此高的旋转速度，从而提出了暗物质的存在。然而，Gaia卫星对恒星三维速度的精确测量表明，从恒星推导出的旋转速度远低于气体速度，这挑战了传统对暗物质质量的估计，需要重新审视气体旋转速度作为质量指示器的可靠性。

Method: 结合多组气体速度观测数据，在径向区间内对速度进行平均，建立气体的大平均旋转曲线；同时收集已发表的Gaia恒星旋转曲线数据，建立恒星的大平均旋转曲线。通过统计分析比较两者差异的置信度。

Result: 恒星速度得到的旋转曲线与气体速度得到的旋转曲线存在显著差异，且置信度很高。恒星旋转曲线随半径增加而明显下降，而气体旋转曲线不下降。两者差异随银心半径增大而增大。

Conclusion: 基于气体旋转速度估算的银河系暗物质质量分数可能被高估。气体更高的旋转速度缺乏合理解释，不太可能是银河系运动学质量的准确指标。这一发现对外部星系基于气体旋转曲线的质量估算也具有重要影响。
...
TL;DR: 本研究利用XMM-Newton和Chandra巡天数据，在z>3的高红移宇宙中识别康普顿厚AGN（CTK AGN），发现其占全部AGN的约17%，且在红移3到6之间没有显著演化，表明最严重的遮蔽吸积阶段是黑洞增长中持续存在的组成部分。


<details>
  <summary>Details</summary>
Motivation: 康普顿厚AGN是理解超大质量黑洞增长和宇宙X射线背景起源的关键但难以捕捉的阶段，需要约束其在高红移下的丰度和演化。

Method: 从811个X射线选源中通过能谱拟合筛选出10个高概率CTK候选体，利用SED模型和多波段数据检验分类，并用24微米光度估算红外光度作为先验更新X射线后验分布，最终计算X射线光度函数。

Result: 加入红外先验后仅3个源保持CTK分类；CTK AGN占3<z<6总AGN群体的17%；CTK比例在z≈6以内无显著演化；整体遮蔽AGN比例随红移升高而增加。

Conclusion: CTK AGN是黑洞增长中持久存在的阶段，在高红移下其比例稳定，暗示典型宿主星系中的星际介质无法产生CTK级别的遮蔽。
...
### [15] [SDSS-V LVM: Revealing the Physical and Chemical Structure of the Helix Nebula](https://arxiv.org/abs/2606.11394)
...
TL;DR: 基于SDSS-V LVM积分场光谱数据，首次对螺旋星云进行了空间连续的全覆盖物理化学结构研究，揭示了其分层电离结构、低密度特征、近太阳氧丰度以及硫异常现象。


<details>
  <summary>Details</summary>
Motivation: 螺旋星云作为最近且最亮的行星状星云之一，此前缺乏基于全覆盖积分场光谱的连续空间研究，无法全面揭示其物理化学结构。

Method: 利用SDSS-V LVM的宽场积分场光谱数据，逐像素分析消光、电子密度与温度、电离结构及化学丰度，重建校准数据立方体并测量41条光学发射线。

Result: 发现星云呈现强分层结构：高电离气体集中于中央空腔，低电离物质主导明亮壳层，中性/过渡区气体分布在外围；典型电子密度约10² cm⁻³；温度不均一，不同电离区间差异数千开尔文；氧丰度近太阳值（12+log(O/H)≈8.7）；存在约1 dex的硫亏损（行星状星云硫异常）；氦和氮丰度接近Type I行星状星云边界。

Conclusion: 表观丰度变化主要由电离效应驱动而非真实化学不均匀性；硫异常在空间全覆盖下仍然存在；螺旋星云的化学增丰程度为中等水平。
...
### [16] [CMB Constraints on Pre-Inflationary Axion Dark Matter Isocurvature](https://arxiv.org/abs/2606.11312)
*Catherine Petretti,Praniti Singh,Matteo Braglia,Xingang Chen,JiJi Fan,Lingfeng Li*

Main category: astro-ph.CO

Recommendation: ★★★★☆ (77/100) - 与 Zotero 兴趣画像中的 dark matter, spt 等主题/作者匹配。

TL;DR: 利用Planck、ACT和SPT的最新CMB数据，对轴子预暴胀模型中产生的冷暗物质等曲率扰动（CDI）给出了最新约束，发现联合数据集对谱指数约束有所改善，但对固定谱指数的等曲率振幅约束并未收紧。


<details>
  <summary>Details</summary>
Motivation: CMB观测与绝热扰动一致，但可能存在等曲率扰动；轴子预暴胀模型可产生具有不同谱倾斜的CDI扰动，需要更新观测约束。

Method: 使用Planck、ACT和SPT最新的CMB各向异性测量数据，研究固定谱指数（从红谱到蓝谱）以及自由谱指数情况下的CDI约束。

Result: 联合数据集对谱指数的约束比Planck单独使用时有所改善；但对固定谱指数的等曲率振幅的约束没有变得更紧。

Conclusion: 给出了轴子CDI扰动的最新观测约束，并讨论了理论意义，特别是对产生蓝谱的模型的影响。
...
### [17] [Nonminimal couplings and preheating effects in $R^2$-Higgs inflation after ACT and SPT](https://arxiv.org/abs/2606.11929)
...
TL;DR: 本文研究了R²-希格斯暴胀模型中非最小希格斯-曲率耦合的高维算子，发现dimension-six算子可以解释ACT和SPT观测偏好的标量谱指数增强，并同时通过Goldstone模式生产实现快速预加热，有助于匹配暴胀能标与CMB参考能标。


<details>
  <summary>Details</summary>
Motivation: 最新的ACT和SPT观测数据偏好更高的标量谱指数n_s，这对现有暴胀模型提出了新的约束。作者希望探究高维非最小希格斯-曲率耦合能否同时解释观测数据和影响早期宇宙的热化过程。

Method: 采用双协变形式论，在R²-希格斯暴胀模型框架下引入dimension-four和dimension-six非最小耦合算子，分析其对标量谱指数的影响及预加热动力学。

Result: dimension-six算子|Φ|²R²和|Φ|⁴R可以容纳CMB+BAO联合分析所偏好的增强标量谱指数n_s，且同一参数区域通过Goldstone模的生产诱导快速预加热。

Conclusion: 若预加热机制能有效实现热化，可帮助匹配暴胀能标与CMB参考能标，为R²-希格斯暴胀模型提供新的观测支持。
...
### [18] [Towards Practical Field-Level Inference for Weak Lensing](https://arxiv.org/abs/2606.12255)
...
TL;DR: 本文比较了场级推断（FLI）与基于功率谱的推断方法，使用相同的弱透镜映射前向建模管线，发现FLI在提取宇宙学信息方面显著优于两点统计量，尤其是当包含小尺度信息时。


<details>
  <summary>Details</summary>
Motivation: 非线性结构增长产生的高阶相关性和形态特征无法被两点统计量完全刻画，而即将到来的巡天将更精确地测量这些特征，因此需要开发能够从中提取更多宇宙学信息的方法。

Method: 使用基于拉格朗日微扰理论和粒子网格（PM）N体演化的800万参数前向模型，通过显式和隐式推断方法，比较场级推断与功率谱推断在弱透镜映射分析中的表现。

Result: 两种FLI方法得到高度一致的后验分布，覆盖检验证实了隐式分析的校准性；相对于功率谱分析，FLI在宇宙学信息上取得显著增益，尤其是在PM前向模型中包含小尺度时。

Conclusion: FLI在提取宇宙学信息方面优于传统两点统计量，但将基于PM的显式FLI应用于观测数据集前仍需解决若干挑战。
...
### [19] [Dark Energy Survey Year 3 results: optimized $w$CDM simulation-based inference with weak lensing map-level hybrid statistics](https://arxiv.org/abs/2606.11309)
...
TL;DR: 本文利用DES Y3弱引力透镜数据，通过贝叶斯模拟推理框架结合信息论压缩统计量，获取了当前最精确的宇宙学参数约束（S8=0.808±0.017, Ωm=0.325±0.024, w<-0.766），相比此前方法将(Ωm, S8, w)的优值提升了60%。


<details>
  <summary>Details</summary>
Motivation: 为了从DES Y3弱引力透镜数据中提取更精确的宇宙学约束，克服传统两点统计方法信息损失和系统误差建模不充分的问题。

Method: 采用基于Gower Street模拟的贝叶斯模拟推理框架，结合信息论方法将弱透镜地图压缩为7个高信息量统计量（包括功率谱和基于神经网络的摘要统计量），并正向建模各类系统误差。

Result: 在wCDM宇宙学模型下获得S8=0.808±0.017, Ωm=0.325±0.024, w<-0.766（68%置信区间）；(Ωm, S8, w)优值较此前最优方法提升60%，较两点分析提升近3倍。

Conclusion: 该混合分析方法是当前仅基于弱引力透镜数据对(Ωm, S8, w)最精确的联合约束，计划将方法应用于DES Y6数据。
...
### [20] [A Unified Halo Mass Function Across Dark Matter Models from High-Resolution Multi-Scale Simulations](https://arxiv.org/abs/2606.12137)
...
TL;DR: 本文通过多种宇宙学模拟校准了暗物质晕质量函数的拟合函数，涵盖了从10^6到10^16太阳质量的广泛范围，精度通常为12%，并考虑了回溅晕去除、有限盒效应等系统误差，为暗物质模型推断提供了关键工具。


<details>
  <summary>Details</summary>
Motivation: 精确测量暗物质晕质量函数，特别是去除回溅晕后的结果，并为多种暗物质范式提供高精度、模型无关的拟合函数，以服务于未来暗物质推断研究。

Method: 使用MultiDark Planck宇宙学盒模拟以及群、银河系、LMC质量级的放大模拟（含CDM和非CDM初始条件），校准晕质量函数和窗口函数的灵活拟合函数，并建模有限盒效应、晕隔离标准、检测效率、人工晕污染等系统误差。

Result: 模型与N体模拟在红移范围内和10个量级质量尺度上高度一致，典型精度12%；在某些功率谱的特定质量区间偏差可达40-50%；结合环境依赖模型后能稳健描述密度对晕质量函数的影响。

Conclusion: 该高精度模型覆盖多种暗物质范式（热 relics、轴子、暗区相互作用模型），对低至10^7太阳质量的晕也准确，是基于数据推断暗物质模型的关键工具。
...
### [21] [Non-linear Structure Formation in Planck+DESI Favoured Interacting Dark Energy Cosmologies](https://arxiv.org/abs/2606.11368)
...
TL;DR: 本文通过自洽的N体模拟研究了相互作用暗能量（IDE）模型在非线性尺度上的效应，发现其对物质功率谱、密度场形态和晕丰度产生尺度依赖的偏离，且标准ΛCDM校准的模型无法捕捉这些非线性特征，强调了在弱引力透镜和大尺度结构分析中需采用模型一致的非线性建模。


<details>
  <summary>Details</summary>
Motivation: 此前研究表明IDE模型有望缓解哈勃张力，但其对S8张力的影响尚不明确，因为现有弱引力透镜和大尺度结构分析要么排除高非线性尺度，要么使用ΛCDM校准的非线性模型来处理非线性区域。

Method: 采用改进的RAMSES代码进行N体模拟，针对转移率Q=ξHρx的IDE模型。由于暗物质欧拉方程相对于ΛCDM不变，相互作用通过修正的背景演化和有效的随时间变化的暗物质粒子质量来纳入。

Result: 在物质功率谱的准线性和非线性区域发现尺度依赖的偏离，密度场形态和晕丰度也发生改变。IDE对非线性结构形成的影响无法被标准ΛCDM校准的模型所捕捉。

Conclusion: 对未来弱引力透镜和大尺度结构约束相互作用暗能量宇宙学而言，采用模型一致的非线性建模至关重要。
...
### [22] [KiDS-Legacy: Joint analysis of second- and third-order cosmic shear](https://arxiv.org/abs/2606.12389)
...
TL;DR: 本文使用KiDS-Legacy数据联合分析二阶和三阶弱引力透镜统计量，显著提高了对Ω_m和S_8的约束精度，三阶统计量通过捕捉非高斯信息打破了参数简并，未发现S_8 tension。


<details>
  <summary>Details</summary>
Motivation: 二阶弱引力透镜相关函数主要对S_8参数组合敏感，难以独立约束Ω_m等宇宙学参数，需要更高阶统计量来打破参数简并并提取更多非高斯信息。

Method: 使用KiDS-Legacy最终数据，联合COSEBIs（2'-300'尺度）和三阶孔径质量矩（4'-32'尺度），并引入红移和质量依赖的本征排列模型、重子修正模型、以及剪切减缩和源聚集修正。

Result: 得到Ω_m=0.297^{+0.056}_{-0.040}和S_8=0.806^{+0.025}_{-0.023}，与仅用二阶统计量相比，Ω_m约束显著收紧，Ω_m-S_8平面的品质因子提高一倍以上。三阶测量通过内部一致性检验，与Planck CMB在1σ内一致。

Conclusion: 三阶宇宙剪切作为成熟探针，能在未来巡天中发挥关键作用，本分析未发现S_8 tension的证据。
...
### [23] [Equilibrium Halo Solutions of the Gross-Pitaevskii-Poisson System: The Role of the Particle Number](https://arxiv.org/abs/2606.11545)
...
TL;DR: 本文系统研究了自引力玻色-爱因斯坦凝聚（GPP系统）的稳态晕状解，明确了玻色子质量、散射长度和总粒子数等微观参数对暗物质晕结构的影响，并建立了从微观粒子性质到可观测星系尺度的直接联系。


<details>
  <summary>Details</summary>
Motivation: 现有暗物质模型需要建立从微观粒子参数到宏观星系观测的定量联系，但GPP系统中各参数对晕结构的影响尚缺乏系统表征。

Method: 通过求解包含显式玻色子质量(m_φ)、散射长度(a_s)和总粒子数(N)的稳态GPP方程，在宽广参数空间内识别基态、激发态和非束缚态解分支，并推导经验标度关系。

Result: 基态分支在(m_φ,N)平面占据特定区域（强烈依赖自相互作用强度）；弱相互作用区重现标准薛定谔-泊松质量半径关系；有限自相互作用展现重力、量子压力和排斥相互作用共同决定的中间态。

Conclusion: 基态解可复现矮星系旋转曲线，但满足莱曼α森林约束的参数配置无法同时匹配观测到的矮星系运动学特征。
...
### [24] [Deep Learning Calibration of the Quasar X-ray/UV Luminosity Relation for Cosmological Applications](https://arxiv.org/abs/2606.12265)
...
TL;DR: 本研究利用深度学习方法重建Ia型超新星哈勃图，检验类星体紫外与X射线光度之间的经验标度关系是否随红移演化。结果表明，该关系存在非线性红移依赖，低红移样本（z<0.7）与高红移样本存在显著差异，需进一步筛选或排除后才可将类星体用作宇宙学探针。


<details>
  <summary>Details</summary>
Motivation: 类星体作为高红移标准烛光具有重要潜力，但其紫外- X射线光度标度关系是否随红移演化尚未明确，需要系统检验。

Method: 采用基于深度学习的LADDER算法重建Pantheon+样本的哈勃图作为参考，结合高斯过程回归和窄带分析方法，检验类星体标度关系的红移依赖性。

Result: z<0.7的样本与z>0.7的样本存在显著差异；标度关系呈现非线性红移依赖，简单线性校正无法解释；该行为源自数据样本本身特征而非宇宙学模型误设。

Conclusion: 要将类星体用作标准烛光，需进一步对标度关系和固有弥散进行建模，或采用更先进的数据处理技术。
